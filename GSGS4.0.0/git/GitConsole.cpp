...
static wxVariant MakeFileBitmapLabel(const wxString& filename)
{
    ...
    gsgs::DataViewTextBitmap tb(filename,
                            bitmaps.GetMimeImageId(FileUtil::GetType(filename, FileUtil::TypeText)));
    wxVariant v;
    v << tb;
    return v;
}
...
// ---------------------------------------------------------------------
void PopulateToolbarOverflow(gsgs::ToolBar* toolbar)
{
    ...
    for(size_t n = 0; n < IDsize; ++n) {
        if(IDs[n] != 0) {
            toolbar->AddTool(IDs[n], labels[n], gsgs_Image().LoadBitmap(bitmapnames[n]));
        } else {
    ...
    , m_git(git)
{
    ...
    m_styler.reset(new gsgs::GenericSTCStyler(m_stcLog));
    m_bitmapLoader = &gsgs_Image();
    
    gsgs_Style().RegisterWindow(m_splitter733);
    ...
    // Error messages will be coloured with red
    {
        ...
        m_styler->AddStyle(words, gsgs::GenericSTCStyler::kError);
    }

    {
        ...
        words.Add("up-to-date");
        m_styler->AddStyle(words, gsgs::GenericSTCStyler::kInfo);
    }
    {
        // warning messages
        wxArrayString words;
        ...
        m_styler->AddStyle(words, gsgs::GenericSTCStyler::kWarning);
    }
    ...
    gsgs_Event().Connect(wxEVT_GIT_CONFIG_CHANGED, wxCommandEventHandler(GitConsole::OnConfigurationChanged),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(GitConsole::OnWorkspaceClosed), NULL,
                                  this);
...
{
    gsgs_Style().UnRegisterWindow(m_splitter733);
    gsgs_Event().Disconnect(wxEVT_GIT_CONFIG_CHANGED,
                                     wxCommandEventHandler(GitConsole::OnConfigurationChanged), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(GitConsole::OnWorkspaceClosed), NULL,
                                     this);
...
void GitConsole::OnResetFile(wxCommandEvent& event)
{
    ...
    GitResetDlg dlg(gsgs_Event().TopFrame(), filesToRevert, filesToRemove);
...
void GitConsole::OnUnversionedFileActivated(wxDataViewEvent& event)
{
    ...
    CHECK_PTR_RET(cd);
    GetManager()->OpenFile(cd->GetPath());
}
...
void GitConsole::OnOpenUnversionedFiles(wxCommandEvent& event)
{
    ...
    std::for_each(paths.begin(), paths.end(), [&](const wxString& filepath) {
        if(!wxDirExists(filepath)) { GetManager()->OpenFile(filepath); }
    });
}
