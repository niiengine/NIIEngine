    ...
    , m_sashPositionH(0)
{
    ...
    m_editEventsHandler.Reset(new gsgs::EditEventsHandler(m_stcBlame));
...
void GitBlameDlg::SetBlame(const wxString& blame, const wxString& args)
{
    ...
    gsgsDEBUG() << "GitBlame is called for file:" << filename << clEndl;
    gsgsDEBUG1() << "GitBlame 'blame':\n" << blame << clEndl;

    // Set blame editor style and fonts
    LexerStyle::Ptr_t lexer = gsgs_Style().GetLexerForFile(wxFileName(filename).GetFullName());
    if(!lexer) { lexer = gsgs_Style().GetLexer("default"); }
    lexer->Apply(m_stcBlame, true);

    LexerStyle::Ptr_t lex = gsgs_EditorConfig().GetLexer("diff");
    if(lex) { lex->Apply(m_stcDiff, true); }

    LexerStyle::Ptr_t textLex = gsgs_EditorConfig().GetLexer("text");
...
void GitBlameDlg::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    wxUnusedVar(event);
    wxDELETE(m_process);
    ...
    for(gsgsStringMap::iterator it = m_diffMap.begin(); it != m_diffMap.end(); ++it) {
...
void GitBlameDlg::OnProcessOutput(gsgs::ProcessEvent& event) { m_commandOutput.Append(event.GetOutput()); }
