...
{
    gsgs::ImageManager&  bl = gsgs_Image();
...
wxBitmap OverlayTool::DoAddBitmap(const wxBitmap& bmp, const wxColour& colour) const
{
    ...
    wxColour c2;
    gsgs_Style().LightColour(colour, c2, 3.0);
    ...
    , m_gitBlameDlg(NULL)
{
    ...
    gsgs_Event().Bind(wxEVT_FILE_CREATED, &GitPlugin::OnFileCreated, this);
    gsgs_Event().Connect(wxEVT_INIT_DONE, wxCommandEventHandler(GitPlugin::OnInitDone), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(GitPlugin::OnWorkspaceLoaded), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(GitPlugin::OnWorkspaceClosed), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_FILE_SAVED, gsgsEventHandler(GitPlugin::OnFileSaved), NULL, this);
    gsgs_Event().Connect(wxEVT_PROJ_FILE_ADDED, gsgsEventHandler(GitPlugin::OnFilesAddedToProject), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_PROJ_FILE_REMOVED, gsgsEventHandler(GitPlugin::OnFilesRemovedFromProject),
                                  NULL, this);
    ...
    gsgs_Event().Connect(wxEVT_CL_FRAME_TITLE, gsgsEventHandler(GitPlugin::OnMainFrameTitle), NULL, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FILE, &GitPlugin::OnFileMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FOLDER, &GitPlugin::OnFolderMenu, this);
    gsgs_Event().Bind(wxEVT_ACTIVE_PROJECT_CHANGED, &GitPlugin::OnActiveProjectChanged, this);
    gsgs_Event().Bind(wxEVT_GSGS_MAINFRAME_GOT_FOCUS, &GitPlugin::OnAppActivated, this);
    gsgs_Event().Bind(wxEVT_FILES_MODIFIED_REPLACE_IN_FILES, &GitPlugin::OnReplaceInFiles, this);
    ...
    m_tabToggler.reset(new gsgs::TabTogglerHelper(_("Git"), m_console, "", NULL));
...
/*******************************************************************************/
void GitPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void GitPlugin::CreatePluginMenu(wxMenu* pluginsMenu)
{
    ...
    gsgs::ImageManager* bmps = m_mgr->GetStdIcons();
...
void GitPlugin::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_FILE_CREATED, &GitPlugin::OnFileCreated, this);
    /*MENU*/
    ...
    /*SYSTEM*/
    gsgs_Event().Disconnect(wxEVT_INIT_DONE, wxCommandEventHandler(GitPlugin::OnInitDone), NULL, this);
    gsgs_Event().Disconnect(wxEVT_FILE_SAVED, gsgsEventHandler(GitPlugin::OnFileSaved), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(GitPlugin::OnWorkspaceLoaded), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_PROJ_FILE_ADDED, gsgsEventHandler(GitPlugin::OnFilesAddedToProject),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CONFIG_CHANGED,
                                     wxCommandEventHandler(GitPlugin::OnWorkspaceConfigurationChanged), NULL, this);
    gsgs_Event().Unbind(wxEVT_ACTIVE_PROJECT_CHANGED, &GitPlugin::OnActiveProjectChanged, this);
    gsgs_Event().Unbind(wxEVT_GSGS_MAINFRAME_GOT_FOCUS, &GitPlugin::OnAppActivated, this);
    gsgs_Event().Unbind(wxEVT_FILES_MODIFIED_REPLACE_IN_FILES, &GitPlugin::OnReplaceInFiles, this);
    ...
    m_eventHandler->Disconnect(XRCID("git_diff_file"), wxEVT_COMMAND_MENU_SELECTED,
                               wxCommandEventHandler(GitPlugin::OnFileDiffSelected), NULL, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FILE, &GitPlugin::OnFileMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FOLDER, &GitPlugin::OnFolderMenu, this);
...
void GitPlugin::DoSetRepoPath(const wxString& repoPath, bool promptUser)
{
    ...
    wxString projectName = gsgs_Workspace().IsValid()
                               ? gsgs_Workspace().GetWorkspace()->GetActiveProjectName()
                               : "";
    ...
    if(!dir.IsEmpty()) {
        ...
        gsgs::StatusBar* sb = m_mgr->GetStatusBar();
        if(sb) {
...
void GitPlugin::OnSwitchLocalBranch(wxCommandEvent& e)
{
    ...
    if(!m_modifiedFiles.empty()) {
        wxMessageBox(_("Modified files found! Commit them first before switching branches..."), wxT("gsgs"),
                     wxICON_ERROR | wxOK, m_topWindow);
        return;
    }

    if(m_localBranchList.GetCount() == 0) {
        wxMessageBox(_("No other local branches found."), wxT("gsgs"), wxICON_INFORMATION | wxOK, m_topWindow);
        return;
    }
...
void GitPlugin::OnSwitchRemoteBranch(wxCommandEvent& e)
{
    ...
    if(m_remoteBranchList.GetCount() == 0) {
        wxMessageBox(_("No remote branches found."), wxT("gsgs"), wxICON_INFORMATION | wxOK, m_topWindow);
        return;
    }
...
void GitPlugin::OnPush(wxCommandEvent& e)
{
    ...
    if(m_remotes.GetCount() == 0) {
        wxMessageBox(_("No remotes found, can't push!"), wxT("gsgs"), wxICON_ERROR | wxOK, m_topWindow);
        return;
    }

    wxStandardID res = ::PromptMessageDialogWithoutCancel(_("Push all local commits?"), "GitPromptBeforePush");
    if(res == wxID_YES) {
...
void GitPlugin::OnPull(wxCommandEvent& e)
{
    ...
    wxStandardID res =
        ::PromptMessageDialogWithoutCancel(_("Save all changes and pull remote changes?"), "GitPullRemoteChanges");
    if(res == wxID_YES) {
        m_mgr->SaveAll();
...
wxString GitPlugin::GetEditorRelativeFilepath() const // Called by OnGitBlame or the git blame dialog
{
    ...
    // We need to be symlink-aware here on Linux, so use RealPath
    wxString realfilepath = RealPath(current->GetFileName().GetFullPath());
    wxFileName fn(realfilepath);
    fn.MakeRelativeTo(RealPath(m_repositoryDirectory));
...
#endif
/*******************************************************************************/
void GitPlugin::OnFileSaved(gsgs::Event& e)
{
    e.Skip();
    std::map<wxString, wxTreeItemId>::const_iterator it;
...
/*******************************************************************************/
void GitPlugin::OnFilesAddedToProject(gsgs::Event& e)
{
    e.Skip();
...
/*******************************************************************************/
void GitPlugin::OnFilesRemovedFromProject(gsgs::Event& e)
{
    e.Skip();
...
void GitPlugin::FinishGitListAction(const gitAction& ga)
{
    ...
    if(ga.action == gitListAll) {
        ...
        gsgsStringSet toColour;
        gsgsStringSet::const_iterator iter = gitFileSet.begin();
...
void GitPlugin::GetCurrentBranchAction(const gitAction& ga)
{
    ...
    if(!m_currentBranch.IsEmpty()) {
        ...
        m_mgr->GetAuiManager()->GetPane(wxT("Workspace View")).Caption(wxT("Workspace View [") + m_currentBranch + wxT("]"));
...
void GitPlugin::UpdateFileTree()
{
    ...
    // gsgs::ProgressDlg *prgDlg = new gsgs::ProgressDlg (m_topWindow, _("Importing files ..."), wxT(""),
...
/*******************************************************************************/
void GitPlugin::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    HideProgress();
    ...
    if(ga.action == gitListAll || ga.action == gitListModified || ga.action == gitResetRepo) {
        ...
        gsgs_Event().PostReloadExternallyModifiedEvent(true);
        ...
        if(ga.action == gitPull) {
            if(m_commandOutput.Contains(wxT("Already"))) {
                ...
                if(m_bActionRequiresTreUpdate) {
                    ...
                    gsgs::FileEvent fsEvent(wxEVT_FILE_SYSTEM_UPDATED);
    ...
    if(ga.action == gitResetRepo || ga.action == gitResetFile) {
        // Reload externally modified files
        gsgs_Debug("Git: posting a 'reload externally modified files' event");
        gsgs_Event().PostReloadExternallyModifiedEvent(true);
    }
...
/*******************************************************************************/
void GitPlugin::OnProcessOutput(gsgs::ProcessEvent& event)
{
...

    {
        if(tmpOutput.Contains("username for")) {
            ...
            m_process->Terminate();
            GitUserEmailDialog userEmailDialog(gsgs_Event().TopFrame());
...
void GitPlugin::InitDefaults()
{
    ...
    if(!repoPath.IsEmpty() && wxFileName::DirExists(repoPath + gsgsPathSep + wxT(".git"))) {
...
/*******************************************************************************/
void GitPlugin::ColourFileTree(gsgs::TreeCtrl* tree, const gsgsStringSet& files, OverlayTool::BmpType bmpType) const
{
    gsgs::Config conf("git.conf");
    GitEntry data;
    conf.ReadItem(&data);
...
{
    gsgs::TreeCtrl* tree = m_mgr->GetWorkspaceTree();
    if(!tree) { return; }
...
void GitPlugin::DoCleanup()
{
    ...
    m_bActionRequiresTreUpdate = false;
    wxDELETE(m_process);
    m_mgr->GetAuiManager()->GetPane(wxT("Workspace View")).Caption(wxT("Workspace View"));
    m_mgr->GetAuiManager()->Update();
...
void GitPlugin::DoSetTreeItemImage(gsgs::TreeCtrl* ctrl, const wxTreeItemId& item, OverlayTool::BmpType bmpType) const
{
    //    gsgs::Config conf("git.conf");
    //    GitEntry data;
    //    conf.ReadItem(&data);
...
void GitPlugin::DoShowDiffViewer(const wxString& headFile, const wxString& fileName)
{
    ...
    // Show diff frame
    FileDiffPanel::FileInfo l(tmpFilePath, _("HEAD version"), true);
    l.deleteOnExit = true;
    FileDiffPanel::FileInfo r(fnWorkingCopy.GetFullPath(), _("Working copy"), false);
    FileDiffFrame* diffView = new FileDiffFrame(gsgs_Event().TopFrame(), l, r, true);
    diffView->Show();
}
...
void GitPlugin::OnRebase(wxCommandEvent& e)
{
    ...
    wxStandardID res = ::PromptMessageDialogWithoutCancel(_("Save all changes and rebase?"), "GitRebaseChanges");
    if(res == wxID_YES) {
        m_mgr->SaveAll();
        if(m_console->IsDirty()) {
            gitAction ga(gitStash, wxT(""));
...
void GitPlugin::OnMainFrameTitle(gsgs::Event& e)
{
    // By default - skip it
    e.Skip();
...
void GitPlugin::OnFileMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void GitPlugin::OnFolderMenu(ContextMenuEvent& event)
{
    event.Skip();
    wxMenu* menu = new wxMenu();
    wxMenu* parentMenu = event.GetMenu();
...
void GitPlugin::OnCommandEnded(gsgs::Event& event)
{
...
void GitPlugin::OnCommandOutput(gsgs::Event& event)
{
...
void GitPlugin::DoExecuteCommands(const GitCmd::Vec_t& commands, const wxString& workingDir)
{
    ...
    command << " --no-pager ";
    m_commandProcessor =
        new gsgs::CmdPrc(command + commands.at(0).baseCommand, workingDir, commands.at(0).processFlags);
    gsgs::CmdPrc* cur = m_commandProcessor;
    for(size_t i = 1; i < commands.size(); ++i) {
        gsgs::CmdPrc* next =
            new gsgs::CmdPrc(command + commands.at(i).baseCommand, workingDir, commands.at(i).processFlags);
        cur = cur->Link(next);
    }
...
void GitPlugin::OnFolderCommit(wxCommandEvent& event)
{
    ...
    if(!diff.IsEmpty()) {
        ...
        ::wxMessageBox(_("All files are up-to-date!"), "gsgs");
    }
}
...
void GitPlugin::OnActiveProjectChanged(gsgs::ProjectSettingsEvent& event)
{
    event.Skip();
...
void GitPlugin::OnFileGitBlame(wxCommandEvent& event)
{
    ...
    // We need to be symlink-aware here on Linux, so use RealPath
    wxString realfilepath = RealPath(m_filesSelected.Item(0));
    wxFileName fn(realfilepath);
    fn.MakeRelativeTo(RealPath(m_repositoryDirectory));
...
void GitPlugin::OnFileCreated(gsgs::FileEvent& event)
{
    event.Skip();
    if(IsGitEnabled()) {
...
void GitPlugin::OnReplaceInFiles(gsgs::FileEvent& event)
{
    event.Skip();
    DoRefreshView(false);
}
