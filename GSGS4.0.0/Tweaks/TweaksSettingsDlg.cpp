...
{
    event.Enable( gsgs_CppWorker().IsOpen() );
}
...
{
    event.Enable( m_checkBoxEnableTweaks->IsChecked() && gsgs_CppWorker().IsOpen() );
}
...
void TweaksSettingsDlg::OnImageSelected(wxPropertyGridEvent& event)
{
    ...
    if ( !m_settings.HasFlag( TweaksSettings::kDontPromptForProjectReload ) ) {
        wxRichMessageDialog dlg(this, _("Icon changes require a workspace reload"), "gsgs", wxOK|wxOK_DEFAULT|wxCANCEL|wxICON_INFORMATION);
