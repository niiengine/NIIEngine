...
#define TWEAKS_ENABLED_EVENT_HANDLER()                                       \
    if(!m_settings.IsEnableTweaks() || !gsgs_CppWorker().IsOpen()) { \
        e.Skip();                                                            \
        return;                                                              \
    }
    ...
    : IPlugin(manager)
{
    ...
    gsgs_Event().Connect(wxEVT_COLOUR_TAB, gsgsColourEventHandler(Tweaks::OnColourTab), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(Tweaks::OnWorkspaceLoaded), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(Tweaks::OnWorkspaceClosed), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_VIEW_BUILD_STARTING,
                                  gsgsEventHandler(Tweaks::OnFileViewBuildTree), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_VIEW_CUSTOMIZE_PROJECT,
                                  gsgsColourEventHandler(Tweaks::OnCustomizeProject), NULL, this);
    gsgs_Event().Connect(wxEVT_GET_TAB_BORDER_COLOUR, gsgsColourEventHandler(Tweaks::OnTabBorderColour), NULL,
                                  this);
}
...
void Tweaks::UnPlug()
{
    m_mgr->GetTheApp()->Disconnect(ID_TWEAKS_SETTINGS, wxEVT_COMMAND_MENU_SELECTED,
                                   wxCommandEventHandler(Tweaks::OnSettings), NULL, this);
    gsgs_Event().Disconnect(wxEVT_COLOUR_TAB, gsgsColourEventHandler(Tweaks::OnColourTab), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(Tweaks::OnWorkspaceLoaded), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(Tweaks::OnWorkspaceClosed), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_VIEW_BUILD_STARTING,
                                     gsgsEventHandler(Tweaks::OnFileViewBuildTree), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_VIEW_CUSTOMIZE_PROJECT,
                                     gsgsColourEventHandler(Tweaks::OnCustomizeProject), NULL, this);
    gsgs_Event().Disconnect(wxEVT_GET_TAB_BORDER_COLOUR, gsgsColourEventHandler(Tweaks::OnTabBorderColour), NULL,
                                     this);
}

Tweaks::~Tweaks() {}

void Tweaks::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void Tweaks::OnColourTab(gsgsColourEvent& e)
{
...
void Tweaks::OnFileViewBuildTree(gsgs::Event& e)
{
    TWEAKS_ENABLED_EVENT_HANDLER();
    m_project2Icon.clear();
...
void Tweaks::OnCustomizeProject(gsgsColourEvent& e)
{
    TWEAKS_ENABLED_EVENT_HANDLER();
...
void Tweaks::OnTabBorderColour(gsgsColourEvent& e)
{
#ifdef __WXGTK__
    ...
    if(m_settings.GetGlobalBgColour().IsOk()) {
        ...
        e.SetBorderColour(gsgs::StyleManager::DarkColour(m_settings.GetGlobalBgColour(), 1.5));
    } else {
        e.Skip();
    }
}
