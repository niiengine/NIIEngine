...
void TweaksSettings::FromJSON(const JSONObject& json)
{
    m_projects.clear();
    m_globalBgColour = json.getObject("m_globalBgColour").toColour();
    m_globalFgColour = json.getObject("m_globalFgColour").toColour();
    m_enableTweaks = json.getObject("m_enableTweaks").toBool(false);
    m_flags = json.getObject("m_flags").toSize_t();
    
    JSONObject arr = json.getObject("projects");
    int size = arr.getArraySize();
    for(int i=0; i<size; ++i) {
        ProjectTweaks tw;
        tw.FromJSON( arr.getArray(i) );
        m_projects.insert( std::make_pair(tw.GetProjectName(), tw) );
    }
}

JSONObject TweaksSettings::ToJSON() const
{
    ...
    JSONObject arr = JSONObject::createArray("projects");
    e.append( arr );
...
TweaksSettings& TweaksSettings::Load()
{
    ...
    wxFileName fn(gsgs_CppWorker().GetPrivateFolder(), "tweaks.conf");
...
void ProjectTweaks::FromJSON(const JSONObject& json)
{
    m_tabBgColour = json.getObject("m_tabBgColour").toColour(wxSystemSettings::GetColour(wxSYS_COLOUR_3DFACE));
    m_tabFgColour = json.getObject("m_tabFgColour").toColour(*wxBLACK);
    m_projectName = json.getObject("m_projectName").toString();
    m_bitmapFilename = json.getObject("m_bitmapFilename").toString();
}

JSONObject ProjectTweaks::ToJSON() const
{
