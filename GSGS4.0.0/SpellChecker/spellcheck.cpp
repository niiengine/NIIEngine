...
{
    if(thePlugin == 0) { thePlugin = new SpellCheck(manager); }
    return thePlugin;
}
// ------------------------------------------------------------
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("gsgs spell checker"));
...
void SpellCheck::Init()
{
    m_topWin = NULL;
    m_pEngine = NULL;
    m_longName = _("gsgs spell-checker");
    ...
    if(m_pEngine) {
        LoadSettings();
        wxString userDictPath = PathManager::Get().GetUserDataDir();
...
// ------------------------------------------------------------
void SpellCheck::CreateToolBar(gsgs::ToolBar* toolbar)
{
...
// ------------------------------------------------------------
void SpellCheck::OnContextMenu(ContextMenuEvent& e)
{
    ...
    if(editor->GetCtrl()->IndicatorValueAt(3, pos) == 1) {
        ...
        if(sugg.GetCount() != 0) { pSubMenu->AppendSeparator(); }
    ...
    if(m_pEngine != NULL) {
        ...
        case wxSTC_LEX_CPP: {
            if(m_mgr->IsOpen()) {
                ...
                if(!GetCheckContinuous()) { editor->ClearUserIndicators(); }
            }
        } break;
        case wxSTC_LEX_NULL: {
...
void SpellCheck::OnContinousCheck(wxCommandEvent& e)
{
    if(m_pEngine != NULL) {
        ...
        if(editor) {
            ...
            switch(editor->GetLexerId()) {
            case wxSTC_LEX_CPP: {
                if(m_mgr->IsOpen()) { m_pEngine->CheckCppSpelling(text); }
            } break;
            default: {
                m_pEngine->CheckSpelling(text);
            } break;
...
void SpellCheck::OnTimer(wxTimerEvent& e)
{
    ...
    if(GetCheckContinuous()) {
        ...
        if((editor == m_pLastEditor) && (m_lastModificationCount == modificationCount)) { return; }
        ...
        switch(editor->GetLexerId()) {
        case wxSTC_LEX_CPP: {
            if(m_mgr->IsOpen()) { m_pEngine->CheckCppSpelling(editor->GetEditorText()); }
        } break;
        default: {
...
void SpellCheck::SetCheckContinuous(bool value)
{
    ...
    gsgs::ToolBarButton* btn = GetManager()->GetToolBar()->FindById(XRCID(s_contCheckID.ToUTF8()));
