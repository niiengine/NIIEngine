...
bool IHunSpell::InitEngine()
{
    ...
    if(!fna.FileExists()) {
        gsgsLogMessage(_("Could not find aff file!"));
        return false;
    }
    wxFileName fnd(dict);

    if(!fnd.FileExists()) {
        gsgsLogMessage(_("Could not find dictionary file!"));
        return false;
    }
...
bool IHunSpell::SaveUserDict(const wxString& filename)
{
    ...
    if(!tf.Exists()) {
        ...
        // Re-read contents from disk in case another gsgs instance has updated the user dictionary.
        for(wxUint32 i = 0; i < tf.GetLineCount(); i++) {
