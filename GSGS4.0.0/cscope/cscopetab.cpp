...
CscopeTab::CscopeTab(wxWindow* parent, IManager* mgr)
    ...
    , m_mgr(mgr)
{
    m_styler.Reset(new gsgs::FindResultsStyler(m_stc));
    ...
    gsgs_Event().Connect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(CscopeTab::OnTheme), NULL, this);
}

CscopeTab::~CscopeTab()
{
    gsgs_Event().Disconnect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(CscopeTab::OnTheme), NULL,
                                     this);
}
...
void CscopeTab::OnTheme(wxCommandEvent& e)
{
    e.Skip();
    m_styler->SetStyles(m_stc);
}
...
{
    CHECK_PTR_RET(gsgs_CppWorker().IsOpen());

    int clickedLine;
    int style = m_styler->HitTest(e, clickedLine);
    if(style == gsgs::FindResultsStyler::LEX_FIF_FILE || style == gsgs::FindResultsStyler::LEX_FIF_HEADER) {
        // Toggle
        ...
        if(iter != m_matchesInStc.end()) {
            wxString wsp_path = gsgs_CppWorker().GetPrivateFolder();
            wxFileName fn(iter->second.GetFile());
            if(!fn.MakeAbsolute(wsp_path)) {
                gsgsLogMessage(wxT("CScope: failed to convert file to absolute path"));
                return;
            }
