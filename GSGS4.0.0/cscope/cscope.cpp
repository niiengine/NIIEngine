...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("CScope Integration for gsgs"));
...
Cscope::Cscope(IManager* manager)
    ...
    , m_topWindow(NULL)
{
    m_longName = _("CScope Integration for gsgs");
    ...
    m_tabHelper.reset(new gsgs::TabTogglerHelper(CSCOPE_NAME, m_cscopeWin, "", NULL));
    ...
    gsgs_Window().addHotKey("cscope_find_user_symbol", "Alt-)", "Plugins::CScope::Find");
    gsgs_Window().addHotKey("cscope_find_symbol", "Alt-0",
                                                   "Plugins::CScope::Find selected text");
    gsgs_Window().addHotKey("cscope_find_global_definition", "Alt-1",
                                                   "Plugins::CScope::Find this global definition");
    gsgs_Window().addHotKey("cscope_functions_calling_this_function", "Alt-2",
                                                   "Plugins::CScope::Find functions called by this function");
    gsgs_Window().addHotKey("cscope_functions_called_by_this_function", "Alt-3",
                                                   "Plugins::CScope::Find functions calling this function");
    gsgs_Window().addHotKey("cscope_create_db", "Alt-4",
                                                   "Plugins::CScope::Create CScope database");
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR, &Cscope::OnEditorContentMenu, this);
}

Cscope::~Cscope() {}

void Cscope::CreateToolBar(gsgs::ToolBar* toolbar)
{
    ...
    gsgs::ImageManager* bitmapLoader = m_mgr->GetStdIcons();
...
void Cscope::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR, &Cscope::OnEditorContentMenu, this);
...
wxString Cscope::DoCreateListFile(bool force)
{
    ...
    wxString privateFolder = gsgs_CppWorker().GetPrivateFolder();
    ...
    if(force || settings.GetRebuildOption() || !list_file.FileExists()) {
        ...
        if(!file.IsOpened()) {
            gsgsDEBUG() << "Failed to open temporary file:" << list_file;
...
void Cscope::DoCscopeCommand(const wxString& command, const wxString& findWhat, const wxString& endMsg)
{
    ...
    req->SetWorkingDir(gsgs_CppWorker().GetPrivateFolder());
...
void Cscope::OnDoSettings(wxCommandEvent& e)
{
    ...
    CScopeSettingsDlg dlg(gsgs_Event().TopFrame());
...
void Cscope::OnCscopeUI(wxUpdateUIEvent& e)
{
    ...
    e.Enable(m_mgr->IsOpen() && isEditor);
}
...
void Cscope::OnEditorContentMenu(ContextMenuEvent& event)
{
    event.Skip();
    IEditor* editor = m_mgr->GetActiveEditor();
