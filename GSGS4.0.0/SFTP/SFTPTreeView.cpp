...
SFTPTreeView::SFTPTreeView(wxWindow * parent, SFTPPlugin * plugin)
    : SFTPTreeViewBase(parent)
    , m_plugin(plugin)
{
    m_bmpLoader = &gsgs_Image();
    m_treeCtrl->SetBitmaps(&m_bmpLoader->GetBitmaps());
    ...
    gsgs_Event().Bind(wxEVT_EDITOR_CLOSING, &SFTPTreeView::OnEditorClosing, this);
    m_treeCtrl->SetDropTarget(new gsgs::FileOrFolderDropTarget(this));
...
SFTPTreeView::~SFTPTreeView()
{
    ...
    gsgs_Event().Unbind(wxEVT_EDITOR_CLOSING, &SFTPTreeView::OnEditorClosing, this);
...
void SFTPTreeView::DoBuildTree(const wxString& initialFolder)
{
    ...
    wxTreeItemId root =
        m_treeCtrl->AddRoot(initialFolder, m_bmpLoader->GetMimeImageId(FileUtil::TypeFolder), wxNOT_FOUND, cd);
...
void SFTPTreeView::OnItemActivated(wxTreeEvent& event)
{
    ...
    if(cd->IsFolder()) {
        ...
        if(m_treeCtrl->IsExpanded(item)) {
            m_treeCtrl->CallAfter(&gsgs::DefaultTreeCtrl::Collapse, item);
        } else {
            m_treeCtrl->CallAfter(&gsgs::DefaultTreeCtrl::Expand, item);
        }
...
{
    SSHAccountManagerDlg dlg(gsgs_Event().TopFrame());
...
void SFTPTreeView::DoCloseSession()
{
    ...
    gsgs::IEditor::List_t modeditors;
    ...
    std::for_each(editors.begin(), editors.end(), [&](gsgs::IEditor* editor) {
        ...
        if(pcd) {
            ...
            if(!GetManager()->CloseEditor(editor)) { modeditors.push_back(editor); }
        }
    });
...
bool SFTPTreeView::DoExpandItem(const wxTreeItemId& item)
{
    ...
    try {
        attributes = m_sftp->List(cd->GetFullPath(), gsgs::SFTP::SFTP_BROWSE_FILES | gsgs::SFTP::SFTP_BROWSE_FOLDERS);

    } catch(gsgs::Exception& e) {
        ::wxMessageBox(e.What(), "SFTP", wxOK | wxICON_ERROR | wxCENTER, gsgs_Event().TopFrame());
        return false;
    }
    ...
    for(; iter != attributes.end(); ++iter) {
        ...
        if(attr->IsFolder()) {
            imgIdx = m_bmpLoader->GetMimeImageId(FileUtil::TypeFolder);
...
void SFTPTreeView::OnMenuNewFile(wxCommandEvent& event)
{
    ...
    defaultValue << "new" << ++s_untitledCounter;
...
wxTreeItemId SFTPTreeView::DoAddFile(const wxTreeItemId& parent, const wxString& path)
{
    try {
        ...
        wxTreeItemId child = m_treeCtrl->AppendItem(
            ...
            m_bmpLoader->GetMimeImageId(FileUtil::GetType(path, FileUtil::TypeText)), wxNOT_FOUND, newFile);
        return child;
...
wxTreeItemId SFTPTreeView::DoAddFolder(const wxTreeItemId& parent, const wxString& path)
{
    try {
        ...
        wxTreeItemId child = m_treeCtrl->AppendItem(
            parent, newCd->GetFullName(), m_bmpLoader->GetMimeImageId(FileUtil::TypeFolder), wxNOT_FOUND, newCd);
...
{
    SFTPImages images;
    gsgs::ToolBarButton* button = m_toolbar->FindById(XRCID("ID_SFTP_CONNECT"));
    CHECK_PTR_RET(button);
    if(IsConnected()) {
        // Disconnect
        DoCloseSession();
        // Update toobar image
        button->SetBmpIdx(m_bmpLoader->GetMimeImageId("disconnected"));
        button->SetLabel(_("Disconnected. Click to connect"));
    } else {
        DoOpenSession();
        if(IsConnected()) {
            // Update toobar image
            button->SetBmpIdx(m_bmpLoader->GetMimeImageId("connected"));
...
void SFTPTreeView::DoOpenSession()
{
    ...
    try {
        gsgs::SSH::Ptr_t ssh(
            new gsgs::SSH(m_account.GetHost(), m_account.GetUsername(), m_account.GetPassword(), m_account.GetPort()));
    ...
    try {
        ...
        if(!s.GetAccount(m_account.GetAccountName(), dummy)) {
            ...
            wxStandardID res = ::PromptMessageDialog(message, "SFTPQuickConnectSaveDlg");
            if(res == wxID_YES) {
        ...
        ::wxMessageBox(e.What(), "gsgs", wxICON_ERROR | wxOK);
        DoCloseSession();
    }
}
...
void SFTPTreeView::OnOpenTerminal(wxCommandEvent& event)
{
    ...
    FileUtil::OpenSSHTerminal(sshClient, connectString, account.GetPassword(), account.GetPort());
}
...
void SFTPTreeView::OnFileDropped(gsgs::Event& event)
{
    wxArrayTreeItemIds items;
    m_treeCtrl->GetSelections(items);
    ...
    SFTPUploadDialog dlg(gsgs_Event().TopFrame());
...
{
    SFTPQuickConnectDlg connectDialog(gsgs_Event().TopFrame());
...
{
    evt.Skip();
    gsgs::IEditor* editor = (gsgs::IEditor*)evt.GetClientData();
    SFTPClientData* pcd = dynamic_cast<SFTPClientData*>(editor->GetClientData("sftp"));
    if(pcd) {
...
void SFTPTreeView::DoLoadSession()
{
    ...
    if(sess.IsOk()) {
        ...
        wxStandardID ans = ::PromptMessageDialog(msg, "sftp-load-session-dlg");
        if(ans == wxID_YES) {
...
void SFTPTreeView::OnExecuteCommand(wxCommandEvent& event)
{
    ...
    try {
        ...
        gsgs::ConsoleFrame* frame = new gsgs::ConsoleFrame(gsgs_Event().TopFrame(), m_sftp->GetSsh());
        frame->Show();
...
void SFTPTreeView::OnRemoteFind(wxCommandEvent& event)
{
    ...
    SFTPGrep grep(gsgs_Event().TopFrame());
    if(grep.ShowModal() != wxID_OK) { return; }

    try {
        if(m_channel && m_channel->IsOpen()) { m_channel->Close(); }
        m_channel.reset(new gsgs::SSHChannel(m_sftp->GetSsh()));
        m_channel->Open();

        // Prepare the UI for new search
        m_plugin->GetOutputPane()->ClearSearchOutput();
        m_plugin->GetOutputPane()->ShowSearchTab();
        GetManager()->ShowOutputPane(_("SFTP Log"));
