...
SFTPWorkerThread* SFTPWorkerThread::gInstance = 0;
...
{
    if(gInstance == 0) { gInstance = new SFTPWorkerThread(); }
    return gInstance;
}
        ...
        try {
            ...
            case eSFTPActions::kDownloadAndOpenWithDefaultApp: {
                ...
                if(req->GetAction() == eSFTPActions::kDownload) {
                    ...
                    m_plugin->CallAfter(&SFTPPlugin::FileDownloadedSuccessfully, cd);

                } else if(req->GetAction() == eSFTPActions::kDownloadAndOpenContainingFolder) {
                    m_plugin->CallAfter(&SFTPPlugin::OpenContainingFolder, req->GetLocalFile());

                } else {
                    m_plugin->CallAfter(&SFTPPlugin::OpenWithDefaultApp, req->GetLocalFile());
                }
                break;
            }
...
{
    wxString accountName = req->GetAccount().GetAccountName();
    gsgs::SSH::Ptr_t ssh(new gsgs::SSH(req->GetAccount().GetHost(), req->GetAccount().GetUsername(),
...
void SFTPWorkerThread::SetSftpPlugin(SFTPPlugin* sftp) { m_plugin = sftp; }
