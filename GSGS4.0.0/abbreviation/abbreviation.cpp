    ...
    , m_config("abbreviations.conf")
{
    ...
    gsgs_Event().Bind(wxEVT_CCBOX_SELECTION_MADE, &AbbreviationPlugin::OnAbbrevSelected, this);
    gsgs_Event().Bind(wxEVT_CCBOX_SHOWING, &AbbreviationPlugin::OnCompletionBoxShowing, this);
...
void AbbreviationPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void AbbreviationPlugin::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_CCBOX_SELECTION_MADE, &AbbreviationPlugin::OnAbbrevSelected, this);
    gsgs_Event().Unbind(wxEVT_CCBOX_SHOWING, &AbbreviationPlugin::OnCompletionBoxShowing, this);
}
...
void AbbreviationPlugin::AddAbbreviations(CodeCompleteEvent& e)
{
    ...
    const wxBitmap & bmp = gsgs_Image().LoadBitmap("replace-blue");
    if(bmp.IsOk()) {
        // search for the old item
        const gsgsStringMap& entries = jsonData.GetEntries();
        std::for_each(entries.begin(), entries.end(), [&](const gsgsStringMap::value_type& vt) {
...
void AbbreviationPlugin::OnAbbrevSelected(CodeCompleteEvent& e)
{
    if(!e.GetEntry()) {
        e.Skip();
        return;
...
void AbbreviationPlugin::InitDefaults()
{
    ...
    gsgs_Window().addHotKey("abbrev_insert", "Ctrl-Alt-SPACE",
...
bool AbbreviationPlugin::InsertExpansion(const wxString& abbreviation)
{
    ...
    // search for the old item
    const gsgsStringMap& entries = jsonData.GetEntries();
    gsgsStringMap::const_iterator iter = entries.find(abbreviation);
...
void AbbreviationPlugin::OnCompletionBoxShowing(CodeCompleteEvent& event)
{
...
void AbbreviationServiceProvider::OnWordComplete(CodeCompleteEvent& event)
{
    event.Skip();
