...
void AbbreviationsSettingsDlg::OnDelete(wxCommandEvent& event)
{
    ...
    if(wxMessageBox(wxString::Format(_("Are you sure you want to delete '%s'"), m_activeItemName.c_str()),
                    _("光栅故事"), wxYES_NO | wxCANCEL | wxCENTER | wxICON_QUESTION, this) != wxYES) {
        return;
    }
...
void AbbreviationsSettingsDlg::DoPopulateItems()
{
    ...
    m_stc->ClearAll();
    gsgsStringMap entries = m_data.GetEntries();
    gsgsStringMap::iterator iter = entries.begin();
...
void AbbreviationsSettingsDlg::DoSelectItem(int item)
{
    if(item >= 0) {
        ...
        gsgsStringMap entries = m_data.GetEntries();
        gsgsStringMap::iterator iter = entries.find(name);
...
void AbbreviationsSettingsDlg::DoSaveCurrent()
{
    ...
    gsgsStringMap entries = m_data.GetEntries();
    gsgsStringMap::iterator iter = entries.find(m_activeItemName);
...
void AbbreviationsSettingsDlg::OnImport(wxCommandEvent& event)
{
    ...
    const gsgsStringMap& newEntries = data.GetEntries();
    const gsgsStringMap& curEntries = m_data.GetEntries();

    gsgsStringMap merged = m_config.MergeStringMaps(newEntries, curEntries);
