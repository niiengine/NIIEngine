    ...
    , m_driver(driver)
{
    ...
    lexer->Apply(m_stc);
    gsgs_Event().Bind(wxEVT_WORKSPACE_CLOSED, &DockerOutputPane::OnWorkspaceClosed, this);

    m_styler.reset(new gsgs::GenericSTCStyler(m_stc));
    {
        wxArrayString words;
    ...
    gsgs::ImageManager* bmps = &gsgs_Image();
...
{
    gsgs_Event().Unbind(wxEVT_WORKSPACE_CLOSED, &DockerOutputPane::OnWorkspaceClosed, this);
}
