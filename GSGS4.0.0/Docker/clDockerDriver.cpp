...
void clDockerDriver::OnProcessOutput(gsgs::ProcessEvent& event)
{
    switch(m_context) {
    case kNone:
        break; //??
...
void clDockerDriver::OnProcessTerminated(gsgs::ProcessEvent& event)
{
...
void clDockerDriver::RemoveContainers(const wxArrayString& ids)
{
    ...
    if(::wxMessageBox(message, "gsgs", wxICON_WARNING | wxYES_NO | wxCANCEL | wxCANCEL_DEFAULT,
...
void clDockerDriver::AttachTerminal(const wxArrayString& names)
{
    ...
    for(size_t i = 0; i < names.size(); ++i) {
        ...
        FileUtil::OpenTerminal(clDockerWorkspace::Get()->GetFileName().GetPath(), command);
    }
}
...
void clDockerDriver::Build(const wxFileName& filepath, const clDockerWorkspaceSettings& settings)
{
    ...
    if(!info) {
        wxMessageBox(wxString() << _("Don't know how to build '") << filepath.GetFullPath() << "'\n"
                     ...
                     "gsgs", wxICON_WARNING | wxOK | wxOK_DEFAULT);
        return;
    }
    ...
    GetManager()->ShowOutputPane(_("Docker"));
...
void clDockerDriver::Run(const wxFileName& filepath, const clDockerWorkspaceSettings& settings)
{
    ...
    command << " " << runOptions;
    FileUtil::OpenTerminal(filepath.GetPath(), command, true);
}
