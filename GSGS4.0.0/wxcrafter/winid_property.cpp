...
gsgsStringSet WinIdProperty::m_winIdSet;
...
void WinIdProperty::UnSerialize(const JSONElement& json)
{
    ...
    if(json.isObjectExist(wxT("m_winid")))
        m_winid = json.getObject(wxT("m_winid")).toString();
    else
        m_winid = json.getObject(wxT("m_value")).toString();
}
