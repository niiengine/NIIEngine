...
bool wxcApp::OnInit()
{
    ...
    gsgs_EditorConfig().SetInstallDir(wxString() << INSTALL_PREFIX << "/share/wxcrafter");
    ...
    gsgs_EditorConfig().SetInstallDir(wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath());
#endif
    gsgs_EditorConfig().Init("", "2.0.2");
    ...
    for(size_t i = 0; i < parser.GetParamCount(); i++) {
        ...
        if(fn.GetExt() == "wxcp") {
            ...
            gsgs_Event().AddPendingEvent(evtOpen);
        }
    }
    ...
    , m_exiting(false)
{
    ...
    gsgs_Event().Bind(wxEVT_NETWORK_COMMAND_EXIT, &MainFrame::OnNetCommandExit, this);
    gsgs_Event().Bind(wxEVT_NETWORK_COMMAND_SHOW_DESIGNER, &MainFrame::OnNetShowDesigner, this);
    gsgs_Event().Bind(wxEVT_NETWORK_COMMAND_LOAD_FILE, &MainFrame::OnNetOpenFile, this);
    gsgs_Event().Bind(wxEVT_NETWORK_COMMAND_CONN_GEN_CODE, &MainFrame::OnNetGenerateCode, this);
    gsgs_Event().Bind(wxEVT_NETWORK_COMMAND_NEW_FORM, &MainFrame::OnNetNewForm, this);

#if STANDALONE_BUILD
    m_bitmaps = new gsgs::ImageManager();
#else
    m_bitmaps = &gsgs_Image();
#endif
    m_mainToolbar->SetMiniToolBar(false);

#if !STANDALONE_BUILD
    m_mainToolbar->AddTool(wxID_BACKWARD, _("Back to gsgs"), m_bitmaps->LoadBitmap("back"));
#endif
...
#if !STANDALONE_BUILD
    m_mainToolbar->Bind(wxEVT_TOOL, &MainFrame::OnSwitchToGSGS, this, wxID_BACKWARD);
    m_mainToolbar->Bind(wxEVT_UPDATE_UI, &MainFrame::OnSwitchToGSGSUI, this, wxID_BACKWARD);
#endif
    ...
    gsgs_Event().Connect(wxEVT_WXC_PROJECT_LOADED, wxCommandEventHandler(MainFrame::OnProjectLoaded), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(MainFrame::OnWorkspaceClosed), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WXGUI_PROJECT_CLOSED, wxCommandEventHandler(MainFrame::OnProjectClosed), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_GSGS_MAINFRAME_GOT_FOCUS,
                                  wxCommandEventHandler(MainFrame::OnGSGSGotFocus), NULL, this);
    gsgs_Event().Connect(wxEVT_WXC_CODE_PREVIEW_PAGE_CHANGED,
...
MainFrame::~MainFrame()
{
    ...
    gsgs_Event().Disconnect(wxEVT_CMD_WXCRAFTER_PROJECT_SYNCHED,
                                     wxCommandEventHandler(MainFrame::OnProjectSynched), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WXC_PROJECT_LOADED, wxCommandEventHandler(MainFrame::OnProjectLoaded), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(MainFrame::OnWorkspaceClosed), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_WXGUI_PROJECT_CLOSED, wxCommandEventHandler(MainFrame::OnProjectClosed),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_GSGS_MAINFRAME_GOT_FOCUS,
                                     wxCommandEventHandler(MainFrame::OnGSGSGotFocus), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WXC_CODE_PREVIEW_PAGE_CHANGED,
                                     wxCommandEventHandler(MainFrame::OnCodeEditorSelected), NULL, this);
...
void MainFrame::OnClose(wxCommandEvent& event)
{
    ...
    gsgs_Event().AddPendingEvent(evtClose);
}
...
void MainFrame::OnBuild(wxCommandEvent& event)
{
    ...
    gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(evtBuild);
...
void MainFrame::OnSwitchToGSGS(wxCommandEvent& event)
{
    if(IsShown()) {
        wxFrame* mainFrame = gsgs_Event().TopFrame();
        if(mainFrame) {
...
void MainFrame::OnSwitchToGSGSUI(wxUpdateUIEvent& event)
{
#if STANDALONE_BUILD
    event.Enable(false);
#else
...
void MainFrame::OnGSGSGotFocus(wxCommandEvent& e) { e.Skip(); }
...
void MainFrame::OnNewProject(wxCommandEvent& event)
{
    ...
    gsgs_Event().ProcessEvent(evtOpen);
}
...
void MainFrame::OnOpenMenu(wxCommandEvent& event)
{
    ...
    wxCommandEvent evtClose(wxEVT_WXC_CLOSE_PROJECT);
    gsgs_Event().ProcessEvent(evtClose);
...
void MainFrame::OnGenerateCode(wxCommandEvent& event)
{
    ...
    gsgs_Event().ProcessEvent(evtGenCode);
}
