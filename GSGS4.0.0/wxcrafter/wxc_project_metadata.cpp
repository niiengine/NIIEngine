...
{
    m_objCounter = json.getObject(wxT("m_objCounter")).toInt();
    m_generatedFilesDir = json.getObject(wxT("m_generatedFilesDir")).toString();
    m_includeFiles = json.getObject(wxT("m_includeFiles")).toArrayString();
    m_bitmapFunction = json.getObject(wxT("m_bitmapFunction")).toString();
    m_bitmapsFile = json.getObject(wxT("m_bitmapsFile")).toString();
    m_GenerateCodeTypes = json.getObject(wxT("m_GenerateCodeTypes")).toInt(wxcGenerateCPPCode);
    m_outputFileName = json.getObject("m_outputFileName").toString();
    m_firstWindowId = json.getObject("m_firstWindowId").toInt(m_firstWindowId);
    m_useEnum = json.getObject("m_useEnum").toBool(true);
    m_useUnderscoreMacro = json.getObject("m_useUnderscoreMacro").toBool(true);
    m_addHandlers = json.getObject("m_addHandlers").toBool(m_addHandlers);

    wxcSettings::Get().MergeCustomControl(json.getObject("m_templateClasses"));
...
void wxcProjectMetadata::Serialize(const wxcWidget::List_t& topLevelsList, const wxFileName& filename)
{
    ...
    root.getObject().append(p.ToJSON());

    // The windows
    JSONElement windows = JSONElement::createArray(wxT("windows"));
    root.getObject().append(windows);
