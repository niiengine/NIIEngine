...
void NewFormWizard::OnSelectVirtualFolder(wxCommandEvent& event)
{
    ...
    if(suggestedName.empty() && &gsgs_CppWorker()) {
        suggestedName = gsgs_CppWorker().GetActiveProjectName();
    }
    VirtualDirectorySelectorDlg selector(this, &gsgs_CppWorker(), suggestedName);
