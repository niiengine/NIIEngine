...
bool ImportFromXrc::ImportProject(ImportDlg::ImportFileData& data) const
{
    ...
    if(!doc.IsOk()) {
        wxMessageBox(_("Failed to load the file to import"), _("光栅故事"), wxICON_ERROR | wxOK, m_Parent);
        return false;
    }
...
bool ImportFromXrc::ParseFile(wxXmlDocument& doc, wxcWidget::List_t& toplevels) const
{
    ...
    while(toplevelnode) {
        ...
        if(tag != wxT("object")) {
            wxMessageBox(_("This doesn't seem to be a valid XRC file. Aborting."), _("光栅故事"), wxICON_ERROR | wxOK,
                         m_Parent);
            return false;
        }
...
wxcWidget* ImportFromXrc::ParseNode(wxXmlNode* node, wxcWidget* parentwrapper, bool& alreadyParented) const
{
    ...
    if(classname == wxT("sizeritem")) {
        ...
        if(!node) {
            ...
            gsgs_Warning(wxT("No object found in a sizeritem"));
            return NULL;
        }
        ...
        if(!node) {
            // I don't think this can happen, but...
            gsgs_Warning(wxString::Format(wxT("No object found in a %s"), classname.c_str()));
            return NULL;
        }
        ...
        node = XmlUtils::FindFirstByTagName(node, wxT("object"));
        if(!node) {
            gsgs_Warning(wxT("No object found in a <button> node"));
            return NULL;
        }
