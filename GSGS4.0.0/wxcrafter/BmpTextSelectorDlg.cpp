...
BmpTextVec_t BmpTextSelectorDlg::FromString(const wxString& text)
{
    BmpTextVec_t vec;
    JSONRoot root(text);
    int size = root.getObject().getArraySize();
    for(int i = 0; i < size; ++i) {
        JSONElement item = root.getObject().getArray(i);
        wxString bitmap = item.getObject("bmp").toString();
        wxString label = item.getObject("label").toString();
...
wxString BmpTextSelectorDlg::ToString(const BmpTextVec_t& vec)
{
    ...
    for(size_t i = 0; i < vec.size(); ++i) {
        ...
        root.getObject().arrayAppend(element);
    }
    wxString asString(root.getObject().getString());
    asString.Replace("\n", "");
    return asString;
}
