...
{
    m_templInfoName = json.getObject(wxT("m_templInfoName")).toString();
    DoUpdateEvents();
...
void CustomControlWrapper::LoadPropertiesFromwxFB(const wxXmlNode* node)
{
    ...
    if(wxclassname.empty() || construction.empty() ||
       declaration.empty()) { // settings and xrcclassname are optional; 'include' probably is too
        gsgs_Warning("Failed to load a Custom Control from wxFB: not all necessary data was available");
        return;
    }
...
void CustomControlWrapper::DoUpdateEvents()
{
    ...
    const gsgsStringMap& events = cct.GetEvents();
    gsgsStringMap::const_iterator iter = events.begin();
