...
// will be initialized in the OnInitDone()
    ...
    gsgs_Event().Bind(wxEVT_SHOW_WORKSPACE_TAB, &wxCrafterPlugin::OnToggleView, this);
    gsgs_Event().Connect(wxEVT_INIT_DONE, wxCommandEventHandler(wxCrafterPlugin::OnInitDone), NULL, this);
    gsgs_Event().Connect(wxEVT_SHOW_WXCRAFTER_DESIGNER, wxCommandEventHandler(wxCrafterPlugin::OnShowDesigner),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_DESIGNER_DELETED, wxCommandEventHandler(wxCrafterPlugin::OnDesignerDelete),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_BITMAP_CODE_GENERATION_DONE,
                                  wxCommandEventHandler(wxCrafterPlugin::OnBitmapCodeGenerationCompleted), NULL, this);
    gsgs_Event().Connect(wxEVT_TREE_ITEM_FILE_ACTIVATED, gsgsEventHandler(wxCrafterPlugin::OnOpenFile),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_NOTIFY_PAGE_CLOSING, wxNotifyEventHandler(wxCrafterPlugin::OnPageClosing), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_CMD_WXCRAFTER_PROJECT_MODIFIED,
                                  wxCommandEventHandler(wxCrafterPlugin::OnProjectModified), NULL, this);
    gsgs_Event().Connect(wxEVT_CMD_WXCRAFTER_PROJECT_SYNCHED,
                                  wxCommandEventHandler(wxCrafterPlugin::OnProjectSynched), NULL, this);
    gsgs_Event().Connect(wxEVT_CMD_PAGE_CHANGED, wxCommandEventHandler(wxCrafterPlugin::OnPageChanged), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_PREVIEW_CTRL_SELECTED,
                                  wxCommandEventHandler(wxCrafterPlugin::OnDesignerItemSelected), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(wxCrafterPlugin::OnWorkspaceClosed),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_BUILD_STARTING, wxCommandEventHandler(wxCrafterPlugin::OnBuildStarting), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_ALL_EDITORS_CLOSING,
                                  wxCommandEventHandler(wxCrafterPlugin::OnAllEditorsClosing), NULL, this);
    gsgs_Event().Connect(wxEVT_ALL_EDITORS_CLOSED, wxCommandEventHandler(wxCrafterPlugin::OnAllEditorsClosed),
                                  NULL, this);
    gsgs_Event().Connect(XRCID("save_file"), wxCommandEventHandler(wxCrafterPlugin::OnSave), NULL, this);
    gsgs_Event().Connect(wxEVT_SAVE_ALL_EDITORS, gsgsEventHandler(wxCrafterPlugin::OnSaveAll), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WXGUI_PROJECT_LOADED, wxCommandEventHandler(wxCrafterPlugin::OnProjectLoaded),
                                  NULL, this);

    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FILE, &wxCrafterPlugin::OnFileContextMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_VIRTUAL_FOLDER, &wxCrafterPlugin::OnVirtualFolderContextMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_PROJECT, &wxCrafterPlugin::OnProjectContextMenu, this);
    gsgs_Event().Bind(wxEVT_PAGE_MODIFIED_UPDATE_UI, &wxCrafterPlugin::OnSaveUI, this);
    // Menu events
    ...
    gsgs_Window().addHotKey("ID_SHOW_DESIGNER", "Ctrl-Shift-F12",
                                                   _("Plugins::wxCrafter::Show the designer"));
#endif
}
...
{
    gsgs_Event().Unbind(wxEVT_PAGE_MODIFIED_UPDATE_UI, &wxCrafterPlugin::OnSaveUI, this);
    gsgs_Event().Unbind(wxEVT_SHOW_WORKSPACE_TAB, &wxCrafterPlugin::OnToggleView, this);
    gsgs_Event().Disconnect(wxEVT_DESIGNER_DELETED, wxCommandEventHandler(wxCrafterPlugin::OnDesignerDelete),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_SHOW_WXCRAFTER_DESIGNER,
                                     wxCommandEventHandler(wxCrafterPlugin::OnShowDesigner), NULL, this);
    gsgs_Event().Disconnect(wxEVT_INIT_DONE, wxCommandEventHandler(wxCrafterPlugin::OnInitDone), NULL, this);
    gsgs_Event().Disconnect(wxEVT_BITMAP_CODE_GENERATION_DONE,
                                     wxCommandEventHandler(wxCrafterPlugin::OnBitmapCodeGenerationCompleted), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_TREE_ITEM_FILE_ACTIVATED, gsgsEventHandler(wxCrafterPlugin::OnOpenFile),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_NOTIFY_PAGE_CLOSING, wxNotifyEventHandler(wxCrafterPlugin::OnPageClosing),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_CMD_WXCRAFTER_PROJECT_MODIFIED,
                                     wxCommandEventHandler(wxCrafterPlugin::OnProjectModified), NULL, this);
    gsgs_Event().Disconnect(wxEVT_CMD_WXCRAFTER_PROJECT_SYNCHED,
                                     wxCommandEventHandler(wxCrafterPlugin::OnProjectSynched), NULL, this);
    gsgs_Event().Disconnect(wxEVT_CMD_PAGE_CHANGED, wxCommandEventHandler(wxCrafterPlugin::OnPageChanged),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_PREVIEW_CTRL_SELECTED,
                                     wxCommandEventHandler(wxCrafterPlugin::OnDesignerItemSelected), NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(wxCrafterPlugin::OnWorkspaceClosed),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_BUILD_STARTING, wxCommandEventHandler(wxCrafterPlugin::OnBuildStarting),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_ALL_EDITORS_CLOSING,
                                     wxCommandEventHandler(wxCrafterPlugin::OnAllEditorsClosing), NULL, this);
    gsgs_Event().Disconnect(wxEVT_ALL_EDITORS_CLOSED,
                                     wxCommandEventHandler(wxCrafterPlugin::OnAllEditorsClosed), NULL, this);
    gsgs_Event().Disconnect(XRCID("save_file"), wxCommandEventHandler(wxCrafterPlugin::OnSave), NULL, this);
    gsgs_Event().Disconnect(wxEVT_SAVE_ALL_EDITORS, gsgsEventHandler(wxCrafterPlugin::OnSaveAll), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_WXGUI_PROJECT_LOADED,
                                     wxCommandEventHandler(wxCrafterPlugin::OnProjectLoaded), NULL, this);

    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FILE, &wxCrafterPlugin::OnFileContextMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_VIRTUAL_FOLDER, &wxCrafterPlugin::OnVirtualFolderContextMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_PROJECT, &wxCrafterPlugin::OnProjectContextMenu, this);
...
void wxCrafterPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void wxCrafterPlugin::OnBitmapCodeGenerationCompleted(wxCommandEvent& e)
{
    if(wxcProjectMetadata::Get().GetGenerateCPPCode()) {
        ...
        const gsgsStringMap& af = wxcProjectMetadata::Get().GetAdditionalFiles();
        gsgsStringMap::const_iterator itr = af.begin();
        for(; itr != af.end(); ++itr) {
        ...
        wxString errMsg;
        ProjectPtr project = gsgs_CppWorker().FindProjectByName(projectName, errMsg);
...
// post event here to codelite to retag the workspace
    ...
    gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(eventRetag);
#endif
}
...
wxMenu* wxCrafterPlugin::DoCreateFolderMenu()
{
    ...
    item->SetBitmap(bmps.Bitmap(wxT("new-form")));
    menu->Append(item);
...
void wxCrafterPlugin::DoGenerateCode(const NewFormDetails& fd)
{
    ...
    if(!wxcpFile.FileExists()) {
        wxFFile fp(wxcpFile.GetFullPath(), wxT("w+b"));
        if(fp.IsOpened()) {
            fp.Close();

        } else {
            wxString msg;
            msg << _("Could not create resource file '") << wxcpFile.GetFullPath() << wxT("'");
    ...
    if(IsTabMode()) {
        ...
        gsgs_Event().AddPendingEvent(evtShowDesigner);
    }
}
...
bool wxCrafterPlugin::DoCreateVirtualFolder(const wxString& vdFullPath)
{
    ...
    wxString name = vdFullPath.AfterLast(wxT(':'));
    wxString parent = vdFullPath.BeforeLast(wxT(':'));
    return m_mgr->CreateVirtualDirectory(parent, name);
}

void wxCrafterPlugin::OnOpenFile(gsgs::Event& e)
{
    e.Skip();
...
bool wxCrafterPlugin::DoReadFileContentAndPrompt(const wxFileName& fn, wxString& content, IEditor** editor)
{
    *editor = NULL;
    ...
    if(!wxCrafter::ReadFileContent(fn.GetFullPath(), content)) {
        ...
        ::wxMessageBox(msg, _("wxCrafter"), wxICON_WARNING | wxOK | wxCENTER, gsgs_Event().TopFrame());
        return false;
    }
    return true;
}
...
void wxCrafterPlugin::OnDebugStarting(gsgs::DebugEvent& e)
{
    if(!m_mainPanel) {
        e.Skip();
        return;
...
void wxCrafterPlugin::DoInitDone(wxObject* obj)
{
    wxUnusedVar(obj);
#if !STANDALONE_BUILD
    wxToolBar* mainToolbar = gsgs_Event().TopFrame()->GetToolBar();
    if(mainToolbar) {
    ...
    if(m_useFrame) {
        m_mainFrame = new MainFrame(gsgs_Event().TopFrame(), m_serverMode);
...
void wxCrafterPlugin::OnSaveAll(gsgs::Event& e)
{
    e.Skip();
    if(wxcProjectMetadata::Get().IsLoaded()) { m_treeView->SaveProject(); }
}

void wxCrafterPlugin::OnFileContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void wxCrafterPlugin::OnVirtualFolderContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void wxCrafterPlugin::OnProjectContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void wxCrafterPlugin::OnReGenerateForProject(wxCommandEvent& e)
{
    ...
    if(GetManager()->GetWorkspace() && GetManager()->GetWorkspace()->IsOpen()) {
        gsgsStringSet all_files;
        wxArrayString projects;
        ProjectPtr activeProject = GetManager()->GetSelectedProject();
        ...
        std::for_each(all_files.begin(), all_files.end(), [&](const wxString& file) {
            if(FileUtil::GetType(file) == FileUtil::TypeWxCrafter) { wxcpFiles.Add(file); }
        });
...
void wxCrafterPlugin::OnToggleView(gsgs::Event& event)
{
...
void wxCrafterPlugin::OnSaveUI(gsgs::Event& event)
{
    event.Skip();
