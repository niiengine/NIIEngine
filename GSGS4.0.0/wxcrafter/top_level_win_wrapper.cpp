    ...
    , m_auiDropDownMenuHelperRegistered(false)
{
    ...
    AddProperty(
        new VirtualFolderProperty(PROP_VIRTUAL_FOLDER, "", _("gsgs's virtual folder for the generated files")));
                                      ...
                                      gsgsStringMap& additionalFiles)
{
    ...
    if(baseClassName.IsEmpty()) { // meaning that it was empty until BASE_CLASS_SUFFIX was appended
        wxMessageBox(wxString("Can not generate code.\nMake sure that all toplevel windows have a valid C++ class name"), wxT("wxCrafter"), wxOK | wxICON_WARNING | wxCENTER);
        return;
    }
    ...
    // Tell gsgs to reload externall modified files
