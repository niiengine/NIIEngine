...
Allocator* Allocator::gInstance = 0;
...
{
    if(gInstance == 0) { gInstance = new Allocator(); }
    return gInstance;
}
...
wxcWidget* Allocator::CreateWrapperFromJSON(const JSONElement& json)
{
    ...
    int type = json.getObject(wxT("m_type")).toInt();
    wrapper = Create(type);
...
int Allocator::GetInsertionType(int controlId, int targetControlId, bool allowPrompt, wxcWidget* selectedWidget) const
{
    ...
    if(type == INSERT_PROMPT_CHILD_OR_SIBLING && allowPrompt) {
        ...
        int res =
            ::PromptMessageDialogWithoutCancel(_("This item can be placed either as a sibling or as a child of the "
