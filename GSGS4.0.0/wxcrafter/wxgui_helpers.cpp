...
wxString wxCrafter::FBToFontstring(const wxString& FBstr)
{
    ...
    if(arr.GetCount() < 6) {
        gsgs_Warning(wxT("Badly-formed wxFB font"));
        return str;
    }
...
void wxCrafter::GetWorkspaceFiles(gsgsStringSet& files)
{
    wxArrayString arrFiles;
    gsgs_CppWorker().GetWorkspaceFiles(arrFiles);
    for(size_t i = 0; i < arrFiles.GetCount(); ++i) {
        files.insert(arrFiles.Item(i));
    }
}

void wxCrafter::GetProjectFiles(const wxString& projectName, gsgsStringSet& files)
{
...
{
#if !STANDALONE_BUILD
    gsgs::SourceFormatEvent event(wxEVT_FORMAT_FILE);
    event.SetFileName(filename.GetFullPath());
    gsgs_Event().ProcessEvent(event);
#endif
}
