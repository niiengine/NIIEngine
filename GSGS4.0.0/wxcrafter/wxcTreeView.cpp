    ...
    , m_loadingProject(false)
{
    ...
    gsgs_Event().Connect(wxEVT_WXGUI_PROJECT_SAVED, wxCommandEventHandler(wxcTreeView::OnProjectSaved), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WXGUI_PROJECT_CLOSED, wxCommandEventHandler(wxcTreeView::OnProjectClosed), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(wxcTreeView::OnWorkspaceLoaded), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(wxcTreeView::OnWorkspaceClosed), NULL,
                                  this);
}
...
wxcTreeView::~wxcTreeView()
{
    ...
    gsgs_Event().Disconnect(wxEVT_WXGUI_PROJECT_SAVED, wxCommandEventHandler(wxcTreeView::OnProjectSaved),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WXGUI_PROJECT_CLOSED, wxCommandEventHandler(wxcTreeView::OnProjectClosed),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(wxcTreeView::OnWorkspaceLoaded),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(wxcTreeView::OnWorkspaceClosed),
                                     NULL, this);
}
...
void wxcTreeView::LoadProject(const wxFileName& filname)
{
    ...
    gsgs_Event().ProcessEvent(showDesignerEvent);
...
void wxcTreeView::SaveProject()
{
    ...
    gsgs_Event().ProcessEvent(evtSave);
}
...
void wxcTreeView::DoRefreshFileList(bool reloadFileList)
{
    ...
    if(m_fileList.IsEmpty() || reloadFileList) {
        ...
        gsgs_CppWorker().GetWorkspaceFiles(allFiles);

        wxArrayString options;
        wxFileName wspfile = gsgs_CppWorker().GetFileName();
        for(size_t i = 0; i < allFiles.GetCount(); ++i) {
            if(FileUtil::GetType(allFiles.Item(i)) == FileUtil::TypeWxCrafter) {
...
void wxcTreeView::OnWorkspaceOpenUI(wxUpdateUIEvent& event) { event.Enable(gsgs_CppWorker().IsOpen()); }
