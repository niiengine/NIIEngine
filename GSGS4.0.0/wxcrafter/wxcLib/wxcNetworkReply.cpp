...
{
    m_replyType = (wxCrafter::eReplyType) json.getObject(wxT("m_replyType")).toInt(wxCrafter::kCommandTypeInvalid);
    m_wxcpFile = json.getObject(wxT("m_wxcpFile")).toString();
    
    m_files.clear();
    JSONElement arr = json.getObject(wxT("m_files"));
    for(int i=0; i<arr.getArraySize(); ++i) {
        m_files.push_back( arr.getArray(i).toString() );
    }
}
