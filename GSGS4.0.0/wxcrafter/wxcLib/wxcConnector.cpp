...
#include "wxcLib/gsgsSocketClient.h"
...
void wxcConnector::DoLaunchAndConnect() 
{
    ...
    if ( pid > 0 ) {
        ...
        if ( nRetries <= 0 ) {
            ...
            throw clSocketException1( message );
        }
...
{
    if ( !m_socket ) {
        throw clSocketException1(wxT("socket not connected!"));
    }

    // Serialize the command
    wxString serializedCommand = command.ToJSON().getString();
