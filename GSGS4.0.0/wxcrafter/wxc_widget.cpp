...
void wxcWidget::DoTraverseAndGenCode(wxArrayString& headers, wxString& ctorCode, wxString& members,
                                     ...
                                     gsgsStringMap& additionalFiles, wxString& dtorCode, wxString& extraFunctionsImpl,
...
{
    m_sizerItem.SetBorder(json.getObject(wxT("border")).toInt(5));
    m_sizerItem.SetProportion(json.getObject(wxT("proportion")).toInt(0));
    m_gbSpan = json.getObject(wxT("gbSpan")).toString();
    m_gbPos = json.getObject(wxT("gbPosition")).toString();

    m_auiPaneInfo.FromJSON(json.getObject("wxAuiPaneInfo"));
    ...
    JSONElement styles = json.getObject(wxT("m_styles"));
    int nCount = styles.getArraySize();
    for(int i = 0; i < nCount; i++) {
        wxString styleName = styles.getArray(i).toString();
        EnableStyle(styleName, true);
    }

    JSONElement sizerFlags = json.getObject(wxT("m_sizerFlags"));
    nCount = sizerFlags.getArraySize();
    for(int i = 0; i < nCount; i++) {
        wxString styleName = sizerFlags.getArray(i).toString();
        EnableSizerFlag(styleName, true);
    }

    // Unserialize the properties
    JSONElement properties = json.getObject(wxT("m_properties"));
    nCount = properties.getArraySize();
    for(int i = 0; i < nCount; i++) {
        JSONElement jsonProp = properties.getArray(i);
        wxString propLabel = jsonProp.getObject(wxT("m_label")).toString();
        if(m_properties.Contains(propLabel)) { m_properties.Item(propLabel)->UnSerialize(jsonProp); }
    }

    // Unserialize the events
    JSONElement events = json.getObject(wxT("m_events"));
    nCount = events.getArraySize();
    for(int i = 0; i < nCount; i++) {
        JSONElement jsonEvent = events.getArray(i);
    ...
    JSONElement children = json.getObject(wxT("m_children"));
    int nChildren = children.getArraySize();
    for(int i = 0; i < nChildren; i++) {
        JSONElement child = children.getArray(i);
        ...
        if(!eventtype.empty()) {
            ...
            if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
                ...
                if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
                    ...
                    gsgs_Warning(wxString::Format("No wxCrafter equivalent for XRCed event: %s", eventname));
                }
            }
        }
    }
    ...
    if(!eventname.empty()) {
        ...
        if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
            ...
            if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
                ...
                gsgs_Warning(wxString::Format("No wxCrafter equivalent for wxSmith event: %s", eventname));
            }
        }
    }
}
    ...
    if(!eventname.empty()) {
        ...
        if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
            ...
            if(edb.Exists(wxXmlResource::GetXRCID(eventtype))) {
                ...
                gsgs_Warning(wxString::Format("No wxCrafter equivalent for wxFormBuilder event: %s", eventname));
            }
        }
    }
}
...
void wxcWidget::GenerateAdditionalFiles(gsgsStringMap& additionalFiles) const { wxUnusedVar(additionalFiles); }
