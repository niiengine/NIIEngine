...
void wxcSettings::Load()
{
    ...
    if(root.isValid()) {
        ...
        m_flags = root.getObject().getObject(wxT("m_annoyDialogs")).toInt(m_flags);
        m_flags &= ~USE_TABBED_MODE;
        m_sashPosition = root.getObject().getObject(wxT("m_sashPosition")).toInt(150);
        m_secondarySashPos = root.getObject().getObject(wxT("m_secondarySashPos")).toInt(-1);
        m_treeviewSashPos = root.getObject().getObject(wxT("m_treeviewSashPos")).toInt(-1);
        m_username = root.getObject().getObject("username").toString();
        m_serialNumber = root.getObject().getObject("serialNumber").toString();
        m_history = root.getObject().getObject("recentFiles").toArrayString();
        m_ts = root.getObject().getObject("activationCode").toInt(nowT);

        JSONElement arr = root.getObject().getObject(wxT("m_templateClasses"));
        m_templateClasses.clear();
        for(int i = 0; i < arr.getArraySize(); ++i) {
            CustomControlTemplate templateControl;
            templateControl.FromJSON(arr.getArray(i));
...
void wxcSettings::Save()
{
    ...
    m_flags &= ~USE_TABBED_MODE;
    root.getObject().addProperty(wxT("m_annoyDialogs"), (int)m_flags);
    root.getObject().addProperty(wxT("m_sashPosition"), m_sashPosition);
    root.getObject().addProperty(wxT("m_secondarySashPos"), m_secondarySashPos);
    root.getObject().addProperty(wxT("m_treeviewSashPos"), m_treeviewSashPos);
    root.getObject().addProperty("serialNumber", m_serialNumber);
    root.getObject().addProperty("username", m_username);
    root.getObject().addProperty("recentFiles", m_history);
    root.getObject().addProperty("activationCode", (int)m_ts);

    JSONElement arr = JSONElement::createArray(wxT("m_templateClasses"));
    root.getObject().append(arr);
...
#else
    // gsgs's plugin, nothing to be done here
    return;
#endif
}
...
{
    m_includeFile = json.getObject("m_includeFile").toString();
    m_allocationLine = json.getObject("m_allocationLine").toString();
    m_className = json.getObject("m_className").toString();
    m_xrcPreviewClass = json.getObject("m_xrcPreviewClass").toString();
    m_events = json.getObject("m_events").getStringMap();
}
