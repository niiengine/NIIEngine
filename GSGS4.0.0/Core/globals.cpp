...
namespace gsgs
{
    ...
    // --------------------------------------------------------
    class InternalEventHandlerData : public wxClientData
    {
        wxObject* m_this;
        EventFunc_t m_funcPtr;
        wxClientData* m_arg;

    public:
        InternalEventHandlerData(wxObject* instance, EventFunc_t func, wxClientData* arg)
            : m_this(instance)
            , m_funcPtr(func)
            , m_arg(arg)
        {
        ...
        InternalEventHandlerData(wxObject* instance, EventFunc_t func)
            : m_this(instance)
            , m_funcPtr(func)
            , m_arg(NULL)
        {
        ...
        virtual ~InternalEventHandlerData() { wxDELETE(m_arg); }

        wxClientData* GetArg() const { return m_arg; }
        EventFunc_t GetFuncPtr() const { return m_funcPtr; }
        wxObject* GetThis() { return m_this; }
    };

    class InternalEventHandler : public wxEvtHandler
    {
    public:
        InternalEventHandler()
        {
            gsgs_Event().Connect(wxEVT_COMMAND_CL_INTERNAL_0_ARGS,
                                          wxCommandEventHandler(InternalEventHandler::OnInternalEvent0), NULL, this);
            gsgs_Event().Connect(wxEVT_COMMAND_CL_INTERNAL_1_ARGS,
                                          wxCommandEventHandler(InternalEventHandler::OnInternalEvent1), NULL, this);
        }

        virtual ~InternalEventHandler()
        {
            gsgs_Event().Disconnect(wxEVT_COMMAND_CL_INTERNAL_0_ARGS,
                                             wxCommandEventHandler(InternalEventHandler::OnInternalEvent0), NULL, this);
            gsgs_Event().Disconnect(wxEVT_COMMAND_CL_INTERNAL_1_ARGS,
                                             wxCommandEventHandler(InternalEventHandler::OnInternalEvent1), NULL, this);
        }
        ...
        {
            InternalEventHandlerData* cd = reinterpret_cast<InternalEventHandlerData*>(e.GetClientObject());
            if(cd) {
                wxObject* obj = cd->GetThis();
                wxClientData* arg = cd->GetArg();
                EventFunc_t func = cd->GetFuncPtr();
                (obj->*func)(arg);
    ...
    InternalEventHandler EventHandlerHelper;
    //--------------------------------------------------------------------------
    class GetTextFromUserDialog : public GetTextFromUserBaseDialog
    {
    public:
        GetTextFromUserDialog::GetTextFromUserDialog(
            wxWindow* parent, const wxString& title, const wxString& message, const wxString& initialValue, int charsToSelect)
            : GetTextFromUserBaseDialog(parent)
        {
            SetTitle(title);
            ...
            CenterOnParent();
            SetName("GetTextFromUserDialog");
        }

        virtual ~GetTextFromUserDialog(){}
    ...
    static wxString DoExpandAllVariables(const wxString& expression, CxxWS* workspace, const wxString& projectName,
    ...
    { 
        return gsgs_Event().SendCommandEvent(eventId, clientData); 
    }
    //--------------------------------------------------------------------------
    bool SendCmdEvent(int eventId, void* clientData, const wxString& str)
    {
        return gsgs_Event().SendCommandEvent(eventId, clientData, str);
    }
    //--------------------------------------------------------------------------
    void PostCmdEvent(int eventId, void* clientData) 
    { 
        gsgs_Event().PostCommandEvent(eventId, clientData); 
    }
    //--------------------------------------------------------------------------
    ...
    bool ReadFileWithConversion(const wxString& fileName, wxString& content, wxFontEncoding encoding, BOM* bom)
    {
        ...
        if(file.IsOpened()) {
            ...
            if(encoding == wxFONTENCODING_DEFAULT) encoding = gsgs_EditorConfig().GetOptions()->GetFileFontEncoding();
    ...
    wxString ExpandVariables(const wxString& expression, ProjectPtr proj, IEditor* editor, const wxString& filename)
    {
        ...
        return ExpandAllVariables(expression, &gsgs_CppWorker(), project_name, wxEmptyString, file);
    }
    //--------------------------------------------------------------------------
    // This functions accepts expression and expand all variables in it
    wxString ExpandAllVariables(const wxString& expression, CxxWS* workspace, const wxString& projectName,
        ...
        for(size_t i = 0; i < expression.Length(); i++) {
            if(expression.GetChar(i) == wxT('`')) {
                ...
                if(!found) {
                    ...
                    gsgsLogMessage(wxT("Syntax error in expression: ") + expression + wxT(": expecting '`'"));
                    return expression;
                } else {
    ...
    //--------------------------------------------------------------------------
    wxString DoExpandAllVariables(const wxString& expression, CxxWS* workspace, const wxString& projectName,
                                  const wxString& confToBuild, const wxString& fileName)
    {
        wxString errMsg;
        ...
        while((retries < 5) && MacroManager::Instance()->FindVariable(output, dummyname, dummyname)) {
            ...
            if(workspace) {
                ...
                if(proj) {
                    ...
                    output.Replace(wxT("$(WorkspacePath)"), workspace->GetFileName().GetPath());
                    ...
                    if(bldConf) {
                        ...
                        id.Replace(wxT("$(WorkspacePath)"), workspace->GetFileName().GetPath());
            ...
            if(workspace) { output.Replace(wxT("$(gsgsPath)"), gsgs_Path().GetStartupDirectory()); }

            // call the environment & workspace variables expand function
            output = gsgs_EditorConfig().ExpandVariables(output, true);
        }
        return output;
    }
    //--------------------------------------------------------------------------
    ...
    bool WriteFileWithBackup(const wxString& file_name, const wxString& content, bool backup)
    {
        if(backup) {
            ...
            if(!wxCopyFile(file_name, backup_name, true)) {
                gsgsLogMessage(wxString::Format(wxT("Failed to backup file %s, skipping it"), file_name.c_str()));
                return false;
            }
        }
        ...
        wxCSConv fontEncConv(gsgs_EditorConfig().GetOptions()->GetFileFontEncoding());
    ...
    { 
        wxColour re; 
        gsgs_Style().LightColour(color,re, level); 
        return re; 
    }
    //--------------------------------------------------------------------------
    ...
    bool IsMSYSEnv()
    {
        ...
        if(firstTime) {
            firstTime = false;
            gsgs_Debug("Testing for MSYS environment...uname -a");
    ...
    bool IsCygwinEnv()
    {
        ...
        if(firstTime) {
            firstTime = false;
            gsgs_Debug("Testing for CYGWIN environment...uname -s");
    ...
    {
        static wxString squashedname;
        if(squashedname.IsEmpty()) 
        {
            ...
            if(squashedname.IsEmpty()) 
                squashedname << wxDateTime::Now().GetTicks();
        }
        return squashedname;
    }
    //--------------------------------------------------------------------------
            ...
            if(files.GetCount() > 0) {
                for(size_t i = 0; i < files.GetCount(); i++) {
                    ...
                    if(!proj->Load(files.Item(i))) {
                        // corrupted xml file?
                        gsgsLogMessage(wxT("Failed to load template project: ") + files.Item(i) + wxT(" (corrupted XML?)"));
                        continue;
                    }
                    list.push_back(proj);
    ...
    {
        DoReadProjectTemplatesFromFolder(gsgs_Path().GetProjectTemplatesDir(), list);
        DoReadProjectTemplatesFromFolder(gsgs_Path().GetUserProjectTemplatesDir(), list, false);
    ...
    wxString wxShellExec(const wxString& cmd, const wxString& projectName)
    {
        ...
        theCommand = gsgs_EditorConfig().ExpandVariables(theCommand, false);
    ...
    wxFileName wxReadLink(const wxFileName& filename)
    {
        ...
        if(wxIsFileSymlink(filename)) {
            ...
            return wxFileName(RealPath(filename.GetFullPath()));
    ...
    //--------------------------------------------------------------------------
    wxString RealPath(const wxString& filepath) // This is readlink on steroids: it also makes-absolute, and dereferences
    ...
    EventDisabler::EventDisabler() { gsgs_Event().DisableEvents(true); }
    //--------------------------------------------------------------------------
    EventDisabler::~EventDisabler() { gsgs_Event().DisableEvents(false); }
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    void PostCall(wxObject* instance, EventFunc_t func, wxClientData* arg)
    {
        InternalEventHandlerData* cd = new InternalEventHandlerData(instance, func, arg);
        wxCommandEvent evt(wxEVT_COMMAND_CL_INTERNAL_1_ARGS);
        evt.SetClientObject(cd);
        gsgs_Event().AddPendingEvent(evt);
    }
    //--------------------------------------------------------------------------
    void PostCall(wxObject* instance, EventFunc_t func)
    {
        InternalEventHandlerData* cd = new InternalEventHandlerData(instance, func);
    ...
    //--------------------------------------------------------------------------
    wxStandardID PromptMessageDialog(const wxString& message, const wxString& dlgId,
    ...
    {
        int res = gsgs_Config().GetAnnoyingDlgAnswer(dlgId, wxNOT_FOUND);
        if(res == wxNOT_FOUND) {

            // User did not save his answer
            wxRichMessageDialog d(gsgs_Event().TopFrame(), message, "光栅故事", style);
            ...
            if(d.IsCheckBoxChecked() && (res != wxID_CANCEL)) {
                ...
                gsgs_Config().SetAnnoyingDlgAnswer(dlgId, res, false);
            }
        }
        return static_cast<wxStandardID>(res);
    }
    //--------------------------------------------------------------------------
    wxStandardID PromptMessageDialogWithoutCancel(const wxString& message, const wxString& dlgId, const wxString& yesLabel,
                                                  const wxString& noLabel, const wxString& checkboxLabel, long style,
                                                  bool checkboxInitialValue)
    {
        return PromptMessageDialog(message, dlgId, yesLabel, noLabel, "", checkboxLabel, style,
    ...
    {
        grid->SetPropertyAttributeAll(wxPG_BOOL_USE_CHECKBOX, wxVariant(true));
    }
    //--------------------------------------------------------------------------
    void CalcHScrollBar(wxStyledTextCtrl* ctrl)
    {
    ...
    //--------------------------------------------------------------------------
    wxString GetUserText(const wxString& title, const wxString& message, const wxString& initialValue,
                               int charsToSelect, wxWindow* parent)
    {
        GetTextFromUserDialog dialog(parent == NULL ? gsgs_Event().TopFrame() : parent, title, message,
    ...
    //--------------------------------------------------------------------------
    void SetManager(IManager* manager) { s_pluginManager = manager; }
    //--------------------------------------------------------------------------
    void StripTerminalColouring(const wxString& buffer, wxString& modbuffer)
    {
        StringUtils::StripTerminalColouring(buffer, modbuffer);
    }
    //--------------------------------------------------------------------------
    bool IsVaidProjectName(const wxString& name)
    {
        return name.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-") == wxString::npos;
    }
    //--------------------------------------------------------------------------
    double gsgsGetContentScaleFactor()
    {
    ...
    {
        if(gsgsGetContentScaleFactor() >= 1.5) {
            return size * 2;
        } else {
            return size;
        }
    ...
    //--------------------------------------------------------------------------
    void Kill(int processID, wxSignal signo, bool kill_whole_group, bool as_superuser)
    {
    #ifdef __WXMSW__
    ...
    //--------------------------------------------------------------------------
    void SetEditorFontEncoding(const wxString& encoding)
    {
    ...
    //--------------------------------------------------------------------------
    bool FindExec(const wxString& name, wxFileName& exepath, const wxArrayString& hint)
    {
        wxString path;
        if(!::wxGetEnv("PATH", &path)) {
            gsgsWARNING() << "Could not read environment variable PATH" << clEndl;
            return false;
        }
    ...
    //--------------------------------------------------------------------------
    int FindMenuItemPosition(wxMenu* menu, int menuItemId)
    {
    ...
    { 
        return FileUtil::NextWord(str, offset, word); 
    }
    //--------------------------------------------------------------------------
    wxString JoinLinesWithEOL(const wxArrayString& lines, int eol)
    {
        wxString glue = "\n";
        switch(eol) {
        case wxSTC_EOL_CRLF:
    ...
    {
        // Calculate the display size only once. If the user changes the display size, he will need to restart gsgs
    ...
    //--------------------------------------------------------------------------
    void FitColumnWidth(wxDataViewCtrl* ctrl)
    {
    #ifndef __WXOSX__
    ...
    {
        DataViewTextBitmap tb(text, imgIndex);
        wxVariant vr;
        vr << tb;
        return vr;
    }
    ...
    {
        DataViewCheckbox cb(checked, imgIndex, label);
        wxVariant vr;
        vr << cb;
        return vr;
    }
    //--------------------------------------------------------------------------
    void SetTLWindowBestSizeAndPosition(wxWindow* win)
    {
    ...
    //--------------------------------------------------------------------------
    void SetDialogBestSizeAndPosition(wxDialog* win)
    {
        if (win)
        {
            WindowManager::Load(win);
