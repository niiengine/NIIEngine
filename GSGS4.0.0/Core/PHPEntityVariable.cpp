...
void PHPEntityVariable::FromJSON(const JSONObject& json)
{
    BaseFromJSON(json);
    json.getObject("type-hint").getValue(m_typeHint);
    json.getObject("expr-hint").getValue(m_expressionHint);
    json.getObject("defaultValue").getValue(m_defaultValue);
}

JSONObject PHPEntityVariable::ToJSON() const
{
    JSONObject json = BaseToJSON("v"); // type variable
