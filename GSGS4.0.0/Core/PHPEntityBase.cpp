...
JSONObject PHPEntityBase::BaseToJSON(const wxString& entityType) const
{
...
void PHPEntityBase::BaseFromJSON(const JSONObject& json)
{
    wxString temp;
    json.getObject("file").getValue(temp);
    m_filename = temp;
    json.getObject("name").getValue(m_shortName);
    json.getObject("fullname").getValue(m_fullname);
    json.getObject("doc").getValue(m_docComment);
    json.getObject("line").getValue(m_line, 0);
    json.getObject("col").getValue(m_column, 0);
