 ...
 * bison.exe -d -l -pjson_ -o gsgsJSONObjectParser.cpp gsgsJSONObjectParser.y
 */
%{
#define YYTOKENTYPE
...
static void ParseJS_ReadUntil(void *scanner, int until);


struct JSONParserData {
    JSLookUpTable::Ptr_t lookup;
    JSObject::Vec_t objStack;
};

static void ParseJS_FinalizeObject(JSONParserData* parserData, const wxString& objType);
#define PARSER_DATA() static_cast<JSONParserData*>(static_cast<JSLexerUserData*>(jsLexerGetUserData(scanner))->parserData)
...
program:
    | literalObject {
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, parserData->lookup->GenerateNewType());
    }
    | literalArray {
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "Array");
    }
    | number { 
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "Number");
    }
    | kJS_STRING { 
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "String");
    }
    | kJS_TRUE { 
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "Boolean");
    }
    | kJS_FALSE { 
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "Boolean");
...
literalArray: '[' { ParseJS_ReadUntil(scanner, ']'); } ']'
    ;
...
keyValue: /* empty */
    ...
    | propertyKey ':' kJS_NULL {
        JSONParserData *parserData = PARSER_DATA();
        ParseJS_FinalizeObject(parserData, "null");
    }
...
// Public API
//--------------------------
JSObject::Ptr_t ParseJSVariable(const wxString& content, JSLookUpTable::Ptr_t lookup)
{
    void* scanner = ::jsLexerNew(content);
    
    // set the parser data
    JSLexerUserData *lexerData = ::jsLexerGetUserData(scanner);
    JSONParserData *parserData = new JSONParserData;
