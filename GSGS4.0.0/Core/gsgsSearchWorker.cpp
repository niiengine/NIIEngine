...
#if !wxUSE_GUI
#include "gsgsEvent.h" // Needed for the definition of wxCommandEvent
#endif
...
namespace gsgs
{
    ...
    void SearchThread::DoSearchFile(const wxString& fileName, const SearchData* data)
    {
        ...
        size_t size = FileUtil::GetFileSize(fileName);
        if(size == 0) { return; }
        wxString fileData;
        fileData.Alloc(size);
        ...
        if(!FileUtil::ReadFileContent(fileName, fileData, fontEncConv)) {
            m_summary.GetFailedFiles().Add(fileName);
            return;
        }
    #else
        if(!FileUtil::ReadFileContent(fileName, fileData, wxConvLibc)) {
            m_summary.GetFailedFiles().Add(fileName);
            return;
        }
    #endif
                                      ...
                                      const wxString& fileName, const SearchData* data, TextStatesPtr statesPtr)
    {
        ...
        if(re.IsValid()) {
            while(re.Matches(modLine)) {
                ...
                iCorrectedCol = FileUtil::UTF8Length(line.c_str(), col);
                iCorrectedLen = FileUtil::UTF8Length(line.c_str(), col + len) - iCorrectedCol;
                SearchResult result;
                                    ...
                                    TextStatesPtr statesPtr)
    {
        ...
        while(pos != wxNOT_FOUND) {
            ...
            if(pos != wxNOT_FOUND) {
                ...
                // correct search Pos and Length owing to non plain ASCII multibyte characters
                iCorrectedCol = FileUtil::UTF8Length(line.c_str(), col);
                iCorrectedLen = FileUtil::UTF8Length(findWhat.c_str(), findWhat.Length());
                SearchResult result;
                result.SetPosition(lineOffset + col);
    ...
    void SearchThread::FilterFiles(wxArrayString& files, const SearchData* data)
    {
        ...
        std::for_each(files.begin(), files.end(), [&](const wxString& filename) {
            ...
            if(FileUtil::WildMatch(mask, filename) && !FileUtil::WildMatch(excludePatterns, filename)) {
    ...
    JSONObject SearchResult::ToJSON() const
    {
    ...
    void SearchResult::FromJSON(const JSONObject& json)
    {
        json.getObject("pos").getValue(m_position, m_position);
        json.getObject("col").getValue(m_column, m_column);
        json.getObject("line").getValue(m_lineNumber, m_lineNumber);
        json.getObject("pattern").getValue(m_pattern, m_pattern);
        json.getObject("file").getValue(m_fileName, m_fileName);
        json.getObject("len").getValue(m_len, m_len);
        json.getObject("flags").getValue(m_flags, m_flags);
        json.getObject("columnInChars").getValue(m_columnInChars, m_columnInChars);
        json.getObject("lenInChars").getValue(m_lenInChars, m_lenInChars);
        // m_findWhat = json.getObject("findWhat").toString(m_findWhat);
        // m_matchState = json.getObject("matchState").toInt(m_matchState);
        // m_scope = json.getObject("scope").toString(m_scope);
    }

    JSONObject SearchSummary::ToJSON() const
    {
        JSONObject json = JSONObject::createObject();
    ...
    void SearchSummary::FromJSON(const JSONObject& json)
    {
        json.getObject("filesScanned").getValue(m_fileScanned, m_fileScanned);
        json.getObject("matchesFound").getValue(m_matchesFound,m_matchesFound);
        json.getObject("elapsed").getValue(m_elapsed, m_elapsed);
        json.getObject("failedFiles").getValue(m_failedFiles);
        json.getObject("findWhat").getValue(m_findWhat);
        json.getObject("replaceWith").getValue(m_replaceWith);
    }
}