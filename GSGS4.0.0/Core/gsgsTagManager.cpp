...
#ifdef __WXMSW__
#define PIPE_NAME "\\\\.\\pipe\\gsgs_indexer_%s"
#else
#define PIPE_NAME "/tmp/gsgs_indexer.%s.sock"
#endif 
...
namespace gsgs
{
    ...
    TagsManager::TagsManager()
        : wxEvtHandler()
        , m_IndexerPath(wxT("gsgs_indexer"))
        , m_IndexerProcess(NULL)
        ...
        , m_encoding(wxFONTENCODING_DEFAULT)
    {
        ...
        m_symbolsCache.reset(new CxxFileCacheSymbols());
    }
    ...
    #ifndef __WXMSW__
            m_IndexerProcess->Terminate();
    #endif
            delete m_IndexerProcess;
    ...
    TagTreePtr TagsManager::ParseSourceFile(const wxFileName& fp, std::vector<CommentPtr>* comments)
    {
        ...
        if(!m_IndexerProcess) {
            gsgsWARNING() << "Indexer process is not running..." << clEndl;
    ...
    TagTreePtr TagsManager::Load(const wxFileName& fileName, TagEntryPtrList* tags)
    {
        TagTreePtr tree;
        TagEntryPtrList tagsByFile;
    ...
    void TagsManager::StartIndexer()
    {
        ...
        if(m_IndexerPath.FileExists() == false) {
            gsgs_Error(wxT("ERROR: Could not locate indexer: %s"), m_IndexerPath.GetFullPath().c_str());
            m_IndexerProcess = NULL;
            return;
        }

        // concatenate the PID to identifies this channel to this instance of gsgs
        cmd << wxT("\"") << m_IndexerPath.GetFullPath() << wxT("\" ") << uid << wxT(" --pid");
        m_IndexerProcess =
            CreateAsyncProcess(this, cmd, IProcessCreateDefault, gsgs_Path().GetUserDataDir());
    }

    void TagsManager::RestartIndexer()
    {
        if(m_IndexerProcess) { m_IndexerProcess->Terminate(); }

        // no need to call StartIndexer(), since it will be called automatically
        // by the termination handler
    }

    void TagsManager::SetIndexerPath(const wxString& path) { m_IndexerPath = path; }

    void TagsManager::OnIndexerTerminated(gsgs::ProcessEvent& event)
    {
        wxUnusedVar(event);
        wxDELETE(m_IndexerProcess);
        StartIndexer();
    }
    ...
    void TagsManager::SourceToTags(const wxFileName& source, wxString& tags)
    {
        ...
        gsgsDEBUG1() << "Sending CTAGS command:" << ctagsCmd << clEndl;
        // connect to the indexer
        if(!client.connect()) {
            gsgsWARNING() << "Failed to connect to indexer process. Indexer ID:" << wxGetProcessId() << clEndl;
            return;
        }

        // send the request
        if(!clIndexerProtocol::SendRequest(&client, req)) {
            gsgsWARNING() << "Failed to send request to indexer. Indexer ID:" << wxGetProcessId() << clEndl;
            return;
        }

        // read the reply
        clIndexerReply reply;
        gsgsDEBUG1() << "SourceToTags: reading indexer reply" << clEndl;
        try {
            std::string errmsg;
            if(!clIndexerProtocol::ReadReply(&client, reply, errmsg)) {
                gsgsWARNING() << "Failed to read indexer reply: " << (wxString() << errmsg) << clEndl;
                RestartIndexer();
                return;
            }
        } catch(std::bad_alloc& ex) {
            gsgsWARNING() << "std::bad_alloc exception caught" << clEndl;
            tags.Clear();
            return;
        }

        gsgsDEBUG1() << "SourceToTags: [" << reply.getTags() << "]" << clEndl;
    ...
    bool TagsManager::IsValidCtagsFile(const wxFileName& filename) const
    {
        ...
        return FileUtil::IsCxxFile(filename) || FileUtil::WildMatch(m_tagsOptions.GetFileSpec(), filename);
    }
                                               ...
                                               std::vector<TagEntryPtr>& candidates)
    {
        ...
        TagEntryPtrList locals;
        TagEntryPtrList scoped;
        TagEntryPtrList globals;
        TagEntryPtrList keywords;
                                             ...
                                             const wxString& text, std::vector<TagEntryPtr>& candidates)
    {
        ...
        if(expression == wxT("::")) {
            ...
            PERF_BLOCK("ProcessExpression")
            {
                ...
                if(!res) {
                    PERF_END();
                    gsgs_Debug(wxT("Failed to resolve %s"), expression.c_str());
                    return false;
                }
            }
        }
    ...
    gsgsTipInfoPtr TagsManager::GetFunctionTip(const wxFileName& fileName, int lineno, const wxString& expr,
                                             const wxString& text, const wxString& word)
    {
        wxString path;
        wxString typeName, typeScope, tmp;
        ...
        if(expression.IsEmpty()) {
            ...
            wxString oper, dummy;
            gsgsDEBUG1() << "Calling ProcessExpression(" << expression << ")" << clEndl;
            bool res = ProcessExpression(fileName, lineno, expression, text, typeName, typeScope, oper, dummy);
            if(!res) {
                gsgsDEBUG1() << "Calling ProcessExpression(" << expression << ") - returned FALSE" << clEndl;
                return NULL;
            }
            ...
            std::vector<TagEntryPtr> tmpCandidates;
            TagsByScopeAndName(scope, word, tmpCandidates, ExactMatch);
            gsgsDEBUG1() << "Building Calltip from tags, size:" << tmpCandidates.size() << clEndl;
        ...
        gsgsTipInfoPtr ct(new gsgs::TipInfo(tips));
        return ct;
    }
    ...
    bool TagsManager::GetFunctionDetails(const wxFileName& fileName, int lineno, TagEntryPtr& tag, Function& func)
    {
                                               ...
                                               std::vector<std::pair<int, int> >* paramLen)
    {
        ...
        CxxVariableScanner varScanner(sig, eCxxStandard::kCxx03, gsgsStringMap(), true);
        ...
        const gsgsStringMap& macrosTable = GetCtagsOptions().GetTokensReversedWxMap();
        std::for_each(vars.begin(), vars.end(), [&](CxxVariable::Ptr_t var) {
            ...
            str_output << var->ToString(toStringFlags,
                                        (flags & Normalize_Func_Reverse_Macro) ? macrosTable : gsgsStringMap());
    ...
    wxString TagsManager::GetCTagsCmd()
    {
        ...
        // build the command, we surround ctags name with double quatations
        cmd << wxT("\"") << m_IndexerPath.GetFullPath() << wxT("\"") << ctagsCmd;
    ...
    wxString TagsManager::DoReplaceMacros(const wxString& name)
    {
        ...
        const gsgsStringMap& iTokens = GetCtagsOptions().GetTokensWxMap();
        gsgsStringMap::const_iterator it;
    ...
    {
        wxFFile fp;
        wxString fileName = wxFileName::CreateTempFileName(wxT("gsgs_mod_file_"), &fp);
        if(fp.IsOpened()) {
            fp.Write(text);
            fp.Close();
            wxString tagsStr;
    ...
    bool TagsManager::IsBinaryFile(const wxString& filepath)
    {
        ...
        FileUtil::FileType type = FileUtil::GetType(filepath);
        if(type == FileUtil::TypeHeader || type == FileUtil::TypeSourceC ||
           type == FileUtil::TypeSourceCpp)
            return false;
    ...
    bool TagsManager::AreTheSame(const TagEntryPtrList& v1, const TagEntryPtrList& v2) const
    {
    ...
    void TagsManager::DoSortByVisibility(TagEntryPtrList& tags)
    {
        TagEntryPtrList publicTags, privateTags, protectedTags;
        for(size_t i = 0; i < tags.size(); ++i) {
    ...
    void TagsManager::GetCXXKeywords(gsgsStringSet& words)
    {
        wxArrayString arr;
        GetCXXKeywords(arr);
        words.clear();
    ...
    TagEntryPtrList TagsManager::ParseBuffer(const wxString& content, const wxString& filename)
    {
        if(!m_IndexerProcess) { return TagEntryPtrList(); }
        ...
        if(!fp.IsOpened()) return TagEntryPtrList();
        ...
        {
            wxLogNull noLog;
            gsgsRemoveFile(tmpfilename);
        }

        TagEntryPtrList tagsVec;
