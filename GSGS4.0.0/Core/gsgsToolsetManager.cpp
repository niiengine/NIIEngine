...
namespace gsgs
{
        ...
        , m_isDefault(false)
    {
        ...
        if(node) {
            ...
            if(!node->XmlHasProp(wxT("GenerateDependenciesFiles"))) {
            ...
            if(!node->XmlHasProp(wxT("ReadObjectsListFromFile"))) {
            ...
            while(child) {
                if(child->GetName() == wxT("Switch")) {
                    ...
                    ft.compilation_line = XmlUtils::ReadString(child, wxT("Operation"));
            ...
            if(GetTool("MAKE").IsEmpty()) {
                BuilderConfigPtr bldr = gsgs_Settings().GetBuilderConfig("");
        ...
        if(m_fileTypes.empty()) {
            AddCmpFileType("cpp", CmpFileKindSource,
                           "$(CXX) $(Operation) \"$(FileFullPath)\" $(CXXFLAGS) "
                           "$(Objective)$(IntermediateDirectory)/$(ObjectName)$(ObjectSuffix) "
                           "$(IncludePath)");
            ...
            AddCmpFileType("c++", CmpFileKindSource,
                           ...
                           "$(IncludePath)");
            AddCmpFileType("c", CmpFileKindSource,
                           "$(CC) $(Operation) \"$(FileFullPath)\" $(CFLAGS) "
                           "$(Objective)$(IntermediateDirectory)/$(ObjectName)$(ObjectSuffix) "
                           "$(IncludePath)");
            ...
            AddCmpFileType("m", CmpFileKindSource,
                           "$(CXX) -x objective-c $(Operation) \"$(FileFullPath)\" $(CXXFLAGS) "
                           "$(Objective)$(IntermediateDirectory)/$(ObjectName)$(ObjectSuffix) "
                           "$(IncludePath)");
            AddCmpFileType("mm", CmpFileKindSource,
                           "$(CXX) -x objective-c++ $(Operation) \"$(FileFullPath)\" "
                           "$(CXXFLAGS) "
                           "$(Objective)$(IntermediateDirectory)/$(ObjectName)$(ObjectSuffix) "
                           "$(IncludePath)");
    ...
    wxXmlNode* Compiler::ToXml() const
    {
        ...
        node->XmlAddProperty(wxT("GenerateDependenciesFiles"), BoolToString(m_generateDependeciesFile));
        node->XmlAddProperty(wxT("ReadObjectsListFromFile"), BoolToString(m_readObjectFilesFromList));
        node->XmlAddProperty(wxT("ObjectNameIdenticalToFileName"), BoolToString(m_objectNameIdenticalToFileName));
        node->XmlAddProperty("CompilerFamily", m_compilerFamily);
        node->XmlAddProperty("IsDefault", BoolToString(m_isDefault));
        ...
        for(; iter != m_switches.end(); iter++) {
            ...
            child->XmlAddProperty(wxT("Name"), iter->first);
        ...
        CmpFileTypeInfos::const_iterator it, itend = m_fileTypes.end();
        for(it = m_fileTypes.begin(); it != itend; it++) {
            wxXmlNode* child = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("File"));
            Compiler::CmpFileTypeInfo ft = it->second;
            child->XmlAddProperty(wxT("Extension"), ft.extension);
            child->XmlAddProperty(wxT("Operation"), ft.compilation_line);

            wxString strKind;
            strKind << (long)ft.kind;
            child->XmlAddProperty(wxT("Kind"), strKind);

            node->AddChild(child);
        }

        wxXmlNode* options = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("Option"));
        options->XmlAddProperty(wxT("Name"), wxT("ObjectSuffix"));
        options->XmlAddProperty(wxT("Value"), m_objectSuffix);
        node->AddChild(options);

        options = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("Option"));
        options->XmlAddProperty(wxT("Name"), wxT("DependSuffix"));
        options->XmlAddProperty(wxT("Value"), m_dependSuffix);
        node->AddChild(options);

        options = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("Option"));
        options->XmlAddProperty(wxT("Name"), wxT("PreprocessSuffix"));
        options->XmlAddProperty(wxT("Value"), m_preprocessSuffix);
        node->AddChild(options);
        ...
        for(; itCmpOption != m_compilerOptions.end(); ++itCmpOption) {
            ...
            pCmpOptionNode->XmlAddProperty(wxT("Name"), cmpOption.name);
        ...
        for(; itLnkOption != m_linkerOptions.end(); ++itLnkOption) {
            ...
            pLnkOptionNode->XmlAddProperty(wxT("Name"), lnkOption.name);
    ...
    {
        CmpFileTypeInfos::iterator iter = m_fileTypes.find(extension.Lower());
    ...
    wxArrayString Compiler::GetDefaultIncludePaths()
    {
        ...
        // Only add the cygwin
        if(::IsCygwinEnv()) { gccCompilers.Add(COMPILER_FAMILY_CYGWIN); }
        ...
        if(GetCompilerFamily() == COMPILER_FAMILY_CLANG || GetCompilerFamily() == COMPILER_FAMILY_GCC ||
           GetCompilerFamily() == COMPILER_FAMILY_CYGWIN || GetCompilerFamily() == COMPILER_FAMILY_MINGW) {
            ...
            if(cmpMacrosFile.Exists()) {
                gsgsDEBUG1() << "Compiler builtin macros are written into:" << cmpMacrosFile.GetFullPath();
                ...
                //                gsgsDEBUG1() << "BUILTIN:" << definitions.Item(i);
                //            }

                {
                    // Delete the file
                    gsgsRemoveFile(cmpMacrosFile.GetFullPath());
                }
            }
        }
    ...
    {
    #ifdef __WXMSW__
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorMinGW()));
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorCLANG()));
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorMSVC()));
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorCygwin()));

    #elif defined(__WXGTK__)
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorGCC()));
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorCLANG()));
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorEosCDT()));
    ...
    #endif
        m_detectors.push_back(ToolsetLocator::Ptr_t(new CompilerLocatorCrossGCC()));
    }
    //---------------------------------------------------------------------------
    ToolsetManager::~ToolsetManager() {}
    //---------------------------------------------------------------------------
    bool ToolsetManager::Locate()
    {
        ...
        ToolsetLocator::Vect_t::iterator iter = m_detectors.begin();
    ...
    //---------------------------------------------------------------------------
    CompilerPtr ToolsetManager::Locate(const wxString& folder)
    {
        m_compilersFound.clear();
        ToolsetLocator::Vect_t::iterator iter = m_detectors.begin();
        for(; iter != m_detectors.end(); ++iter) {
    ...
    //---------------------------------------------------------------------------
    bool ToolsetManager::FoundMinGWCompiler() const
    {
        for(size_t i = 0; i < m_compilersFound.size(); ++i) {
    ...
    void ToolsetManager::MSWSuggestToDownloadMinGW(bool prompt)
    {
    #ifdef __WXMSW__
        if(!prompt ||
           ::wxMessageBox(_("Could not locate any MinGW compiler installed on your machine, would you like to "
            ...
            gsgsStringMap mingwCompilers;
            wxArrayString options;

            // Load the compilers list from the website
            wxURL url("http://niiengine.com/compilers.json");
            ...
            if(url.GetError() == wxURL_NOERR) {
                ...
                gsgs::JSON root(dataRead);
                gsgs::JSONObject compilers = root.getObject().getObject("Compilers");
                gsgs::JSONObject arr = compilers.getObject("MinGW");
                int count = arr.getArraySize();
                for(int i = 0; i < count; ++i) {
                    JSONObject compiler = arr.getArray(i);
                    wxString tstr1, tstr2;
                    compiler.getObject("Name").getValue(tstr1);
                    compiler.getObject("URL").getValue(tstr2);

                    mingwCompilers.insert(std::make_pair(tstr1, tstr2));
                    options.Add(tstr1);
                }

                if(options.IsEmpty()) {
                    ::wxMessageBox(_("Unable to fetch compilers list from the website\nhttp://www.niiengine.com/compilers.json"),
                                   "gssg",
                ...
                if(!selection.IsEmpty()) {
                    // Reset the compiler detection flag so next time gsgs is restarted, it will
                    // rescan the machine
                    gsgs_Config().Write(kConfigBootstrapCompleted, false);
    ...
    //---------------------------------------------------------------------------
    void ToolsetManager::MSWFixClangToolChain(CompilerPtr compiler, const ToolsetLocator::CompilerVec_t& allCompilers)
    {
