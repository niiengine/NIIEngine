...
void PHPEntityFunction::Store(PHPLookupTable* lookup)
{
    ...
    try {
        ...
        gsgs_Warning("PHPEntityFunction::Store: %s", exc.GetMessage());
    }
}
...
void PHPEntityFunction::FromJSON(const JSONObject& json)
{
    BaseFromJSON(json);
    json.getObject("returns").getValue(m_strReturnValue);
    json.getObject("signature").getValue(m_strSignature);
}

JSONObject PHPEntityFunction::ToJSON() const
{
    JSONObject json = BaseToJSON("f");
