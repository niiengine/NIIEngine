...
namespace gsgs
{
    LanguageServerProtocol::LanguageServerProtocol(const wxString& name, eNetworkType netType, wxEvtHandler* owner)
        ...
        , m_owner(owner)
    {
        gsgs_Event().Bind(wxEVT_FILE_SAVED, &LanguageServerProtocol::OnFileSaved, this);
        gsgs_Event().Bind(wxEVT_FILE_CLOSED, &LanguageServerProtocol::OnFileClosed, this);
        gsgs_Event().Bind(wxEVT_FILE_LOADED, &LanguageServerProtocol::OnFileLoaded, this);
        gsgs_Event().Bind(wxEVT_ACTIVE_EDITOR_CHANGED, &LanguageServerProtocol::OnEditorChanged, this);
    ...
    {
        gsgs_Event().Unbind(wxEVT_FILE_SAVED, &LanguageServerProtocol::OnFileSaved, this);
        gsgs_Event().Unbind(wxEVT_FILE_CLOSED, &LanguageServerProtocol::OnFileClosed, this);
        gsgs_Event().Unbind(wxEVT_FILE_LOADED, &LanguageServerProtocol::OnFileLoaded, this);
        gsgs_Event().Unbind(wxEVT_ACTIVE_EDITOR_CHANGED, &LanguageServerProtocol::OnEditorChanged, this);
    ...
    {
        FileUtil::FileType type = FileUtil::GetType(fn, FileUtil::TypeText);
        switch (type) {
        ...
        case FileUtil::TypeJS:
            return "javascript";
        case FileUtil::TypeDockerfile:
            return "dockerfile";
        case FileUtil::TypeJava:
            return "java";
        case FileUtil::TypePython:
            return "python";
        case FileUtil::TypeMakefile:
            return "makefile";
        case FileUtil::TypePhp:
            return "php";
        case FileUtil::TypeSQL:
            return "sql";
    ...
    void LanguageServerProtocol::DoStart()
    {
        ...
        gsgsDEBUG() << GetLogPrefix() << "Starting...";
        gsgsDEBUG() << GetLogPrefix() << "Command:" << m_lspCommand;
        gsgsDEBUG() << GetLogPrefix() << "Root folder:" << m_rootFolder;
        for (const wxString& lang : m_languages) {
            gsgsDEBUG() << GetLogPrefix() << "Language:" << lang;
        }
        LSPStartupInfo info;
    ...
    void LanguageServerProtocol::OnFunctionCallTip(CodeCompleteEvent& event)
    {
        event.Skip();
    ...
    void LanguageServerProtocol::OnCodeComplete(CodeCompleteEvent& event)
    {
        event.Skip();
        IEditor* editor = dynamic_cast<IEditor*>(event.GetEditor());
        CHECK_PTR_RET(editor);
    ...
    void LanguageServerProtocol::OnFindSymbolDecl(CodeCompleteEvent& event)
    {
        event.Skip();
        IEditor* editor = dynamic_cast<IEditor*>(event.GetEditor());
        CHECK_PTR_RET(editor);
    ...
    void LanguageServerProtocol::OnFindSymbolImpl(CodeCompleteEvent& event)
    {
        event.Skip();
        IEditor* editor = dynamic_cast<IEditor*>(event.GetEditor());
        CHECK_PTR_RET(editor);
    ...
    void LanguageServerProtocol::OnFindSymbol(CodeCompleteEvent& event)
    {
        event.Skip();
        IEditor* editor = dynamic_cast<IEditor*>(event.GetEditor());
        CHECK_PTR_RET(editor);
    ...
    void LanguageServerProtocol::SendCloseRequest(const wxFileName& filename)
    {
        if (m_filesSent.count(filename.GetFullPath()) == 0) {
            gsgsDEBUG() << GetLogPrefix() << "LanguageServerProtocol::FileClosed(): file" << filename << "is not opened";
            return;
        }
    ...
    void LanguageServerProtocol::SendSaveRequest(const wxFileName& filename, const wxString& fileContent)
    {
        ...
        // req.Send(this);
        // GetManager()->SetStatusMessage(wxString() << "[LSP] re-parsing file: " << filename.GetFullName(), 1);
    ...
    // Event handlers
    //===------------------------------------------------------------------
    void LanguageServerProtocol::OnFileLoaded(gsgs::Event& event)
    {
        event.Skip();
    ...
    void LanguageServerProtocol::OnFileClosed(gsgs::Event& event)
    {
        event.Skip();
        SendCloseRequest(event.GetFileName());
    }

    void LanguageServerProtocol::OnFileSaved(gsgs::Event& event)
    {
        event.Skip();
    ...
    void LanguageServerProtocol::OpenEditor(IEditor* editor)
    {
        ...
        if (editor && ShouldHandleFile(editor)) {
            if (m_filesSent.count(editor->GetFileName().GetFullPath())) {
                gsgsDEBUG() << "OpenEditor->SendChangeRequest called for:" << editor->GetFileName().GetFullName();
                SendChangeRequest(editor->GetFileName(), editor->GetCtrl()->GetText());
            }
            else {
                gsgsDEBUG() << "OpenEditor->SendOpenRequest called for:" << editor->GetFileName().GetFullName();
    ...
    void LanguageServerProtocol::ProcessQueue()
    {
        ...
        if (m_Queue.IsWaitingReponse()) {
            gsgsDEBUG() << "LSP is busy, will not send message";
            return;
        }
        LSP::MessageWithParams::Ptr_t req = m_Queue.Get();
        if (!IsRunning()) {
            gsgsDEBUG() << GetLogPrefix() << "is down.";
            return;
        }
        ...
        m_Queue.Pop();
        if (!req->GetStatusMessage().IsEmpty()) { GetManager()->SetStatusMessage(req->GetStatusMessage(), 1); }
    }
    ...
    void LanguageServerProtocol::OnNetConnected(gsgs::Event& event)
    {
        ...
        gsgsDEBUG() << GetLogPrefix() << "Sending initialize request...";
    ...
    void LanguageServerProtocol::OnNetError(gsgs::Event& event)
    {
        gsgsDEBUG() << GetLogPrefix() << "Socket error." << event.GetString();
        DoClear();
    ...
    void LanguageServerProtocol::OnNetDataReady(gsgs::Event& event)
    {
        gsgsDEBUG() << GetLogPrefix() << event.GetString();
        ...
        while (true) {
            ...
            if (res.IsOk()) {
                if (IsInitialized()) {
                    ...
                    if (res.Has("error")) {
                        gsgsDEBUG() << GetLogPrefix() << "received an error message";
                        ...
                        case LSP::ResponseError::kErrorCodeMethodNotFound: {
                            ...
                            GetManager()->SetStatusMessage(wxString() << GetLogPrefix() << _("method: ")
                        ...
                        if (msg_ptr && msg_ptr->As<LSP::Request>()) {
                            gsgsDEBUG() << GetLogPrefix() << "received a response";
                            ...
                            if (editor) {
                                ...
                                if (false && preq->IsPositionDependantRequest() &&
                                    ...
                                    gsgsDEBUG() << "Response is no longer valid. Discarding its result";
                                }
                                else {
                            ...
                            // Get the URI
                            gsgsDEBUG() << GetLogPrefix() << "Received diagnostic message";
                            ...
                            gsgsDEBUG() << GetLogPrefix() << "received an unsupported message";
                        }
                    }
                }
                else {
                    ...
                    if (res.GetId() == m_initializeRequestID) {
                        gsgsDEBUG() << GetLogPrefix() << "initialization completed";
                        m_initializeRequestID = wxNOT_FOUND;
                        m_state = kInitialized;
                        ...
                        gsgsDEBUG() << GetLogPrefix() << "Server not initialized. This message is ignored";
                    }
                }
    ...
    {
        gsgsDEBUG() << GetLogPrefix() << "Going down";
        m_network->Close();
    }
