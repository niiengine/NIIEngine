...
#define DEBUG_MESSAGE(x) gsgs_Debug1(x.c_str())
...
namespace gsgs
{
    ...
    wxDEFINE_EVENT(wxEVT_PARSE_THREAD_SUGGEST_COLOUR_TOKENS, gsgs::Event);
    wxDEFINE_EVENT(wxEVT_PARSE_THREAD_SOURCE_TAGS, gsgs::Event);
    //-----------------------------------------------------------------------------------
    static const wxString& WriteGSGSCCHelperFile()
    {
        ...
        if(filepath.IsEmpty()) {
            wxFileName tmpFile(gsgs_Path().GetUserDataDir(), "gsgs_templates.hpp");
        ...
        FileUtil::WriteFileContent(wxFileName(filepath), buffer);
        return filepath;
    }
    //-----------------------------------------------------------------------------------
    ...
    void ParseThread::ProcessRequest(ThreadRequest* request)
    {
        ...
        if(!req->_workspaceFiles.empty()) {
            ...
            for(std::string& filename : req->_workspaceFiles) {
                if(FileUtil::IsCxxFile(wxString() << filename)) { filtered_list.push_back(std::move(filename)); }
            }
    ...
    void ParseThread::ProcessSimple(ParseRequest* req)
    {
        ...
        gsgsDEBUG1() << "Parsing saved file:" << file << clEndl;
        // Skip binary file
        if(TagsManagerST::Get()->IsBinaryFile(file)) {
            gsgsDEBUG1() << "File:" << file << "is binady and will be skipped" << clEndl;
            return;
        }
        ...
        gsgsDEBUG1() << "Parsed file output: [" << tags << "]" << clEndl;
    ...
    void ParseThread::GetFileListToParse(const wxString& filename, wxArrayString& arrFiles)
    {
...

        {
            ...
            FileOpen::Get()->ClearSearchPath();
            for(size_t i = 0; i < includePaths.GetCount(); i++) {
                FileOpen::Get()->AddSearchPath(includePaths.Item(i).mb_str(wxConvUTF8).data());
            }

            for(size_t i = 0; i < excludePaths.GetCount(); i++) {
                FileOpen::Get()->AddExcludePath(excludePaths.Item(i).mb_str(wxConvUTF8).data());
            }
        ...
        const FileOpen::Set_t& fileSet = FileOpen::Get()->GetResults();
    ...
    void ParseThread::ProcessParseAndStore(ParseRequest* req)
    {
        ...
        const wxString& hackfile = WriteGSGSCCHelperFile();
    ...
    void ParseThread::FindIncludedFiles(ParseRequest* req, std::set<wxString>* newSet)
    {
        ...
        // Clear the results once
        {
            ...
            for(size_t i = 0; i < searchPaths.GetCount(); i++) {
                ...
                FileOpen::Get()->AddSearchPath(path.data());
            }
            ...
            for(size_t i = 0; i < excludePaths.GetCount(); i++) {
                ...
                FileOpen::Get()->AddExcludePath(path.data());
            }
            ...
            newSet->insert(FileOpen::Get()->GetResults().begin(), FileOpen::Get()->GetResults().end());
        }
    }
    //--------------------------------------------------------------------------------------
    // Parse Request Class
    ...
    {
        FileOpen::Set_t* matches = new FileOpen::Set_t;
        {
            wxString file = req->getFile();
            ...
            FileOpen::Set_t& incls = FileOpen::Get()->GetIncludeStatements();
            matches->swap(incls);
            // Cleanup
            FileOpen::Get()->ClearResults();
            FileOpen::Get()->ClearSearchPath();
    ...
    void ParseThread::ProcessColourRequest(ParseRequest* req)
    {
        ...
        if(FileUtil::ReadFileContent(req->getFile(), content)) {
            ...
            if(req->_evtHandler) {
                gsgs::Event event(wxEVT_PARSE_THREAD_SUGGEST_COLOUR_TOKENS);
                wxArrayString res;
                res.Add(flatClasses);
    ...
    void ParseThread::ProcessSourceToTags(ParseRequest* req)
    {
        ...
        // Fire the event
        gsgs::Event event(wxEVT_PARSE_THREAD_SOURCE_TAGS);
