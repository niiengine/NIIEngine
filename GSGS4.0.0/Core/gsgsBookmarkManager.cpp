    ...
    : m_activeBookmarkType(smt_bookmark1)
{
    ...
    gsgs_Event().Connect(wxEVT_EDITOR_SETTINGS_CHANGED, wxCommandEventHandler(BookmarkManager::OnEditorSettingsChanged), NULL, this);
}

BookmarkManager::~BookmarkManager()
{
    gsgs_Event().Disconnect(wxEVT_EDITOR_SETTINGS_CHANGED, wxCommandEventHandler(BookmarkManager::OnEditorSettingsChanged), NULL, this);
}

BookmarkManager& BookmarkManager::singleton()
{
...
wxMenu* BookmarkManager::CreateBookmarksSubmenu(wxMenu* parentMenu)
{
    ...
    int current = gsgs_Bookmark().GetActiveBookmarkType();
