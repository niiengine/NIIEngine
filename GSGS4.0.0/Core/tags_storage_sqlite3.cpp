...
void TagsStorageSQLite::OpenDatabase(const wxFileName& fileName)
{
    ...
    try {
        ...
        gsgsWARNING() << "Failed to open file:" << m_fileName.GetFullPath() << "." << e.GetMessage();
    }
}
...
void TagsStorageSQLite::RecreateDatabase()
{
    try {
        ...
        if(gsgsRemoveFile(m_fileName.GetFullPath()) == false) {
...
wxSQLite3ResultSet TagsStorageSQLite::Query(const wxString& sql, const wxFileName& path)
{
    ...
    try {
        ...
        gsgsWARNING() << "Query error:" << sql << "." << e.GetMessage();
    }
...
void TagsStorageSQLite::ExecuteUpdate(const wxString& sql)
{
    try {
        ...
        gsgsWARNING() << "ExecuteUpdate error:" << sql << "." << e.GetMessage();
    }
}
...
{
    try {
        bool match_path = (!partialName.IsEmpty() && partialName.Last() == gsgsPathSep);
...
{
    if(GetUseCache()) {
        gsgsDEBUG1() << "Testing cache for" << sql << clEndl;
        if(m_cache.Get(sql, tags) == true) {
            gsgsDEBUG1() << "[CACHED ITEMS]" << sql << clEndl;
            return;
        }
    }

    gsgsDEBUG1() << "Entry not found in cache" << sql << clEndl;
    gsgsDEBUG1() << "Fetching from disk..." << clEndl;
    tags.reserve(500);
    try {
        ...
        gsgsWARNING() << "TagsStorageSQLite::DoFetchTags() error:" << e.GetMessage() << clEndl;
    }
    gsgsDEBUG1() << "Fetching from disk...done" << clEndl;
    if(GetUseCache()) {
        gsgsDEBUG1() << "Updating cache" << clEndl;
        m_cache.Store(sql, tags);
        gsgsDEBUG1() << "Updating cache...done (" << tags.size() << "entries)" << clEndl;
    }
}
...
{
    if(GetUseCache()) {
        gsgs_Debug1(wxT("Testing cache for: %s"), sql);
        if(m_cache.Get(sql, kinds, tags) == true) {
            gsgs_Debug1(wxT("[CACHED ITEMS] %s"), sql);
            return;
        }
    }

    gsgs_Debug1("Fetching from disk");
    try {
        wxSQLite3ResultSet ex_rs;
        ex_rs = Query(sql);
    ...
    gsgs_Debug1("Fetching from disk...done");
    if(GetUseCache()) {
        gsgs_Debug1("updating cache");
        m_cache.Store(sql, kinds, tags);
        gsgs_Debug1("updating cache...done");
    }
}
                                         ...
                                         wxArrayString& defMacros)
{
    ...
    try {
        ...
        gsgs_Debug(wxT("%s"), exc.GetMessage().c_str());
    }
}
...
void TagsStorageSQLite::GetTagsByName(const wxString& prefix, std::vector<TagEntryPtr>& tags, bool exactMatch)
{
    try {
        ...
        gsgs_Debug(wxT("%s"), e.GetMessage().c_str());
    }
}
                                                  ...
                                                  std::vector<wxString>& nonWorkspaceSymbols)
{
    ...
    try {
        ...
        gsgsDEBUG() << "SplitSymbols error:" << sql << ":" << e.GetMessage() << clEndl;
    }
}

const wxString& TagsStorageSQLite::GetVersion() const
{
    static const wxString gTagsDatabaseVersion(wxT("gsgs Version 11.1"));
...
void TagsStorageSQLite::GetTagsByPartName(const wxArrayString& parts, std::vector<TagEntryPtr>& tags)
{
    ...
    try {
        ...
        gsgsWARNING() << sql << ":" << e.GetMessage() << clEndl;
    }
}
