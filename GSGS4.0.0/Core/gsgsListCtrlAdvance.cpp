...
namespace gsgs
{
    ...
    BEGIN_EVENT_TABLE(ListCtrlAdvance, wxListCtrl)
        EVT_LEFT_DOWN(ListCtrlAdvance::OnLeftDown)
        END_EVENT_TABLE()

        ListCtrlAdvance::ListCtrlAdvance(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
        : wxListCtrl(parent, id, pos, size, style | wxLC_REPORT)
    {
        DoInitialize();
    }

    ListCtrlAdvance::~ListCtrlAdvance()
    {
    ...
    void ListCtrlAdvance::DoInitialize()
    {
    ...
    bool ListCtrlAdvance::IsChecked(long item)
    {
    ...
    void ListCtrlAdvance::Check(long item, bool b)
    {
        wxListItem list_item;
        list_item.SetId(item);
        list_item.SetColumn(0);
    ...
    long ListCtrlAdvance::AppendRow()
    {
        long item;
    ...
    wxString ListCtrlAdvance::GetText(long item, long col)
    {
        wxListItem list_item;
        list_item.SetId(item);
    ...
    void ListCtrlAdvance::SetTextColumn(long item, long col, const wxString& text)
    {
        wxListItem list_item;
        list_item.SetId(item);
        list_item.SetColumn(col);
    ...
    void ListCtrlAdvance::OnLeftDown(wxMouseEvent& event)
    {
    ...
    bool ListCtrlAdvance::SetItemClientData(long item, wxClientData* clientData)
    {
        return SetItemPtrData(item, (wxUIntPtr)clientData);
    }

    void ListCtrlAdvance::SetCheckboxRow(long item, bool checked)
    {
        wxListItem list_item;
        list_item.SetId(item);
        list_item.SetColumn(0);
    ...
    long ListCtrlAdvance::GetSelection() const
    {
