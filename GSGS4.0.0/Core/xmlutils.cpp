...
namespace gsgs
{
    wxXmlNode *XmlUtils::FindNodeByName(const wxXmlNode *parent, const wxString &tagName, const wxString &name)
    {
        if (parent)
        {
            ...
            while (child)
            {
                ...
                {
                    if ((child->XmlGetPropVal(wxT("Name"), wxEmptyString) == name) || (child->XmlGetPropVal(wxT("name"), wxEmptyString) == name))
                    {
                        return child;
                    }
                }
    ...
    {
        wxXmlProperty *prop = node->XmlGetProperties();
        while (prop) {
    ...
    {
        wxString value;
        if (!node->XmlGetPropVal(propName, &value)) {
            return false;
        }
    ...
    bool XmlUtils::StaticReadObject(wxXmlNode* root, const wxString& name, ObjectData* obj)
    {
        //find the object node in the xml file

        wxXmlNode *node = XmlUtils::FindNodeByName(root, wxT("Info"), name);
        if (node) {

            // Check to see if we need a version check
            wxString objectVersion = obj->GetVersion();
            if (objectVersion.IsEmpty() == false) {
                if (node->XmlGetPropVal(wxT("Version"), wxT("")) != objectVersion) {
                    return false;
                }
            }
    ...
    bool XmlUtils::StaticWriteObject(wxXmlNode* root, const wxString& name, ObjectData* obj)
    {
        if (!root)
            return false;

        Serializer arch;
        wxXmlNode *child = XmlUtils::FindNodeByName(root, wxT("Info"), name);
        if (child) {
            wxXmlNode *n = root;
            n->RemoveChild(child);
            delete child;
        ...
        child = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("Info"));
        root->AddChild(child);

        wxString objectVersion = obj->GetVersion();
        if (objectVersion.IsEmpty() == false)
            child->XmlAddProperty(wxT("Version"), objectVersion);
    ...
    bool XmlUtils::SaveXmlToFile(wxXmlDocument* doc, const wxString& filename)
    {
        ...
        if (doc->Save(sos))
        {
            return FileUtil::WriteFileUTF8(filename, content);
        }
        return false;
    }
}