...
wxDEFINE_EVENT(wxPHP_PARSE_STARTED, gsgs::ParseEvent);
wxDEFINE_EVENT(wxPHP_PARSE_ENDED, gsgs::ParseEvent);
wxDEFINE_EVENT(wxPHP_PARSE_PROGRESS, gsgs::ParseEvent);
...
PHPEntityBase::Ptr_t PHPLookupTable::FindScope(const wxString& fullname)
{
    ...
    if(scopeName.EndsWith("\\") && scopeName.length() > 1) { scopeName.RemoveLast(); }
...
void PHPLookupTable::Open(const wxFileName& dbfile)
{
    try {
        ...
        gsgsWARNING() << "PHPLookupTable::Open" << e.GetMessage() << clEndl;
    }
}
...
void PHPLookupTable::CreateSchema()
{
    ...
    try {
        ...
        st.Bind(st.GetParamIndex(":SCHEMA_NAME"), "GSGSPHP");
        wxSQLite3ResultSet res = st.ExecuteQuery();
        if(res.NextRow()) { schemaVersion = res.GetString("SCHEMA_VERSION"); }
    } catch(wxSQLite3Exception& e) {
        wxUnusedVar(e);
    }
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::CreateSchema: %s", e.GetMessage());
    }
}
...
void PHPLookupTable::UpdateSourceFile(PHPSourceFile& source, bool autoCommit)
{
    try {
        ...
        gsgs_Warning("PHPLookupTable::SaveSourceFile: %s", e.GetMessage());
    }
}
                                                    ...
                                                    bool parentIsNamespace)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::DoFindMemberOf: %s", e.GetMessage());
    }
    return PHPEntityBase::Ptr_t(NULL);
}
...
{
    if(!excludeSelf) { parents.push_back(cls->GetDbId()); }
...
PHPEntityBase::Ptr_t PHPLookupTable::DoFindScope(const wxString& fullname, ePhpScopeType scopeType)
{
    ...
    try {
        ...
        if(scopeType != kPhpScopeTypeAny) { sql << " AND SCOPE_TYPE = " << static_cast<int>(scopeType); }
        sql << " LIMIT 2 ";
        ...
        gsgs_Warning("PHPLookupTable::FindScope: %s", e.GetMessage());
    }
    return PHPEntityBase::Ptr_t(NULL);
}
...
PHPEntityBase::Ptr_t PHPLookupTable::DoFindScope(wxLongLong id, ePhpScopeType scopeType)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::DoFindScope: %s", e.GetMessage());
    }
    return PHPEntityBase::Ptr_t(NULL);
}
...
PHPEntityBase::List_t PHPLookupTable::LoadFunctionArguments(wxLongLong parentId)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::LoadFunctionArguments: %s", e.GetMessage());
    }
    return matches;
}
...
void PHPLookupTable::DoAddNameFilter(wxString& sql, const wxString& nameHint, size_t flags)
{
    ...
    if(name.IsEmpty()) {
        sql.Trim();
        if(sql.EndsWith("AND") || sql.EndsWith("and")) { sql.RemoveLast(3); }
        sql << " ";
        return;
    }
...
void PHPLookupTable::LoadAllByFilter(PHPEntityBase::List_t& matches, const wxString& nameHint, eLookupFlags flags)
{
    try {
        ...
        gsgsWARNING() << "PHPLookupTable::LoadAllByFilter:" << e.GetMessage() << clEndl;
    }
}
                                             ...
                                             const wxString& nameHint, eLookupFlags flags)
{
    ...
    try {
        ...
        gsgsWARNING() << "PHPLookupTable::LoadFromTableByNameHint:" << sql << ":" << e.GetMessage() << clEndl;
    }
}
...
void PHPLookupTable::DeleteFileEntries(const wxFileName& filename, bool autoCommit)
{
    try {
        ...
        if(autoCommit) m_db.Rollback();
        gsgs_Warning("PHPLookupTable::DeleteFileEntries: %s", e.GetMessage());
    }
}
...
void PHPLookupTable::Close()
{
    try {
        ...
        gsgs_Warning("PHPLookupTable::Close: %s", e.GetMessage());
    }
}
    ...
    try {
...

        {
            ...
            while(res.NextRow()) {
                ...
                if(isStaticFunction) {
                    ...
                    // Non static function.
                    if(!(flags & kLookupFlags_Static)) { matches.push_back(match); }
                }
            }
        }
...

        {
            ...
            while(res.NextRow()) {
                ...
                if(bAddIt) { matches.push_back(match); }
            }
            DoFixVarsDocComment(matches, parentId);
        }

    } catch(wxSQLite3Exception& e) {
        gsgs_Warning("PHPLookupTable::FindChildren: %s", e.GetMessage());
    }
}
...
wxLongLong PHPLookupTable::GetFileLastParsedTimestamp(const wxFileName& filename)
{
    try {
        ...
        st.Bind(st.GetParamIndex(":FILE_NAME"), filename.GetFullPath());
        wxSQLite3ResultSet res = st.ExecuteQuery();
        if(res.NextRow()) { return res.GetInt64("LAST_UPDATED"); }
    } catch(wxSQLite3Exception& e) {
        gsgs_Warning("PHPLookupTable::FindChildren: %s", e.GetMessage());
    }
    return 0;
...
void PHPLookupTable::UpdateFileLastParsedTimestamp(const wxFileName& filename)
{
    try {
        ...
        gsgs_Warning("PHPLookupTable::UpdateFileLastParsedTimestamp: %s", e.GetMessage());
    }
}
...
void PHPLookupTable::ClearAll(bool autoCommit)
{
    try {
        ...
        if(autoCommit) m_db.Rollback();
        gsgs_Warning("PHPLookupTable::ClearAll: %s", e.GetMessage());
    }
}
...
PHPEntityBase::Ptr_t PHPLookupTable::FindFunction(const wxString& fullname)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::FindFunction: %s", e.GetMessage());
    }
    return PHPEntityBase::Ptr_t(NULL);
}
                                                     ...
                                                     const wxString& shortNameContains)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::FindNamespaces: %s", e.GetMessage());
    }
    return matches;
}
...
PHPEntityBase::Ptr_t PHPLookupTable::FindFunctionByLineAndFile(const wxFileName& filename, int line)
{
    try {
        ...
        gsgs_Warning("PHPLookupTable::FindFunctionByLineAndFile: %s", e.GetMessage());
    }
    return NULL;
}
...
void PHPLookupTable::ResetDatabase()
{
    ...
    if(curfile.IsOk() && curfile.Exists()) {
        ...
        wxLogNull noLog;
        if(!gsgsRemoveFile(curfile.GetFullPath())) {
            // gsgs_Warning("PHPLookupTable::ResetDatabase: failed to remove file '%s'", curfile.GetFullPath());
        }
    }
    Open(curfile);
}
...
bool PHPLookupTable::CheckDiskImage(wxSQLite3Database& db, const wxFileName& filename)
{
    try {
        ...
        if(res.NextRow()) {
            ...
            gsgsDEBUG() << "PHP: 'PRAGMA quick_check' returned:" << value << clEndl;
        ...
        gsgsWARNING() << "PHP: exception caught:" << exec.GetMessage() << clEndl;
        gsgsWARNING() << "PHP: database image is corrupted:" << filename.GetFullPath() << clEndl;
        return false;
    }
    return true;
}
...
void PHPLookupTable::EnsureIntegrity(const wxFileName& filename)
{
    ...
    if(db.IsOpen()) {
        if(!CheckDiskImage(db, filename)) {
            ...
            db.Close();
            wxLogNull noLog;
            gsgsRemoveFile(filename.GetFullPath());
        }
    }
}
...
PHPEntityBase::List_t PHPLookupTable::FindSymbol(const wxString& name)
{
    ...
    try {
        ...
        gsgs_Warning("PHPLookupTable::FindSymbol: %s", e.GetMessage());
    }
    return matches;
}
    ...
    std::for_each(matches.begin(), matches.end(), [&](PHPEntityBase::Ptr_t match) {
        if(match->Is(kEntityTypeVariable)) {
            if(docs.count(match->GetShortName())) {
                ...
                if(!docvar->GetType().IsEmpty()) { match->Cast<PHPEntityVariable>()->SetTypeHint(docvar->GetType()); }
            }
        }
    });
}
...
{
    if(m_allClasses.count(classname) == 0) { m_allClasses.insert(classname); }
}
...
{
    // locate the scope
    gsgsDEBUG() << "Rebuilding PHP class cache..." << clEndl;
    m_allClasses.clear();
    size_t count = 0;
    try {
        wxString sql;
        ...
        gsgsWARNING() << "PHPLookupTable::RebuildClassCache:" << e.GetMessage() << clEndl;
        return;
    }
    gsgsDEBUG() << "Loading" << count << "class names into the cache" << clEndl;
    gsgsDEBUG() << "Rebuilding PHP class cache...done" << clEndl;
}
...
PHPEntityBase::Ptr_t PHPLookupTable::FindFunctionNearLine(const wxFileName& filename, int lineNumber)
{
    try {
        ...
        gsgsWARNING() << "PHPLookupTable::FindFunctionNearLine:" << e.GetMessage() << clEndl;
    }
    return PHPEntityBase::Ptr_t(NULL);
}
...
size_t PHPLookupTable::FindFunctionsByFile(const wxFileName& filename, PHPEntityBase::List_t& functions)
{
    ...
    try {
        ...
        gsgsWARNING() << "SQLite 3 error:" << e.GetMessage() << clEndl;
    }
...
void PHPLookupTable::ParseFolder(const wxString& folder, const wxString& filemask, eUpdateMode updateMode)
{
    ...
    std::for_each(files.begin(), files.end(), [&](const wxString& file) {
        try {
            ...
            wxString content;
            if(!FileUtil::ReadFileContent(fnFile, content, wxConvISO8859_1)) {
                gsgsWARNING() << "PHP: Failed to read file:" << fnFile << "for parsing";
                return;
            }
            gsgsDEBUG1() << "Parsing PHP file:" << fnFile;
            ...
            gsgsWARNING() << "PHPLookupTable::ParseFolder:" << e.GetMessage();
        }
    });
}
