...
void PHPEntityClass::FromJSON(const JSONObject& json)
{
    BaseFromJSON(json);
    json.getObject("extends").getValue(m_extends);
    json.getObject("implements").getValue(m_implements);
    json.getObject("traits").getValue(m_traits);
}

JSONObject PHPEntityClass::ToJSON() const
{
    JSONObject json = BaseToJSON("c");
