...
namespace gsgs
{
                                     ...
                                     wxString& scopeTemplateInitList) // output
    {
        ...
        const gsgsStringMap& ignoreTokens = GetTagsManager()->GetCtagsOptions().GetTokensWxMap();
        m_locals.clear();
        {
    ...
    //-----------------------------------------------------------------------------------
    bool Language::FunctionFromPattern(TagEntryPtr tag, Function& foo)
    {
    ...
    //-----------------------------------------------------------------------------------
    void Language::DoFixFunctionUsingCtagsReturnValue(Function& foo, TagEntryPtr tag)
    {
    ...
    //-----------------------------------------------------------------------------------
    void Language::DoReplaceTokens(wxString& inStr, const gsgsStringMap& ignoreTokens)
    {
        if(inStr.IsEmpty()) 
            return;

        gsgsStringMap::const_iterator iter = ignoreTokens.begin();
    ...
    {
        gsgsStringMap typeMap = GetTagsManager()->GetCtagsOptions().GetTypesMap();
        // HACK1: Let the user override the parser decisions
        wxString path = entryPath.IsEmpty() ? token->GetPath() : entryPath;
        gsgsStringMap::const_iterator where = typeMap.find(path);
    ...
    wxString Language::ApplyCtagsReplacementTokens(const wxString& in)
    {
        ...
        const gsgsStringMap& replacementMap = GetTagsManager()->GetCtagsOptions().GetTokensWxMap();
        gsgsStringMap::const_iterator iter = replacementMap.begin();
        ...
        for(size_t i = 0; i < lines.GetCount(); i++) {
            ...
            for(; iter != replacements.end(); iter++) {
                ::ReplacePatternA(outStr, *iter, outStr);
            }
    ...
    {
        CxxVariableScanner scanner(scope, eCxxStandard::kCxx11, GetTagsManager()->GetCtagsOptions().GetTokensWxMap(), false);
