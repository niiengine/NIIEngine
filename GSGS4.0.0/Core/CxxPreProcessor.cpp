...
namespace gsgs
{
    ...
    void CxxPreProcessor::Parse(const wxFileName& filename, size_t options)
    {
        ...
        try {
            //gsgs_Debug("Calling CxxPreProcessor::Parse for file '%s'\n", filename.GetFullPath());
            m_options = options;
    ...
    CxxPreProcessor::ExpandInclude(const wxFileName& currentFile, const wxString& includeStatement, wxFileName& outFile)
    {
        ...
        for(size_t i = 0; i < paths.GetCount(); ++i) {
            ...
            // gsgs_Debug(" ... Checking include file: %s\n", fn.GetFullPath());
            struct stat buff;
            if((stat(tmpfile.mb_str(wxConvUTF8).data(), &buff) == 0)) {
                //gsgs_Debug1(" ==> Creating scanner for file: %s\n", tmpfile);
                ...
                if(fixedFileName.FileExists()) {
                    ...
                    //gsgs_Debug("Including a folder :/ : %s", fixedFileName.GetFullPath());
                }
            }
        }
