...
ProjectSettings::ProjectSettings(wxXmlNode* node)
{
    ...
    if(!m_globalSettings) {
        // gsgsLogMessage(wxT("ProjectSettings : Create global settings because it doesn't exists"));
...
bool Project::Create(const wxString& name, const wxString& description, const wxString& path, const wxString& projType)
{
    ...
    wxXmlNode* root = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("gsgs_Project"));
    m_doc.SetRoot(root);
...
bool Project::IsFileExist(const wxString& filePath, wxString& fileNameInProject)
{
    ...
    std::find_if(m_filesTable.begin(), m_filesTable.end(), [&](const FilesMap_t::value_type& vt) {
        if(filePath == RealPath(vt.second->GetFilename())) {
...
//---------------------------------------------------------------------------
wxString Project::GetName() const { return m_doc.GetRoot()->XmlGetPropVal(wxT("Name"), wxEmptyString); }
//---------------------------------------------------------------------------
...
void Project::RecursiveAdd(wxXmlNode* xmlNode, ProjectTreePtr& ptp, ProjectTreeNode* nodeParent)
{
    ...
    while(parent) {
        nameList.push_front(parent->XmlGetPropVal(wxT("Name"), wxEmptyString));
    ...
    key += xmlNode->XmlGetPropVal(wxT("Name"), wxEmptyString);

    // Create the tree node data
    ProjectItem item;
    if(xmlNode->GetName() == wxT("Project")) {
        item =
            ProjectItem(key, xmlNode->XmlGetPropVal(wxT("Name"), wxEmptyString), wxEmptyString, ProjectItem::TypeProject);

    } else if(xmlNode->GetName() == wxT("VirtualDirectory")) {
        item = ProjectItem(key, xmlNode->XmlGetPropVal(wxT("Name"), wxEmptyString), wxEmptyString,
                           ProjectItem::TypeVirtualDirectory);

    } else if(xmlNode->GetName() == wxT("File")) {
        wxFileName filename(xmlNode->XmlGetPropVal(wxT("Name"), wxEmptyString));
        ...
        while(!Q.empty()) {
            ...
            while(child) {
                if(child->GetName() == wxT("File")) {
                    wxFileName fileName(child->XmlGetPropVal(wxT("Name"), wxEmptyString));
                    fileName.MakeAbsolute(m_fileName.GetPath());
...
void Project::CopyTo(const wxString& new_path, const wxString& new_name, const wxString& description)
{
    ...
    std::for_each(m_filesTable.begin(), m_filesTable.end(), [&](const FilesMap_t::value_type& vt) {
        ...
        file_node->XmlAddProperty(wxT("Name"), fn.GetFullName());
        ...
        default:
            ...
            if(!rcNode) {
                ...
                rcNode->XmlAddProperty(wxT("Name"), wxT("resources"));
    ...
    while(!Q.empty()) {
        ...
        while(child) {
            if(child->GetName() == "File" && folder) {
                ...
                wxString folderName = child->XmlGetPropVal("Name", wxEmptyString);
...
wxArrayString Project::GetDependencies(const wxString& configuration) const
{
    ...
    while(node) {
        if(node->GetName() == wxT("Dependencies") && node->XmlGetPropVal(wxT("Name"), wxEmptyString) == configuration) {
            // we have our match
            wxXmlNode* child = node->GetChildren();
            while(child) {
                if(child->GetName() == wxT("Project")) { result.Add(XmlUtils::ReadString(child, wxT("Name"))); }
...
void Project::SetDependencies(wxArrayString& deps, const wxString& configuration)
{
    ...
    node->XmlAddProperty(wxT("Name"), configuration);
    ...
    for(size_t i = 0; i < deps.GetCount(); i++) {
        ...
        child->XmlAddProperty(wxT("Name"), deps.Item(i));
        node->AddChild(child);
    }
...
//---------------------------------------------------------------------------
bool Project::GetUserData(const wxString& name, ObjectData* obj)
{
...
//---------------------------------------------------------------------------
bool Project::SetUserData(const wxString& name, ObjectData* obj)
{
    if(!m_doc.IsOk()) { return false; }
    ...
    dataNode->XmlAddProperty(wxT("Name"), name);
...
wxString Project::GetProjectInternalType() const
{
    if(!m_doc.GetRoot()) 
        return "";
    return m_doc.GetRoot()->XmlGetPropVal(wxT("InternalType"), wxEmptyString);
}
//---------------------------------------------------------------------------
void Project::GetAllPluginsData(std::map<wxString, wxString>& pluginsDataMap)
{
    ...
    while(child) {
        if(child->GetName() == wxT("Plugin")) {
            ...
            pluginsDataMap[child->XmlGetPropVal(wxT("Name"), wxEmptyString)] = content;
        }
        child = child->GetNext();
    }
}
...
void Project::SetPluginData(const wxString& pluginName, const wxString& data, bool saveToXml)
{
    ...
    if(!plugin) {
        ...
        plugin->XmlAddProperty(wxT("Name"), pluginName);
    }
...
void Project::ConvertToUnixFormat(wxXmlNode* parent)
{
    ...
    while(child) {
        ...
        if(child->GetName() == wxT("VirtualDirectory")) {
            ...
            wxXmlProperty* props = child->XmlGetProperties();
            // Convert the path to unix format
            while(props) {
                if(props->GetName() == wxT("Name")) {
                    wxString val = props->GetValue();
...
wxString Project::GetBestPathForVD(const wxString& vdPath)
{
    ...
    for(size_t i = 0; i < subDirs.GetCount(); i++) {
        wxFileName fn(bestPath + gsgsPathSep + subDirs.Item(i), wxEmptyString);
        if(fn.DirExists()) {
            bestPath << gsgsPathSep << subDirs.Item(i);
            matches++;
        } else {
            break;
        }
...
wxArrayString Project::DoBacktickToPreProcessors(const wxString& backtick)
{
    ...
    CompilerCmdParser cclp(cmpOption);
...
wxArrayString Project::DoBacktickToIncludePath(const wxString& backtick)
{
    wxArrayString paths;
    wxString cmpOption = DoExpandBacktick(backtick);
    CompilerCmdParser cclp(cmpOption, GetFileName().GetPath());
...
//---------------------------------------------------------------------------
wxString Project::GetProjectIconName() const { return m_doc.GetRoot()->XmlGetPropVal(wxT("IconIndex"), "gear16"); }
//---------------------------------------------------------------------------
                                    ...
                                    wxArrayString& regexes)
{
    ...
    for(size_t n = 0; n < excludePaths.GetCount(); ++n) {
        ...
        fn.MakeRelativeTo(RealPath(toplevelDir));
...
wxString Project::DoFormatVirtualFolderName(const wxXmlNode* node) const
{
    ...
    while(p) {
        if(p->GetName() == "VirtualDirectory")
            q.push_front(p->XmlGetPropVal("Name", ""));
        else
            break;
        p = p->GetParent();
    }
...
//---------------------------------------------------------------------------
const gsgsStringSet& Project::GetExcludeConfigForFile(const wxString& filename) const
{
...
//---------------------------------------------------------------------------
void Project::SetExcludeConfigsForFile(const wxString& filename, const gsgsStringSet& configs)
{
    clProjectFile::Ptr_t pfile = GetFile(filename);
    if(!pfile) { return; }
...
//---------------------------------------------------------------------------
static void ProcessIncludes(const wxArrayString& paths, const wxString& wd, gsgsStringSet& res,
                            std::vector<wxString>& pathsVec)
{
    // Convert the include paths to full path, prepend -I and add them
    for(const wxString& path : paths) {
        wxFileName fnPath(path, "");
        if(!wd.IsEmpty() && fnPath.IsRelative()) { fnPath.MakeAbsolute(wd); }
...
//---------------------------------------------------------------------------
static void ProcessMacros(const wxArrayString& macros, gsgsStringSet& res)
{
...
//---------------------------------------------------------------------------
wxString Project::GetCompileLineForCXXFile(const gsgsStringMap& compilersGlobalPaths, BuildConfigPtr buildConf,
                                           const wxString& filenamePlaceholder, bool cxxFile)
{
    ...
    if(compilersGlobalPaths.count(compiler->GetName())) {
        ...
        for(const wxString& compilerPath : compilerPaths) {
...
wxString Project::DoExpandBacktick(const wxString& backtick)
{
    ...
    if(cmpOption.StartsWith(wxT("$(shell "), &tmp) || cmpOption.StartsWith(wxT("`"), &tmp)) {
        ...
        wxString expandedValue;
        {
            ...
            // Check the cache
            if(!gsgs_Workspace().GetBacktickValue(cmpOption, expandedValue)) {
                ...
                gsgs_Workspace().SetBacktickValue(cmpOption, expandedValue);
            }
...
//---------------------------------------------------------------------------
void Project::CreateCompileCommandsJSON(JSONObject& compile_commands, const gsgsStringMap& compilersGlobalPaths)
{
    ...
    std::for_each(m_filesTable.begin(), m_filesTable.end(), [&](const FilesMap_t::value_type& vt) {
        ...
        FileUtil::FileType fileType = FileUtil::GetType(fullpath);
        if(fileType == FileUtil::TypeSourceC) {
            compilePattern = cFilePattern;
        } else if(fileType == FileUtil::TypeSourceCpp) {
            compilePattern = cxxFilePattern;
        } else if(fileType == FileUtil::TypeHeader) {
            compilePattern = cxxFilePattern;
        }
        ...
        if(!compilePattern.IsEmpty()) {
            ...
            compile_commands.addProperty(json);
        }
    });
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void Project::AssociateToWorkspace(CxxWS* workspace) 
{ 
    m_workspace = workspace; 
}
//---------------------------------------------------------------------------
CxxWS* Project::GetWorkspace()
{
    if(!m_workspace) 
    {
...
//---------------------------------------------------------------------------
const CxxWS* Project::GetWorkspace() const
{
    if(!m_workspace) {
        return &gsgs_CppWorker();
...
//---------------------------------------------------------------------------
void Project::GetCompilers(gsgsStringSet& compilers)
{
...
//---------------------------------------------------------------------------
void Project::ReplaceCompilers(gsgsStringMap& compilers)
{
    ProjectSettingsPtr pSettings = GetSettings();
    CHECK_PTR_RET(pSettings);
    ...
    if(buildConf && !buildConf->IsCustomBuild()) {
        ...
        for(size_t i = 0; i < optionsArr.GetCount(); ++i) {
            ...
            if(expandedCmpOption != cmpOption) {
                ...
                CompilerCmdParser cclp(expandedCmpOption, GetFileName().GetPath());
...
void Project::GetUnresolvedMacros(const wxString& configName, wxArrayString& vars) const
{
    ...
    if(buildConfig) {
        ...
        if(reEnvironmentVar.IsValid()) {
            ...
            includePaths = MacroManager::Instance()->Expand(includePaths, GetManager(), GetName(), configName);
            while(true) {
            ...
            libPaths = MacroManager::Instance()->Expand(libPaths, GetManager(), GetName(), configName);
            while(true) {
...
void Project::GetFiles(const wxString& vdFullPath, wxArrayString& files)
{
    ...
    clProjectFolder::Ptr_t parentFolder = vdFullPath.IsEmpty() ? GetRootFolder() : GetFolder(vdFullPath);
    if(!parentFolder) return;
    const gsgsStringSet& filesSet = parentFolder->GetFiles();
...
void Project::RemoveExcludeConfigForFile(const wxString& filename, const wxString& configName)
{
    ...
    gsgsStringSet& excludeConfigs = pfile->GetExcludeConfigs();
...
//===--------------------------------------------------------------------------------
void Project::CreateCompileFlags(const gsgsStringMap& compilersGlobalPaths)
{
    BuildConfigPtr buildConf = GetBuildConfiguration();
    if(!buildConf) { return; }

    CompilerPtr cmp = (buildConf) ? buildConf->GetCompiler() : nullptr;
    gsgsStringSet macroSet;
    std::vector<wxString> pathsVec;
    ...
    if(buildConf->IsCustomBuild() && (buildConf->GetBuilder() != "CMake")) {
        ...
        gsgsStringSet pathsSet;

        CompilerCmdParser cxxParser(cxxFilePattern);
        CompilerCmdParser cParser(cFilePattern);
    ...
    FileUtil::WriteFileContent(compile_flags, compile_flags_content);
}
//---------------------------------------------------------------------------
    ...
    while(!q.empty()) {
        ...
        while(child) {
            if(child->GetName() == "VirtualDirectory") {
                wxString name = child->XmlGetPropVal("Name", "");
...
clProjectFile::Ptr_t clProjectFolder::AddFile(Project* project, const wxString& fullpath)
{
    ...
    node->XmlAddProperty(wxT("Name"), tmp.GetFullPath(wxPATH_UNIX));
...
void clProjectFile::Delete(Project* project, bool deleteXml)
{
    ...
    if(is_excluded) { project->m_excludeFiles.erase(GetFilename()); }
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void clProjectFile::SetExcludeConfigs(Project* project, const gsgsStringSet& excludeConfigs)
{
