...
wxDECLARE_EXPORTED_EVENT(_gsgsAPI, wxPHP_PARSE_STARTED, gsgs::ParseEvent);
wxDECLARE_EXPORTED_EVENT(_gsgsAPI, wxPHP_PARSE_ENDED, gsgs::ParseEvent);
wxDECLARE_EXPORTED_EVENT(_gsgsAPI, wxPHP_PARSE_PROGRESS, gsgs::ParseEvent);
...
public:
    ...
    typedef SharedPtr<PHPLookupTable> Ptr_t;
                                             ...
                                             GoindDownFunc pFuncGoingDown, bool parseFuncBodies)
{
    try {
        ...
        {
            gsgs::ParseEvent event(wxPHP_PARSE_STARTED);
        ...
        for(size_t i = 0; i < files.GetCount(); ++i) {
            ...
            {
                gsgs::ParseEvent event(wxPHP_PARSE_PROGRESS);
                event.SetTotalFiles(files.GetCount());
            ...
            if(FileUtil::GetType(fnFile.GetFullName()) != FileUtil::TypePhp) { reParseNeeded = false; }

            if(reParseNeeded) {
                // For performance reaons, load the file into memory and then parse it
                wxFileName fnSourceFile(files.Item(i));
                wxString content;
                if(!FileUtil::ReadFileContent(fnSourceFile, content, wxConvISO8859_1)) {
                    gsgsWARNING() << "PHP: Failed to read file:" << fnSourceFile << "for parsing" << clEndl;
                    continue;
                }
        ...
        gsgsDEBUG1() << _("PHP: parsed ") << files.GetCount() << " in " << elapsedMs << " milliseconds" << clEndl;

        {
            gsgs::ParseEvent event(wxPHP_PARSE_ENDED);
            event.SetTotalFiles(files.GetCount());
        ...
        gsgsWARNING() << "PHPLookupTable::UpdateSourceFiles:" << e.GetMessage() << clEndl;
    }
}
