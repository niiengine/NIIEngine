...
namespace gsgs
{
    ...
    //-------------------------------------------------------------------------------
    wxString GetBestLabel(Command::Ptr_t command)
    {
        wxString label, text;
        if(command) {
    ...
    //-------------------------------------------------------------------------------
    Command::Ptr_t CommandProcessorBase::GetOpenCommand()
    {
        Command::Ptr_t command(NULL);

        size_t size = GetCommands().size();
        if(size && GetCommands().at(size - 1)->IsOpen()) { command = GetCommands().at(size - 1); }
    ...
    void CommandProcessorBase::DecrementCurrentCommand()
    {
        ...
        if(GetOpenCommand()) { ProcessOpenCommand(); }
    ...
    void CommandProcessorBase::SetUserLabel(const wxString& label)
    {
        ...
        Command::Ptr_t command = GetActiveCommand();
        if(command) { command->SetUserLabel(label); }
    }
    //-------------------------------------------------------------------------------
    void CommandProcessorBase::OnTBUnRedo(wxCommandEvent& event)
    {
        PopulateUnRedoMenu(dynamic_cast<gsgs::ToolBar*>(event.GetEventObject()), event.GetId());
    }
    //-------------------------------------------------------------------------------
    void CommandProcessorBase::PopulateUnRedoMenu(gsgs::ToolBar* tb, wxWindowID toolId)
    {
        CHECK_PTR_RET(tb);
        wxMenu menu;
    ...
    void CommandProcessorBase::DoPopulateUnRedoMenu(wxMenu& menu, bool undoing)
    {
        ...
        if(undoing) {
            if(GetCommands().size() > 0) {
                for(Command::Vec_t::const_reverse_iterator iter = GetCommands().rbegin() + GetNextUndoCommand();
                    iter != GetCommands().rend(); ++iter) {
                    Command::Ptr_t command = *iter;
                    if(command) {
                        wxString label;
                        if(!command->GetUserLabel().empty()) {
                            if(command->GetName().Contains(":")) { label = command->GetName().BeforeFirst(':') + ": "; }
                            label << command->GetUserLabel();
                        } else {
            ...
            for(Command::Vec_t::const_iterator iter = GetCommands().begin() + GetCurrentCommand() + 1;
    ...
    void CommandProcessorBase::PrepareLabelledStatesMenu(wxMenu* editmenu)
    {
        ...
        if(menuitem) { editmenu->Delete(menuitem); }
    ...
    void CommandProcessorBase::PopulateLabelledStatesMenu(wxMenu* menu)
    {
        ...
        int id = FIRST_MENU_ID;
        for(Command::Vec_t::const_iterator iter = GetCommands().begin(); iter != GetCommands().end(); ++iter) {
            Command::Ptr_t command = *iter;
            if(command && !command->GetUserLabel().empty()) { menu->Append(id, command->GetUserLabel()); }
    ...
    void CommandProcessorBase::OnLabelledStatesMenuItem(wxCommandEvent& event)
    {
        ...
        if(index < GetCurrentCommand()) {
            ...
            for(int n = 0; n < count; ++n) {
                if(DoUndo()) { DecrementCurrentCommand(); }
            }
        } else {
            const int count = index - GetCurrentCommand();
            for(int n = 0; n < count; ++n) {
                if(DoRedo()) { IncrementCurrentCommand(); }
            }
        }
    }
    //-------------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------------
    Command::Ptr_t CommandProcessorBase::GetActiveCommand() const
    {
        Command::Ptr_t command(NULL);
