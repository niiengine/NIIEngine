...
void PHPEntityFunctionAlias::Store(PHPLookupTable* lookup)
{
    try {
        ...
        gsgs_Warning("PHPEntityFunctionAlias::Store: %s", exc.GetMessage());
    }
}
...
void PHPEntityFunctionAlias::FromJSON(const JSONObject& json)
{
    BaseFromJSON(json);
    json.getObject("realName").getValue(m_realname);
    json.getObject("scope").getValue(m_scope);
    if(json.isObjectExist("func")) {
        JSONObject func = json.getObject("func");
        m_func.Reset(new PHPEntityFunction());
        m_func->FromJSON(func);
    }
}

JSONObject PHPEntityFunctionAlias::ToJSON() const
{
    JSONObject json = BaseToJSON("a");
    json.addProperty("realName", m_realname);
    json.addProperty("scope", m_scope);
    if(m_func) {
        JSONObject func = m_func->ToJSON();
