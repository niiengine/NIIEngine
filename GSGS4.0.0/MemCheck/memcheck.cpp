    ...
    , m_memcheckProcessor(NULL)
{
    ...
    // gsgs_Event().Connect( wxEVT_INIT_DONE, wxCommandEventHandler(MemCheckPlugin::OnInitDone), NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(MemCheckPlugin::OnWorkspaceLoaded),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(MemCheckPlugin::OnWorkspaceClosed),
                                  NULL, this);
    ...
    m_tabHelper.reset(new gsgs::TabTogglerHelper(_("MemCheck"), m_outputView, "", NULL));
...
void MemCheckPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void MemCheckPlugin::UnPlug()
{
    ...
    // gsgs_Event().Disconnect( wxEVT_INIT_DONE, wxCommandEventHandler(MemCheckPlugin::OnInitDone), NULL,
    // this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(MemCheckPlugin::OnWorkspaceLoaded),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(MemCheckPlugin::OnWorkspaceClosed),
                                     NULL, this);
...
{
    CHECK_CL_SHUTDOWN()
    CxxWS* workspace = m_mgr->GetWorkspace();
...
void MemCheckPlugin::OnProcessOutput(gsgs::Event& event)
{
    m_mgr->AppendOutputTabText(kOutputTab_Output, event.GetString());
}

void MemCheckPlugin::OnProcessTerminated(gsgs::Event& event)
{
...
wxString MemCheckPlugin::PrepareCommand(const wxString& projectName, wxString& wd)
{
    ...
    if(!proj) {
        gsgsWARNING() << "MemCheckPlugin::PrepareCommand(): could not find project:" << projectName;
        return wxEmptyString;
    }

    BuildConfigPtr bldConf = gsgs_CppWorker().GetProjBuildConf(projectName, wxEmptyString);
    if(!bldConf) {
        gsgsWARNING() << "MemCheckPlugin::PrepareCommand(): failed to find project configuration for project:"
                    << projectName;
        return wxEmptyString;
    }
