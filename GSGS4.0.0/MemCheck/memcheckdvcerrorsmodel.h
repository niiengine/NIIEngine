...
// File name            : memcheckdvcerrorsmodel.h
//
// -------------------------------------------------------------------------
// A
//              _____           _      _     _ _
...
#ifndef __MemCheckDVCErrorsModel_GUARD__H__
#define __MemCheckDVCErrorsModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a MemCheckDVCErrorsModel_Item class, simply use:
 * @code
 * MemCheckDVCErrorsModel_Item* itemData = reinterpret_cast<MemCheckDVCErrorsModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class MemCheckDVCErrorsModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    MemCheckDVCErrorsModel_Item*           m_parent;
    wxVector<MemCheckDVCErrorsModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    MemCheckDVCErrorsModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~MemCheckDVCErrorsModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<MemCheckDVCErrorsModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(MemCheckDVCErrorsModel_Item* child) {
        wxVector<MemCheckDVCErrorsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(MemCheckDVCErrorsModel_Item* child) {
        wxVector<MemCheckDVCErrorsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(MemCheckDVCErrorsModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<MemCheckDVCErrorsModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(MemCheckDVCErrorsModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<MemCheckDVCErrorsModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<MemCheckDVCErrorsModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    MemCheckDVCErrorsModel_Item* GetParent() {
        return m_parent;
    }
...
class MemCheckDVCErrorsModel : public wxDataViewModel
{
protected:
    wxVector<MemCheckDVCErrorsModel_Item*> m_data;
    unsigned int m_colCount;

public:
    MemCheckDVCErrorsModel();
    virtual ~MemCheckDVCErrorsModel();
    ...
    void SetData(const wxVector<MemCheckDVCErrorsModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<MemCheckDVCErrorsModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<MemCheckDVCErrorsModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __MemCheckDVCErrorsModel_GUARD__H__
