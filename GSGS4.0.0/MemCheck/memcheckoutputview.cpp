...
void MemCheckOutputView::LoadErrors()
{
    ...
    if(m_mgr->IsOpen())
        m_workspacePath = m_mgr->GetWorkspace()->GetFileName().GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR);
    else
...
void MemCheckOutputView::ApplyFilterSupp(unsigned int mode)
{
    ...
    case FILTER_STRING:
        ...
        for(MemCheckIterTools::ErrorListIterator it = MemCheckIterTools::Factory(errorList, wxEmptyString, iterFlags);
            it != errorList.end(); ++it) {
            if(m_checkBoxInvert->IsChecked() != StringUtils::Search((*it).toString().wc_str(), offset,
