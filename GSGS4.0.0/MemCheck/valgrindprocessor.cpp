...
wxArrayString ValgrindMemcheckProcessor::GetSuppressionFiles()
{
    ...
    if(gsgs_CppWorker().IsOpen() && m_settings->GetValgrindSettings().GetSuppFileInPrivateFolder()) {
        wxTextFile defaultSupp(
            wxFileName(gsgs_CppWorker().GetPrivateFolder(), "valgrind.memcheck.supp").GetFullPath());
                                                    ...
                                                    wxString& command_args)
{
    ...
    if(m_settings->GetValgrindSettings().GetOutputInPrivateFolder() || m_outputLogFileName.IsEmpty()) {
        if(gsgs_CppWorker().IsOpen())
            m_outputLogFileName =
                wxFileName(gsgs_CppWorker().GetPrivateFolder(), "valgrind.memcheck.log.xml").GetFullPath();
        else
            m_outputLogFileName =
                wxFileName(PathManager::Get().GetTempDir(), "valgrind.memcheck.log.xml").GetFullPath();
    }
