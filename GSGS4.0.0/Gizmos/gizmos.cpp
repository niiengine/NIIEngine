...
static wxString MI_NEW_CODELITE_PLUGIN = wxT("Create new gsgs plugin...");
...
void WizardsPlugin::CreateToolBar(gsgs::ToolBar* toolbar)
{
    wxUnusedVar(toolbar);
...
void WizardsPlugin::CreatePluginMenu(wxMenu* pluginsMenu)
{
    ...
    item = new wxMenuItem(menu, ID_MI_NEW_CODELITE_PLUGIN, _("New gsgs Plugin Wizard..."), wxEmptyString,
                          wxITEM_NORMAL);
    menu->Append(item);
...
void WizardsPlugin::DoCreateNewPlugin()
{
    ...
    if(wiz.Run(data)) {
        ...
        if(!fn.MakeRelativeTo(wxFileName(data.GetProjectPath()).GetPath())) {
            gsgsLogMessage(wxT("Warning: Failed to convert paths to relative path."));
        }
        ...
        if(!ReadFileWithConversion(filename, content)) {
            wxMessageBox(_("Failed to load wizard's file 'plugin.cpp.wizard'"), _("光栅故事"), wxICON_WARNING | wxOK);
            return;
        }
        ...
        gsgs::SourceFormatEvent evtFormat(wxEVT_FORMAT_STRING);
        evtFormat.SetInputString(content);
        evtFormat.SetFileName(srcFile.GetFullPath());
        gsgs_Event().ProcessEvent(evtFormat);
        ...
        wxFileName cmakeFile(gsgs_Path().GetDataDir(), "CMakeLists.txt.plugin.wizard");
        cmakeFile.AppendDir("templates");
        cmakeFile.AppendDir("gizmos");
        if(cmakeFile.FileExists()) {
            wxString cmakeContent;
            if(FileUtil::ReadFileContent(cmakeFile, cmakeContent)) {
                ...
                FileUtil::WriteFileContent(cmakeFile, cmakeContent);
            }
        }
...
{
    NewClassDlg* dlg = new NewClassDlg(gsgs_Event().TopFrame(), m_mgr);
...
void WizardsPlugin::CreateClass(NewClassInfo& info)
{
    ...
    // inactive one.
    OptionsConfig * options = gsgs_EditorConfig().GetOptions(); // Globals first
    wxString TargetProj = info.virtualDirectory.BeforeFirst(wxT(':'));
    if(!TargetProj.empty()) {
        gsgs_CppWorker().GetLocalWorkspace()->GetOptions(options,
    ...
    wxString srcFile;
    srcFile << info.path << gsgsPathSep << info.fileName << wxT(".cpp");

    wxString hdrFile;
    hdrFile << info.path << gsgsPathSep << info.fileName << headerExt;
    ...
    if(info.isSingleton) { header << separator << wxT("static ") << info.name << wxT("* gInstance;\n\n"); }
    ...
    if(!info.isInline) {
        ...
        if(info.isSingleton) {
            ...
            cpp << nsPrefix << info.name << wxT("* ") << nsPrefix << info.name << wxT("::gInstance = NULL;\n\n");
        } else {
            cpp << "\n";
        }
        ...
        if(info.isSingleton) {
            ...
            cpp << wxT("{\n");
            cpp << separator << wxT("if (gInstance == NULL) {\n");
            cpp << separator << separator << wxT("gInstance = new ") << info.name << wxT("();\n");
            cpp << separator << wxT("}\n");
            cpp << separator << wxT("return gInstance;\n");
            cpp << wxT("}\n\n");

            cpp << wxT("void ") << info.name << wxT("::Release()\n");
            cpp << wxT("{\n");
            cpp << separator << wxT("if (gInstance) {\n");
            cpp << separator << separator << wxT("delete gInstance;\n");
            cpp << separator << wxT("}\n");
            cpp << separator << wxT("gInstance = NULL;\n");
            cpp << wxT("}\n\n");
        }
    ...
    bool smartAddFiles = gsgs_EditorConfig().GetOptions()->GetOptions() & OptionsConfig::Opt_SmartAddFiles;
    ...
    gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(parseEvent);
}
