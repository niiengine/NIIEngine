...
NewClassDlg::NewClassDlg(wxWindow* parent, IManager* mgr)
    ...
    , m_mgr(mgr)
{
    gsgs_EditorConfig().ReadObject(wxT("NewClassDlgData"), &m_options);
...
bool NewClassDlg::ValidateInput()
{
    ...
    if(!IsValidCppIndetifier(m_textClassName->GetValue())) {
        ...
        wxMessageBox(msg, _("光栅故事"), wxOK | wxICON_WARNING);
        return false;
    }
    ...
    if(GetClassFile().IsEmpty()) {
        wxMessageBox(_("Empty file name"), _("光栅故事"), wxOK | wxICON_WARNING);
        return false;
    }

    wxString cpp_file;
    cpp_file << GetClassPath() << gsgsPathSep << GetClassFile() << wxT(".cpp");
    if(wxFileName::FileExists(cpp_file)) {
        if(wxMessageBox(
               ...
               _("光栅故事"), wxYES_NO | wxICON_WARNING) == wxNO) {
            return false;
        }
    }
    wxString h_file;
    h_file << GetClassPath() << gsgsPathSep << GetClassFile() << wxT(".h");
    ...
    if(GetVirtualDirectoryPath().IsEmpty()) {
        wxMessageBox(_("Please select a virtual directory"), _("光栅故事"), wxOK | wxICON_WARNING);
        return false;
    }
    return true;
}
...
void NewClassDlg::DoSaveOptions()
{
    ...
    gsgs_EditorConfig().WriteObject(wxT("NewClassDlgData"), &m_options);
}
