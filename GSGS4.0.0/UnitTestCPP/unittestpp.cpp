    ...
    , m_proc(NULL)
{
    ...
    gsgs_Event().Connect(wxEVT_CMD_EXECUTE_ACTIVE_PROJECT, gsgsExecuteEventHandler(UnitTestPP::OnRunProject),
                                  NULL, this);
    ...
    m_tabHelper.reset(new gsgs::TabTogglerHelper(_("UnitTest++"), m_outputPage, "", NULL));
...
void UnitTestPP::CreateToolBar(gsgs::ToolBar* toolbar)
{
...
void UnitTestPP::CreatePluginMenu(wxMenu* pluginsMenu)
{
    ...
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR, &UnitTestPP::OnEditorContextMenu, this);
}
...
void UnitTestPP::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR, &UnitTestPP::OnEditorContextMenu, this);
    wxDELETE(m_proc);
    m_output.Clear();
}
...
void UnitTestPP::OnNewClassTest(wxCommandEvent& e)
{
    ...
    if(GetUnitTestProjects().empty()) {
        if(wxMessageBox(
               ...
               _("光栅故事"), wxYES_NO | wxCANCEL) == wxYES) {
...
void UnitTestPP::DoCreateSimpleTest(const wxString& name, const wxString& projectName, const wxString& filename)
{
    ...
    if(!proj) {
        // no such project!
        wxMessageBox(_("Could not find the target project"), _("gsgs"), wxOK | wxICON_ERROR);
        return;
    }
...
void UnitTestPP::OnRunUnitTestsUI(wxUpdateUIEvent& e)
{
    ...
    if(m_proc) {
        ...
        e.Enable(gsgs_CppWorker().IsOpen() && gsgs_CppWorker().GetActiveProject() &&
                 gsgs_CppWorker().GetActiveProject()->GetProjectInternalType() == wxT("UnitTest++"));
    }
}
...
IEditor* UnitTestPP::DoAddTestFile(const wxString& filename, const wxString& projectName)
{
    ...
    if(wxFileName::FileExists(filename) == false) {
        ...
        if(!file.IsOpened()) {
            wxMessageBox(wxString::Format(_("Could not create target file '%s'"), filename.c_str()), _("光栅故事"),
                         wxICON_WARNING | wxOK);
            return NULL;
        }
...
void UnitTestPP::OnProcessRead(gsgs::ProcessEvent& e) { m_output << e.GetOutput(); }

void UnitTestPP::OnProcessTerminated(gsgs::ProcessEvent& e)
{
    wxDELETE(m_proc);
    ...
    if(summary.totalTests == 0) {
        ::wxMessageBox(_("Project contains 0 tests. Nothing to be done"), "gsgs");
        return;
    }
...
void UnitTestPP::OnRunProject(gsgs::ExecuteEvent& e)
{
    e.Skip();
    // Sanity
    ...
    ProjectPtr pProj = gsgs_CppWorker().GetProject(e.GetTargetName());
    CHECK_PTR_RET(pProj);
...
void UnitTestPP::OnEditorContextMenu(ContextMenuEvent& e)
{
    e.Skip();
    IEditor* editor = m_mgr->GetActiveEditor();
