...
UnitTestsPage::UnitTestsPage(wxWindow* parent, IManager* mgr)
    ...
    , m_mgr(mgr)
{
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(UnitTestsPage::OnWorkspaceClosed), NULL,
                                  this);
}

UnitTestsPage::~UnitTestsPage()
{
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(UnitTestsPage::OnWorkspaceClosed),
                                     NULL, this);
}
