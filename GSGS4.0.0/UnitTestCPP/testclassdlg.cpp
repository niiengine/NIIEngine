...
void TestClassDlg::OnButtonOk(wxCommandEvent& e)
{
    ...
    if(m_checkListMethods->GetCount() == 0) {
        wxMessageBox(_("There are no tests to generate"), _("光栅故事"), wxICON_WARNING | wxOK);
        return;
    }
    EndModal(wxID_OK);
}
...
void TestClassDlg::OnShowClassListDialog(wxCommandEvent& e)
{
    ...
    gsgs::OpenResourceDialog dlg(m_manager->GetTheApp()->GetTopWindow(), m_manager, "");
