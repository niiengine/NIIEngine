...
protected:
    ...
    virtual bool setup(void)
    {
        ...
        mRoot = N_new Engine("niiengine");//,  mConfigPath + "ogre.cfg", mResourcePath + "Ogre.log");
...
//        setupResources();
        ...
        ResourceLoadManager::getSingleton().setupAll();
    ...
    virtual bool initializeShaderGenerator(SpaceManager* sceneMgr)
    {
        if(RTShader::ShaderGenerator::initialize())
        {
            ...
            VFS * vfs = N_Only(ResourceLoad).getVFS("RTShaderLib", "Popular");
            if(vfs)
            {
            ...
            ResourceLoadManager::getSingleton().add(shaderCoreLibsPath , "FileSystem");
...
#endif
                ResourceLoadManager::getSingleton().add(archName, typeName, secName);
