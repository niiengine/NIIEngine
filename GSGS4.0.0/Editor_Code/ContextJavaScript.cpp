...
namespace gsgs
{
    ContextJavaScript::ContextJavaScript(Editor* editor)
        : ContextBase(editor)
    {
    ...
    void ContextJavaScript::ApplySettings()
    {
        ...
        if(gsgs_EditorConfig().IsOk()) { lexPtr = gsgs_EditorConfig().GetLexer(GetName()); }
    ...
    ContextBase* ContextJavaScript::NewInstance(Editor* container) { return new ContextJavaScript(container); }
