...
namespace gsgs
{
        ...
        , m_errorsFirstLine(false)
    {
        ...
        m_styler.Reset(new FindResultsStyler());
        CreateViewImpl();
        ...
        gsgs_Event().Connect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(OutputTabWindow::OnTheme), NULL,
                                      this);
    ...
    OutputTabWindow::~OutputTabWindow()
    {
        ...
        gsgs_Event().Disconnect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(OutputTabWindow::OnTheme), NULL, this);
    ...
    void OutputTabWindow::InitStyle(wxStyledTextCtrl* sci, int lexer, bool folding)
    {
        ...
        {
            sci->StyleSetBackground(i, gsgs_Style().GetOutputPaneBgColour());
            sci->StyleSetForeground(i, gsgs_Style().GetOutputPaneFgColour());
        }

        wxFont defFont = wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
        wxFont font = gsgs_EditorConfig().GetLexer("text")->GetFontForSyle(0);
    ...
    void OutputTabWindow::CreateViewImpl()
    {
    ...
    void OutputTabWindow::AppendText(const wxString & text)
    {
        if(m_sci) 
        {
            ...
            { 
                gsgs_Manager()->ShowOutputPane(m_name); 
            }
            //----------------------------------------------
            // enable writing
            m_sci->SetReadOnly(false);
            ...
            wxString modText;
            ::StripTerminalColouring(text, modText);
    ...
    void OutputTabWindow::OnToggleHoldOpen(wxCommandEvent& e)
    {
        ...
        if(book && (sel = book->GetSelection()) != wxNOT_FOUND) {
            gsgs_EditorConfig().SetPaneStickiness(book->GetPageText(sel), e.IsChecked());
        }
    }

    void OutputTabWindow::OnTheme(wxCommandEvent& e)
    {
        e.Skip();
