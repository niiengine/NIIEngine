...
#ifndef __CompilersFoundModel_GUARD__H__
#define __CompilersFoundModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a CompilersFoundModel_Item class, simply use:
 * @code
 * CompilersFoundModel_Item* itemData = reinterpret_cast<CompilersFoundModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class CompilersFoundModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    CompilersFoundModel_Item*           m_parent;
    wxVector<CompilersFoundModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    CompilersFoundModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~CompilersFoundModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<CompilersFoundModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(CompilersFoundModel_Item* child) {
        wxVector<CompilersFoundModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(CompilersFoundModel_Item* child) {
        wxVector<CompilersFoundModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(CompilersFoundModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<CompilersFoundModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(CompilersFoundModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<CompilersFoundModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<CompilersFoundModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    CompilersFoundModel_Item* GetParent() {
        return m_parent;
    }
...
class CompilersFoundModel : public wxDataViewModel
{
protected:
    wxVector<CompilersFoundModel_Item*> m_data;
    unsigned int m_colCount;

public:
    CompilersFoundModel();
    virtual ~CompilersFoundModel();
    ...
    void SetData(const wxVector<CompilersFoundModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<CompilersFoundModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<CompilersFoundModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __CompilersFoundModel_GUARD__H__
