...
namespace gsgs
{
    ...
    #define VALIDATE_WORKSPACE() \
        if(gsgs_Workspace().IsOpen() == false) { return; }

    #define VALIDATE_WORKSPACE_FALSE() \
        if(!gsgs_Workspace().IsOpen()) { return false; }
    ...
    public:
        AddFunctionsImpDlg(wxWindow* parent, const TagEntryPtrList& tags, const wxString& targetFile);
    ...
    //----------------------------------------------------------------------------------
    AddFunctionsImpDlg::AddFunctionsImpDlg(wxWindow* parent, const TagEntryPtrList& tags, const wxString& targetFile)
        : AddFunctionsImplBaseDlg(parent)
    {
        m_dvListCtrl->SetBitmaps(&gsgs_Image().GetBitmaps());
        int functionIndex = gsgs_Image().GetMimeImageId(gsgs::ImageManager::kFunctionPublic);
    ...
    END_EVENT_TABLE()
    //----------------------------------------------------------------------------------
    ContextCpp::ContextCpp(Editor* container)
        : ContextBase(container)
        , m_rclickMenu(NULL)
    {
        Initialize();
        SetName("c++");
        gsgs_Event().Connect(wxEVT_CC_SHOW_QUICK_NAV_MENU,
                                      clCodeCompletionEventHandler(ContextCpp::OnShowCodeNavMenu), NULL, this);
        gsgs_Event().Bind(wxEVT_CCBOX_SELECTION_MADE, &ContextCpp::OnCodeCompleteFiles, this);
    }
    //----------------------------------------------------------------------------------
        ...
        , m_rclickMenu(NULL)
    {
        gsgs_Event().Connect(wxEVT_CC_SHOW_QUICK_NAV_MENU,
                                      clCodeCompletionEventHandler(ContextCpp::OnShowCodeNavMenu), NULL, this);
        gsgs_Event().Unbind(wxEVT_CCBOX_SELECTION_MADE, &ContextCpp::OnCodeCompleteFiles, this);
    }
    //----------------------------------------------------------------------------------
    ContextCpp::~ContextCpp()
    {
        gsgs_Event().Disconnect(wxEVT_CC_SHOW_QUICK_NAV_MENU,
                                         clCodeCompletionEventHandler(ContextCpp::OnShowCodeNavMenu), NULL, this);
        wxDELETE(m_rclickMenu);
    }
    //----------------------------------------------------------------------------------
    ContextBase* ContextCpp::NewInstance(Editor* container) { return new ContextCpp(container); }
    //----------------------------------------------------------------------------------
    ...
    bool ContextCpp::GetHoverTip(int pos)
    {
        ...
        if(gsgs_Frame()->GetMainBook()->GetActiveEditor(true) != &rCtrl) { return false; }
    ...
    //----------------------------------------------------------------------------------
    wxString ContextCpp::GetImageString(const gsgs::TagEntry& entry)
    {
    ...
    void ContextCpp::OnAddIncludeFile(wxCommandEvent& e)
    {
        ...
        AddIncludeFileDlg dlg(gsgs_Frame(), choice, rCtrl.GetText(), FindLineToAddInclude());
    ...
    void ContextCpp::DisplayFilesCompletionBox(const wxString& word)
    {
        ...
        if(!files.IsEmpty()) {
            ...
            for(size_t i = 0; i < files.GetCount(); ++i) {
                ...
                int imgID = 0;
                switch(FileUtil::GetType(fn.GetFullName())) {
                ...
                if(FileUtil::GetType(fn.GetFullName()) == FileUtil::TypeHeader ||
                   FileUtil::GetType(fn.GetFullName()) == FileUtil::TypeOther) {
                    entries.push_back(wxCodeCompletionBoxEntry::New(files.Item(i), imgID));
                }
            }
            CodeBoxManager::Get().ShowCompletionBox(&GetCtrl(), entries, bitmaps, CodeBox::kNone,
                                                                wxNOT_FOUND, this);
        }
    }
    //----------------------------------------------------------------------------------
    ...
    TagEntryPtr ContextCpp::GetTagAtCaret(bool scoped, bool impl)
    {
        ...
        if(!gsgs_Workspace().IsOpen()) return NULL;
        ...
        if(tags.empty()) {
            ...
            if(token.getOffset() != wxString::npos) {
                ...
                Editor* editor = gsgs_Frame()->GetMainBook()->OpenFile(rCtrl.GetFileName().GetFullPath(),
        ...
        SelectSymbolDialogEntry::List_t entries;
        std::for_each(tags.begin(), tags.end(), [&](TagEntryPtr tag) {
            SelectSymbolDialogEntry e;
            e.bmp = CodeBox::GetBitmap(tag);
        ...
        SelectSymbolDialog dlg(gsgs_Event().TopFrame(), entries);
    ...
    void ContextCpp::SwapFiles(const wxFileName& fileName)
    {
        ...
        otherFile.SetExt(FileUtil::GetType(fileName.GetFullName()) == FileUtil::TypeHeader ? "cpp" : "h");

        wxStandardID res = ::PromptMessageDialogWithoutCancel(_("No matched file was found, would you like to create one?"),
    ...
    //----------------------------------------------------------------------------------
    bool ContextCpp::FindSwappedFile(const wxFileName& rhs, gsgsStringSet& others)
    {
        CHECK_JS_RETURN_FALSE();
        ...
        if(FileUtil::GetType(rhs.GetFullName()) == FileUtil::TypeSourceC ||
           FileUtil::GetType(rhs.GetFullName()) == FileUtil::TypeSourceCpp) {
        ...
        if(others.empty()) {
            ...
            gsgs_Manager()->GetWorkspaceFiles(files, true);
    ...
    bool ContextCpp::TryOpenFile(const wxFileName& fileName, bool lookInEntireWorkspace)
    {
        if(fileName.FileExists()) {
            // we got a match
            wxString proj = gsgs_Manager()->GetProjectNameByFile(fileName.GetFullPath());
            return gsgs_Frame()->GetMainBook()->OpenFile(fileName.GetFullPath(), proj, wxNOT_FOUND, wxNOT_FOUND,
        ...
        for(size_t i = 0; i < files.size(); i++) {
            if(files.at(i).GetFullName() == fileName.GetFullName()) {
                wxString proj = gsgs_Manager()->GetProjectNameByFile(files.at(i).GetFullPath());
                return gsgs_Frame()->GetMainBook()->OpenFile(files.at(i).GetFullPath(), proj, wxNOT_FOUND,
                                                                   wxNOT_FOUND,
                                                                   (enum OF_extra)(OF_PlaceNextToCurrent | OF_AddJump));
            }
        }
    ...
    void ContextCpp::DoMakeDoxyCommentString(DoxygenComment& dc, const wxString& blockPrefix, wxChar keywordPrefix)
    {
        ...
        classPattern = ExpandAllVariables(classPattern, &gsgs_CppWorker(), editor.GetProjectName(), wxEmptyString,
                                          editor.GetFileName().GetFullPath());
        funcPattern = ExpandAllVariables(funcPattern, &gsgs_CppWorker(), editor.GetProjectName(), wxEmptyString,
                                         editor.GetFileName().GetFullPath());
    ...
    void ContextCpp::OnInsertDoxyComment(wxCommandEvent& event)
    {
        ...
        TagEntryPtrList tags = TagsManagerST::Get()->ParseBuffer(text);
        if(!tags.empty()) {
            ...
            // Join the lines back
            wxString doxyBlock = ::JoinLinesWithEOL(lines, GetCtrl().GetEOL());
    ...
    void ContextCpp::OnGenerateSettersGetters(wxCommandEvent& event)
    {
        ...
        if(tag->GetFile().CmpNoCase(editor.GetFileName().GetFullPath()) != 0) {
            ...
            msg << _("Would you like gsgs to open this file for you?");

            if(wxMessageBox(msg, _("光栅故事"), wxYES_NO) == wxYES) {
                wxString projectName = gsgs_Manager()->GetProjectNameByFile(tag->GetFile());
                gsgs_Frame()->GetMainBook()->OpenFile(tag->GetFile(), projectName, tag->GetLine());
            }
            return;
        }
        ...
        SettersGettersDlg dlg(gsgs_Event().TopFrame());
        if(!dlg.Init(tags, tag->GetFile(), lineno)) {
            ::wxMessageBox(_("Seems like you have all the getters/setters you need..."), _("gsgs"));
            return;
        }

        if(dlg.ShowModal() == wxID_OK) {
            EditorStateLocker locker(editor.GetCtrl());
    ...
    void ContextCpp::OnUpdateUI(wxUpdateUIEvent& event)
    {
        ...
        bool workspaceOpen = gsgs_Workspace().IsOpen();
    ...
    void ContextCpp::OnDbgDwellStart(wxStyledTextEvent& event)
    {
        ...
        if(gsgs_Manager()->GetDebuggerTip() && gsgs_Manager()->GetDebuggerTip()->IsShown()) return;
        ...
        if(dbgr && dbgr->IsRunning() && gsgs_Manager()->DbgCanInteract()) {
            if(gsgs_Manager()->GetDebuggerTip()->IsShown() && gsgs_Manager()->GetDebuggerTip()->m_expression == word) {
    ...
    void ContextCpp::OnMoveImpl(wxCommandEvent& e)
    {
        ...
        if(match) {
            ...
            if(DoGetFunctionBody(curPos, blockStartPos, blockEndPos, content)) {
                ...
                MoveFuncImplDlg dlg(gsgs_Event().TopFrame(), body, targetFile);
                if(dlg.ShowModal() == wxID_OK) {
                    ...
                    Editor* implEditor = gsgs_Frame()->GetMainBook()->OpenFile(targetFile);
                    if(implEditor) {

                        // Ensure that the file state is remained
                        int insertedLine = wxNOT_FOUND;
                        {
                            EditorStateLocker locker(implEditor->GetCtrl());
    ...
    void ContextCpp::OnOverrideParentVritualFunctions(wxCommandEvent& e)
    {
        ...
        if(scopeName.IsEmpty() || scopeName == wxT("<global>")) {
            wxMessageBox(_("Cant resolve scope properly. Found <") + scopeName + wxT(">"), _("光栅故事"),
        ...
        if(dlg.ShowModal() == wxID_OK) {
            ...
            Editor* implEditor = gsgs_Frame()->GetMainBook()->OpenFile(implFile);
            if(implEditor) {
                int insertedLine = wxNOT_FOUND;
                wxString sourceContent = implEditor->GetText();
        ...
        gsgs_Frame()->GetMainBook()->OpenFile(GetCtrl().GetFileName().GetFullPath());
    }
    //----------------------------------------------------------------------------------
    void ContextCpp::OnAddMultiImpl(wxCommandEvent& e)
    {
        ...
        if(scopeName.IsEmpty() || scopeName == wxT("<global>")) {
            wxMessageBox(_("'Add Functions Implementation' can only work inside valid scope, got (") + scopeName + wxT(")"),
                         _("光栅故事"), wxICON_INFORMATION | wxOK);
            return;
        }
        ...
        if(dlg.ShowModal() == wxID_OK) {
            ...
            // Open the C++ file
            Editor* editor = gsgs_Frame()->GetMainBook()->OpenFile(targetFile, wxEmptyString, 0);
            if(!editor) { return; }
    ...
    void ContextCpp::OnAddImpl(wxCommandEvent& e)
    {
        ...
        if(match) {
            ...
            if(DoGetFunctionBody(curPos, blockStartPos, blockEndPos, content)) {
                ...
                wxMessageBox(_("Function '") + tag->GetName() + _("' already has a body"), _("光栅故事"),
                             wxICON_WARNING | wxOK);
                return;
            }
    ...
    //----------------------------------------------------------------------------------
    void ContextCpp::DoFormatEditor(Editor* editor)
    {
        gsgs::SourceFormatEvent formatEvent(wxEVT_FORMAT_STRING);
        formatEvent.SetInputString(editor->GetText());
        formatEvent.SetFileName(editor->GetFileName().GetFullPath());
        gsgs_Event().ProcessEvent(formatEvent);
    ...
    void ContextCpp::OnFileSaved()
    {
        ...
        if(!IsJavaScript()) {
            ...
            ParseRequest* parsingRequest = new ParseRequest(gsgs_Manager());
            ...
            gsgs_Manager()->UpdatePreprocessorFile(&GetCtrl());
        }
    }
    //----------------------------------------------------------------------------------
    void ContextCpp::ApplySettings()
    {
        ...
        if(gsgs_EditorConfig().IsOk()) { lexPtr = gsgs_EditorConfig().GetLexer(wxT("C++")); }
        ...
        gsgs_EditorConfig().ReadObject(wxT("build_tab_settings"), &cmpColoursOptions);
        wxColour tmpcolour;
        gsgs_Style().LightColour(cmpColoursOptions.GetErrorColour(), tmpcolour, 9.0);
        rCtrl.StyleSetBackground(eAnnotationStyleError, tmpcolour);
        ...
        // Warning
        gsgs_Style().LightColour(cmpColoursOptions.GetErrorColour(), tmpcolour, 9.0);
        rCtrl.StyleSetBackground(eAnnotationStyleWarning, tmpcolour);
    ...
    void ContextCpp::OnRenameGlobalSymbol(wxCommandEvent& e)
    {
        ...
        if(!gsgs_Frame()->GetMainBook()->SaveAll(true, false)) return;

        // Get list of projects to work on
        wxArrayString projectsCandidateList, projects;
        gsgs_CppWorker().GetProjectList(projectsCandidateList);
        if(projectsCandidateList.IsEmpty()) return;

        if(projectsCandidateList.GetCount() > 1) {
            SelectProjectsDlg projectSelectorDlg(gsgs_Event().TopFrame());
        ...
        for(size_t i = 0; i < projects.GetCount(); ++i) {
            gsgs_Manager()->GetProjectFiles(projects.Item(i), filesArr);
        }
        ...
        if(RefactoringEngine::Instance()->IsBusy()) {
            ::wxMessageBox(_("Refactoring engine is busy with another request. Please try again later"), "gsgs",
                           wxOK | wxICON_WARNING);
            return;
        }
    ...
    void ContextCpp::ReplaceInFiles(const wxString& word, const CppToken::Vec_t& li)
    {
        ...
        Editor* current = gsgs_Frame()->GetMainBook()->GetActiveEditor();
        ...
        for(CppToken::Vec_t::const_iterator iter = li.begin(); iter != li.end(); ++iter) {
            ...
            if(!editor || editor->GetFileName().GetFullPath() != file_name) {
                editor = gsgs_Frame()->GetMainBook()->OpenFile(file_name, wxEmptyString, 0);
        ...
        if(current) {
            gsgs_Frame()->GetMainBook()->SelectPage(current);
        ...
        if(success) { GetManager()->GetStatusBar()->SetMessage(_("Symbol renamed")); }
    }
    //----------------------------------------------------------------------------------
    ...
    {
        CHECK_JS_RETURN_VOID();
        if(gsgs_Manager()->GetRetagInProgress()) return;

        Editor& editor = GetCtrl();
        gsgs_Manager()->RetagFile(editor.GetFileName().GetFullPath());
    ...
    void ContextCpp::MakeCppKeywordsTags(const wxString& word, std::vector<TagEntryPtr>& tags)
    {
        ...
        LexerStyle::Ptr_t lexPtr;
        // Read the configuration file
        if(gsgs_EditorConfig().IsOk()) { lexPtr = gsgs_EditorConfig().GetLexer(this->GetName()); }
        ...
        for(size_t i = 0; i < wordsArr.GetCount(); i++) {
            ...
            if(s2.StartsWith(s1) || s2.Lower().StartsWith(s1.Lower())) {
                gsgs::TagEntryPtr tag(new gsgs::TagEntry());
    ...
    {
        CHECK_JS_RETURN_FALSE();
        gsgsDEBUG1() << "ContextCpp::DoCodeComplete(" << pos << ") is called" << clEndl;
        ...
        gsgsDEBUG1() << "Triggering char is:" << ch << clEndl;
        switch(ch) {
        case '.':
        ...
        if(showFuncProto) {
            gsgsDEBUG1() << "Function prototype is requested..." << clEndl;
            ...
            gsgsDEBUG1() << "Function prototype is requested for:" << expr << "|" << word << clEndl;
    ...
    void ContextCpp::DoOpenWorkspaceFile()
    {
        ...
        if(fileToOpen.IsEmpty() == false) { gsgs_Frame()->GetMainBook()->OpenFile(fileToOpen); }
    }
    //----------------------------------------------------------------------------------
    void ContextCpp::DoCreateFile(const wxFileName& fn)
    {
        // get the file name from the user
        wxString new_file = wxGetTextFromUser(_("New File Name:"), _("Create File"), fn.GetFullPath(), gsgs_Frame());
        if(new_file.IsEmpty()) {
            // user clicked cancel
            return;
        }
        ...
        if(GetCtrl().GetProject().IsEmpty() == false) {
            ProjectPtr p = gsgs_Manager()->GetProject(GetCtrl().GetProject());
            if(p) {
                wxString vd = p->GetVDByFileName(GetCtrl().GetFileName().GetFullPath());
                vd.Prepend(p->GetName() + wxT(":"));

                if(vd.IsEmpty() == false) {
                    gsgs_Frame()->GetWorkspaceTab()->GetFileView()->CreateAndAddFile(new_file, vd);
                }
            }
        } else {
            // just a plain file
    ...
    //----------------------------------------------------------------------------------
    wxString ContextCpp::GetExpression(long pos, bool onlyWord, Editor* editor, bool forCC)
    {
        if(IsJavaScript()) { return wxEmptyString; }
    ...
    void ContextCpp::OnSyncSignatures(wxCommandEvent& e)
    {
        ...
        Editor* editor = gsgs_Frame()->GetMainBook()->OpenFile(tag->GetFile(), wxEmptyString, 0);
        if(!editor) return;
    ...
    //----------------------------------------------------------------------------------
    bool ContextCpp::DoGetSingatureRange(int line, int& start, int& end, Editor* ctrl)
    {
        CHECK_JS_RETURN_FALSE();
        start = wxNOT_FOUND;
        end = wxNOT_FOUND;
    ...
    //----------------------------------------------------------------------------------
    void ContextCpp::OnShowCodeNavMenu(CodeCompleteEvent& e)
    {
        Editor* editor = dynamic_cast<Editor*>(e.GetEditor());
    ...
    //----------------------------------------------------------------------------------
    void ContextCpp::OnCodeCompleteFiles(CodeCompleteEvent& event)
    {
