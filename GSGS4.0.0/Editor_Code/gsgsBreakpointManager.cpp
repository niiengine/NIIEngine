...
namespace gsgs
{
    ...
    bool BreakPointManager::AddBreakpointByAddress(const wxString& address)
    {
        BreakpointInfo bp;
    ...
    bool BreakPointManager::AddBreakpointByLineno(const wxString& file,
                                           const int lineno,
    ...
    bool BreakPointManager::AddBreakpoint(const BreakpointInfo& bp)
    {
    ...
    void BreakPointManager::AddBreakpoint()
    {
        BreakptPropertiesDlg dlg(NULL);
        dlg.SetTitle(_("Create a breakpoint or watchpoint"));

        Editor* const editor = gsgs_Frame()->GetMainBook()->GetActiveEditor();
        BreakpointInfo bp;
        bp.Create(
        ...
        if(AddBreakpoint(dlg.b)) {
            ...
            gsgs_Frame()->GetMainStatusBar()->SetMessage(msg);
        }
    }

    void BreakPointManager::GetBreakpoints(std::vector<BreakpointInfo>& li)
    {
    ...
    BreakpointInfo& BreakPointManager::GetBreakpoint(const wxString& fileName, const int lineno)
    {
    ...
    const BreakpointInfo& BreakPointManager::GetBreakpoint(const wxString& fileName, const int lineno) const
    {
        wxFileName fn(fileName);
        fn.Normalize(wxPATH_NORM_ALL & ~wxPATH_NORM_LONG);
    ...
    void BreakPointManager::GetTooltip(const wxString& fileName, int lineno, wxString& tip, wxString& title)
    {
    ...
    void BreakPointManager::DeleteAllBreakpointMarkers()
    {
        EditorVec_t editors;
    ...
    void BreakPointManager::RefreshBreakpointMarkers()
    {
    ...
    void BreakPointManager::DoRefreshFileBreakpoints(Editor* editor)
    {
    ...
    void BreakPointManager::DoProvideBestBP_Type(Editor* editor, const std::vector<BreakpointInfo>& li)
    {
    ...
    void BreakPointManager::DebuggerStopped()
    {
    ...
    bool BreakPointManager::DelBreakpoint(double id)
    {
        ...
        if(dbgr && dbgr->IsRunning()) {
            if(id > FIRST_INTERNAL_ID) {
                ...
                if(dbgr->RemoveBreak(id)) {
                    ...
                    gsgs_Manager()->UpdateAddLine(msg + wxString::Format(_("%i deleted"), (int)id));
                }
                if(contIsNeeded) {
                    dbgr->Continue();
                }
    ...
    bool BreakPointManager::DelBreakpointByLineno(const wxString& file, const int lineno)
    {
    ...
    void BreakPointManager::ApplyPendingBreakpoints()
    {
    ...
    void BreakPointManager::DelAllBreakpoints()
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
        if(dbgr && dbgr->IsRunning()) {
            bool contIsNeeded = PauseDebuggerIfNeeded();
    ...
    void BreakPointManager::SetAllBreakpointsEnabledState(bool enabled)
    {
    ...
    void BreakPointManager::SetBreakpointDebuggerID(const int internal_id, const int debugger_id)
    {
        ...
        for(; iter != m_bps.end(); ++iter) {
            if(iter->internal_id == internal_id) {
                ...
                if(debugger_id == -1) {
                    ...
                    gsgs_Manager()->UpdateAddLine(msg);
        ...
        gsgsLogMessage(wxT("SetBreakpointDebuggerID(): Failed to match internal_id to debugger_id"));
    }

    // Set a breakpoint's ignore count
    bool BreakPointManager::IgnoreByLineno(const wxString& file, const int lineno)
    {
    ...
    void BreakPointManager::EditBreakpointByLineno(const wxString& file, const int lineno)
    {
        const BreakpointInfo& bp = GetBreakpoint(file, lineno);
        if(bp.IsNull()) return;
    ...
    void BreakPointManager::EditBreakpoint(int index, bool& bpExist)
    {
        // sanity
        bpExist = true;
        if(index < 0 || index >= (int)m_bps.size()) {
            gsgsLogMessage(wxT("BreakPointManager::EditBreakpoint: Insane index"));
            bpExist = false;
            return;
        }
    ...
    void BreakPointManager::ReconcileBreakpoints(const std::vector<BreakpointInfo>& li)
    {
    ...
    void BreakPointManager::BreakpointHit(double id)
    {
        int index = FindBreakpointById(id, m_bps);
    ...
    std::set<wxString> BreakPointManager::GetFilesWithBreakpointMarkers()
    {
    ...
    int BreakPointManager::FindBreakpointById(double id, const std::vector<BreakpointInfo>& li)
    {
        std::vector<BreakpointInfo>::const_iterator iter = li.begin();
        for(; iter != li.end(); ++iter) {
    ...
    bool BreakPointManager::CanThisBreakpointBeUpdated(const BreakpointInfo& a, const BreakpointInfo& b) const
    {
    ...
    bool BreakPointManager::SetBPIgnoreCount(double bid, const int ignorecount)
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
        if(dbgr && dbgr->IsRunning()) {
    ...
    bool BreakPointManager::SetBPCommands(const BreakpointInfo& bp)
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
        if(dbgr && dbgr->IsRunning()) {
    ...
    bool BreakPointManager::SetBPConditon(const BreakpointInfo& bp)
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
        if(dbgr && dbgr->IsRunning()) {
    ...
    bool BreakPointManager::SetBPEnabledState(double bid, const bool enable)
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
        if(dbgr && dbgr->IsRunning()) {
    ...
    void BreakPointManager::SetBestBPType(BreakpointInfo& bp)
    {
        if(bp.bp_type == BP_type_watchpt) {
            return;
        }
    ...
    bool BreakPointManager::PauseDebuggerIfNeeded()
    {
        IDebugger* dbgr = DebuggerMgr::Get().GetActiveDebugger();
    ...
    void BreakPointManager::DeleteAllBreakpointsByFileName(const wxString& fileName)
    {
        std::vector<BreakpointInfo>::iterator iter = m_bps.begin();
    ...
    void BreakPointManager::SetBreakpoints(const std::vector<BreakpointInfo>& bps)
    {
        // append the breakpoint list
        m_bps.insert(m_bps.end(), bps.begin(), bps.end());
    }

    bool BreakPointManager::AreThereEnabledBreakpoints(bool enabled /*= true*/)
    {
        for(size_t i = 0; i < m_bps.size(); ++i) {
            BreakpointInfo& bp = m_bps.at(i);
    ...
    void BreakPointManager::SaveSession(SessionEntry& session) { session.SetBreakpoints(m_bps); }

    void BreakPointManager::LoadSession(const SessionEntry& session)
    {
    ...
    void BreakPointManager::DragBreakpoint(Editor* editor, int line, wxBitmap bitmap)
    {
    ...
    void BreakPointManager::DropBreakpoint(const BreakpointInfo& bp, int newline)
    {
    ...
    myDragImage::myDragImage(Editor* ed, const wxBitmap& bitmap, const BreakpointInfo& bp)
    ...
    void myDragImage::OnEndDrag(wxMouseEvent& event)
    {
        ...
        if((newline + 1) != m_bp.lineno) {
            gsgs_Manager()->GetBreakpointsMgr()->DropBreakpoint(m_bp, newline);
        }
    }

    void BreakPointManager::RefreshBreakpointsForEditor(Editor* editor) { DoRefreshFileBreakpoints(editor); }

    bool BreakPointManager::IsDuplicate(const BreakpointInfo& bp, const std::vector<BreakpointInfo>& bpList)
    {
    ...
    void BreakPointManager::DoRemoveDuplicateBreakpoints()
    {
    ...
    int BreakPointManager::DelBreakpointByAddress(const wxString& address)
    {
    ...
    void BreakPointManager::GetAllMemoryBreakpoints(BreakpointInfoVec_t& memoryBps)
    {
