...
namespace gsgs
{
    ...
    {
        m_tabgroups.clear();
        gsgs_Event().Unbind(wxEVT_WORKSPACE_LOADED, &TabgroupManager::OnWorkspaceLoaded, this);
        gsgs_Event().Unbind(wxEVT_WORKSPACE_CLOSED, &TabgroupManager::OnWorkspaceClosed, this);
    }
    //-----------------------------------------------------------------------------
    ...
    {
        wxFileName TabgrpPath = wxFileName::DirName(gsgs_CppWorker().GetPrivateFolder() + wxT("/tabgroups/"));
    ...
    void TabgroupManager::LoadKnownTabgroups(bool isGlobal /*=false*/)
    {
        ...
        wxString tabgroupsDir = isGlobal ?
            gsgs_Path().GetUserDataDir() + "/tabgroups" : GetTabgroupDirectory();
    ...
    void TabgroupManager::LoadTabgroupData(bool isGlobal, const wxString& tabgroup)
    {
        ...
        if(gsgs_Session().GetSession(filepath, session, "tabgroup", tabgroupTag)) {
    ...
    wxXmlNode* TabgroupManager::FindTabgroupItem(wxXmlDocument& doc, const wxString& filepath, const wxString& itemfilepath)
    {
        ...
        if(TabInfoArrayNode) {
            ...
            while(TabInfoNode) {
                ...
                if(child && child->XmlGetPropVal(wxT("Value"), wxEmptyString) == itemfilepath) {
    ...
    void TabgroupManager::OnWorkspaceLoaded(wxCommandEvent& evt)
    {
        ...
        fnWorkspaceFile.AppendDir(".gsgsworkspace");
        ...
        gsgs_Frame()->GetWorkspacePane()->GetServiceTab()->DisplayTabgroups(false);
    }
    //-----------------------------------------------------------------------------
    ...
    TabgroupManager::TabgroupManager()
    {
        gsgs_Event().Bind(wxEVT_WORKSPACE_LOADED, &TabgroupManager::OnWorkspaceLoaded, this);
        gsgs_Event().Bind(wxEVT_WORKSPACE_CLOSED, &TabgroupManager::OnWorkspaceClosed, this);
    }
    //-----------------------------------------------------------------------------
}