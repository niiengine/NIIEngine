...
------------------------------------------------------------------------------
*/
#ifndef gsgsEDITOR_EDITOR_H
#define gsgsEDITOR_EDITOR_H
...
namespace gsgs
{
    ...
    { 
        eAnnotationStyleError = 128, 
        eAnnotationStyleWarning 
    };
    /**
     * @class BPtoMarker
    ...
    {
    public:
        TextCommand(CLC_types type, const wxString & name="") : Command(type, name){}
        virtual ~TextCommand() {}
    ...
    class InsertTextCommand : public TextCommand
    {
    public:
        InsertTextCommand(const wxString& name="insertion:") : TextCommand(CLC_insert, name) {}
        virtual ~InsertTextCommand() {}
    protected:

    };

    class DeleteTextCommand : public TextCommand
    {
    public:
        DeleteTextCommand(const wxString& name="deletion:") : TextCommand(CLC_delete, name) {}
        virtual ~DeleteTextCommand() {}
    protected:

    };

    class TextProcessor : public CommandProcessorBase
    {
    public:
        TextProcessor();
        ...
        inline void SetParent(Editor* parent)           { m_parent = parent; }
        
        inline Editor* GetParent() const                { return m_parent; }

        inline void Reset() { // Like Clear() but retain m_initialCommand. Used when an editor is reloaded
           m_commands.clear();
        }
    ...
    /**
     * \ingroup LiteEditor
     * Editor gsgs editing component based on Scintilla
     * Editor provides most of the C++/C editing capablities including:
     * -# Auto Completion
     * -# Find and replace
     * -# Bookmarks
     * -# Folding
     ...
     * and many many more
     */
    class Editor : public wxStyledTextCtrl, public IEditor
    {
    public:
        struct SelectionInfo 
        {
             ...
             */
            inline void AddSelection(int from, int to)  { selections.push_back(std::make_pair(from, to)); }

            /**
             * @brief return the number of selections
             */
            inline size_t GetCount() const              { return this->selections.size(); }
             ...
             * @param to
             */
            inline void At(size_t i, int& from, int& to) const
            {
             ...
             */
            inline void Clear()                         { this->selections.clear(); }

            /**
             * @brief is this object OK?
             */
            inline bool IsOk() const                    { return !this->selections.empty(); }

            /**
             * @brief sort the selections from top to bottom
             */
            inline void Sort()                          { std::sort(this->selections.begin(), this->selections.end(), SelectorSorter()); }
        ...
        public:
            ...
            inline bool IsValid(wxStyledTextCtrl* ctrl) const
            {
                return ctrl->PositionFromLine(ctrl->GetFirstVisibleLine()) == m_firstOffset && m_hasMarkers;
            }

            // setters/getters
            inline void SetFirstOffset(int firstOffset)             { this->m_firstOffset = firstOffset; }
            inline void SetHasMarkers(bool hasMarkers)              { this->m_hasMarkers = hasMarkers; }
            inline void SetWord(const wxString& word)               { this->m_word = word; }
            inline int GetFirstOffset() const                       { return m_firstOffset; }
            inline bool IsHasMarkers() const                        { return m_hasMarkers; }
    ...
    public:
        ...
        inline IManager* GetManager()                               { return m_mgr; }

        void SetShowNumber(bool set);

        inline bool GetShowNumber() const                           { return mShowNumber;}

        void SetShowSymbol(bool set);

        inline bool GetShowSymbol() const                           { return mShowSymbol; }
        /**
         * @brief gsgs preferences updated
         */
        void PreferencesChanged();
        /**
         * @brief are the CC annotations visible?
         */
        inline bool IsHasCCAnnotation() const                       { return m_hasCCAnnotation; }

        void ClearCCAnnotations();

        inline void SetEditorBitmap(const wxBitmap& bitmap)         { this->m_editorBitmap = bitmap; }

        inline const wxBitmap& GetEditorBitmap() const              { return m_editorBitmap; }

        void NotifyMarkerChanged(int lineNumber = wxNOT_FOUND);

        inline static FindReplaceData& GetFindReplaceData()         { return m_findReplaceData; }

        inline void SetPreProcessorsWords(const wxString& word)     { this->m_preProcessorsWords = word; }

        inline const wxString& GetPreProcessorsWords() const        { return m_preProcessorsWords; }

        void SetLineVisible(int lineno);

        inline void SetReloadingFile(const bool& reloadingFile)     { this->m_reloadingFile = reloadingFile; }

        inline const bool& GetReloadingFile() const                 { return m_reloadingFile; }

        inline EditorTipWindow * GetFunctionTip() const             { return mFuncTip; }

        bool IsFocused() const;

        inline TextProcessor& GetCommandsProcessor()                { return m_commandsProcessor; }
        ...
        inline void SetFileName(const wxFileName& name)             { m_fileName = name; }

        // Return the project name
        inline const wxString& GetProject() const                   { return m_project; }
        // Set the project name
        inline void SetProject(const wxString& proj)                { m_project = proj; }
         ...
         * on its stack
         */
        inline bool CanGotoPreviousDefintion()                      { return NavMgr::Get()->CanPrev(); }
         ...
         */
        void LoadCollapsedFoldsFromArray(const EditorStateLocker::VecInt_t& folds);
        /**
         * Store any collapsed folds to a vector, so they can be serialised
         */
        void StoreCollapsedFoldsToArray(EditorStateLocker::VecInt_t& folds) const;
         ...
         */
        virtual OptionsConfig * GetOptions() { return m_options; }
        ...
        inline bool IsContextMenuOn() const                     { return m_popupIsOn; }
         ...
         */
        inline time_t GetEditorLastModifiedTime() const { return m_modifyTime; }
        inline void SetEditorLastModifiedTime(time_t modificationTime) { m_modifyTime = modificationTime; }
        ...
        const wxColour & GetTextColour() const { m_selTextColour; }

        const wxColour & GetTextBgColour() const { m_selTextBgColour; }
         ...
         */
        virtual void ShowCalltip(gsgsTipInfoPtr tip);
    ...
    private:
        ...
        void OnDLeftDown(wxMouseEvent& event);
    ...
    protected:
        ...
        EditorTipWindow* mFuncTip;
        BoxTipWindow* mEditTip;
        ...
        TextProcessor m_commandsProcessor;
    ...
    typedef std::vector<Editor*> EditorVec_t;
    class ListenerScrollBar;
    class EditorScrolledPanel : public ScrolledWindow
    {
    public:
        EditorScrolledPanel(wxWindow * parent, wxWindowID id = wxID_ANY, const wxPoint & pos = wxDefaultPosition,
            const wxSize & size = wxDefaultSize, long style = 0);
        ~EditorScrolledPanel();

        Editor * getEditor() const                          { return mEditor; }

        void NotifyVPos() { DoPositionVScrollbar(); }

        void NotifyHPos() { DoPositionHScrollbar(); }
    ...
    protected:
        Editor * mEditor;
        EditorScrollBar * m_lhsb;
        EditorScrollBar * m_lvsb;
    };
    typedef std::vector<EditorScrolledPanel*> EditorPanelVec_t;
}
#endif
