...
void WordCompleter::OnWordComplete(CodeCompleteEvent& event) { m_plugin->OnWordComplete(event); }
...
void WordCompletionPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void WordCompletionPlugin::OnWordComplete(CodeCompleteEvent& event)
{
    ...
    gsgsStringSet words = m_dictionary->GetWords();
    ...
    if(activeEditor->IsModified()) {
        ...
        gsgsStringSet unsavedBufferWords;
        wxStyledTextCtrl* stc = activeEditor->GetCtrl();
    ...
    LexerStyle::Ptr_t lexer = gsgs_Style().GetLexerForFile(activeEditor->GetFileName().GetFullName());
    if(lexer) {
        wxString keywords;
    ...
    gsgsStringSet filterdSet;
    if(filter.IsEmpty()) {
        filterdSet.swap(words);
    } else {
        for(gsgsStringSet::iterator iter = words.begin(); iter != words.end(); ++iter) {
            wxString word = *iter;
    ...
    for(gsgsStringSet::iterator iter = filterdSet.begin(); iter != filterdSet.end(); ++iter) {
...
{
    WordCompletionSettingsDlg dlg(gsgs_Event().TopFrame());
    dlg.ShowModal();
}
