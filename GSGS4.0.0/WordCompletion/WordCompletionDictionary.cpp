...
{
    gsgs_Event().Bind(wxEVT_ACTIVE_EDITOR_CHANGED, &WordCompletionDictionary::OnEditorChanged, this);
    gsgs_Event().Bind(wxEVT_ALL_EDITORS_CLOSED, &WordCompletionDictionary::OnAllEditorsClosed, this);
    gsgs_Event().Bind(wxEVT_FILE_SAVED, &WordCompletionDictionary::OnFileSaved, this);
...
WordCompletionDictionary::~WordCompletionDictionary()
{
    gsgs_Event().Unbind(wxEVT_ACTIVE_EDITOR_CHANGED, &WordCompletionDictionary::OnEditorChanged, this);
    gsgs_Event().Unbind(wxEVT_ALL_EDITORS_CLOSED, &WordCompletionDictionary::OnAllEditorsClosed, this);
    gsgs_Event().Unbind(wxEVT_FILE_SAVED, &WordCompletionDictionary::OnFileSaved, this);
...
void WordCompletionDictionary::OnEditorChanged(wxCommandEvent& event)
{
    ...
    ::GetManager()->GetAllEditors(allEditors);

    std::for_each(allEditors.begin(), allEditors.end(), [&](IEditor* editor) {
        openEditors.Add(editor->GetFileName().GetFullPath());
    });

    std::for_each(m_files.begin(), m_files.end(), [&](const std::pair<wxString, gsgsStringSet>& p) {
...
void WordCompletionDictionary::OnSuggestThread(const WordCompletionThreadReply& reply)
{
    ...
    std::map<wxString, gsgsStringSet>::iterator iter = m_files.find(reply.filename.GetFullPath());
...
void WordCompletionDictionary::DoCacheActiveEditor(bool overwrite)
{
    ...
    m_files.insert(std::make_pair(activeEditor->GetFileName().GetFullPath(), gsgsStringSet()));
...
void WordCompletionDictionary::OnFileSaved(gsgs::Event& event)
{
    event.Skip();
    DoCacheActiveEditor(true);
}

gsgsStringSet WordCompletionDictionary::GetWords() const
{
    gsgsStringSet words;
    std::for_each(m_files.begin(), m_files.end(), [&](const std::pair<wxString, gsgsStringSet>& p){
