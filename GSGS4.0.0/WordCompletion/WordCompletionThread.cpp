...
void WordCompletionThread::ProcessRequest(gsgs::ThreadRequest* request)
{
    WordCompletionThreadRequest* req = dynamic_cast<WordCompletionThreadRequest*>(request);
    CHECK_PTR_RET(req);

    gsgsStringSet suggestsions;
...
void WordCompletionThread::ParseBuffer(const wxString& buffer, gsgsStringSet& suggest)
{
#if 0
