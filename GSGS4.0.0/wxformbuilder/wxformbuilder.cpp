...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("wxFormBuilder integration with gsgs"));
    ...
    , m_openWithWxFbSepItem(NULL)
{
    ...
    m_longName = _("wxFormBuilder integration with gsgs");
    ...
    gsgs_Event().Connect(wxEVT_TREE_ITEM_FILE_ACTIVATED, gsgsEventHandler(wxFormBuilder::OnOpenFile),
                                  NULL, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FILE, &wxFormBuilder::OnShowFileContextMenu, this);
}

wxFormBuilder::~wxFormBuilder() {}

void wxFormBuilder::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
{
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FILE, &wxFormBuilder::OnShowFileContextMenu, this);
}
...
void wxFormBuilder::DoCreateWxFormBuilderProject(const wxFBItemInfo& data)
{
    ...
    if(proj) {
        ...
        if(!ReadFileWithConversion(fbpFile.GetFullPath().c_str(), content)) {
            wxMessageBox(wxString::Format(_("Failed to read file '%s'"), fbpFile.GetFullPath().c_str()), _("光栅故事"),
        ...
        if(!WriteFileWithBackup(fbpFile.GetFullPath().c_str(), content, false)) {
            wxMessageBox(wxString::Format(_("Failed to write file '%s'"), fbpFile.GetFullPath().c_str()), _("光栅故事"),
                         wxOK | wxCENTER | wxICON_WARNING);
            return;
        }
...
void wxFormBuilder::OpenWithWxFb(wxCommandEvent& e)
{
    ...
    if(item.m_item.IsOk() && item.m_itemType == ProjectItem::TypeFile) {
        if(item.m_fileName.GetExt() == wxT("fbp")) {
            ...
            wxMessageBox(_("Please select a 'fbp' (Form Builder Project) file only"), _("光栅故事"),
...
void wxFormBuilder::OnOpenFile(gsgs::Event& e)
{
    e.Skip();
...
void wxFormBuilder::OnWxFBTerminated(gsgs::ProcessEvent& e)
{
    if(e.GetProcess()) { delete e.GetProcess(); }
}

void wxFormBuilder::OnShowFileContextMenu(ContextMenuEvent& event)
{
    event.Skip();
