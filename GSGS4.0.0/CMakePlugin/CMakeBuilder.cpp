...
wxString CMakeBuilder::GetWorkspaceBuildFolder(bool wrapWithQuotes)
{
    ...
    wxString workspaceConfig = gsgs_CppWorker().GetBuildMatrix()->GetSelectedConfigurationName();
...
{
    ProjectPtr p = gsgs_CppWorker().GetProject(project);
    wxASSERT(p);
