...
CMake::CMake(const wxFileName& path)
    : m_path(path)
    , m_version("?")
    , m_dbFileName(gsgs_Path().GetUserDataDir(), "cmake.db")
{
    // Prepare database
    PrepareDatabase();
    
    // Register the CMake builder
    gsgs_Build().AddBuilder(new CMakeBuilder());
}
...
CMake::PrepareDatabase()
{
    ...
    try {
        ...
        gsgs_Error("CMake DoPrepareDatabase error: %s", e.GetMessage());
    }
}
...
CMake::LoadFromDatabase()
{
    ...
    try
    {
        ...
        gsgs_Error("Error occurred while loading data from CMake database: %s", e.GetMessage());
    }
...
{
    if (!m_dbInitialized) {
        gsgs_Warning("CMake: can't store data into database. Database was not initialized properly");
        return;
    }
    ...
    try
    {
        ...
        gsgs_Error("An error occurred while storing CMake data into database: %s", e.GetMessage());
    }
}
