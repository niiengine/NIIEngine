...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("CMake integration for gsgs"));
    info.SetVersion("0.8");
    ...
    , m_cmake(NULL)
{
    ...
    m_configuration.reset(new CMakeConfiguration(gsgs_Path().GetUserDataDir() +
                                                 gsgsPathSep + "config/cmake.ini"));
    ...
    if(IsPaneDetached()) 
    {
        DockablePane* cp = new DockablePane(book->GetParent()->GetParent(), book, HELP_TAB_NAME, false, bmp, wxSize(200, 200));
    ...
    // Bind events
    gsgs_Event().Bind(wxEVT_SHOW_WORKSPACE_TAB, &CMakePlugin::OnToggleHelpTab, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_PROJECT, &CMakePlugin::OnProjectContextMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_WORKSPACE, &CMakePlugin::OnWorkspaceContextMenu, this);
    gsgs_Event().Bind(wxEVT_PROJ_FILE_ADDED, &CMakePlugin::OnFileAdded, this);
    gsgs_Event().Bind(wxEVT_PROJ_FILE_REMOVED, &CMakePlugin::OnFileRemoved, this);
...
{
    const CxxWS * workspace = m_mgr->GetWorkspace();
    wxASSERT(workspace);

    return wxFileName::DirName(workspace->GetFileName().GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME));
}
//--------------------------------------------------------------------------
...
{
    wxASSERT(m_mgr);
    ConfigTool* configTool = m_mgr->GetConfigTool();
    wxASSERT(configTool);
...
//--------------------------------------------------------------------------
void CMakePlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
//--------------------------------------------------------------------------
...
void CMakePlugin::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_SHOW_WORKSPACE_TAB, &CMakePlugin::OnToggleHelpTab, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_PROJECT, &CMakePlugin::OnProjectContextMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_WORKSPACE, &CMakePlugin::OnWorkspaceContextMenu, this);
    gsgs_Event().Unbind(wxEVT_PROJ_FILE_ADDED, &CMakePlugin::OnFileAdded, this);
    gsgs_Event().Unbind(wxEVT_PROJ_FILE_REMOVED, &CMakePlugin::OnFileRemoved, this);
...
//--------------------------------------------------------------------------
void CMakePlugin::OnToggleHelpTab(gsgs::Event& event)
{
...
//--------------------------------------------------------------------------
void CMakePlugin::OnProjectContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
//--------------------------------------------------------------------------
void CMakePlugin::OnCMakeOutput(gsgs::ProcessEvent & event)
{
    m_mgr->AppendOutputTabText(kOutputTab_Build, event.GetOutput());
}
//--------------------------------------------------------------------------
void CMakePlugin::OnCMakeTerminated(gsgs::ProcessEvent & event)
{
    m_mgr->AppendOutputTabText(kOutputTab_Build, event.GetOutput());
...
void CMakePlugin::OnOpenCMakeLists(wxCommandEvent & event)
{
    ...
    if(openWorkspaceCMakeLists) {
        cmakelists = gsgs_CppWorker().GetFileName();
    } else {
...
void CMakePlugin::OnExportCMakeLists(wxCommandEvent & event)
{
    ProjectPtr proj = (event.GetId() == XRCID("cmake_export_active_project")) ? 
        gsgs_CppWorker().GetActiveProject() : GetSelectedProject();

    CHECK_PTR_RET(proj);

    CMakeGenerator generator;
    if(generator.Generate(proj)) { gsgs_Event().PostReloadExternallyModifiedEvent(); }
}
//--------------------------------------------------------------------------
void CMakePlugin::OnWorkspaceContextMenu(ContextMenuEvent& event)
{
    event.Skip();
    CHECK_COND_RET(gsgs_CppWorker().IsOpen());

    ProjectPtr p = gsgs_CppWorker().GetActiveProject();
    CHECK_COND_RET(p);
    ...
    wxFileName workspaceFile = gsgs_CppWorker().GetFileName();
...
//--------------------------------------------------------------------------
void CMakePlugin::OnFileRemoved(gsgs::Event& event)
{
    event.Skip();
    CHECK_COND_RET(gsgs_CppWorker().IsOpen());

    // The affected project is passed in the string member of the event
    ProjectPtr p = gsgs_CppWorker().GetProject(event.GetString());
    CHECK_PTR_RET(p);
...
//--------------------------------------------------------------------------
void CMakePlugin::OnFileAdded(gsgs::Event& event) { OnFileRemoved(event); }
//--------------------------------------------------------------------------
...
// Apply the environment variables before we do anything here
    ...
    if(!proc) 
    {
        ::wxMessageBox(_("Failed to execute:\n") + command, "GSGS", wxICON_ERROR | wxOK | wxCENTER,
            gsgs_Event().TopFrame());
        return;
    }
