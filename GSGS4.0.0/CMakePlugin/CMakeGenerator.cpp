...
bool CMakeGenerator::CheckExists(const wxFileName& filename)
{
    ...
    if(filename.Exists()) {
        // Check to see if this file was generated by gsgs
        wxString content;
        FileUtil::ReadFileContent(filename, content);
        if(!content.StartsWith(CODELITE_CMAKE_PREFIX)) {
            wxString msg;
            msg << _("A custom ") << CMakePlugin::CMAKELISTS_FILE << _(" exists.\nWould you like to overwrite it?\n")
                << "( " << filename.GetFullPath() << " )";
            wxStandardID answer =
                ::PromptMessageDialogWithoutCancel(msg, "CMakePluginOverwriteDlg", _("Overwrite"), _("Don't Overwrite"),
            ...
            // A gsgs generated CMakeLists.txt file
...
bool CMakeGenerator::Generate(ProjectPtr p)
{
    ...
    for(size_t i = 0; i < projectsArr.GetCount(); ++i) {
        ...
        if(!cmakeFileContent.IsEmpty()) {
            ...
            FileUtil::WriteFileContent(cmakelists, cmakeFileContent);
        }
    }
    ...
    mainProjectContent << "# This setting is useful for providing JSON file used by gsgs for code completion\n";
    ...
    FileUtil::WriteFileContent(fnMainCMakeLists, mainProjectContent);
    return true;
}
...
wxString CMakeGenerator::GenerateProject(ProjectPtr project, bool topProject, const wxString& configName)
{
    ...
    wxFileName fnWorkspace = gsgs_CppWorker().GetFileName();
    ...
    std::for_each(allFiles.begin(), allFiles.end(), [&](const Project::FilesMap_t::value_type& vt) {
        ...
        if(!file->IsExcludeFromConfiguration(buildConf->GetName())) {
            ...
            file_name.Replace("\\", "/");
            if(FileUtil::GetType(fn.GetFullName()) == FileUtil::TypeSourceC) {
                cSources.Add(file_name);
            } else if(FileUtil::GetType(fn.GetFullName()) == FileUtil::TypeSourceCpp) {
                cppSources.Add(file_name);
            } else if(FileUtil::GetType(fn.GetFullName()) == FileUtil::TypeResource) {
    ...
    if(topProject) {
        ...
        for(size_t i = 0; i < projectsArr.GetCount(); ++i) {
            ProjectPtr pProj = gsgs_CppWorker().GetProject(projectsArr.Item(i));
...
bool CMakeGenerator::CanGenerate(ProjectPtr project)
{
    ...
    for(size_t i = 0; i < cmakelistsArr.size(); ++i) {
        ...
        wxString content;
        if(fn.Exists() && FileUtil::ReadFileContent(fn, content)) {
...
bool CMakeGenerator::IsCustomCMakeLists(const wxFileName& fn)
{
    if(fn.FileExists()) {
        wxFileName fn;
        wxString content;
        if(FileUtil::ReadFileContent(fn, content)) { return !content.StartsWith(CODELITE_CMAKE_PREFIX); }
    }
    return false;
}
