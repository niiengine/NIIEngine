...
namespace gsgs
{
    ...
    bool DialogAbout::Create(const DialogAboutInfo& info)
    {
        ...
        // the LayoutEditor::OnExit() will not be called and proper cleanup
        // will not be performed when the main frame is deleted
        if(!wxDialog::Create(wxGetApp().GetLayoutEditorFrame(), wxID_ANY, _("About ") + info.GetName(),
        ...
        wxStaticText* const label = gsgs_Style().createStaticText(this, wxID_ANY, nameAndVersion);
        ...
        if(info.HasContacts())
        {
            ...
            if(contactsCount == contactsEmailCount)
            {
                ...
                for(size_t i=0; i<contactsCount; i++)
                {
                    ...
                    wxStaticText* const contact = gsgs_Style().createStaticText(this, wxID_ANY, contacts[i] + wxT(" - "));
                    wxStaticText* const beginTag = gsgs_Style().createStaticText(this, wxID_ANY, wxT("<"));
                    wxHyperlinkCtrl* const contactEmail = new wxHyperlinkCtrl(this, wxID_ANY, contactsEmail[i], wxT("mailto:")  + contactsEmail[i]);
                    wxStaticText* const endTag = gsgs_Style().createStaticText(this, wxID_ANY, wxT(">"));
        ...
        if (info.HasFootnote())
        {
            ...
            wxStaticText* const footnote = gsgs_Style().createStaticText(this, wxID_ANY, info.GetFootnote());
    ...
    void DialogAbout::AddText(const wxString& text, const wxSizerFlags& flags)
    {
        ...
        if ( !text.empty() )
            AddControl(gsgs_Style().createStaticText(this, wxID_ANY, text), flags);
    }
    ...
    void DialogAbout::AddCollapsiblePane(const wxString& title, const wxString& text, long style)
    {
        ...
        wxStaticText* const txt = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, text, wxDefaultPosition, wxDefaultSize, style);
    ...
    void DialogAbout::AddCollapsiblePane(const wxString& title, const wxArrayString& names, const wxArrayString& emails, const wxArrayString& nicks, const wxArrayString& descs, long style)
    {
        ...
        if( (namesCount == emailsCount) && (namesCount == nicksCount) )
        {
            ...
            for(size_t i=0; i<namesCount; i++)
            {
                ...
                // since it will be handled internally by wxWidgets wxSizer::Add()
                wxStaticText* const name = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, names[i], wxDefaultPosition, wxDefaultSize, style);
                (void)horizontalSizer->Add(name, 0, wxSTRETCH_NOT, BORDER);

                // Add nick element (if it exists) - non stretching and border.
                if(nicks[i].Length() > 0)
                {
                    wxStaticText* const nick = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, wxT(" (") + nicks[i] + wxT(")"), wxDefaultPosition, wxDefaultSize, style);
                ...
                {
                    wxStaticText* const beginTag = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, wxT(" - <"), wxDefaultPosition, wxDefaultSize, style);
                    wxHyperlinkCtrl* const email = new wxHyperlinkCtrl(pane->GetPane(), wxID_ANY, emails[i], wxT("mailto:") + emails[i]);
                    wxStaticText* const endTag = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, wxT(">"), wxDefaultPosition, wxDefaultSize, style);
                ...
                if(descs[i].Length() > 0)
                {
                    wxStaticText* const desc = gsgs_Style().createStaticText(pane->GetPane(), wxID_ANY, wxT(" - ") + descs[i], wxDefaultPosition, wxDefaultSize, style);
