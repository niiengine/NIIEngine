    ...
    , m_mgr(mgr)
{
    ...
    } else if(editor && FileUtil::IsCxxFile(editor->GetFileName())) {
...
void CodeFormatterDlg::InitDialog()
{
    ...
    LexerStyle::Ptr_t lexer = gsgs_EditorConfig().GetLexer("C++");
    if(lexer) {
        lexer->Apply(m_textCtrlPreview, true);
        lexer->Apply(m_textCtrlPreview_Clang, true);
    }

    LexerStyle::Ptr_t phpLexer = gsgs_EditorConfig().GetLexer("php");
    if(phpLexer) {
...
void CodeFormatterDlg::UpdatePreviewText(wxStyledTextCtrl*& textCtrl, const wxString& text)
{
    ...
    gsgs::STCLineKeeper lk(textCtrl);
