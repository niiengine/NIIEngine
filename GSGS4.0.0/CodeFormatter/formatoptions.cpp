...
void FormatOptions::DeSerialize(gsgs::Serializer& arch)
{
...
void FormatOptions::AutodetectSettings()
{
    ...
    if(m_PHPCSFixerPhar.IsEmpty() || !phpcsfixerFormatPhar.Exists()) {
        if(FindExec("php-cs-fixer", phpcsfixerFormatPhar)) {
    ...
    if(m_PhpcbfPhar.IsEmpty() || !phpcbfFormatPhar.Exists()) {
        if(FindExec("phpcbf", phpcbfFormatPhar)) {
...
void FormatOptions::Serialize(gsgs::Serializer& arch)
{
...
{
    int indentWidth = gsgs_EditorConfig().GetOptions()->GetIndentWidth();
    bool useTabs = gsgs_EditorConfig().GetOptions()->GetIndentUsesTabs();
...
bool FormatOptions::GetPhpFixerCommand(const wxFileName& fileName, wxString& command)
{
    ...
    if(php.IsEmpty()) {
        gsgsDEBUG() << "CodeForamtter: GetPhpFixerCommand(): empty php command" << clEndl;
        return false;
    }
    ::WrapWithQuotes(php);
    phar = GetPHPCSFixerPhar();
    if(phar.IsEmpty()) {
        gsgsDEBUG() << "CodeForamtter: GetPhpFixerCommand(): empty php-cs-fixer phar path" << clEndl;
        return false;
    }
    ::WrapWithQuotes(phar);
...
wxString FormatOptions::GetPhpFixerRules(const wxFileName& fileName)
{
    ...
    JSONObject rules = root.getObject();
    ...
    if(m_PHPCSFixerPharRules & (kPcfShortArray | kPcfLongArray)) {
        JSONObject array_syntax = JSONObject::createObject("array_syntax");
    ...
    if(m_PHPCSFixerPharRules &
       (kPcfAlignDoubleArrow | kPcfStripDoubleArrow | kPcfAlignEquals | kPcfStripEquals | kPcfIgnoreDoubleArrow)) {
        JSONObject binary_operator_spaces = JSONObject::createObject("binary_operator_spaces");
    ...
    if(m_PHPCSFixerPharRules & (kPcfConcatSpaceNone | kPcfConcatSpaceOne)) {
        JSONObject concat_space = JSONObject::createObject("concat_space");
    ...
    wxString rulesString = rules.getString(false);
    if(rulesString == "{}") {
        return "";
    }
...
bool FormatOptions::GetPhpcbfCommand(const wxFileName& fileName, wxString& command)
{
    ...
    php = m_optionsPhp.GetPhpExe();
    if(php.IsEmpty()) {
        gsgsDEBUG() << "CodeForamtter: GetPhpcbfCommand(): empty php command" << clEndl;
        return false;
    }
    ::WrapWithQuotes(php);
    ...
    phar = GetPhpcbfPhar();
    if(phar.IsEmpty()) {
        gsgsDEBUG() << "CodeForamtter: GetPhpcbfCommand(): empty phpcbf phar path" << clEndl;
        return false;
    }
    ::WrapWithQuotes(phar);
