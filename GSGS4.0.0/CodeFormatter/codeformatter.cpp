    ...
    : Plugin(manager)
{
    ...
    gsgs_Event().Connect(wxEVT_FORMAT_STRING, gsgsSourceFormatEventHandler(CodeFormatter::OnFormatString), NULL, this);
    gsgs_Event().Connect(wxEVT_FORMAT_FILE, gsgsSourceFormatEventHandler(CodeFormatter::OnFormatFile), NULL, this);
    ...
    gsgs_Event().Bind(wxEVT_BEFORE_EDITOR_SAVE, gsgsEventHandler(CodeFormatter::OnBeforeFileSave), this);
    gsgs_Event().Bind(wxEVT_PHP_SETTINGS_CHANGED, &CodeFormatter::OnPhpSettingsChanged, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FOLDER, &CodeFormatter::OnContextMenu, this);
...
void CodeFormatter::CreateToolBar(gsgs::ToolBar* toolbar)
{
    // support both toolbars icon size
    int size = m_mgr->GetToolbarIconSize();

    gsgs::ImageManager* bmpLoader = m_mgr->GetStdIcons();
    toolbar->AddSpacer();
...
{
    if(FileUtil::IsCxxFile(fileName)) {
    ...
    if(FileUtil::IsFileType(fileName, FileUtil::TypeXml) ||
       FileUtil::IsFileType(fileName, FileUtil::TypeXRC) ||
       FileUtil::IsFileType(fileName, FileUtil::TypeWorkspace) ||
       FileUtil::IsFileType(fileName, FileUtil::TypeProject)) {
        return kFormatEngineWxXmlDocument;
    }

    if(FileUtil::IsJavascriptFile(fileName) || FileUtil::IsJavaFile(fileName)) {
...
void CodeFormatter::DoFormatPreview(wxString& content, const wxString& ext, const FormatterEngine& engine)
{
    ...
    if(gsgs_Workspace().IsValid()) {
        wxString projectname = gsgs_Workspace().GetWorkspace()->GetActiveProjectName();
        wxFileName filename = gsgs_Workspace().GetWorkspace()->GetProjectFileName(projectname);
...
void CodeFormatter::DoFormatWithTempFile(const wxFileName& fileName, wxString& content, const FormatterEngine& engine)
{
    ...
    FileUtil::Deleter fd(tempFileName);

    if(!FileUtil::WriteFileContent(tempFileName, content)) {
        gsgsWARNING() << "CodeFormatter: Failed to save file: " << tempFileName << clEndl;
        return;
    }

    DoFormatFile(tempFileName, engine);

    if(!FileUtil::ReadFileContent(tempFileName, content)) {
        gsgsWARNING() << "CodeFormatter: Failed to load file: " << tempFileName << clEndl;
    }
}

void CodeFormatter::DoFormatFile(const wxFileName& fileName, const FormatterEngine& engine)
{
    gsgsDEBUG() << "CodeFormatter formatting file: " << fileName << clEndl;
    ...
    gsgsDEBUG() << "CodeFormatte file formatted: " << fileName << clEndl;
}
...
{
    gsgsDEBUG() << "CodeFormatter running: " << command << clEndl;
...
void CodeFormatter::DoFormatWithClang(const wxFileName& fileName)
{
    if(m_options.GetClangFormatExe().IsEmpty()) {
        gsgsWARNING() << "CodeFormatter: Missing clang_format exec" << clEndl;
        return;
    }
                                      ...
                                      const int& selStart, const int& selEnd)
{
    ...
    if(cursorPosition != wxNOT_FOUND) {
        ...
        JSON root(metadata);
        root.getObject().getObject("cursor").getValue(cursorPosition, wxNOT_FOUND);
...
void CodeFormatter::DoFormatFileAsString(const wxFileName& fileName, const FormatterEngine& engine)
{
    ...
    if(!FileUtil::ReadFileContent(fileName, content)) {
        gsgsWARNING() << "CodeFormatter: Failed to load file: " << fileName << clEndl;
        return;
    }
    ...
    if(!FileUtil::WriteFileContent(fileName, content)) {
        gsgsWARNING() << "CodeFormatter: Failed to save file: " << fileName << clEndl;
        return;
    }
}
...
void CodeFormatter::DoFormatWithWxXmlDocument(const wxFileName& fileName)
{
    ...
    if(!doc.Load(filePaht) || !doc.Save(filePaht, m_mgr->GetEditorSettings()->GetIndentWidth())) {
        gsgsWARNING() << "CodeFormatter: Failed to format XML file: " << fileName << clEndl;
        return;
    }
}
...
void CodeFormatter::OnContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void CodeFormatter::UnPlug()
{
    ...
    m_mgr->GetTheApp()->Disconnect(XRCID("format_files"), wxEVT_COMMAND_MENU_SELECTED,
                                   wxCommandEventHandler(CodeFormatter::OnFormatFiles), NULL, this);
    gsgs_Event().Disconnect(wxEVT_FORMAT_STRING, gsgsSourceFormatEventHandler(CodeFormatter::OnFormatString), NULL, this);
    gsgs_Event().Disconnect(wxEVT_FORMAT_FILE, gsgsSourceFormatEventHandler(CodeFormatter::OnFormatFile), NULL, this);
    gsgs_Event().Unbind(wxEVT_BEFORE_EDITOR_SAVE, gsgsEventHandler(CodeFormatter::OnBeforeFileSave), this);
    gsgs_Event().Unbind(wxEVT_PHP_SETTINGS_CHANGED, &CodeFormatter::OnPhpSettingsChanged, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FOLDER, &CodeFormatter::OnContextMenu, this);
}

IManager* CodeFormatter::GetManager() { return m_mgr; }

void CodeFormatter::OnFormatString(gsgs::SourceFormatEvent& e)
{
...
{
    OptionsConfig * options = m_mgr->GetEditorSettings();
...
void CodeFormatter::OnFormatFile(gsgs::SourceFormatEvent& e)
{
...
void CodeFormatter::OnFormatProject(wxCommandEvent& event)
{
    ...
    ProjectPtr pProj = gsgs_CppWorker().GetProject(selectedItem.m_text);
    CHECK_PTR_RET(pProj);
...
void CodeFormatter::OnBeforeFileSave(gsgs::Event& e)
{
    e.Skip();
...
void CodeFormatter::OnPhpSettingsChanged(gsgs::Event& event)
{
    event.Skip();
    m_optionsPhp.Load();
}
