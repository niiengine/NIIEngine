...
typedef BOOL (*DBG_BREAK_PROC_FUNC_PTR)(HANDLE);
    ...
    , m_internalBpId(wxNOT_FOUND)
{
    ...
    if(Kernel32Dll) {
        ...
        if(SetConsoleCtrlHandler((PHANDLER_ROUTINE)CtrlHandler, TRUE) == FALSE) {
            gsgsLogMessage(wxString::Format(wxT("failed to install ConsoleCtrlHandler: %d"), GetLastError()));
        }
    }
    if(SetConsoleCtrlHandler((PHANDLER_ROUTINE)CtrlHandler, TRUE) == FALSE) {
        gsgsLogMessage(wxString::Format(wxT("failed to install ConsoleCtrlHandler: %d"), GetLastError()));
    ...
    gsgs_Event().Connect(wxEVT_GDB_STOP_DEBUGGER, wxCommandEventHandler(DbgGdb::OnKillGDB), NULL, this);
}
...
#endif
    gsgs_Event().Disconnect(wxEVT_GDB_STOP_DEBUGGER, wxCommandEventHandler(DbgGdb::OnKillGDB), NULL, this);
}
...
DbgCmdHandler* DbgGdb::PopHandler(const wxString& id)
{
    ...
    //        --it;
    //        if(it->first != oldId) {
    //            gsgs_Warning("Request to process handler %s while handler %s is still in the queue!!", id, it->first);
    //        }
    //        ++it;
    //    }
    m_handlers.erase(id);
...
bool DbgGdb::WriteCommand(const wxString& command, DbgCmdHandler* handler)
{
    ...
    if(!ExecuteCmd(cmd)) {
        gsgs_Warning("Failed to send command: %s", cmd);
        return false;
    }
...
bool DbgGdb::Stop()
{
    ...
    if(!m_attachedMode) {
        Kill((int)m_debuggeePid, wxSIGKILL, true, (m_info.flags & DebuggerInformation::kRunAsSuperuser));
    }
...
bool DbgGdb::Interrupt()
{
    if(m_debuggeePid > 0) {
        ...
        Kill((int)m_debuggeePid, wxSIGINT, false, (m_info.flags & DebuggerInformation::kRunAsSuperuser));
        // kill(m_debuggeePid, SIGINT);
        return true;
#endif
    } else {
        ::wxMessageBox(_("Can't interrupt debuggee process: I don't know its PID!"), wxT("gsgs"));
    }
    return false;
}
...
void DbgGdb::OnProcessEnd(gsgs::ProcessEvent& e)
{
    DoCleanup();
    m_observer->UpdateGotControl(DBG_EXITED_NORMALLY);
}
...
bool DbgGdb::DoLocateGdbExecutable(const wxString& debuggerPath, wxString& dbgExeName)
{
    ...
    if(ExeLocator::locate(dbgExeName, actualPath) == false) {
        wxMessageBox(wxString::Format(wxT("Failed to locate gdb! at '%s'"), dbgExeName.c_str()), wxT("gsgs"));
        return false;
    }
    ...
    codelite_gdbinit_file << wxFileName::GetTempDir() << gsgsPathSep << wxT("codelite_gdbinit.txt");
...
void DbgGdb::OnDataRead(gsgs::ProcessEvent& e)
{
