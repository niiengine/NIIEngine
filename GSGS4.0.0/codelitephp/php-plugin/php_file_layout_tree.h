...
public:
    PHPFileLayoutTree(wxWindow* parent, gsgs::IEditor* editor, IManager* manager);
    PHPFileLayoutTree(wxWindow* parent);
    virtual ~PHPFileLayoutTree();

    void Construct();
    void AdvanceSelection(bool forward);
    void SetEditor(gsgs::IEditor* editor) { this->m_editor = editor; }
    void SetManager(IManager* manager) { this->m_manager = manager; }
    gsgs::IEditor* GetEditor() { return m_editor; }
