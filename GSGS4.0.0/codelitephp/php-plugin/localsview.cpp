...
{
    gsgs_Event().Bind(wxEVT_XDEBUG_LOCALS_UPDATED, &LocalsView::OnLocalsUpdated, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_SESSION_ENDED, &LocalsView::OnXDebugSessionEnded, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_SESSION_STARTED, &LocalsView::OnXDebugSessionStarted, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_PROPERTY_GET, &LocalsView::OnProperytGet, this);
...
{
    gsgs_Event().Unbind(wxEVT_XDEBUG_LOCALS_UPDATED, &LocalsView::OnLocalsUpdated, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_SESSION_ENDED, &LocalsView::OnXDebugSessionEnded, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_SESSION_STARTED, &LocalsView::OnXDebugSessionStarted, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_PROPERTY_GET, &LocalsView::OnProperytGet, this);
}
...
{
    e.Skip();
    gsgs_Debug("LocalsView::OnXDebugSessionEnded(): Debug sessions started - cleaning all locals view");
...
{
    e.Skip();
    gsgs_Debug("Inside OnLocalsUpdated");
...
{
    e.Skip();
    gsgs_Debug("LocalsView::OnXDebugSessionStarted(): Debug sessions started - cleaning all locals view");
    // Clear the variables view
    ClearView();
    m_localsExpandedItemsFullname.clear();
    m_localsExpandedItems.Clear();
...
void LocalsView::OnCopyValue(wxCommandEvent& event)
{
    ...
    for(size_t i = 0; i < items.size(); ++i) {
        ...
        textToCopy << v << gsgs_EditorConfig().GetOptions()->GetEOLAsString();
    }
