    ...
    , m_scanInProgress(true)
{
    ...
    gsgs_Event().Connect(wxEVT_CMD_EXECUTE_ACTIVE_PROJECT,
                                  gsgsExecuteEventHandler(PHPWorkspaceView::OnRunActiveProject), NULL, this);
    gsgs_Event().Bind(wxEVT_CMD_STOP_EXECUTED_PROGRAM, &PHPWorkspaceView::OnStopExecutedProgram, this);
    gsgs_Event().Bind(wxEVT_CMD_IS_PROGRAM_RUNNING, &PHPWorkspaceView::OnIsProgramRunning, this);
    gsgs_Event().Connect(wxEVT_ACTIVE_EDITOR_CHANGED, wxCommandEventHandler(PHPWorkspaceView::OnEditorChanged),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_PHP_FILE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnFileRenamed), NULL, this);
    gsgs_Event().Bind(wxPHP_PARSE_ENDED, &PHPWorkspaceView::OnPhpParserDone, this);
    gsgs_Event().Bind(wxPHP_PARSE_PROGRESS, &PHPWorkspaceView::OnPhpParserProgress, this);
    gsgs_Event().Bind(wxEVT_PHP_WORKSPACE_LOADED, &PHPWorkspaceView::OnWorkspaceLoaded, this);
    gsgs_Event().Bind(wxEVT_PHP_WORKSPACE_RENAMED, &PHPWorkspaceView::OnWorkspaceRenamed, this);
    gsgs_Event().Bind(wxEVT_FINDINFILES_DLG_SHOWING, &PHPWorkspaceView::OnFindInFilesShowing, this);
    gsgs_Event().Bind(wxEVT_FINDINFILES_DLG_DISMISSED, &PHPWorkspaceView::OnFindInFilesDismissed, this);
    gsgs_Event().Bind(wxEVT_FILE_SAVEAS, &PHPWorkspaceView::OnFileSaveAs, this);
    Bind(wxEVT_PHP_PROJECT_FILES_SYNC_END, &PHPWorkspaceView::OnProjectSyncCompleted, this);

    gsgs::ImageManager* bl = m_mgr->GetStdIcons();
    m_treeCtrlView->SetBitmaps(&bl->GetBitmaps());

    // Allow the PHP view to accepts folders
    m_treeCtrlView->SetDropTarget(new gsgs::FileOrFolderDropTarget(this));
...
{
    gsgs_Event().Disconnect(wxEVT_CMD_EXECUTE_ACTIVE_PROJECT,
                                     gsgsExecuteEventHandler(PHPWorkspaceView::OnRunActiveProject), NULL, this);
    gsgs_Event().Unbind(wxEVT_CMD_STOP_EXECUTED_PROGRAM, &PHPWorkspaceView::OnStopExecutedProgram, this);
    gsgs_Event().Unbind(wxEVT_CMD_IS_PROGRAM_RUNNING, &PHPWorkspaceView::OnIsProgramRunning, this);
    gsgs_Event().Disconnect(wxEVT_ACTIVE_EDITOR_CHANGED,
                                     wxCommandEventHandler(PHPWorkspaceView::OnEditorChanged), NULL, this);
    gsgs_Event().Disconnect(wxEVT_PHP_FILE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnFileRenamed), NULL,
                                     this);
    gsgs_Event().Unbind(wxPHP_PARSE_ENDED, &PHPWorkspaceView::OnPhpParserDone, this);
    gsgs_Event().Unbind(wxPHP_PARSE_PROGRESS, &PHPWorkspaceView::OnPhpParserProgress, this);
    gsgs_Event().Unbind(wxEVT_PHP_WORKSPACE_LOADED, &PHPWorkspaceView::OnWorkspaceLoaded, this);
    gsgs_Event().Unbind(wxEVT_PHP_WORKSPACE_RENAMED, &PHPWorkspaceView::OnWorkspaceRenamed, this);
    gsgs_Event().Unbind(wxEVT_FINDINFILES_DLG_SHOWING, &PHPWorkspaceView::OnFindInFilesShowing, this);
    gsgs_Event().Unbind(wxEVT_FINDINFILES_DLG_DISMISSED, &PHPWorkspaceView::OnFindInFilesDismissed, this);
    ...
    gsgs_Event().Unbind(wxEVT_FILE_SAVEAS, &PHPWorkspaceView::OnFileSaveAs, this);
...
void PHPWorkspaceView::OnFolderDropped(gsgs::Event& event)
{
    const wxArrayString& folders = event.GetStrings();
    if(folders.GetCount() != 1) {
        ::wxMessageBox(_("Can only import one folder at a time"), "gsgs", wxOK | wxICON_ERROR | wxCENTER);
        return;
    }
    ...
    for(size_t i = 0; i < workspaceFiles.size(); ++i) {
        if(FileUtil::GetType(workspaceFiles.Item(i)) == FileUtil::TypeWorkspacePHP) {
    ...
    if(!PHPWorkspace::Get()->IsOpen()) {
        ...
        if(!workspaceFileName.IsDirWritable()) {
            ...
            message << _("Failed to create workspace '") << workspaceFileName.GetFullPath() << "'\n"
                    << _("Permission denied.");
            ::wxMessageBox(message, "gsgs", wxOK | wxICON_ERROR | wxCENTER);
            return;
        }
        ...
        if(!workspaceFile.IsEmpty()) {
            ...
            ::wxMessageBox(
                ...
                "gsgs", wxOK | wxICON_WARNING | wxCENTER);
            return;
        }
    ...
    if(item.IsOk()) {
        ...
        if(data) {
            ...
            case ItemData::Kind_File: {
                ...
                // file context menu
                ContextMenuEvent menuEvent(wxEVT_CONTEXT_MENU_FILE);
            ...
            case ItemData::Kind_Workspace: {
                ...
                menuItem->SetBitmap(gsgs_Image().LoadBitmap("debugger_restart"));
            ...
            case ItemData::Kind_Project: {
                ...
                ContextMenuEvent folderMenuEvent(wxEVT_CONTEXT_MENU_FOLDER);
                folderMenuEvent.SetMenu(&menu);
                folderMenuEvent.SetPath(wxFileName(data->GetFile()).GetPath());
                gsgs_Event().ProcessEvent(folderMenuEvent);
...
void PHPWorkspaceView::LoadWorkspaceView()
{
    ...
    for(; iter_project != projects.end(); ++iter_project) {
        ...
        wxTreeItemId projectItemId = m_treeCtrlView->AppendItem(
            root, iter_project->second->GetName(), bl->GetMimeImageId(FileUtil::TypeProject),
            bl->GetMimeImageId(FileUtil::TypeProjectExpanded), data);
    ...
    if(selection.IsOk()) {
        ...
        if(itemData && itemData->IsProject()) {
            if(wxMessageBox(wxString() << _("Are you sure you want to remove project '") << itemData->GetProjectName()
                                       << "'?",
                            _("光栅故事"), wxYES_NO | wxCANCEL | wxCANCEL_DEFAULT, wxTheApp->GetTopWindow()) == wxYES) {
...
void PHPWorkspaceView::OnNewFile(wxCommandEvent& e)
{
    ...
    if(data->IsFolder() || data->IsProject()) {
        wxString filename =
            ::GetUserText(_("New File"), _("Set the file name:"), "new.php", wxStrlen("new"));
...
void PHPWorkspaceView::OnDeleteFolder(wxCommandEvent& e)
{
    ...
    if(wxMessageBox(msg, wxT("gsgs"), wxYES_NO | wxCANCEL | wxICON_WARNING | wxCENTER) != wxYES) return;
...
int PHPWorkspaceView::DoGetItemImgIdx(const wxString& filename)
{
    ...
    if(idx == wxNOT_FOUND) { idx = m_mgr->GetStdIcons()->GetMimeImageId(FileUtil::TypeText); }
    return idx;
}
...
void PHPWorkspaceView::OnRenameFile(wxCommandEvent& e)
{
    ...
    wxString new_name = ::GetUserText(_("New file name:"), _("Rename file"), oldFileName.GetFullName());
    ...
    gsgs::IEditor* editor = m_mgr->FindEditor(oldFileName.GetFullPath());
    if(editor) {
...
void PHPWorkspaceView::DoDeleteSelectedFileItem()
{
    ...
    wxStandardID res = ::PromptMessageDialogWithoutCancel(msg, "PHPDeleteFiles", _("Yes"), _("No"),
    ...
    for(size_t i = 0; i < items.GetCount(); ++i) {
        ...
        if(itemData->IsFolder()) {
            ...
            res = ::PromptMessageDialogWithoutCancel(msg, "PHPDeleteFolder", _("Yes"), _("No"),
                                                     _("Remember my answer and don't ask me again"),
                                                     wxYES_NO | wxICON_QUESTION | wxNO_DEFAULT);
            ...
            gsgsRemoveFile(itemData->GetFile());
    ...
    {
        gsgs::FileEvent evt(wxEVT_FILE_DELETED);
    ...
    {
        gsgs::FileEvent evt(wxEVT_FOLDER_DELETED);
...
void PHPWorkspaceView::OnRunProject(wxCommandEvent& e)
{
    ...
    PHPDebugStartDlg debugDlg(gsgs_Event().TopFrame(), PHPWorkspace::Get()->GetActiveProject(), m_mgr);
...
void PHPWorkspaceView::OnActiveProjectSettings(wxCommandEvent& event)
{
    if(!PHPWorkspace::Get()->GetActiveProject()) {
        ::wxMessageBox(_("No active project is set !?\nPlease set an active project and try again"), "gsgs",
...
void PHPWorkspaceView::OnRunActiveProject(gsgs::ExecuteEvent& e)
{
    if(PHPWorkspace::Get()->IsOpen()) {
        gsgs_Debug("Running active project...");
        CHECK_COND_RET(PHPWorkspace::Get()->GetActiveProject());
        // Test which file we want to debug
        PHPDebugStartDlg dlg(gsgs_Event().TopFrame(), PHPWorkspace::Get()->GetActiveProject(), m_mgr);
...
void PHPWorkspaceView::OnIsProgramRunning(gsgs::ExecuteEvent& e)
{
    if(PHPWorkspace::Get()->IsOpen()) {
...
void PHPWorkspaceView::OnStopExecutedProgram(gsgs::ExecuteEvent& e)
{
...
void PHPWorkspaceView::OnNewClass(wxCommandEvent& e)
{
    ...
    if(dlg.ShowModal() == wxID_OK) {
        ...
        wxString eolString = gsgs_EditorConfig().GetOptions()->GetEOLAsString();
...
void PHPWorkspaceView::DoOpenSSHAccountManager()
{
    ...
    SFTPBrowserDlg dlg(gsgs_Event().TopFrame(),
                       _("Select the remote folder corrseponding to the current workspace file"), "",
                       gsgs::SFTP::SFTP_BROWSE_FOLDERS); // Browse for folders only
...
void PHPWorkspaceView::OnPhpParserDone(gsgs::ParseEvent& event)
{
    event.Skip();
    ReportParseThreadDone();
}

void PHPWorkspaceView::OnPhpParserProgress(gsgs::ParseEvent& event)
{
    event.Skip();
    ReportParseThreadProgress(event.GetCurfileIndex(), event.GetTotalFiles());
}

void PHPWorkspaceView::OnPhpParserStarted(gsgs::ParseEvent& event) { event.Skip(); }
...
wxTreeItemId PHPWorkspaceView::DoAddFolder(const wxString& project, const wxString& path)
{
    ...
    int imgId = m_mgr->GetStdIcons()->GetMimeImageId(FileUtil::TypeFolder);
    int imgIdExpanded = m_mgr->GetStdIcons()->GetMimeImageId(FileUtil::TypeFolderExpanded);
    wxString curpath;
                                            ...
                                            const wxString& content)
{
    ...
    if(FileUtil::WriteFileContent(file, content)) {
        // Update the UI
        // Add the file
        ItemData* itemData = new ItemData(ItemData::Kind_File);
...
void PHPWorkspaceView::OnOpenInExplorer(wxCommandEvent& e)
{
    ...
    FileUtil::OpenFileExplorerAndSelect(file);
}
...
void PHPWorkspaceView::OnOpenShell(wxCommandEvent& e)
{
    ...
    FileUtil::OpenTerminal(file.GetPath());
}
...
void PHPWorkspaceView::OnAddExistingProject(wxCommandEvent& e)
{
    ...
    const wxString ALL("gsgs PHP Projects (*.phprj)|*.phprj");
    ...
    if(dlg.ShowModal() == wxID_OK && !dlg.GetPath().IsEmpty()) {
        ...
        if(!PHPWorkspace::Get()->AddProject(projectToAdd, errmsg)) {
            if(!errmsg.IsEmpty()) { ::wxMessageBox(errmsg, "gsgs", wxICON_WARNING | wxOK | wxCENTER); }
            return;
        }
        LoadWorkspaceView();
    }
...
void PHPWorkspaceView::OnRenameFolder(wxCommandEvent& e)
{
    ...
    wxString new_name = ::GetUserText(_("Rename folder"), _("Folder name:"), data->GetFolderName());
    if(new_name.IsEmpty()) return;
...
void PHPWorkspaceView::OnFindInFilesDismissed(gsgs::FindInFilesEvent& e)
{
    e.Skip();
    if(PHPWorkspace::Get()->IsOpen()) {
        // store the find in files mask
        gsgs_Config().Write("FindInFiles/PHP/LookIn", e.GetPaths());
        gsgs_Config().Write("FindInFiles/PHP/Mask", e.GetFileMask());
    }
}

void PHPWorkspaceView::OnFindInFilesShowing(gsgs::FindInFilesEvent& e)
{
    e.Skip();
    ...
    wxString outot;
    gsgs_Config().Read("FindInFiles/PHP/Mask", outot, mask);
    e.SetFileMask(outot);
    ...
    if(m_treeCtrlView->IsShownOnScreen() && m_treeCtrlView->HasFocus()) {
        ...
        lookIn << "<Entire Workspace>\n"
               << "-*vendor*";
        gsgs_Config().Read("FindInFiles/PHP/LookIn", outot, lookIn);
        e.SetPaths(outot);
    }
}

void PHPWorkspaceView::OnWorkspaceSyncStart(gsgs::Event& event)
{
    m_scanInProgress = true;
...
void PHPWorkspaceView::OnWorkspaceSyncEnd(gsgs::Event& event)
{
...
{
    GetManager()->GetStatusBar()->SetMessage(text, timeOut);
}

void PHPWorkspaceView::OnFileSaveAs(gsgs::FileEvent& event)
{
    event.Skip();
    // File "Save as"
...
void PHPWorkspaceView::OnProjectSyncCompleted(gsgs::Event& event)
{
        ...
        if(!targetFolder.IsEmpty()) {
            ...
            for(size_t i = 0; i < m_draggedFiles.size(); ++i) {
                ...
                if(newFile.FileExists()) {
                    wxStandardID answer = ::PromptMessageDialogWithoutCancel(
                ...
                if(::wxCopyFile(srcfile, newFile.GetFullPath(), true)) {
                    if(gsgsRemoveFile(srcfile)) {
                        syncNeeded = true;
                        gsgs::FileEvent rmEvent(wxEVT_FILE_DELETED);
