    ...
    , m_timer(NULL)
{
    ...
    gsgs_Config().Read("PHP/OpenResourceDialog/SearchString", lastStringTyped, wxString());
...
void OpenResourceDlg::DoInitialize()
{
    ...
    m_dvListCtrl->SetBitmaps(&bmpLoader->GetBitmaps());

    CLASS_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kClass);
    FUNC_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kFunctionPublic);
    CONST_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kEnumerator);
    DEFINE_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kMacro);
    VARIABLE_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kMemberPublic);
    NAMESPACE_IMG_ID = bmpLoader->GetMimeImageId(gsgs::ImageManager::kNamespace);
...
OpenResourceDlg::~OpenResourceDlg()
{
    ...
    gsgs_Config().Write("PHP/OpenResourceDialog/SearchString", m_textCtrlFilter->GetValue());
}
...
void OpenResourceDlg::DoGetResources(const wxString& filter)
{
    ...
    for(; iter != matches.end(); ++iter) {
        ...
        if(FileUtil::FuzzyMatch(filter, match->GetFullName())) {
...
ResourceVector_t OpenResourceDlg::DoGetFiles(const wxString& filter)
{
    ...
    for(size_t i = 0; i < m_allFiles.size(); i++) {
        ...
        if(FileUtil::FuzzyMatch(filter, filename)) {
...
int OpenResourceDlg::DoGetImgIdx(const ResourceItem* item)
{
    ...
    case ResourceItem::kRI_File:
        return gsgs_Image().GetMimeImageId(item->filename.GetFullName());
