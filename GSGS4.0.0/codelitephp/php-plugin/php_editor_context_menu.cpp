...
PHPEditorContextMenu* PHPEditorContextMenu::gInstance = 0;
...
PHPEditorContextMenu::~PHPEditorContextMenu()
{
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR, &PHPEditorContextMenu::OnContextMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR_MARGIN, &PHPEditorContextMenu::OnMarginContextMenu, this);
...
{
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR, &PHPEditorContextMenu::OnContextMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR_MARGIN, &PHPEditorContextMenu::OnMarginContextMenu, this);
...
{
    if(gInstance == 0) {
        gInstance = new PHPEditorContextMenu();
    }
    return gInstance;
}
...
void PHPEditorContextMenu::DoBuildMenu(wxMenu* menu, gsgs::IEditor* editor)
{
...
bool PHPEditorContextMenu::IsLineMarked()
{
    ...
    // 128 is the mask representing a bookmark (refer to enum marker_mask_type in gsgsEditor.h)
...
bool PHPEditorContextMenu::IsIncludeOrRequireStatement(wxString& includeWhat)
{
    ...
    gsgs::IEditor* editor = m_manager->GetActiveEditor();
...
void PHPEditorContextMenu::DoContextMenu(gsgs::IEditor* editor, wxCommandEvent& e)
{
...
void PHPEditorContextMenu::OnContextMenu(ContextMenuEvent& e)
{
    e.Skip();
    gsgs::IEditor* editor = m_manager->GetActiveEditor();
...
void PHPEditorContextMenu::OnMarginContextMenu(ContextMenuEvent& e)
{
    e.Skip();
    gsgs::IEditor* editor = m_manager->GetActiveEditor();
    if(editor && IsPHPFile(editor)) {
...
void PHPEditorContextMenu::OnInsertDoxyComment(wxCommandEvent& e)
{
    ...
    if(editor) {
        ...
        if(entry) {
            ...
            wxString doxyBlock = ::JoinLinesWithEOL(lines, ctrl->GetEOLMode());
...
void PHPEditorContextMenu::OnGenerateSettersGetters(wxCommandEvent& e)
{
    ...
    if(editor) {
        ...
        wxString textToAdd;
        PHPSettersGettersDialog dlg(gsgs_Event().TopFrame(), editor, m_manager);
