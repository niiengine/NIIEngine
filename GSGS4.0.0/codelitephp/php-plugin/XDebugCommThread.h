...
class XDebugComThread : public wxThread
{
    ...
    int m_port;
    gsgs::SocketServer m_server;
    wxString m_host;
    int m_waitForConnTimeout;
    
protected:
    void DoSendCommand(const wxString& command, gsgs::SocketBase::Ptr_t client);
    bool DoReadReply(std::string& reply, gsgs::SocketBase::Ptr_t client);
    void Stop();
