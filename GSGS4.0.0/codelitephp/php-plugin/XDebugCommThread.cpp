...
{
    gsgsDEBUG() << "gsgs >>> XDebugComThread started" << clEndl;
    //----------------------------------------------------------------
    ...
    try {
        ...
        gsgsDEBUG() << "gsgs >>> Creating server on:" << connectionString << clEndl;
        m_server.Start(connectionString);

        gsgsDEBUG() << "gsgs >>> Listening on" << m_host << ":" << m_port << clEndl;
        ...
        do {
            ...
            ++retry;
            gsgsDEBUG() << "gsgs >>> Waiting for connection.." << clEndl;

        } while(!TestDestroy() && !client);

        gsgs_Debug("gsgs >>> Successfully accepted connection from XDebug!");
        ...
        gsgs_Debug("XDebugComThread caught an exception: %s", e.what());
        m_xdebugMgr->CallAfter(&XDebugManager::OnCommThreadTerminated);
        return NULL;
    }
...
bool XDebugComThread::DoReadReply(std::string& reply, gsgs::SocketBase::Ptr_t client)
{
    if(!client) {
        return false;
    }
...
void XDebugComThread::DoSendCommand(const wxString& command, gsgs::SocketBase::Ptr_t client)
{
    // got message, process it
    if(!client) {
        return;
    }
    gsgsDEBUG(wxString() << "gsgs >>> " << command);
