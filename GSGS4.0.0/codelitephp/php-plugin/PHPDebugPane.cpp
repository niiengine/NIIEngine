...
{
    Hide();
    gsgs_Event().Bind(wxEVT_XDEBUG_STACK_TRACE, &PHPDebugPane::OnUpdateStackTrace, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_SESSION_STARTED, &PHPDebugPane::OnXDebugSessionStarted, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_SESSION_STARTING, &PHPDebugPane::OnXDebugSessionStarting, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_BREAKPOINTS_UPDATED, &PHPDebugPane::OnRefreshBreakpointsView, this);
    gsgs_Event().Bind(wxEVT_XDEBUG_SESSION_ENDED, &PHPDebugPane::OnXDebugSessionEnded, this);
    gsgs_Event().Bind(wxEVT_EDITOR_CONFIG_CHANGED, &PHPDebugPane::OnSettingsChanged, this);
    ...
    m_tbBreakpoints->AddTool(wxID_DELETE, _("Delete"), gsgs_Image().LoadBitmap("minus"),
                             _("Delete the selected breakpoints"));
    m_tbBreakpoints->AddTool(wxID_CLEAR, _("Delete all breakpoints"),
                             gsgs_Image().LoadBitmap("clean"), _("Delete all breakpoints"));
    ...
    m_bitmaps.push_back(gsgs_Image().LoadBitmap("forward"));
    m_bitmaps.push_back(gsgs_Image().LoadBitmap("placeholder"));
    m_dvListCtrlStackTrace->SetBitmaps(&m_bitmaps);
}

PHPDebugPane::~PHPDebugPane()
{
    gsgs_Event().Unbind(wxEVT_XDEBUG_STACK_TRACE, &PHPDebugPane::OnUpdateStackTrace, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_SESSION_STARTED, &PHPDebugPane::OnXDebugSessionStarted, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_SESSION_STARTING, &PHPDebugPane::OnXDebugSessionStarting, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_BREAKPOINTS_UPDATED, &PHPDebugPane::OnRefreshBreakpointsView, this);
    gsgs_Event().Unbind(wxEVT_XDEBUG_SESSION_ENDED, &PHPDebugPane::OnXDebugSessionEnded, this);
    gsgs_Event().Unbind(wxEVT_EDITOR_CONFIG_CHANGED, &PHPDebugPane::OnSettingsChanged, this);
}
...
void PHPDebugPane::OnCallStackItemActivated(wxDataViewEvent& event)
{
    ...
    gsgs_Event().AddPendingEvent(eventOpenFile);
}
...
void PHPDebugPane::OnDeleteBreakpoint(wxCommandEvent& event)
{
    ...
    for(size_t i = 0; i < items.GetCount(); ++i) {
        ...
        gsgs_Event().AddPendingEvent(eventDelBP);
    }
}
...
{
    e.Skip();
    // LexerStyle::Ptr_t lex = gsgs_EditorConfig().GetLexer("php");
    // if(lex) {
