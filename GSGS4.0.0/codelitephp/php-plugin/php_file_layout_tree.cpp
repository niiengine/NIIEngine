...
PHPFileLayoutTree::PHPFileLayoutTree(wxWindow* parent, gsgs::IEditor* editor, IManager* manager)
...
void PHPFileLayoutTree::Construct()
{
    ...
    wxImageList* images = new wxImageList(GetScaledSize(16), GetScaledSize(16), true);
...
wxTreeItemId PHPFileLayoutTree::RecurseSearch(const wxTreeItemId& item, const wxString& word)
{
    ...
    if(item != GetRootItem()) {
        if(FileUtil::FuzzyMatch(word, GetItemText(item))) {
            return item;
        }
    }
