...
{
    gsgs_Event().Connect(wxEVT_CMD_RETAG_WORKSPACE, wxCommandEventHandler(PHPCodeCompletion::OnRetagWorkspace),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_CMD_RETAG_WORKSPACE_FULL,
                                  wxCommandEventHandler(PHPCodeCompletion::OnRetagWorkspace), NULL, this);

    gsgs_Event().Bind(wxEVT_FILE_SAVED, gsgsEventHandler(PHPCodeCompletion::OnFileSaved), this);

    gsgs_Event().Connect(wxEVT_CC_CODE_COMPLETE_LANG_KEYWORD,
                                  clCodeCompletionEventHandler(PHPCodeCompletion::OnCodeCompleteLangKeywords), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_CC_CODE_COMPLETE_BOX_DISMISSED,
                                  clCodeCompletionEventHandler(PHPCodeCompletion::OnCodeCompletionBoxDismissed), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_CC_GENERATE_DOXY_BLOCK,
                                  clCodeCompletionEventHandler(PHPCodeCompletion::OnInsertDoxyBlock), NULL, this);
    gsgs_Event().Connect(wxEVT_CC_JUMP_HYPER_LINK,
                                  clCodeCompletionEventHandler(PHPCodeCompletion::OnQuickJump), NULL, this);
    gsgs_Event().Bind(wxPHP_PARSE_ENDED, &PHPCodeCompletion::OnParseEnded, this);
    gsgs_Event().Bind(wxEVT_CC_UPDATE_NAVBAR, &PHPCodeCompletion::OnUpdateNavigationBar, this);
    gsgs_Event().Bind(wxEVT_NAVBAR_SCOPE_MENU_SHOWING, &PHPCodeCompletion::OnNavigationBarMenuShowing, this);
    gsgs_Event().Bind(wxEVT_NAVBAR_SCOPE_MENU_SELECTION_MADE,
...
PHPCodeCompletion::~PHPCodeCompletion()
{
    gsgs_Event().Unbind(wxEVT_CC_UPDATE_NAVBAR, &PHPCodeCompletion::OnUpdateNavigationBar, this);
    gsgs_Event().Disconnect(wxEVT_CMD_RETAG_WORKSPACE,
                                     wxCommandEventHandler(PHPCodeCompletion::OnRetagWorkspace), NULL, this);
    gsgs_Event().Disconnect(wxEVT_CMD_RETAG_WORKSPACE_FULL,
                                     wxCommandEventHandler(PHPCodeCompletion::OnRetagWorkspace), NULL, this);

    gsgs_Event().Unbind(wxEVT_FILE_SAVED, gsgsEventHandler(PHPCodeCompletion::OnFileSaved), this);
    gsgs_Event().Disconnect(wxEVT_CC_CODE_COMPLETE_LANG_KEYWORD,
                                     clCodeCompletionEventHandler(PHPCodeCompletion::OnCodeCompleteLangKeywords), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_CC_CODE_COMPLETE_BOX_DISMISSED,
                                     clCodeCompletionEventHandler(PHPCodeCompletion::OnCodeCompletionBoxDismissed),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_CC_GENERATE_DOXY_BLOCK,
                                     clCodeCompletionEventHandler(PHPCodeCompletion::OnInsertDoxyBlock), NULL, this);
    gsgs_Event().Disconnect(wxEVT_CC_JUMP_HYPER_LINK,
                                     clCodeCompletionEventHandler(PHPCodeCompletion::OnQuickJump), NULL, this);
    gsgs_Event().Unbind(wxPHP_PARSE_ENDED, &PHPCodeCompletion::OnParseEnded, this);
    gsgs_Event().Unbind(wxEVT_NAVBAR_SCOPE_MENU_SHOWING, &PHPCodeCompletion::OnNavigationBarMenuShowing, this);
    gsgs_Event().Unbind(wxEVT_NAVBAR_SCOPE_MENU_SELECTION_MADE,
                                 &PHPCodeCompletion::OnNavigationBarMenuSelectionMade, this);
    // code completion events
...
void PHPCodeCompletion::DoShowCompletionBox(const PHPEntityBase::List_t& entries, PHPExpression::Ptr_t expr)
{
    ...
    for(size_t i = 0; i < tags.size(); ++i) {
        wxCodeCompletionBoxEntry::Ptr_t ccEntry = CodeBox::TagToEntry(tags.at(i));
        ccEntry->SetComment(tags.at(i)->GetComment());
        ccEntries.push_back(ccEntry);
    }
    gsgs::CodeBoxManager::Get().ShowCompletionBox(m_manager->GetActiveEditor()->GetCtrl(), ccEntries,
                                                        CodeBox::kRefreshOnKeyType, wxNOT_FOUND);
}

void PHPCodeCompletion::OnCodeCompletionBoxDismissed(CodeCompleteEvent& e) { e.Skip(); }
...
void PHPCodeCompletion::OnCodeComplete(CodeCompleteEvent& e)
{
    e.Skip(true);
    if(PHPWorkspace::Get()->IsOpen()) {
        ...
        if(editor && IsPHPFile(editor)) {
            ...
            // Update the settings
            TagsOptionsData d;
            //gsgs::Config ccConfig("code-completion.conf");
            gsgs_Config().ReadItem(&d);
...
void PHPCodeCompletion::OnFunctionCallTip(CodeCompleteEvent& e)
{
    e.Skip();
    if(PHPWorkspace::Get()->IsOpen()) {
        ...
        if(editor) {
            ...
            if(IsPHPFile(editor)) {
                ...
                if(resolved) {
                    ...
                    // so we simply place our match into TagEntryPtrList structure and pass it to the editor
                    TagEntryPtrList tags;
                    tags.push_back(DoPHPEntityToTagEntry(resolved));
                    gsgsTipInfoPtr callTip(new TipInfo(tags));
...
void PHPCodeCompletion::OnFindSymbol(CodeCompleteEvent& e)
{
    e.Skip();
    if(PHPWorkspace::Get()->IsOpen()) {
        if(!CanCodeComplete(e)) return;
        ...
        if(editor) {
            ...
            if(symbols.size() == 1) {
                ...
                // Convert the matches to SelectSymbolDialogEntry::List_t
                ...
                std::for_each(symbols.begin(), symbols.end(), [&](PHPEntityBase::Ptr_t entry) {
                    ...
                    wxBitmap bmp = CodeBox::GetBitmap(tag);

                    SelectSymbolDialogEntry m;
                    m.bmp = bmp;
...
void PHPCodeCompletion::OnTypeinfoTip(CodeCompleteEvent& e)
{
    if(PHPWorkspace::Get()->IsOpen()) {
        if(!CanCodeComplete(e)) return;
...
PHPLocation::Ptr_t PHPCodeCompletion::FindDefinition(gsgs::IEditor* editor, int pos)
{
...
void PHPCodeCompletion::OnCodeCompleteLangKeywords(CodeCompleteEvent& e)
{
...
PHPEntityBase::Ptr_t PHPCodeCompletion::GetPHPEntityAtPos(gsgs::IEditor* editor, int pos)
{
    return DoGetPHPEntryUnderTheAtPos(editor, pos, false);
}

/* static */
bool PHPCodeCompletion::CanCodeComplete(CodeCompleteEvent& e)
{
...
void PHPCodeCompletion::OnFileSaved(gsgs::Event& event)
{
    event.Skip();
...
PHPEntityBase::Ptr_t PHPCodeCompletion::DoGetPHPEntryUnderTheAtPos(gsgs::IEditor* editor, int pos, bool forFunctionCalltip)
{
...
void PHPCodeCompletion::Open(const wxFileName& workspaceFile)
{
    ...
    fnDBFile.AppendDir(".gsgsworkspace");
...
void PHPCodeCompletion::OnInsertDoxyBlock(CodeCompleteEvent& e)
{
    e.Skip();
...
void PHPCodeCompletion::OnSymbolsCached() { gsgsLogMessage("PHP Symbols cached into OS cache"); }

void PHPCodeCompletion::OnSymbolsCacheError() { gsgsLogMessage("Error encountered while caching PHP symbols"); }

void PHPCodeCompletion::OnQuickJump(CodeCompleteEvent& e)
{
    e.Skip();
    if(PHPWorkspace::Get()->IsOpen()) {
        e.Skip(false);
...
void PHPCodeCompletion::GotoDefinition(gsgs::IEditor* editor, int pos)
{
    CHECK_PTR_RET(editor);
...
void PHPCodeCompletion::DoSelectInEditor(gsgs::IEditor* editor, const wxString& what, int from)
{
    if(editor) {
...
void PHPCodeCompletion::GetMembers(gsgs::IEditor* editor, PHPEntityBase::List_t& members, wxString& scope)
{
    members.clear();
    scope.clear();
    if(!editor) { return; }
...
{
    // Open the file (make sure we use the 'OpenFile' so we will get a browsing record)
    gsgs::IEditor* editor = m_manager->OpenFile(entry->GetFilename().GetFullPath(), wxEmptyString, entry->GetLine());
    if(editor) {
...
void PHPCodeCompletion::OnParseEnded(gsgs::ParseEvent& event)
{
    event.Skip();
    m_lookupTable.RebuildClassCache();
}

void PHPCodeCompletion::OnUpdateNavigationBar(CodeCompleteEvent& e)
{
    e.Skip();
    if(!GetManager()->GetNavigationBar()->IsShown()) return;
    gsgs::IEditor* active_editor = GetManager()->GetActiveEditor();
    gsgs::IEditor* editor = dynamic_cast<gsgs::IEditor*>(e.GetEditor());
    if(!editor || !active_editor || editor != active_editor) return;
    if(FileUtil::GetTypeFromExtension(editor->GetFileName()) != FileUtil::TypePhp) return;
    ...
    if(!func) {
        GetManager()->GetNavigationBar()->SetMessage("", "");
        return;
    }
    ...
    GetManager()->GetNavigationBar()->SetMessage(className, functionName);
}

void PHPCodeCompletion::OnNavigationBarMenuSelectionMade(gsgs::Event& e)
{
    e.Skip();
    gsgs::IEditor* editor = GetManager()->GetActiveEditor();
    if(!editor || FileUtil::GetTypeFromExtension(editor->GetFileName()) != FileUtil::TypePhp) {
...
void PHPCodeCompletion::OnNavigationBarMenuShowing(ContextMenuEvent& e)
{
    e.Skip();
    gsgs::IEditor* editor = GetManager()->GetActiveEditor();
    if(!editor || FileUtil::GetTypeFromExtension(editor->GetFileName()) != FileUtil::TypePhp) return;
