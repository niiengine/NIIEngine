...
public:
    ...
    JSONObject ToJSON(JSONObject& e) const;
    void FromJSON(const JSONObject& e);
     ...
     */
    void GetWorkspaceFiles(gsgsStringSet& workspaceFiles, const wxString& filter) const;
    /**
     * @brief return list of files from the entire workspace
     * note that files are returned as fullpath
     */
    void GetWorkspaceFiles(gsgsStringSet& workspaceFiles, wxProgressDialog* progress = NULL) const;
    /**
...
protected:
    ...
    void OnProjectSyncEnd(gsgs::Event& event);
protected:
    wxFileName m_workspaceFile;
    PHPProject::Map_t m_projects;
    PHPExecutor m_executor;
    IManager* m_manager;
    gsgsStringSet m_inSyncProjects;
    wxEvtHandler* m_projectSyncOwner;
    // IWorkspace API
};

wxDECLARE_EVENT(wxEVT_PHP_WORKSPACE_FILES_SYNC_START, gsgs::Event);
wxDECLARE_EVENT(wxEVT_PHP_WORKSPACE_FILES_SYNC_END, gsgs::Event);
