...
PHPWorkspace* PHPWorkspace::gInstance = 0;

wxDEFINE_EVENT(wxEVT_PHP_WORKSPACE_FILES_SYNC_START, gsgs::Event);
wxDEFINE_EVENT(wxEVT_PHP_WORKSPACE_FILES_SYNC_END, gsgs::Event);
...
bool PHPWorkspace::Close(bool saveBeforeClose, bool saveSession)
{
    ...
    gsgs_Event().AddPendingEvent(phpEvent);
...
bool PHPWorkspace::Open(const wxString& filename, wxEvtHandler* view, bool createIfMissing)
{
    ...
    // wxBusyInfo busy(_("Scanning for workspace files..."), gsgs_Event().TopFrame());
    // wxYieldIfNeeded();
    wxBusyCursor bc;
    ...
    GetManager()->AddWorkspaceToRecentlyUsedList(GetFilename());
...
void PHPWorkspace::SetProjectActive(const wxString& project)
{
    ...
    if(activeProject) {
        gsgs::ProjectSettingsEvent evt(wxEVT_ACTIVE_PROJECT_CHANGED);
...
void PHPWorkspace::GetWorkspaceFiles(gsgsStringSet& workspaceFiles, wxProgressDialog* progress) const
{
...
void PHPWorkspace::GetWorkspaceFiles(gsgsStringSet& workspaceFiles, const wxString& filter) const
{
    ...
    if(filter.IsEmpty()) {
        ...
        gsgsStringSet::iterator iter = all_files.begin();
...
void PHPWorkspace::DoNotifyFilesRemoved(const wxArrayString& files)
{
    if(!files.IsEmpty()) {
        ...
        wxYieldIfNeeded();
        gsgs_Event().PostFileRemovedEvent(files);
    }
}
...
void PHPWorkspace::FromJSON(const JSONObject& e)
{
    m_projects.clear();
    if(e.isObjectExist("projects")) {
        PHPProject::Ptr_t firstProject;
        JSONObject projects = e.getObject("projects");
        int count = projects.getArraySize();
        for(int i = 0; i < count; ++i) {
            PHPProject::Ptr_t p(new PHPProject());
            wxString project_file;
            projects.getArray(i).getValue(project_file);
...
#define PHP_WORKSPACE_IDE wxString("gsgs")

JSONObject PHPWorkspace::ToJSON(JSONObject& e) const
{
    JSONObject metadata = JSONObject::createObject("metadata");
    e.addProperty(metadata);
    ...
    JSONObject projectsArr = JSONObject::createArray("projects");
    e.addProperty(projectsArr);
...
void PHPWorkspace::Rename(const wxString& newname)
{
    ...
    if(!::wxRenameFile(m_workspaceFile.GetFullPath(), new_path.GetFullPath())) {
        ...
        ::wxMessageBox(msg, "gsgs", wxOK | wxCENTER | wxICON_ERROR, gsgs_Event().TopFrame());
        return;
    }
    ...
    gsgs_Event().AddPendingEvent(evtRetag);
}
...
void PHPWorkspace::GetWorkspaceFiles(wxArrayString& workspaceFiles, wxProgressDialog* progress) const
{
    ...
    gsgsStringSet::const_iterator iter = files.begin();
...
void PHPWorkspace::ParseWorkspace(bool full)
{
    ...
    if(full) {
        ...
        fnDatabaseFile.AppendDir(".gsgsworkspace");

        wxLogNull noLog;
        bool bRemoved = gsgsRemoveFile(fnDatabaseFile.GetFullPath());
...
void PHPWorkspace::SyncWithFileSystemAsync(wxEvtHandler* owner)
{
    ...
    if(owner) {
        gsgs::Event event(wxEVT_PHP_WORKSPACE_FILES_SYNC_START);
    ...
    if(!m_projects.empty()) {
        ...
        if(owner) {
            ...
            gsgs::Event endEvent(wxEVT_PHP_WORKSPACE_FILES_SYNC_END);
            owner->AddPendingEvent(endEvent);
        }
    }
}

void PHPWorkspace::OnProjectSyncEnd(gsgs::Event& event)
{
    const wxString& name = event.GetString();
    if(m_inSyncProjects.count(name) == 0) {
        gsgsWARNING() << "PHPWorkspace::OnProjectSyncEnd: unable to find project '" << name << "' in the workspace..."
                    << clEndl;
        return;
    }

    gsgsDEBUG() << "PHPWorkspace::OnProjectSyncEnd: project" << name << "completed sync" << clEndl;
    ...
    if(m_inSyncProjects.empty()) {
        gsgsDEBUG() << "PHPWorkspace::OnProjectSyncEnd: all projects completed sync" << clEndl;
