...
{
    if(IsRunning()) {
        ::wxMessageBox(_("Another process is already running"), wxT("gsgs"), wxOK | wxICON_INFORMATION,
    ...
    if(cmd.IsEmpty()) {
        ::wxMessageBox(errmsg, wxT("gsgs"), wxOK | wxICON_INFORMATION, wxTheApp->GetTopWindow());
        return false;
    }
    wxString wd;
    if(proj) {
    ...
    if(!xdebugSessionName.IsEmpty()) {
        ...
        ConsoleFrame* console = new ConsoleFrame(gsgs_Event().TopFrame());
        console->Show();
