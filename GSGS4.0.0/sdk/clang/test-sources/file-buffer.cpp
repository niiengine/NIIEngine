...
void ClangWorkerThread::ProcessRequest(ThreadRequest* request)
{
    ...
    FileUtil::FileType type = FileUtil::GetType(task->GetFileName());
    ...
    gsgs_Event().AddPendingEvent(eEnd);
}
...
char** ClangWorkerThread::MakeCommandLine(ClangThreadRequest* req, int& argc, FileUtil::FileType fileType)
{
    bool isHeader = !(fileType == FileUtil::TypeSourceC || fileType == FileUtil::TypeSourceCpp);
...
void ClangWorkerThread::DoSetStatusMsg(const wxString& msg)
{
    ...
    e.SetInt(0);
    e.SetId(10);
    gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(e);
}
