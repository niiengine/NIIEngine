...
static bool checkMinSizes(const std::vector<Library::ArgumentChecks::MinSize> &minsizes, const Token * const ftok, const MathLib::bigint getArraySize, const Token **charSizeToken, const Settings * const settings)
{
    if (charSizeToken)
    ...
    for (std::vector<Library::ArgumentChecks::MinSize>::const_iterator minsize = minsizes.begin(); minsize != minsizes.end(); ++minsize) {
        ...
        case Library::ArgumentChecks::MinSize::STRLEN: {
            if (settings->library.isargformatstr(ftok,minsize->arg)) {
                ...
                if (len > getArraySize + 2)
                    error = true;
            } else {
                const Token *strtoken = argtok->getValueTokenMaxStrLength();
                if (strtoken && Token::getStrLength(strtoken) >= getArraySize)
                    error = true;
            }
        }
        break;
        case Library::ArgumentChecks::MinSize::SIZEOF:
            if (argtok->tokType() == Token::eString && Token::getStrLength(argtok) >= getArraySize)
                error = true;
            break;
...
void CheckBufferOverrun::checkFunctionParameter(const Token &ftok, unsigned int paramIndex, const ArrayInfo &arrayInfo, const std::list<const Token *>& callstack)
{
    ...
    if (minsizes) {
        MathLib::bigint getArraySize = arrayInfo.element_size();
        if (getArraySize == 0)
            return;
        for (std::size_t i = 0; i < arrayInfo.num().size(); ++i)
            getArraySize *= arrayInfo.num(i);
        ...
        if (checkMinSizes(*minsizes, &ftok, getArraySize, &charSizeToken, mSettings))
...
std::string CheckBufferOverrun::MyFileInfo::toString() const
{
    ...
    for (std::map<std::string, MathLib::bigint>::const_iterator it = getArraySize.begin(); it != getArraySize.end(); ++it) {
        ret << "    <ArraySize"
            << " array=\"" << ErrorLogger::toxml(it->first) << '\"'
            << " size=\"" << it->second << "\"/>\n";
    }
...
Check::FileInfo* CheckBufferOverrun::getFileInfo(const Tokenizer *tokenizer, const Settings *settings) const
{
    ...
    for (std::list<Variable>::const_iterator it = varlist.begin(); it != varlist.end(); ++it) {
        ...
        if (!var.isStatic() && var.isArray() && var.dimensions().size() == 1U && var.dimension(0U) > 0U)
            fileInfo->getArraySize[var.name()] = var.dimension(0U);
    }
    ...
    for (const tinyxml2::XMLElement *e = xmlElement->FirstChildElement(); e; e = e->NextSiblingElement()) {
        if (e->Name() == ArrayUsage) {
            ...
            if (!array || !size || !MathLib::isInt(size))
                continue;
            fileInfo->getArraySize[array] = MathLib::toLongNumber(size);
        }
    }
...
bool CheckBufferOverrun::analyseWholeProgram(const std::list<Check::FileInfo*> &fileInfo, const Settings&, ErrorLogger &errorLogger)
{
    ...
    for (std::list<Check::FileInfo*>::const_iterator it = fileInfo.begin(); it != fileInfo.end(); ++it) {
        ...
        // merge array info
        for (std::map<std::string, MathLib::bigint>::const_iterator it2 = fi->getArraySize.begin(); it2 != fi->getArraySize.end(); ++it2) {
            const std::map<std::string, MathLib::bigint>::const_iterator allit = all.getArraySize.find(it2->first);
            if (allit == all.getArraySize.end())
                all.getArraySize[it2->first] = it2->second;
            else
                all.getArraySize[it2->first] = -1;
        }
    }

    // Check buffer usage
    for (std::map<std::string, struct MyFileInfo::ArrayUsage>::const_iterator it = all.arrayUsage.begin(); it != all.arrayUsage.end(); ++it) {
        const std::map<std::string, MathLib::bigint>::const_iterator sz = all.getArraySize.find(it->first);
        if (sz != all.getArraySize.end() && sz->second > 0 && sz->second < it->second.index) {
