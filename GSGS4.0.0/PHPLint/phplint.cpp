    ...
    , m_process(NULL)
{
    ...
    gsgs_Event().Bind(wxEVT_FILE_LOADED, &PHPLint::OnLoadFile, this);
    gsgs_Event().Bind(wxEVT_FILE_SAVED, &PHPLint::OnSaveFile, this);
    gsgs_Event().Bind(wxEVT_PHP_SETTINGS_CHANGED, &PHPLint::OnPhpSettingsChanged, this);
}

PHPLint::~PHPLint() {}

void PHPLint::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void PHPLint::OnMenuCommand(wxCommandEvent& e)
{
    ...
    PHPLintDlg dlg(gsgs_Event().TopFrame());
...
void PHPLint::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_FILE_LOADED, &PHPLint::OnLoadFile, this);
    gsgs_Event().Unbind(wxEVT_FILE_SAVED, &PHPLint::OnSaveFile, this);
    gsgs_Event().Unbind(wxEVT_PHP_SETTINGS_CHANGED, &PHPLint::OnPhpSettingsChanged, this);
}

void PHPLint::OnLoadFile(gsgs::Event& e)
{
    e.Skip();
...
void PHPLint::OnSaveFile(gsgs::Event& e)
{
    e.Skip();
...
void PHPLint::DoCheckFile(const wxFileName& filename)
{
    ...
    if(!php.Exists()) {
        GetManager()->SetStatusMessage(_("PHPLint: can not lint file. Missing PHP executable path"), 5);
        return;
    }
...
void PHPLint::QueuePhpcsCommand(const wxString& phpPath, const wxString& file)
{
    ...
    if(!phpcs.Exists()) {
        gsgsDEBUG() << "PHPLint: Could not find the PHP-CS application. Ignoring" << clEndl;
        return;
    }
...
void PHPLint::QueuePhpmdCommand(const wxString& phpPath, const wxString& file)
{
    ...
    if(!phpmd.Exists()) {
        gsgsDEBUG() << "PHPLint: Could not find the PHPMD application. Ignoring" << clEndl;
        return;
    }
...
void PHPLint::QueuePhpstanCommand(const wxString& phpPath, const wxString& file)
{
    ...
    if(!phpstan.Exists()) {
        gsgsDEBUG() << "PHPLint: Could not find the Phpstan application. Ignoring" << clEndl;
        return;
    }
...
void PHPLint::DispatchCommand(const wxString& command)
{
    ...
    if(!m_process) {
        ...
        gsgsWARNING() << "PHPLint: Could not run command:" << command << clEndl;
        DoProcessQueue();
    }
}

void PHPLint::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    gsgsDEBUG() << "PHPLint: process terminated. output:" << m_output << clEndl;
    wxDELETE(m_process);
...
void PHPLint::OnProcessOutput(gsgs::ProcessEvent& event) { m_output << event.GetOutput(); }
...
void PHPLint::ProcessPhpError(const wxString& lintOutput)
{
    ...
    if(reLine.Matches(lintOutput)) {
        ...
        gsgsDEBUG() << "PHPLint: searching editor for file:" << filename << clEndl;
...
void PHPLint::MarkError(wxString& errorMessage, const wxString& strLine, IEditor*& editor, bool isWarning)
{
    ...
    if(strLine.ToCLong(&nLine)) {
        gsgsDEBUG() << "PHPLint: adding error marker @%d" << (nLine - 1) << clEndl;
...
void PHPLint::OnPhpSettingsChanged(gsgs::Event& event)
{
    event.Skip();
    m_settingsPhp.Load();
}
