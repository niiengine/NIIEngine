...
void CodeLiteLLDBApp::RunDebugger(const LLDBCommand& command)
{
    ...
    if(m_debugger.IsValid()) {
        ...
        CmdParser parser(command.GetCommandArguments());
...
#endif
        // bool isValid = m_target.LaunchSimple(argv, envp, wd).IsValid();
...
void CodeLiteLLDBApp::AcceptNewConnection()
{
    ...
    try {
        ...
        // exit
        throw gsgs::SocketException("Failed to accept new connection");
    }
}
