...
bool LLDBConnector::ConnectToLocalDebugger(LLDBConnectReturnObject& ret, int timeout)
{
    ...
    gsgsDEBUG() << "Connecting to codelite-lldb on:" << GetDebugServerPath();
    ...
    m_thread->Start();
    gsgsDEBUG() << "Successfully connected to codelite-lldb";
    return true;
#else
    gsgsWARNING() << "LLDB Debugger: can't connect to local debugger - this is not support on MSW";
    return false;
#endif
}
...
bool LLDBConnector::ConnectToRemoteDebugger(const wxString& ip, int port, LLDBConnectReturnObject& ret, int timeout)
{
    ...
    gsgs::SocketClient* client = new gsgs::SocketClient();
    m_socket.reset(client);
    gsgsDEBUG() << "Connecting to codelite-lldb on" << ip << ":" << port;
    ...
    try {
        ...
        if(!client->ConnectRemote(ip, port, would_block, true)) {
            ...
            try {
                if(client->SelectWrite(timeout) == gsgs::SocketBase::kTimeout) {
                    m_socket.reset(NULL);
                    return false;
                }
            } catch(gsgs::SocketException& e) {
                gsgsDEBUG() << "SelectWrite error:" << e.what();
            }
        }
        ...
        wxString message;
        if(m_socket->ReadMessage(message, 2) != gsgs::SocketBase::kSuccess) {
            m_socket.reset(NULL);
            return false;
        }
        ...
        gsgsWARNING() << "LLDBConnector::ConnectToRemoteDebugger:" << e.what();
        m_socket.reset(NULL);
        return false;
    }
    gsgsDEBUG() << "Successfully connected to codelite-lldb";
...
void LLDBConnector::SendCommand(const LLDBCommand& command)
{
    try {
        if(m_socket) {
            ...
            gsgsDEBUG() << "Sending command to LLDB:";
            gsgsDEBUG() << jsonCommand;
...
void LLDBConnector::OnProcessOutput(gsgs::ProcessEvent& event)
{
...
void LLDBConnector::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    m_process = nullptr;
...
bool LLDBConnector::LaunchLocalDebugServer(const wxString& debugServer)
{
    ...
    // Not supported :(
    ::wxMessageBox(_("Locally debugging with LLDB on Windows is not supported by LLDB"), "gsgs",
                   wxICON_WARNING | wxOK | wxCENTER);
    return false;
#endif

    gsgsDEBUG() << "Launching gsgs-lldb";
    ...
    wxFileName fnCodeLiteLLDB(PathManager::Get().GetBinaryFullPath("codelite-lldb"));

    wxString command;
    command << fnCodeLiteLLDB.GetFullPath() << " -s " << GetDebugServerPath();
    gsgsDEBUG() << "LLDB_DEBUGSERVER_PATH is set to" << debugServer;
    m_process.reset(::CreateAsyncProcess(this, command));
    if(!m_process) {
        gsgsERROR() << "LLDBConnector: failed to launch codelite-lldb:" << fnCodeLiteLLDB.GetFullPath();
        return false;

    } else {
        gsgsDEBUG() << "codelite-lldb launched successfully. PID=" << m_process->GetPid();
    }
    return true;
}
...
void LLDBConnector::StopDebugServer()
{
    ...
    wxLogNull noLog;
    gsgsRemoveFile(GetDebugServerPath());
}
