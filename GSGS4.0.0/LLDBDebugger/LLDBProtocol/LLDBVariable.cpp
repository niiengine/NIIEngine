...
void LLDBVariable::FromJSON(const JSONObject& json)
{
    m_name = json.getObject("m_name").toString();
    m_value = json.getObject("m_value").toString();
    m_summary = json.getObject("m_summary").toString();
    m_type = json.getObject("m_type").toString();
    m_expression = json.getObject("m_expression").toString();
    m_valueChanged = json.getObject("m_valueChanged").toBool(false);
    m_lldbId = json.getObject("m_lldbId").toInt();
    m_hasChildren = json.getObject("m_hasChildren").toBool(false);
    m_isWatch = json.getObject("m_isWatch").toBool(m_isWatch);
}

JSONObject LLDBVariable::ToJSON() const
{
