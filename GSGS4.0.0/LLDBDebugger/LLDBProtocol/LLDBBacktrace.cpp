...
void LLDBBacktrace::FromJSON(const JSONObject& json)
{
    m_callstack.clear();
    m_threadId = json.getObject("m_threadId").toInt(0);
    m_selectedFrameId = json.getObject("m_selectedFrameId").toInt(0);
    JSONObject arr = json.getObject("m_callstack");
    for(int i = 0; i < arr.getArraySize(); ++i) {
        LLDBBacktrace::Entry entry;
        entry.FromJSON(arr.getArray(i));
        m_callstack.push_back(entry);
    }
}

JSONObject LLDBBacktrace::ToJSON() const
{
    ...
    JSONObject arr = JSONObject::createArray("m_callstack");
    json.append(arr);
...
void LLDBBacktrace::Entry::FromJSON(const JSONObject& json)
{
    id = json.getObject("id").toInt(0);
    line = json.getObject("line").toInt(0);
    filename = json.getObject("filename").toString();
    functionName = json.getObject("functionName").toString();
    address = json.getObject("address").toString();
}

JSONObject LLDBBacktrace::Entry::ToJSON() const
{
    JSONObject json = JSONObject::createObject();
