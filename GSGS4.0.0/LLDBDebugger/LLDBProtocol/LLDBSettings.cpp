    ...
    , m_proxyPort(13610)
{
    ...
    for(size_t i = 0; i < lldbServerOptions.size(); ++i) {
        if(::FindExec(lldbServerOptions.Item(i), exePath)) {
            break;
        }
    }

#elif defined(__WXOSX__)
    exePath = wxFileName(PathManager::Get().GetBinaryFullPath("debugserver"));
#endif
...
{
    wxFileName fn(PathManager::Get().GetUserDataDir(), "lldb.perspective");
    fn.AppendDir("config");
...
void LLDBSettings::FromJSON(const JSONObject& json)
{
    m_arrItems = json.getObject("m_maxArrayElements").toSize_t(m_arrItems);
    m_stackFrames = json.getObject("m_maxCallstackFrames").toSize_t(m_stackFrames);
    m_flags = json.getObject("m_flags").toSize_t(m_flags);
    m_types = json.getObject("m_types").toString(s_DefaultTypes);
    m_proxyPort = json.getObject("m_proxyPort").toInt(13610);
    m_proxyIp = json.getObject("m_proxyIp").toString("127.0.0.1");
    m_lastLocalFolder = json.getObject("m_lastLocalFolder").toString();
    m_lastRemoteFolder = json.getObject("m_lastRemoteFolder").toString();
    m_debugserver = json.getObject("m_debugserver").toString(m_debugserver);
}

JSONObject LLDBSettings::ToJSON() const
{
...
{
    wxFileName fn(PathManager::Get().GetUserDataDir(), "lldb.conf");
    fn.AppendDir("config");
