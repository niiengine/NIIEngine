...
#ifndef __LLDBBreakpointModel_GUARD__H__
#define __LLDBBreakpointModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a LLDBBreakpointModel_Item class, simply use:
 * @code
 * LLDBBreakpointModel_Item* itemData = reinterpret_cast<LLDBBreakpointModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class LLDBBreakpointModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    LLDBBreakpointModel_Item*           m_parent;
    wxVector<LLDBBreakpointModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    LLDBBreakpointModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~LLDBBreakpointModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<LLDBBreakpointModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(LLDBBreakpointModel_Item* child) {
        wxVector<LLDBBreakpointModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(LLDBBreakpointModel_Item* child) {
        wxVector<LLDBBreakpointModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(LLDBBreakpointModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<LLDBBreakpointModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(LLDBBreakpointModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<LLDBBreakpointModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<LLDBBreakpointModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    LLDBBreakpointModel_Item* GetParent() {
        return m_parent;
    }
...
class LLDBBreakpointModel : public wxDataViewModel
{
protected:
    wxVector<LLDBBreakpointModel_Item*> m_data;
    unsigned int m_colCount;

public:
    LLDBBreakpointModel();
    virtual ~LLDBBreakpointModel();
    ...
    void SetData(const wxVector<LLDBBreakpointModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<LLDBBreakpointModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<LLDBBreakpointModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __LLDBBreakpointModel_GUARD__H__
