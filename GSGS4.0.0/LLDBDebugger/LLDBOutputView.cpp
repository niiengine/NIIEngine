...
{
    m_tbBreakpoints->AddTool(wxID_DELETE, _("Delete Breakpoint"), gsgs_Image().LoadBitmap("minus"));
    m_tbBreakpoints->AddTool(wxID_CLEAR, _("Delete All Breakpoints"),
                             gsgs_Image().LoadBitmap("clean"));
    m_tbBreakpoints->AddTool(wxID_NEW, _("New Breakpoint"), gsgs_Image().LoadBitmap("plus"));
    ...
    gsgs_Event().TopFrame()->Bind(wxEVT_COMMAND_MENU_SELECTED, &LLDBOutputView::OnSelectAll, this,
                                           wxID_SELECTALL);
    gsgs_Event().TopFrame()->Bind(wxEVT_COMMAND_MENU_SELECTED, &LLDBOutputView::OnCopy, this, wxID_COPY);
...
LLDBOutputView::~LLDBOutputView()
{
    ...
    gsgs_Event().TopFrame()->Unbind(wxEVT_COMMAND_MENU_SELECTED, &LLDBOutputView::OnSelectAll, this,
                                             wxID_SELECTALL);
    gsgs_Event().TopFrame()->Unbind(wxEVT_COMMAND_MENU_SELECTED, &LLDBOutputView::OnCopy, this, wxID_COPY);
}
...
{
    event.Skip();
    gsgsDEBUG() << "Setting LLDB breakpoints to:";
    for(size_t i = 0; i < event.GetBreakpoints().size(); ++i) {
        gsgsDEBUG() << event.GetBreakpoints().at(i)->ToString();
    }
...
{
    LLDBNewBreakpointDlg dlg(gsgs_Event().TopFrame());
