...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("LLDB Debugger for gsgs"));
...
{
    m_longName = _("LLDB Debugger for gsgs");
    ...
    // UI events
    gsgs_Event().Connect(wxEVT_DBG_IS_PLUGIN_DEBUGGER, gsgsDebugEventHandler(LLDBPlugin::OnIsDebugger), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_DBG_UI_START, gsgsDebugEventHandler(LLDBPlugin::OnDebugStart), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_CONTINUE, gsgsDebugEventHandler(LLDBPlugin::OnDebugContinue), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_NEXT, gsgsDebugEventHandler(LLDBPlugin::OnDebugNext), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_STEP_IN, gsgsDebugEventHandler(LLDBPlugin::OnDebugStepIn), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_STEP_OUT, gsgsDebugEventHandler(LLDBPlugin::OnDebugStepOut), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_STOP, gsgsDebugEventHandler(LLDBPlugin::OnDebugStop), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_IS_RUNNING, gsgsDebugEventHandler(LLDBPlugin::OnDebugIsRunning), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_CAN_INTERACT, gsgsDebugEventHandler(LLDBPlugin::OnDebugCanInteract), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_DBG_UI_TOGGLE_BREAKPOINT, gsgsDebugEventHandler(LLDBPlugin::OnToggleBreakpoint),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_INTERRUPT, gsgsDebugEventHandler(LLDBPlugin::OnToggleInterrupt), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_BUILD_STARTING, gsgsBuildEventHandler(LLDBPlugin::OnBuildStarting), NULL, this);
    gsgs_Event().Connect(wxEVT_INIT_DONE, wxCommandEventHandler(LLDBPlugin::OnInitDone), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_EXPR_TOOLTIP, gsgsDebugEventHandler(LLDBPlugin::OnDebugTooltip), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_QUICK_DEBUG, gsgsDebugEventHandler(LLDBPlugin::OnDebugQuickDebug), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_DBG_UI_CORE_FILE, gsgsDebugEventHandler(LLDBPlugin::OnDebugCoreFile), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_DELETE_ALL_BREAKPOINTS,
                                  gsgsDebugEventHandler(LLDBPlugin::OnDebugDeleteAllBreakpoints), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_ATTACH_TO_PROCESS,
                                  gsgsDebugEventHandler(LLDBPlugin::OnDebugAttachToProcess), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_ENABLE_ALL_BREAKPOINTS,
                                  gsgsDebugEventHandler(LLDBPlugin::OnDebugEnableAllBreakpoints), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_DISABLE_ALL_BREAKPOINTS,
                                  gsgsDebugEventHandler(LLDBPlugin::OnDebugDisableAllBreakpoints), NULL, this);
    gsgs_Event().Connect(wxEVT_DBG_UI_NEXT_INST, gsgsDebugEventHandler(LLDBPlugin::OnDebugNextInst), NULL, this);
    gsgs_Event().Bind(wxEVT_DBG_UI_STEP_I, &LLDBPlugin::OnDebugVOID, this); // not supported

    gsgs_Event().Connect(wxEVT_DBG_UI_SHOW_CURSOR, gsgsDebugEventHandler(LLDBPlugin::OnDebugShowCursor), NULL,
                                  this);
    gsgs_Event().Bind(wxEVT_TOOLTIP_DESTROY, &LLDBPlugin::OnDestroyTip, this);
    wxTheApp->Bind(wxEVT_MENU, &LLDBPlugin::OnSettings, this, XRCID("lldb_settings"));

    gsgs_Event().Bind(wxEVT_COMMAND_MENU_SELECTED, &LLDBPlugin::OnRunToCursor, this,
                               lldbRunToCursorContextMenuId);
    gsgs_Event().Bind(wxEVT_COMMAND_MENU_SELECTED, &LLDBPlugin::OnJumpToCursor, this,
...
{
    gsgs_Event().Unbind(wxEVT_TOOLTIP_DESTROY, &LLDBPlugin::OnDestroyTip, this);
    ...
    // UI events
    gsgs_Event().Disconnect(wxEVT_DBG_IS_PLUGIN_DEBUGGER, gsgsDebugEventHandler(LLDBPlugin::OnIsDebugger), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_START, gsgsDebugEventHandler(LLDBPlugin::OnDebugStart), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_CONTINUE, gsgsDebugEventHandler(LLDBPlugin::OnDebugContinue), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_NEXT, gsgsDebugEventHandler(LLDBPlugin::OnDebugNext), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_STOP, gsgsDebugEventHandler(LLDBPlugin::OnDebugStop), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_IS_RUNNING, gsgsDebugEventHandler(LLDBPlugin::OnDebugIsRunning), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_CAN_INTERACT, gsgsDebugEventHandler(LLDBPlugin::OnDebugCanInteract), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_STEP_IN, gsgsDebugEventHandler(LLDBPlugin::OnDebugStepIn), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_STEP_OUT, gsgsDebugEventHandler(LLDBPlugin::OnDebugStepOut), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_TOGGLE_BREAKPOINT,
                                     gsgsDebugEventHandler(LLDBPlugin::OnToggleBreakpoint), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_INTERRUPT, gsgsDebugEventHandler(LLDBPlugin::OnToggleInterrupt), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_BUILD_STARTING, gsgsBuildEventHandler(LLDBPlugin::OnBuildStarting), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_INIT_DONE, wxCommandEventHandler(LLDBPlugin::OnInitDone), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_EXPR_TOOLTIP, gsgsDebugEventHandler(LLDBPlugin::OnDebugTooltip), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_QUICK_DEBUG, gsgsDebugEventHandler(LLDBPlugin::OnDebugQuickDebug), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_CORE_FILE, gsgsDebugEventHandler(LLDBPlugin::OnDebugCoreFile), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_DELETE_ALL_BREAKPOINTS,
                                     gsgsDebugEventHandler(LLDBPlugin::OnDebugDeleteAllBreakpoints), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_ATTACH_TO_PROCESS,
                                     gsgsDebugEventHandler(LLDBPlugin::OnDebugAttachToProcess), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_ENABLE_ALL_BREAKPOINTS,
                                     gsgsDebugEventHandler(LLDBPlugin::OnDebugEnableAllBreakpoints), NULL, this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_DISABLE_ALL_BREAKPOINTS,
                                     gsgsDebugEventHandler(LLDBPlugin::OnDebugDisableAllBreakpoints), NULL, this);
    gsgs_Event().Unbind(wxEVT_DBG_UI_STEP_I, &LLDBPlugin::OnDebugVOID, this); // Not supported
    gsgs_Event().Disconnect(wxEVT_DBG_UI_NEXT_INST, gsgsDebugEventHandler(LLDBPlugin::OnDebugNextInst), NULL,
                                     this);
    gsgs_Event().Disconnect(wxEVT_DBG_UI_SHOW_CURSOR, gsgsDebugEventHandler(LLDBPlugin::OnDebugShowCursor), NULL,
                                     this);
    wxTheApp->Unbind(wxEVT_MENU, &LLDBPlugin::OnSettings, this, XRCID("lldb_settings"));

    gsgs_Event().Unbind(wxEVT_COMMAND_MENU_SELECTED, &LLDBPlugin::OnRunToCursor, this,
                                 lldbRunToCursorContextMenuId);
    gsgs_Event().Unbind(wxEVT_COMMAND_MENU_SELECTED, &LLDBPlugin::OnJumpToCursor, this,
                                 lldbJumpToCursorContextMenuId);
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
//-----------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugContinue(gsgs::DebugEvent& event)
{
    event.Skip();
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugStart(gsgs::DebugEvent& event)
{
    ...
    CL_DEBUG("LLDB: Initial working directory is restored to: " + ::wxGetCwd());
    {
        ...
        wxString errMsg;
        ProjectPtr pProject = gsgs_CppWorker().FindProjectByName(event.GetProjectName(), errMsg);
        if(!pProject) {
        ...
        BuildConfigPtr bldConf = gsgs_CppWorker().GetProjBuildConf(pProject->GetName(), wxEmptyString);
        if(!bldConf) {
...

        {
            ...
            if(!bldConf->IsGUIProgram() && !isWindows) {
                m_debuggerTerminal.Launch(gsgs::DebuggerTerminalPOSIX::MakeExeTitle(execToDebug.GetFullPath(), args));
                ...
                if(m_debuggerTerminal.IsValid()) {
                    ...
                    // Failed to launch it...
                    DoCleanup();
                    ::wxMessageBox(_("Failed to start terminal for debugger"), "gsgs",
                                   wxICON_ERROR | wxOK | wxCENTER);
                    return;
                }
            }
            ...
            //////////////////////////////////////////////////////////////////////
            // Initiate the connection to gsgs-lldb
            //////////////////////////////////////////////////////////////////////
            ...
            if(m_connector.Connect(retObj, settings, 5)) {
                ...
                DoCleanup();
                wxString message;
                message << _("Could not connect to gsgs-lldb at '")
                        << (settings.IsUsingRemoteProxy() ? settings.GetTcpConnectString()
                                                          : m_connector.GetConnectString())
                        << "'";
                ::wxMessageBox(message, "gsgs", wxICON_ERROR | wxOK | wxCENTER);
                return;
            }
        }
    }
...
void LLDBPlugin::OnLLDBExited(LLDBEvent& event)
{
    ...
    gsgs::DebugEvent e2(wxEVT_DEBUG_ENDED);
    gsgs_Event().AddPendingEvent(e2);
...
void LLDBPlugin::OnLLDBStarted(LLDBEvent& event)
{
    ...
    {
        gsgs::DebugEvent eventStarted(wxEVT_DEBUG_STARTED);
...
void LLDBPlugin::OnLLDBStopped(LLDBEvent& event)
{
    ...
    if(event.GetInterruptReason() == kInterruptReasonNone) {
        ...
        if(m_raisOnBpHit) { gsgs_Event().TopFrame()->Raise(); }
        ...
        if(!m_stopReasonPrompted && event.ShouldPromptStopReason(message)) {
            ...
            wxString msg;
            msg << "Program stopped\nStop reason: " << message;
            ::wxMessageBox(msg, "gsgs", wxICON_ERROR | wxOK | wxCENTER);
        }
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugNext(gsgs::DebugEvent& event)
{
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugStop(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    CL_DEBUG("LLDB    >> Stop");
    m_connector.Stop();
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugIsRunning(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    event.SetAnswer(m_connector.IsRunning());
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugCanInteract(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    event.SetAnswer(m_connector.IsCanInteract());
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugStepIn(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    m_connector.StepIn();
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugStepOut(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    m_connector.StepOut();
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnIsDebugger(gsgs::DebugEvent& event)
{
    event.Skip();
...
void LLDBPlugin::LoadLLDBPerspective()
{
    ...
    wxAuiPaneInfo& pi = m_mgr->GetAuiManager()->GetPane("Output View");
...
void LLDBPlugin::DestroyUI()
{
    ...
    if(m_callstack) {
        wxAuiPaneInfo& pi = m_mgr->GetAuiManager()->GetPane(LLDB_CALLSTACK_PANE_NAME);
        if(pi.IsOk()) { m_mgr->GetAuiManager()->DetachPane(m_callstack); }
        m_callstack->Destroy();
        m_callstack = NULL;
    }
    if(m_breakpointsView) {
        wxAuiPaneInfo& pi = m_mgr->GetAuiManager()->GetPane(LLDB_BREAKPOINTS_PANE_NAME);
        if(pi.IsOk()) { m_mgr->GetAuiManager()->DetachPane(m_breakpointsView); }
        m_breakpointsView->Destroy();
        m_breakpointsView = NULL;
    }
    if(m_localsView) {
        wxAuiPaneInfo& pi = m_mgr->GetAuiManager()->GetPane(LLDB_LOCALS_PANE_NAME);
        if(pi.IsOk()) { m_mgr->GetAuiManager()->DetachPane(m_localsView); }
        m_localsView->Destroy();
        m_localsView = NULL;
    }

    if(m_threadsView) {
        wxAuiPaneInfo& pi = m_mgr->GetAuiManager()->GetPane(LLDB_THREADS_PANE_NAME);
        if(pi.IsOk()) { m_mgr->GetAuiManager()->DetachPane(m_threadsView); }
...
void LLDBPlugin::InitializeUI()
{
    ...
    if(!m_breakpointsView) {
        ...
        m_mgr->GetAuiManager()->AddPane(m_breakpointsView, wxAuiPaneInfo()
                                                                   .MinSize(200, 200)
                                                                   .Right()
                                                                   .Position(1)
                                                                   .Layer(10)
    ...
    if(!m_callstack) {
        ...
        m_mgr->GetAuiManager()->AddPane(m_callstack, wxAuiPaneInfo()
                                                             .MinSize(200, 200)
                                                             .Right()
                                                             .Position(2) // top one
    ...
    if(!m_localsView) {
        ...
        m_mgr->GetAuiManager()->AddPane(m_localsView,
                                            wxAuiPaneInfo()
                                                .MinSize(200, 200)
                                                .Bottom()
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnToggleBreakpoint(gsgs::DebugEvent& event)
{
...
void LLDBPlugin::OnLLDBCrashed(LLDBEvent& event)
{
    ...
    if(!m_connector.IsGoingDown()) {
        ...
        m_connector.SetGoingDown(true);
        ::wxMessageBox(_("LLDB crashed! Terminating debug session"), "gsgs", wxOK | wxICON_ERROR | wxCENTER);
        OnLLDBExited(event);
    }
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnToggleInterrupt(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    event.Skip();
...
{
    event.Skip();
    LLDBSettingDialog dlg(gsgs_Event().TopFrame());
    if(dlg.ShowModal() == wxID_OK) { dlg.Save(); }
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnInitDone(wxCommandEvent& event) { event.Skip(); }
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugTooltip(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugQuickDebug(gsgs::DebugEvent& event)
{
    if(!DoInitializeDebugger(event, true,
                             gsgs::DebuggerTerminalPOSIX::MakeExeTitle(event.GetExecutableName(), event.GetArguments()))) {
        return;
    }
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugCoreFile(gsgs::DebugEvent& event)
{
    if(event.GetDebuggerName() != LLDB_DEBUGGER_NAME) {
        event.Skip();
        return;
...
#ifdef __WXMSW__
    ::wxMessageBox(_("Debug core file with LLDB is not supported under Windows"), "gsgs",
                   wxOK | wxCENTER | wxICON_WARNING);
    return;
#endif

    if(!DoInitializeDebugger(event, false, gsgs::DebuggerTerminalPOSIX::MakeCoreTitle(event.GetCoreFile()))) { return; }
    ...
    if(m_connector.Connect(retObj, settings, 5)) {
        ...
        DoCleanup();
        wxString message;
        message << _("Could not connect to gsgs-lldb at '") << m_connector.GetConnectString() << "'";
        ::wxMessageBox(message, "gsgs", wxICON_ERROR | wxOK | wxCENTER);
        return;
    }
}
//-----------------------------------------------------------------------------------
bool LLDBPlugin::DoInitializeDebugger(gsgs::DebugEvent& event, bool redirectOutput, const wxString& terminalTitle)
{
    if(event.GetDebuggerName() != LLDB_DEBUGGER_NAME) {
        event.Skip();
        return false;
    ...
    if(m_connector.IsRunning()) {
        ...
        ::wxMessageBox(_("Another debug session is already in progress. Please stop it first"), "gsgs",
                       wxOK | wxCENTER | wxICON_WARNING);
        return false;
    }
    ...
    if(redirectOutput && !isWindows) {
        ...
        if(m_debuggerTerminal.IsValid()) {
            ...
            // Failed to launch it...
            DoCleanup();
            ::wxMessageBox(_("Failed to start terminal for debugger"), "gsgs", wxICON_ERROR | wxOK | wxCENTER);
            return false;
        }
    }
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugAttachToProcess(gsgs::DebugEvent& event)
{
    if(event.GetDebuggerName() != LLDB_DEBUGGER_NAME) {
        event.Skip();
        return;
...
#ifdef __WXMSW__
    ::wxMessageBox(_("Attach to process with LLDB is not supported under Windows"), "gsgs",
                   wxOK | wxCENTER | wxICON_WARNING);
    return;
#endif

    if(!DoInitializeDebugger(event, true, gsgs::DebuggerTerminalPOSIX::MakePidTitle(event.GetInt()))) return;
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugDeleteAllBreakpoints(gsgs::DebugEvent& event)
{
    event.Skip();
    m_connector.DeleteAllBreakpoints();
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugDisableAllBreakpoints(gsgs::DebugEvent& event) 
{ 
    event.Skip(); 
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugEnableAllBreakpoints(gsgs::DebugEvent& event) 
{ 
    event.Skip();
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugVOID(gsgs::DebugEvent& event) 
{ 
    CHECK_IS_LLDB_SESSION(); 
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugNextInst(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
    if(m_connector.IsCanInteract()) { m_connector.NextInstruction(); }
}
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDebugShowCursor(gsgs::DebugEvent& event)
{
    CHECK_IS_LLDB_SESSION();
...
//-----------------------------------------------------------------------------------
void LLDBPlugin::OnDestroyTip(gsgs::Event& e)
{
    e.Skip();
    if(m_tooltip) {
        m_tooltip->Destroy();
