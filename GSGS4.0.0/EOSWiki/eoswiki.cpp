...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("gsgs for EOS"));
...
EOSWiki::~EOSWiki() {}
//-----------------------------------------------------------------------------------
void EOSWiki::CreateToolBar(gsgs::ToolBar* toolbar)
{
...
void EOSWiki::OnNewProject(wxCommandEvent& event)
{
    ...
    if(EOSProjectWizard::GetToolchainPath().IsEmpty()) {
        ::wxMessageBox(_("Could not find EOS CDT installation. Please install it and try again"), "gsgs",
                       wxICON_WARNING | wxOK | wxOK_DEFAULT);
        return;
    }
    EOSProjectWizard dlg(gsgs_Event().TopFrame());
...
void EOSWiki::CreateProject(const EOSProjectData& data)
{
    ...
    if(gsgs_CppWorker().IsOpen()) {
        ProjectPtr proj = gsgs_CppWorker().GetProject(data.GetName());
        if(proj) {
            ::wxMessageBox(wxString() << _("A project with the same name: '") << data.GetName() << "'\nalready exists",
                           "gsgs", wxICON_ERROR | wxOK | wxOK_DEFAULT);
            return;
        }
    }
    ...
    if(!gsgs_CppWorker().IsOpen()) {
        ...
        if(!gsgs_CppWorker().CreateWorkspace(data.GetName(), data.GetPath(), errMsg)) {
    ...
    // Create new project
    if(!gsgs_CppWorker().CreateProject(data.GetName(), data.GetPath(), PROJECT_TYPE_EXECUTABLE, "", true,
                                               errMsg)) {
    ...
    if(action == kLoadWorkspace) {
        ...
        loadEvent.SetEventObject(gsgs_Event().TopFrame());
        loadEvent.SetString(gsgs_CppWorker().GetFileName().GetFullPath());
        gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(loadEvent);
    } else {
        // reload
        wxCommandEvent reloadEvent(wxEVT_MENU, XRCID("reload_workspace"));
        reloadEvent.SetEventObject(gsgs_Event().TopFrame());
        gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(reloadEvent);
    }
}
//-----------------------------------------------------------------------------------
void EOSWiki::CreateSampleFile(ProjectPtr proj, const EOSProjectData& d)
{
    ...
    if(FileUtil::WriteFileContent(sourceFile, fileContent)) {
        proj->AddFile(sourceFile.GetFullPath(), "src");
    } else {
        gsgsWARNING() << "Failed to write file content:" << sourceFile;
        return;
    }
}
//-----------------------------------------------------------------------------------
...
{
    wxFileName fnZip(gsgs_Path().GetDataDir(), "eoswiki.zip");
    if(!fnZip.FileExists()) {
        gsgsWARNING() << "EOS Wiki resource file can't be found:" << fnZip;
        return;
    }

    gsgs::ZipReader zip(fnZip);
...
{
    wxFileName resourceFile(gsgs_Path().GetTempDir(), "");
    ...
    // Load the file
    wxString content;
    if(!FileUtil::ReadFileContent(resourceFile, content)) { return ""; }
    return content;
}
//-----------------------------------------------------------------------------------
void EOSWiki::ReplacePlaceHolders(wxString& text, const gsgsStringMap& m)
{
    std::for_each(m.begin(), m.end(), [&](const gsgsStringMap::value_type& vt) { text.Replace(vt.first, vt.second); });
}
//-----------------------------------------------------------------------------------
void EOSWiki::CreateCMakeListsFile(ProjectPtr proj, const EOSProjectData& d)
{
    ...
    if(FileUtil::WriteFileContent(cmakeListTxt, fileContent)) {
        proj->CreateVirtualDir("resources");
        proj->AddFile(cmakeListTxt.GetFullPath(), "resources");
    } else {
        gsgsWARNING() << "Failed to write file content:" << cmakeListTxt;
    }
}
//-----------------------------------------------------------------------------------