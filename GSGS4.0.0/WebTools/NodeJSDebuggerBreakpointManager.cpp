...
{
    gsgs_Event().Bind(wxEVT_WORKSPACE_LOADED, &NodeJSBptManager::OnWorkspaceOpened, this);
    gsgs_Event().Bind(wxEVT_WORKSPACE_CLOSED, &NodeJSBptManager::OnWorkspaceClosed, this);
    gsgs_Event().Bind(wxEVT_ACTIVE_EDITOR_CHANGED, &NodeJSBptManager::OnEditorChanged, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_STOPPED, &NodeJSBptManager::OnDebuggerStopped, this);
    gsgs_Event().Bind(wxEVT_FILE_SAVED, &NodeJSBptManager::OnFileSaved, this);
}

NodeJSBptManager::~NodeJSBptManager()
{
    gsgs_Event().Unbind(wxEVT_WORKSPACE_LOADED, &NodeJSBptManager::OnWorkspaceOpened, this);
    gsgs_Event().Unbind(wxEVT_WORKSPACE_CLOSED, &NodeJSBptManager::OnWorkspaceClosed, this);
    gsgs_Event().Unbind(wxEVT_ACTIVE_EDITOR_CHANGED, &NodeJSBptManager::OnEditorChanged, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_STOPPED, &NodeJSBptManager::OnDebuggerStopped, this);
    gsgs_Event().Unbind(wxEVT_FILE_SAVED, &NodeJSBptManager::OnFileSaved, this);
}
...
void NodeJSBptManager::OnWorkspaceOpened(wxCommandEvent& event)
{
    ...
    if(FileUtil::GetType(fileName.GetFullPath()) == FileUtil::TypeWorkspaceNodeJS) {
...
void NodeJSBptManager::OnDebuggerStopped(gsgs::DebugEvent& event)
{
    event.Skip();
...
void NodeJSBptManager::OnFileSaved(gsgs::Event& event)
{
    event.Skip();
...
void NodeJSBptManager::DeleteByID(const wxString& bpid)
{
    ...
    const NodeJSBreakpoint& bp = *iter;
    IEditor* editor = GetManager()->FindEditor(bp.GetFilename());
