    ...
    , m_plugin(plugin)
{
    ...
    gsgs_Event().Bind(wxEVT_CCBOX_SELECTION_MADE, &XMLCodeCompletion::OnCodeCompleted, this);
...
{
    gsgs_Event().Unbind(wxEVT_CCBOX_SELECTION_MADE, &XMLCodeCompletion::OnCodeCompleted, this);
...
void XMLCodeCompletion::SuggestClosingTag(IEditor* editor, bool html)
{
    ...
    gsgs::CodeBoxManager::Get().ShowCompletionBox(editor->GetCtrl(), entries, bitmaps, 0, GetWordStartPos(editor),
                                                        this);
}
...
void XMLCodeCompletion::XmlCodeComplete(IEditor* editor)
{
    ...
    if(ch == '/') {
        ...
        gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(event);
    }
}
...
void XMLCodeCompletion::OnCodeCompleted(CodeCompleteEvent& event)
{
    event.Skip();
...
void XMLCodeCompletion::OnCodeComplete(CodeCompleteEvent& event)
{
    event.Skip();
