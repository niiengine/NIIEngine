...
class UnixHelperThread : public gsgs::Thread
{
...
public:
    ...
    void* Entry()
    {
        ...
        while(!TestDestroy()) {
            ...
            if(rc < 0) {
                ...
                if(!buffer.empty()) {
                    if(m_owner) {
                        wxString mod_buffer;
                        StripTerminalColouring(buffer, mod_buffer);
        ...
        if(m_owner) {
            gsgs::ProcessEvent endEvent(wxEVT_ASYNC_PROCESS_TERMINATED);
...
void clNodeProcess::OnProcessOutput(gsgs::ProcessEvent& event)
{
    wxPrintf("%s\n", event.GetOutput());
...
void clNodeProcess::OnProcessTerminated(gsgs::ProcessEvent& event)
{
...
void clNodeProcess::Write(const wxString& command)
{
    ...
    JSONElement element = root.getObject();
    int commandId = element.getObject("id").toInt();
    wxString commandString = element.getObject("command").toString();
