    ...
    : NodeJSCliDebuggerPaneBase(parent)
{
    ...
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_UPDATE_CONSOLE, &NodeDebuggerPane::OnConsoleOutput, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_UPDATE_CALLSTACK, &NodeDebuggerPane::OnUpdateBacktrace, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_STOPPED, &NodeDebuggerPane::OnDebuggerStopped, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_MARK_LINE, &NodeDebuggerPane::OnMarkLine, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_INTERACT, &NodeDebuggerPane::OnInteract, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_UPDATE_BREAKPOINTS_VIEW, &NodeDebuggerPane::OnUpdateBreakpoints,
                               this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_EVAL_RESULT, &NodeDebuggerPane::OnEvalResult, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_CREATE_OBJECT, &NodeDebuggerPane::OnCreateObject, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_STARTED, &NodeDebuggerPane::OnDebugSessionStarted, this);
    gsgs_Event().Bind(wxEVT_TOOLTIP_DESTROY, &NodeDebuggerPane::OnDestroyTip, this);
    gsgs_Event().Bind(wxEVT_NODEJS_DEBUGGER_LOCAL_OBJECT_PROPERTIES, &NodeDebuggerPane::OnLocalProperties,
                               this);
    ...
    m_tbBreakpoints->AddTool(wxID_CLEAR, _("Clear all breakpoints"),
                             gsgs_Image().LoadBitmap("clean"));
    m_tbBreakpoints->AddTool(wxID_REMOVE, _("Delete breakpoint"), gsgs_Image().LoadBitmap("minus"));
...
NodeDebuggerPane::~NodeDebuggerPane()
{
    ...
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_UPDATE_CONSOLE, &NodeDebuggerPane::OnConsoleOutput, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_UPDATE_CALLSTACK, &NodeDebuggerPane::OnUpdateBacktrace, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_STOPPED, &NodeDebuggerPane::OnDebuggerStopped, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_MARK_LINE, &NodeDebuggerPane::OnMarkLine, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_INTERACT, &NodeDebuggerPane::OnInteract, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_UPDATE_BREAKPOINTS_VIEW, &NodeDebuggerPane::OnUpdateBreakpoints,
                                 this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_EVAL_RESULT, &NodeDebuggerPane::OnEvalResult, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_CREATE_OBJECT, &NodeDebuggerPane::OnCreateObject, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_STARTED, &NodeDebuggerPane::OnDebugSessionStarted, this);
    gsgs_Event().Unbind(wxEVT_TOOLTIP_DESTROY, &NodeDebuggerPane::OnDestroyTip, this);
    gsgs_Event().Unbind(wxEVT_NODEJS_DEBUGGER_LOCAL_OBJECT_PROPERTIES, &NodeDebuggerPane::OnLocalProperties,
                                 this);
}
...
void NodeDebuggerPane::OnUpdateBacktrace(clDebugCallFramesEvent& event)
{
    ...
    for(size_t i = 0; i < m_frames.size(); ++i) {
        ...
        if(i == 0) {
            gsgs::DebugEvent event(wxEVT_NODEJS_DEBUGGER_MARK_LINE);
...
void NodeDebuggerPane::OnDebuggerStopped(gsgs::DebugEvent& event)
{
    event.Skip();
...
void NodeDebuggerPane::OnMarkLine(gsgs::DebugEvent& event)
{
    event.Skip();
    NodeJSWorkspace::Get()->GetDebugger()->ClearDebuggerMarker();
...
void NodeDebuggerPane::OnUpdateBreakpoints(gsgs::DebugEvent& event)
{
    event.Skip();
    m_dvListCtrlBreakpoints->DeleteAllItems([](wxUIntPtr data) {
        wxStringClientData* scd = reinterpret_cast<wxStringClientData*>(data);
...
void NodeDebuggerPane::OnInteract(gsgs::DebugEvent& event)
{
    event.Skip();
    if(!event.IsAnswer()) {
        m_dvListCtrlCallstack->DeleteAllItems();
...
void NodeDebuggerPane::OnRunTerminalCommand(gsgs::Event& event)
{
...
void NodeDebuggerPane::OnConsoleOutput(gsgs::DebugEvent& event)
{
    // Dont call event.Skip() here
    m_terminal->AddTextRaw(event.GetString());
}

void NodeDebuggerPane::OnEval(gsgs::Event& event)
{
...
void NodeDebuggerPane::OnDestroyTip(gsgs::Event& event)
{
    event.Skip();
    DoDestroyTip();
}
...
void NodeDebuggerPane::OnDebugSessionStarted(gsgs::DebugEvent& event)
{
    event.Skip();
    DoPrintStartupMessages();
}
...
void NodeDebuggerPane::OnLocalProperties(gsgs::DebugEvent& event)
{
