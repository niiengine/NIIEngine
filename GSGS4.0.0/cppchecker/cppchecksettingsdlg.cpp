...
CppCheckSettingsDialog::CppCheckSettingsDialog(wxWindow* parent, CppCheckSettings* settings, ConfigTool* conf,
    ...
    , m_defaultpath(defaultpath)
{
    ...
    gsgsStringMap::const_iterator iter = settings->GetSuppressedWarningsStrings1()->begin();
...
{
    wxString newitem =
        wxGetTextFromUser("Enter a definition e.g. 'FOO' or 'BAR=1' (not '-DFoo')", "gsgs", "", this);
    if(!newitem.empty()) {
...
{
    wxString newitem =
        wxGetTextFromUser("Enter a definition NOT to check e.g. 'FOO' or 'BAR=1' (not '-UFoo')", "gsgs", "", this);
    if(!newitem.empty()) {
