...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("CppChecker integration for gsgs IDE"));
    ...
    , m_fileProcessed(1)
{
    ...
    m_longName = _("CppCheck integration for gsgs IDE");
    ...
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(CppCheckPlugin::OnWorkspaceClosed),
                                  NULL, this);

    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR, &CppCheckPlugin::OnEditorContextMenu, this);
    ...
    m_tabHelper.reset(new gsgs::TabTogglerHelper(_("CppCheck"), m_view, "", NULL));
    m_tabHelper->SetOutputTabBmp(m_mgr->GetStdIcons()->LoadBitmap("check-all"));
}

CppCheckPlugin::~CppCheckPlugin() {}

void CppCheckPlugin::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void CppCheckPlugin::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR, &CppCheckPlugin::OnEditorContextMenu, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(CppCheckPlugin::OnWorkspaceClosed),
                                     NULL, this);
...
void CppCheckPlugin::OnCheckFileEditorItem(wxCommandEvent& e)
{
    if(m_cppcheckProcess) {
        gsgsLogMessage(_("CppCheckPlugin: CppCheck is currently busy please wait for it to complete the current check"));
        return;
    }
    ...
    if(editor) {
        ...
        if(!projectName.IsEmpty()) { proj = gsgs_CppWorker().GetProject(projectName); }
...
void CppCheckPlugin::OnCheckWorkspaceItem(wxCommandEvent& e)
{
    ...
    if(item.m_itemType == ProjectItem::TypeWorkspace) {
        ...
        for(size_t i = 0; i < tmpfiles.size(); i++) {
            if(FileUtil::GetType(tmpfiles.at(i).GetFullPath()) == FileUtil::TypeSourceC ||
               FileUtil::GetType(tmpfiles.at(i).GetFullPath()) == FileUtil::TypeSourceCpp) {
...
ProjectPtr CppCheckPlugin::FindSelectedProject()
{
    ...
    if(!m_mgr->GetWorkspace() || !m_mgr->IsOpen()) { return proj; }
...
void CppCheckPlugin::OnCppCheckTerminated(gsgs::ProcessEvent& e)
{
    m_filelist.Clear();
    wxDELETE(m_cppcheckProcess);
...
void CppCheckPlugin::GetFileListFromDir(const wxString& root)
{
    ...
    for(size_t i = 0; i < tmparr.GetCount(); i++) {
        switch(FileUtil::GetType(tmparr.Item(i))) {
...
void CppCheckPlugin::DoProcess(ProjectPtr proj)
{
    ...
    gsgs_Debug("CppCheck: Working directory: %s", gsgs_Path().GetBinFolder());
    gsgs_Debug("CppCheck: Command: %s", command);
    m_cppcheckProcess = CreateAsyncProcess(this, command, IProcessCreateDefault, gsgs_Path().GetBinFolder());
#elif defined(__WXOSX__)
    gsgs_Debug("CppCheck: Working directory: %s", gsgs_Path().GetDataDir());
    gsgs_Debug("CppCheck: Command: %s", command);
    m_cppcheckProcess = CreateAsyncProcess(this, command, IProcessCreateDefault, gsgs_Path().GetDataDir());
...
void CppCheckPlugin::SetTabVisible(bool clearContent)
{
    ...
    wxAuiManager* aui = m_mgr->GetAuiManager();
    if(aui) {
...
wxString CppCheckPlugin::DoGetCommand(ProjectPtr proj)
{
    ...
    wxString cmd, path;
    path = gsgs_Path().GetBinaryFullPath("codelite_cppcheck");
    ::WrapWithQuotes(path);
    ...
    cmd << fileList << " ";
    gsgs_Debug("cppcheck command: %s", cmd);
    ::WrapInShell(cmd);
    return cmd;
}
...
wxString CppCheckPlugin::DoGenerateFileList()
{
    ...
    wxFileName fnFileList(gsgs_CppWorker().GetPrivateFolder(), "cppcheck.list");
...
void CppCheckPlugin::OnCppCheckReadData(gsgs::ProcessEvent& e)
{
    e.Skip();
    m_view->AppendLine(e.GetOutput());
}

void CppCheckPlugin::OnEditorContextMenu(ContextMenuEvent& event)
{
    event.Skip();
    IEditor* editor = m_mgr->GetActiveEditor();
    CHECK_PTR_RET(editor);

    if(FileUtil::IsCxxFile(editor->GetFileName())) {
