    ...
    , m_plugin(plugin)
{
    ...
    DoInitStyle();
    gsgs_Event().Connect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(CppCheckReportPage::OnTheme),
                                  NULL, this);
}
...
CppCheckReportPage::~CppCheckReportPage()
{
    ...
    gsgs_Event().Disconnect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(CppCheckReportPage::OnTheme),
                                     NULL, this);
}
...
void CppCheckReportPage::DoInitStyle()
{
    ...
    LexerStyle::Ptr_t config = gsgs_EditorConfig().GetLexer("text");
    if(config) {
        ...
        for(int i = 0; i <= wxSTC_STYLE_DEFAULT; i++) {
            ...
            m_stc->StyleSetBackground(i, gsgs_Style().GetOutputPaneBgColour());
            m_stc->StyleSetForeground(i, gsgs_Style().GetOutputPaneFgColour());
            m_stc->StyleSetFont(i, defFont);
        }
    }
}

void CppCheckReportPage::OnTheme(wxCommandEvent& e)
{
    e.Skip();
    DoInitStyle();
}
