    ...
    : SymbolTree(parent, id, pos, size, style)
{
    ...
    gsgs_Event().Bind(wxEVT_CXX_SYMBOLS_CACHE_UPDATED, &svSymbolTree::OnCacheUpdated, this);
    gsgs_Event().Bind(wxEVT_CXX_SYMBOLS_CACHE_INVALIDATED, &svSymbolTree::OnCacheInvalidated, this);
...
void svSymbolTree::BuildTree(const wxFileName& fn, bool force)
{
    ...
    if(!TagsManagerST::Get()->GetFileCache()->Find(fn, tags)) {
        ...
        gsgsDEBUG() << "Outline: symbols not found, building cache...";
        TagsManagerST::Get()->GetFileCache()->RequestSymbols(fn);
    } else {
        gsgsDEBUG() << "Outline: symbols found in cache, building tree...";
        DoBuildTree(tags, fn);
    }
}

wxTreeItemId svSymbolTree::DoAddIncludeFiles(const wxFileName& fn, const FileOpen::Set_t& includes)
{
...
{
    wxUnusedVar(e);
    FileOpen::Set_t* includes = (FileOpen::Set_t*)e.GetClientData();
    if(includes) { delete includes; }
}

void svSymbolTree::ClearCache() { m_currentTags.clear(); }

void svSymbolTree::OnCacheInvalidated(gsgs::Event& e)
{
    e.Skip();
    gsgsDEBUG() << "Outline: symbols for file" << e.GetFileName() << "were invalidated";
    gsgsDEBUG() << "Outline: Rebuilding cache for file:" << e.GetFileName();
    TagsManagerST::Get()->GetFileCache()->RequestSymbols(e.GetFileName());
}

void svSymbolTree::OnCacheUpdated(gsgs::Event& e)
{
    e.Skip();
    gsgsDEBUG() << "Outline: symbols cache updated for file:" << e.GetFileName();
    if(GetActiveEditorFile() != e.GetFileName()) {
        gsgsDEBUG() << "Outline: active editor does not match the current event filename:" << GetActiveEditorFile()
                  << "vs" << e.GetFileName();
        return;
    }
...
wxString svSymbolTree::GetActiveEditorFile() const
{
    wxString currentEditor =
        GetManager()->GetActiveEditor() ? GetManager()->GetActiveEditor()->GetFileName().GetFullPath() : "";
    return currentEditor;
}

void svSymbolTree::DoBuildTree(TagEntryPtrList& tags, const wxFileName& filename)
{
    ...
    gsgsDEBUG() << "Outline: DoBuildTree is called";
    if(TagsManagerST::Get()->AreTheSame(m_currentTags, tags)) {
        gsgsDEBUG() << "Outline: symbols are the same, DoBuildTree will do nothing";
        return;
    }
