...
wxDEFINE_EVENT(wxEVT_TERMINAL_CTRL_READY, gsgs::Event);
wxDEFINE_EVENT(wxEVT_TERMINAL_CTRL_OUTPUT, gsgs::Event);
wxDEFINE_EVENT(wxEVT_TERMINAL_CTRL_STDERR, gsgs::Event);
wxDEFINE_EVENT(wxEVT_TERMINAL_CTRL_DONE, gsgs::Event);
wxDEFINE_EVENT(wxEVT_TERMINAL_CTRL_SET_TITLE, gsgs::Event);
...
void wxTerminalCtrl::PostCreate()
{
    ...
    if(m_shell) {
        gsgs::Event readyEvent(wxEVT_TERMINAL_CTRL_READY);
        ...
        if(IsPrintTTY()) {
            ...
            FileUtil::WriteFileContent(wxFileName(m_ttyfile), GetPTS());
        }
    }
...
void wxTerminalCtrl::OnProcessOutput(gsgs::ProcessEvent& event)
{
    if(m_style & wxTERMINAL_CTRL_USE_EVENTS) {
        gsgs::Event outputEvent(wxEVT_TERMINAL_CTRL_OUTPUT);
        outputEvent.SetString(event.GetOutput());
        outputEvent.SetEventObject(this);
        GetEventHandler()->AddPendingEvent(outputEvent);
    }
    const char* cfile = event.GetOutput().mb_str(wxConvISO8859_1).data();//??
    //AppendText(event.GetOutput());
    AppendText(cfile);
}

void wxTerminalCtrl::OnProcessStderr(gsgs::ProcessEvent& event)
{
    if(m_style & wxTERMINAL_CTRL_USE_EVENTS) {
        gsgs::Event outputEvent(wxEVT_TERMINAL_CTRL_STDERR);
        outputEvent.SetString(event.GetOutput());
        outputEvent.SetEventObject(this);
        GetEventHandler()->AddPendingEvent(outputEvent);
    }
...
void wxTerminalCtrl::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    if(IsPauseOnExit()) {
...
{
    if(m_style & wxTERMINAL_CTRL_USE_EVENTS) {
        gsgs::Event outputEvent(wxEVT_TERMINAL_CTRL_DONE);
        outputEvent.SetEventObject(this);
        GetEventHandler()->AddPendingEvent(outputEvent);
    }
