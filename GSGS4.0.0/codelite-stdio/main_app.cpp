...
bool MainApp::OnInit()
{
    ...
    // Open the log file
    wxFileName logfile(PathManager::Get().GetUserDataDir(), "codelite-stdio.log");
...
void MainApp::OnProcessOutput(gsgs::ProcessEvent& event) { gsgsDEBUG() << "STDOUT:" << event.GetOutput(); }

void MainApp::OnProcessStderr(gsgs::ProcessEvent& event) { gsgsDEBUG() << "STDERR:" << event.GetOutput(); }

void MainApp::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    gsgsDEBUG() << "Process terminated";
    wxDELETE(m_child);
    wxExit();
}
...
void MainApp::DoStartup()
{
    ...
    FileUtil::ReadFileContent(fn, fileContent);
