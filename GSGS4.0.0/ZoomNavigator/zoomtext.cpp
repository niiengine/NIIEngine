                   ...
                   const wxString& name)
{
    ...
    znConfigItem data;
    gsgs::Config conf("zoom-navigator.conf");
    conf.ReadItem(&data);
    ...
    gsgs_Event().Connect(wxEVT_ZN_SETTINGS_UPDATED, wxCommandEventHandler(ZoomText::OnSettingsChanged), NULL,
                                  this);
    gsgs_Event().Connect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(ZoomText::OnTheme), NULL, this);
...
ZoomText::~ZoomText()
{
    gsgs_Event().Disconnect(wxEVT_ZN_SETTINGS_UPDATED, wxCommandEventHandler(ZoomText::OnSettingsChanged),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_GSGS_THEME_CHANGED, wxCommandEventHandler(ZoomText::OnTheme), NULL,
                                     this);
...
{
    if(!editor) {
        editor = GetManager()->GetActiveEditor();
    }
    if(!editor) {
        DoClear();
        return;
    ...
    LexerStyle::Ptr_t lexer = gsgs_EditorConfig().GetLexerForFile(m_filename);
    if(!lexer) {
        lexer = gsgs_EditorConfig().GetLexer("Text");
    }
...
void ZoomText::OnTheme(wxCommandEvent& e)
{
    e.Skip();
    UpdateLexer(NULL);
}
