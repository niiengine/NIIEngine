    ...
    , m_caretBlockStyle(2)
{
    ...
    m_mgr = manager;
    status_vim = NULL;
    gsgs_Event().Bind(wxEVT_ACTIVE_EDITOR_CHANGED, &VimManager::OnEditorChanged, this);
    gsgs_Event().Bind(wxEVT_EDITOR_CLOSING, &VimManager::OnEditorClosing, this);
    gsgs_Event().Bind(wxEVT_WORKSPACE_CLOSING, &VimManager::OnWorkspaceClosing, this);
    gsgs_Event().Bind(wxEVT_ALL_EDITORS_CLOSING, &VimManager::OnAllEditorsClosing, this);
}

VimManager::~VimManager()
{
    gsgs_Event().Unbind(wxEVT_ACTIVE_EDITOR_CHANGED, &VimManager::OnEditorChanged, this);
    gsgs_Event().Unbind(wxEVT_EDITOR_CLOSING, &VimManager::OnEditorClosing, this);
    gsgs_Event().Unbind(wxEVT_WORKSPACE_CLOSING, &VimManager::OnWorkspaceClosing, this);
    gsgs_Event().Unbind(wxEVT_ALL_EDITORS_CLOSING, &VimManager::OnAllEditorsClosing, this);
}
...
void VimManager::CloseCurrentEditor()
{
    ...
    gsgs_Event().TopFrame()->GetEventHandler()->AddPendingEvent(eventClose);
