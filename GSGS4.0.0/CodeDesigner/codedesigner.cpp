...
extern "C" EXPORT PluginInfo GetPluginInfo()
{
    ...
    info.SetDescription(_("CodeDesigner RAD integration with gsgs"));
...
CodeDesigner::CodeDesigner(IManager* manager)
    ...
    , m_addFileMenu(true)
{
    m_longName = _("CodeDesigner RAD integration with gsgs");
...
//-----------------------------------------------------------------------------------
gsgs::ToolBar* CodeDesigner::CreateToolBar(wxWindow* parent)
{
    // Create the toolbar to be used by the plugin
    gsgs::ToolBar* tb(NULL);
    ...
    /*
        ...
        if (m_mgr->AllowToolbar()) {
            ...
            tb = new gsgs::ToolBar(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, clTB_DEFAULT_STYLE);
...
void CodeDesigner::OnOpenWithCD(wxCommandEvent& e)
{
    ...
    if(item.m_item.IsOk() && item.m_itemType == ProjectItem::TypeFile) {
        if(item.m_fileName.GetExt() == wxT("cdp")) {
            ...
            wxMessageBox(_("Select a 'cdp' (CodeDesigner Project) file only"), _("光栅故事"),
...
void CodeDesigner::DoLaunchCD(const wxString& file)
{
    ...
    if(cdpath.IsEmpty()) {
        wxMessageBox(_("Failed to launch CodeDesigner, no path specified\nPlease set CodeDesigner path from Plugins -> "
                       "CodeDesigner -> Settings..."), _("光栅故事"), wxOK | wxCENTER | wxICON_WARNING);
        return;
    }
...
void CodeDesigner::DoCreateCDProject(CDItemInfo& info)
{
    ...
    if(proj) {
        ...
        if(!ReadFileWithConversion(cdpFile.GetFullPath().c_str(), content)) {
            wxMessageBox(wxString::Format(_("Failed to read file '%s'"), cdpFile.GetFullPath().c_str()), _("光栅故事"),
        ...
        if(!WriteFileWithBackup(cdpFile.GetFullPath().c_str(), content, false)) {
            wxMessageBox(wxString::Format(_("Failed to write file '%s'"), cdpFile.GetFullPath().c_str()), _("光栅故事"),
                         wxOK | wxCENTER | wxICON_WARNING);
            return;
        }
