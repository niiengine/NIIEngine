...
void SvnCommand::OnProcessOutput(gsgs::ProcessEvent& event)
{
    m_output.Append(event.GetOutput());
    gsgsDEBUG1() << "Subversion:" << m_output << clEndl;
}

void SvnCommand::OnProcessTerminated(gsgs::ProcessEvent& event)
{
    if(m_handler) {
        gsgsDEBUG1() << "Subversion:" << m_output << clEndl;;
