    ...
    , m_clientVersion(1700) // 1.7.0 (1*1000 + 7*100 + 0)
{
    ...
    gsgs_Event().Connect(wxEVT_GET_ADDITIONAL_COMPILEFLAGS, gsgsBuildEventHandler(Subversion2::OnGetCompileLine),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CONFIG_CHANGED,
                                  wxCommandEventHandler(Subversion2::OnWorkspaceConfigChanged), NULL, this);
    gsgs_Event().Connect(wxEVT_PROJ_FILE_REMOVED, gsgsEventHandler(Subversion2::OnProjectFileRemoved),
                                  NULL, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FOLDER, &Subversion2::OnFolderContextMenu, this);
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_FILE, &Subversion2::OnFileContextMenu, this);
    gsgs_Event().Bind(wxEVT_FILE_DELETED, &Subversion2::OnFileDeleted, this);
    gsgs_Event().Bind(wxEVT_FOLDER_DELETED, &Subversion2::OnFolderDeleted, this);
    gsgs_Event().Bind(wxEVT_GOTO_ANYTHING_SHOWING, &Subversion2::OnGotoAnythingShowing, this);
    // Register common SVN actins into the "Goto Anything" manager
    // gsgs::GotoEntryManager::Get().Add(gsgs::GotoEntry("Svn > Commit", "", XRCID("svn_commit")));
    // gsgs::GotoEntryManager::Get().Add(gsgs::GotoEntry("Svn > Update", "", XRCID("svn_update")));
}

Subversion2::~Subversion2() {}

void Subversion2::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
{
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FOLDER, &Subversion2::OnFolderContextMenu, this);
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_FILE, &Subversion2::OnFileContextMenu, this);
    gsgs_Event().Unbind(wxEVT_FILE_DELETED, &Subversion2::OnFileDeleted, this);
    gsgs_Event().Unbind(wxEVT_FOLDER_DELETED, &Subversion2::OnFolderDeleted, this);
    gsgs_Event().Unbind(wxEVT_GOTO_ANYTHING_SHOWING, &Subversion2::OnGotoAnythingShowing, this);
    ...
    GetManager()->GetTheApp()->Connect(XRCID("svn_explorer_show_changes"), wxEVT_COMMAND_MENU_SELECTED,
                                       wxCommandEventHandler(Subversion2::OnShowFileChanges), NULL, this);
    gsgs_Event().Disconnect(wxEVT_GET_ADDITIONAL_COMPILEFLAGS,
                                     gsgsBuildEventHandler(Subversion2::OnGetCompileLine), NULL, this);
...
void Subversion2::EnsureVisible()
{
    ...
    wxAuiPaneInfo& pi = GetManager()->GetAuiManager()->GetPane("Output View");
...
void Subversion2::DoInitialize()
{
    ...
    m_tabToggler.reset(new gsgs::TabTogglerHelper(svnCONSOLE_TEXT, m_subversionView, "", NULL));
...
void Subversion2::OnFileExplorerDiff(wxCommandEvent& event)
{
    ...
    diffAgainst = GetUserText(_("Svn Diff"), _("Insert base revision to diff against:"), "BASE", wxNOT_FOUND,
...
{
    wxString configDir(gsgs_Path().GetUserDataDir());
    if(wxFileName::DirExists(configDir) == false) { wxMkdir(configDir); }

    configDir << gsgsPathSep << wxT("subversion");
    return configDir;
}
...
void Subversion2::RecreateLocalSvnConfigFile()
{
    wxString configFile;
    wxString configDir = GetUserConfigDir();
    configFile << configDir << gsgsPathSep << wxT("config");
...
void Subversion2::OnGetCompileLine(gsgs::BuildEvent& event)
{
...
void Subversion2::OnProjectFileRemoved(gsgs::Event& event)
{
    event.Skip();
...
void Subversion2::OnFileExplorerRenameItem(wxCommandEvent& event)
{
    ...
    if(!m_selectedFile.IsOk()) {
        wxString folderName = workingDirectory.GetDirs().Last();
        workingDirectory.RemoveLastDir();
        wxString newname = ::GetUserText(_("Svn Rename"), _("New name:"), folderName, folderName.length());
        ...
        wxString newname = ::GetUserText(_("Svn Rename"), _("New name:"), m_selectedFile.GetFullName(),
...
void Subversion2::DoCommit(const wxArrayString& files, const wxString& workingDirectory, wxCommandEvent& event)
{
    ...
    SvnCommitDialog dlg(gsgs_Event().TopFrame(), files, svnInfo.m_sourceUrl, this, workingDirectory);
    if(dlg.ShowModal() == wxID_OK) {
        wxArrayString actualFiles = dlg.GetPaths();
        if(actualFiles.IsEmpty()) return;

        // Store the commit message into a temporary file
        wxFileName tmpFile(gsgs_Path().GetTempDir(), ".svn-commit");
        if(!FileUtil::WriteFileContent(tmpFile, dlg.GetMesasge())) {
            ::wxMessageBox(_("Fail to write commit message to a temporary file!"), "gsgs",
...
bool Subversion2::NormalizeDir(wxString& wd)
{
    ...
    if(wd.Last() == gsgsPathSep) { wd.RemoveLast(); }
    return true;
}
...
{
    if(!m_mgr->GetWorkspace() || !m_mgr->IsOpen()) { return; }
    ...
    gsgsDEBUG() << "excludeBinary=" << excludeBinary;
    ...
    std::vector<wxString> aryRepoList;
    {
        ...
        for(size_t i1 = 0; i1 < repoListOutput.GetCount(); i1++) {
            if(repoListOutput.Item(i1).Last() != '/') {
                fn.Assign(workDir + gsgsPathSep + repoListOutput.Item(i1));
...
void Subversion2::OnFolderContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void Subversion2::OnFileContextMenu(ContextMenuEvent& event)
{
    event.Skip();
...
void Subversion2::OnFileDeleted(gsgs::FileEvent& event)
{
    event.Skip();
    DoFilesDeleted(event.GetPaths());
}

void Subversion2::OnFolderDeleted(gsgs::FileEvent& event)
{
    event.Skip();
...
void Subversion2::ShowRecentChangesDialog(const SvnShowDiffChunk::List_t& changes)
{
    ...
    SvnShowRecentChangesDlg dlg(gsgs_Event().TopFrame(), changes);
    dlg.ShowModal();
}

void Subversion2::OnGotoAnythingShowing(gsgs::GotoEvent& e)
{
    e.Skip();
    // Add our entries
    e.GetEntries().push_back(gsgs::GotoEntry("Svn > Commit", "", XRCID("gotoanything_svn_commit")));
    e.GetEntries().push_back(gsgs::GotoEntry("Svn > Update", "", XRCID("gotoanything_svn_update")));
}
