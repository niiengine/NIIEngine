    ...
    , m_process(NULL)
{
    ...
    m_stcMessageHelper.Reset(new gsgs::EditEventsHandler(m_stcMessage));
    m_stcDiffHelper.Reset(new gsgs::EditEventsHandler(m_stcDiff));
...
void SvnCommitDialog::OnProcessOutput(gsgs::ProcessEvent& e) { m_output << e.GetOutput(); }

void SvnCommitDialog::OnProcessTerminatd(gsgs::ProcessEvent& e)
{
...
void SvnCommitDialog::DoCreateToolbar()
{
    m_toolbar->AddTool(XRCID("commit-history"), _("Commit History"),
                       gsgs_Image().LoadBitmap("history"), "", wxITEM_DROPDOWN);
    m_toolbar->AddTool(wxID_CLEAR, _("Clear History"), gsgs_Image().LoadBitmap("clear"));
    m_toolbar->Realize();
