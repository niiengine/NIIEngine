...
EVT_MENU(wxID_ADD, SubversionView::OnAdd)
EVT_MENU(wxID_OPEN, SubversionView::OnOpenUnverionsedFiles)
 ...
 * Handle diff "gsgs-echo" command output
...
SubversionView::SubversionView(wxWindow* parent, Subversion2* plugin)
    ...
    , m_gsgsEcho(NULL)
{
    gsgs_Style().RegisterWindow(m_splitter);

    m_dvListCtrl->SetBitmaps(&gsgs_Image().GetBitmaps());
    m_dvListCtrlUnversioned->SetBitmaps(&gsgs_Image().GetBitmaps());

    CreatGUIControls();
    m_themeHelper = new ThemeHandlerHelper(this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(SubversionView::OnWorkspaceLoaded),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(SubversionView::OnWorkspaceClosed),
                                  NULL, this);
    gsgs_Event().Connect(wxEVT_FILE_SAVED, gsgsEventHandler(SubversionView::OnFileSaved), NULL, this);
    gsgs_Event().Connect(wxEVT_PROJ_FILE_ADDED, gsgsEventHandler(SubversionView::OnFileAdded), NULL,
                                  this);
    gsgs_Event().Bind(wxEVT_FILE_RENAMED, &SubversionView::OnFileRenamed, this);
    gsgs_Event().Connect(wxEVT_ACTIVE_EDITOR_CHANGED,
                                  wxCommandEventHandler(SubversionView::OnActiveEditorChanged), NULL, this);

    gsgs_Event().Bind(wxEVT_GSGS_MAINFRAME_GOT_FOCUS, &SubversionView::OnAppActivated, this);

    ::CalcHScrollBar(m_sci);
...
{
    gsgs_Style().UnRegisterWindow(m_splitter);
...
void SubversionView::CreatGUIControls()
{
    ...
    // Add toolbar
    // Create the toolbar
    gsgs::ImageManager* bmpLdr = m_plugin->GetManager()->GetStdIcons();
    gsgs::ToolBar* tb = new gsgs::ToolBar(this);
    // Common svn actions
...
#define SVN_CLEAR_DATAVIEW_CTRL(view)                                             \
    {                                                                             \
        int count = view->GetItemCount();                                         \
        for(int i = 0; i < count; ++i) {                                          \
            SvnTreeData* d = (SvnTreeData*)view->GetItemData(view->RowToItem(i)); \
            wxDELETE(d);                                                          \
        }                                                                         \
        view->DeleteAllItems();                                                   \
    }
...
{
    SVN_CLEAR_DATAVIEW_CTRL(m_dvListCtrl);
    SVN_CLEAR_DATAVIEW_CTRL(m_dvListCtrlUnversioned);
}
                                ...
                                const wxArrayString& ignoreFiles, bool fileExplorerOnly, const wxString& sRootDir)
{
    ...
    if(!fileExplorerOnly) {
        ClearAll();
        DoAddChangedFiles("M", modifiedFiles);
        DoAddChangedFiles("A", newFiles);
        DoAddChangedFiles("D", deletedFiles);
        DoAddChangedFiles("C", conflictedFiles);
        DoAddChangedFiles("L", lockedFiles);
        DoAddUnVersionedFiles(unversionedFiles);
        // DoLinkEditor();
    }
}
...
int SubversionView::GetImageIndex(const wxFileName& filepath) const
{
    ...
    int imageId = loader->GetMimeImageId(FileUtil::GetType(filepath.GetFullName(), FileUtil::TypeText));
    if(wxFileName::DirExists(filepath.GetFullPath())) { imageId = loader->GetMimeImageId(FileUtil::TypeFolder); }
    return imageId;
}
...
void SubversionView::DoAddUnVersionedFiles(const wxArrayString& files)
{
    std::for_each(files.begin(), files.end(), [&](const wxString& filepath) {
        wxFileName fn(DoGetCurRepoPath() + gsgsPathSep + filepath);
...
void SubversionView::OnCommit(wxCommandEvent& event)
{
    ...
    if(paths.IsEmpty()) {
        ::wxMessageBox(_("Nothing to commit!"), "gsgs");
        return;
    }
...
void SubversionView::OnFileAdded(gsgs::Event& event)
{
    event.Skip();
...
void SubversionView::OnFileRenamed(gsgs::FileEvent& event)
{
    ...
    if((event.GetEventObject() != this) && m_plugin->GetSvnClientVersion() &&
       (m_plugin->GetSettings().GetFlags() & SvnRenameFileInRepo)) {
        ...
        gsgs::FileEvent e(wxEVT_FILE_RENAMED);
...
void SubversionView::OnItemActivated(wxDataViewEvent& event)
{
    ...
    if(ssd.GetFlags() & SvnUseExternalDiff) {
        ...
        // C:\src\gsgs\gsgsphp\.svn\pristine\ae\ae25b80b53f432c6124c455ef815679df6ed4ea4.svn-base
        // C:\src\gsgs\gsgsphp\php-plugin\XDebugManager.cpp
        ...
        wxString echo = wxFileName(gsgs_Path().GetBinaryFullPath("gsgs-echo")).GetFullPath();
        command << ::WrapWithQuotes(echo);

        // make sure we kill previous gsgs-echo executable
        wxDELETE(m_gsgsEcho);
        ...
        wxArrayString lines;
        {
            ...
            m_gsgsEcho = ::CreateAsyncProcessCB(this, cmdHandler, command);
        }
    }
}
...
{
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_LOADED, wxCommandEventHandler(SubversionView::OnWorkspaceLoaded),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_WORKSPACE_CLOSED, wxCommandEventHandler(SubversionView::OnWorkspaceClosed),
                                     NULL, this);
    gsgs_Event().Disconnect(wxEVT_FILE_SAVED, gsgsEventHandler(SubversionView::OnFileSaved), NULL, this);
    gsgs_Event().Disconnect(wxEVT_PROJ_FILE_ADDED, gsgsEventHandler(SubversionView::OnFileAdded), NULL,
                                     this);
    gsgs_Event().Unbind(wxEVT_FILE_RENAMED, &SubversionView::OnFileRenamed, this);
    gsgs_Event().Disconnect(wxEVT_ACTIVE_EDITOR_CHANGED,
                                     wxCommandEventHandler(SubversionView::OnActiveEditorChanged), NULL, this);
    gsgs_Event().Unbind(wxEVT_GSGS_MAINFRAME_GOT_FOCUS, &SubversionView::OnAppActivated, this);
...
void SubversionView::OnOpenFile(wxCommandEvent& event)
{
    ...
    for(int i = 0; i < count; i++) {
        ...
        if(data && (data->GetType() == SvnTreeData::SvnNodeTypeFile)) {
            paths.Add(DoGetCurRepoPath() + gsgsPathSep + data->GetFilepath());
        }
    }
...
void SubversionView::OnLock(wxCommandEvent& event)
{
    ...
    if(paths.IsEmpty()) return;
    for(size_t i = 0; i < paths.size(); i++) {
        wxFileName fn(DoGetCurRepoPath() + gsgsPathSep + paths.Item(i));
...
void SubversionView::DoRootDirChanged(const wxString& path)
{
    if(path == wxEmptyString) {
        ...
        if(gsgs_CppWorker().IsOpen()) {
            gsgs_CppWorker().GetLocalWorkspace()->SetCustomData(wxT("SubversionPath"), path);
            gsgs_CppWorker().GetLocalWorkspace()->Flush();
        }
        DoChangeRootPathUI(path);
        BuildTree();
...
void SubversionView::UpdateStatusBar(const wxString& path)
{
    ...
    gsgs::StatusBar* sb = GetManager()->GetStatusBar();
    if(sb) {
        if(m_plugin->IsPathUnderSvn(path)) {
            const wxBitmap & bmp = gsgs_Image().LoadBitmap("subversion");
...
void SubversionView::OnRename(wxCommandEvent& event)
{
    ...
    if(paths.IsEmpty()) return;
    for(size_t i = 0; i < paths.size(); i++) {
        wxFileName oldname(DoGetCurRepoPath() + gsgsPathSep + paths.Item(i));
...
void SubversionView::OnFileSaved(gsgs::Event& event)
{
    event.Skip();
    OnRefreshView(event);
}
...
void SubversionView::FinishDiff(wxString output, wxFileName fileBeingDiffed)
{
    ...
    FileDiffPanel::FileInfo l(leftFile, title_left, true);
    FileDiffPanel::FileInfo r(rightFile, title_right, false);
...
void SubversionView::OnCloseView(wxCommandEvent& event)
{
    if(!m_curpath.IsEmpty()) {
        ...
        GetManager()->GetStatusBar()->SetSourceControlBitmap(wxNullBitmap, "", "");
    }
}
...
void SubversionView::OnUnversionedItemActivated(wxDataViewEvent& event)
{
    ...
    CHECK_PTR_RET(d);
    wxFileName fn(DoGetCurRepoPath() + gsgsPathSep + d->GetFilepath());
