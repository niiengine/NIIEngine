...
{
    gsgs::Config conf("svn-commit.conf");
    conf.Read("messages", m_messages, wxArrayString());
}
...
{
    wxString formattedMessage = _FormatMessage(message);
...
wxString CommitMessagesCache::_FormatMessage(const wxString& message)
{
