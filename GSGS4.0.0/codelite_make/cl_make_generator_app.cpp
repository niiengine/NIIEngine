...
static const wxCmdLineEntryDesc g_cmdDesc[] = {
    ...
    { wxCMD_LINE_OPTION, "w", "workspace", "gsgs workspace file", wxCMD_LINE_VAL_STRING,
    ...
    { wxCMD_LINE_OPTION, "p", "project",
      "Project to build, if non given gsgs will build the active project as defined in the workspace",
    ...
    { wxCMD_LINE_OPTION, "s", "settings",
      ...
      "By default, gsgs-make will load the compiler definitions from\n"
      "%appdata%\\gsgs\\config\\build_settings.xml (or the equivalent path on\n"
...
bool clMakeGeneratorApp::OnInit()
{
    ...
    if(!gsgs_Settings().Load("2.1", m_buildSettingsXml)) {
    ...
    wxString errmsg;
    if(!gsgs_CppWorker().OpenWorkspace(fnWorkspace.GetFullPath(), errmsg)) {
        Error(wxString() << "Error while loading workspace: " << fnWorkspace.GetFullPath() << ". " << errmsg);
        return false;
    }

    if(m_project.IsEmpty()) { m_project = gsgs_CppWorker().GetActiveProjectName(); }

    // Set the active project to the configuration set the by the user
    BuildMatrixPtr buildMatrix = gsgs_CppWorker().GetBuildMatrix();
    ...
    ProjectPtr project = gsgs_CppWorker().FindProjectByName(m_project, errmsg);
    if(!project) {
    ...
    BuildConfigPtr bldConf = gsgs_CppWorker().GetProjBuildConf(m_project, projectConfiguraion);
    if(!bldConf) {
...
void clMakeGeneratorApp::DoGenerateCompileCommands()
{
    ...
    JSON json(gsgs_CppWorker().CreateCompileCommandsJSON());
    // Save the file
    json.save(fn);
}
