    ...
    : Plugin(manager)
{
    ...
    m_modified = false;
    m_clipboard.Empty();
    gsgs_Event().Bind(wxEVT_CONTEXT_MENU_EDITOR, &SnipWiz::OnEditorContextMenu, this);
}

//------------------------------------------------------------
SnipWiz::~SnipWiz()
{
    if(m_modified) m_StringDb.Save(m_pluginPath + defaultTmplFile);
}
//------------------------------------------------------------

void SnipWiz::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
//------------------------------------------------------------
...
void SnipWiz::UnPlug()
{
    ...
    gsgs_Event().Unbind(wxEVT_CONTEXT_MENU_EDITOR, &SnipWiz::OnEditorContextMenu, this);
    DetachDynMenus();
}
...
long SnipWiz::GetCurrentIndentation(IEditor* pEditor, long pos)
{
    ...
    for(long i = 0; i < (long)buffer.Len(); i++)
        if(buffer.GetChar(i) == wxT('\t')) tabCount++;
...
void SnipWiz::OnEditorContextMenu(ContextMenuEvent& event)
{
    event.Skip();
    IEditor* editor = m_mgr->GetActiveEditor();
