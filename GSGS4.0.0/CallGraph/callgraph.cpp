...
/*!
 * brief Class for include plugin to gsgs.
 */
...
void CallGraph::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
void CallGraph::OnShowCallGraph(wxCommandEvent& event)
{
    ...
    ConfigTool* config_tool = m_mgr->GetConfigTool();
    ...
    CxxWS* ws = m_mgr->GetWorkspace();
    if(!ws) 
        return MessageBox(_("Unable to get opened workspace."), wxICON_ERROR);

    wxFileName ws_cfn = ws->GetFileName();
    ...
    wxFileName cfn(projPath + gsgsPathSep + projOutputFn);
    cfn.Normalize();
    ...
    wxFileName gmon_cfn(cfn.GetPath() + gsgsPathSep + GMON_FILENAME_OUT);
    gmon_cfn.Normalize();
    ...
    if(wxFileExists(output_png_fn)) gsgsRemoveFile(output_png_fn);
