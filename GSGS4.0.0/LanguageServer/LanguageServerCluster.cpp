...
{
    gsgs_Event().Bind(wxEVT_WORKSPACE_CLOSED, &LanguageServerCluster::OnWorkspaceClosed, this);
    gsgs_Event().Bind(wxEVT_WORKSPACE_LOADED, &LanguageServerCluster::OnWorkspaceOpen, this);
    gsgs_Event().Bind(wxEVT_COMPILE_COMMANDS_JSON_GENERATED,
...
LanguageServerCluster::~LanguageServerCluster()
{
    gsgs_Event().Unbind(wxEVT_WORKSPACE_CLOSED, &LanguageServerCluster::OnWorkspaceClosed, this);
    gsgs_Event().Unbind(wxEVT_WORKSPACE_LOADED, &LanguageServerCluster::OnWorkspaceOpen, this);
...
void LanguageServerCluster::OnSymbolFound(LSPEvent& event)
{
    ...
    gsgsDEBUG() << "LSP: Opening file:" << fn << "(" << location.GetRange().GetStart().GetLine() << ":"
              << location.GetRange().GetStart().GetCharacter() << ")";

    // Manage the browser (BACK and FORWARD) ourself
    BrowseRecord from;
    IEditor* oldEditor = GetManager()->GetActiveEditor();
    if(oldEditor) { from = oldEditor->CreateBrowseRecord(); }
    IEditor* editor = GetManager()->OpenFile(fn.GetFullPath(), "", wxNOT_FOUND, OF_None);
    if(editor) {
...
void LanguageServerCluster::OnSignatureHelp(LSPEvent& event)
{
    ...
    editor->ShowCalltip(new TipInfo(tags));
}
...
void LanguageServerCluster::OnCompletionReady(LSPEvent& event)
{
    ...
    CodeBoxManager::Get().ShowCompletionBox(GetManager()->GetActiveEditor()->GetCtrl(), items);
}
...
{
    {
        ...
        if(!server) { return; }
        gsgsDEBUG() << "Restarting LSP server:" << name;
        server->Stop();
...
void LanguageServerCluster::StartServer(const LanguageServerEntry& entry)
{
    if(entry.IsEnabled()) {
        if(!entry.IsValid()) {
            gsgsWARNING() << "LSP Server" << entry.GetName()
        ...
        if(!entry.GetArgs().IsEmpty()) {
            ...
            wxString project;
            if(gsgs_CppWorker().IsOpen()) { project = gsgs_CppWorker().GetActiveProjectName(); }
            entryArgs = MacroManager::Instance()->Expand(entryArgs, GetManager(), project);
        ...
        if(gsgs_Workspace().GetWorkspace()) {
            rootDir = gsgs_Workspace().GetWorkspace()->GetFileName().GetPath();
        }
        gsgsDEBUG() << "Starting lsp:";
        gsgsDEBUG() << "Connection string:" << entry.GetConnectionString();
        gsgsDEBUG() << "lspCommand:" << lspCommand;
        gsgsDEBUG() << "entry.GetWorkingDirectory():" << entry.GetWorkingDirectory();
        gsgsDEBUG() << "rootDir:" << rootDir;
        gsgsDEBUG() << "entry.GetLanguages():" << entry.GetLanguages();
...
void LanguageServerCluster::LSPSignatureHelpToTagEntries(TagEntryPtrList& tags, const LSP::SignatureHelp& sighelp)
{
...
void LanguageServerCluster::OnCompileCommandsGenerated(gsgs::Event& event)
{
    event.Skip();
