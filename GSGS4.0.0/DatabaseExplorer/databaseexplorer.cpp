...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("DatabaseExplorer for gsgs"));
    ...
    : Plugin(manager)
{
    ...
    gsgs_Event().Connect(wxEVT_TREE_ITEM_FILE_ACTIVATED,
                                  gsgsEventHandler(DatabaseExplorer::OnOpenWithDBE), NULL, this);
    gsgs_Event().Bind(wxEVT_SHOW_WORKSPACE_TAB, &DatabaseExplorer::OnToggleTab, this);
    ...
    m_longName = _("DatabaseExplorer for gsgs");
    m_shortName = wxT("DatabaseExplorer");

    gsgs_Window().addHotKey("wxEVT_EXECUTE_SQL", "Ctrl-J", _("Execute SQL"));
...
void DatabaseExplorer::CreateToolBar(gsgs::ToolBar* toolbar) { wxUnusedVar(toolbar); }
...
{
    gsgs_Event().Disconnect(wxEVT_TREE_ITEM_FILE_ACTIVATED,
                                     gsgsEventHandler(DatabaseExplorer::OnOpenWithDBE), NULL, this);
    gsgs_Event().Unbind(wxEVT_SHOW_WORKSPACE_TAB, &DatabaseExplorer::OnToggleTab, this);
...
void DatabaseExplorer::OnOpenWithDBE(gsgs::Event& e)
{
    // get the file name
    e.Skip();
    if(FileUtil::IsFileType(e.GetFileName(), FileUtil::TypeDatabase)) {
        e.Skip(false);
...
void DatabaseExplorer::OnToggleTab(gsgs::Event& event)
{
    ...
    if(event.IsSelected()) {
        // show it
        GetManager()->GetWorkspacePaneNotebook()->AddPage(m_dbViewerPanel, _("DbExplorer"), true);
    } else {
