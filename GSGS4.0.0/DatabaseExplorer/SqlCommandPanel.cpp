...
class clStatusBarMessage
{
public:
    clStatusBarMessage(const wxString& start, const wxString& end = wxEmptyString)
        : m_startMessage(start)
        , m_endMessage(end)
    {
        if(m_endMessage.IsEmpty()) {
            m_endMessage = GetManager()->GetStatusBar()->GetText();
        }
        GetManager()->SetStatusMessage(m_startMessage);
    }
    virtual ~clStatusBarMessage() { GetManager()->SetStatusMessage(m_endMessage); }
private:
...
{
    LexerStyle::Ptr_t lexerSQL = gsgs_EditorConfig().GetLexer("SQL");
    if(lexerSQL) {
    ...
    m_editHelper.Reset(new gsgs::EditEventsHandler(m_scintillaSQL));
...
void SQLCommandPanel::OnHistoryToolClicked(wxAuiToolBarEvent& event)
{
    ...
    if(auibar) {
        ...
        gsgs::Config conf(DBE_CONFIG_FILE);
        conf.ReadItem(&settings);
        settings.GetRecentOpen();
