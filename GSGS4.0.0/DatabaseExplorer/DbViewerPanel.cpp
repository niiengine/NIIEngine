    ...
    : _DbViewerPanel(parent)
{
    ...
    // replace the icons...
    gsgs::ImageManager* bmpLoader = pManager->GetStdIcons();
...
void DbViewerPanel::RefreshDbView()
{
    ...
    m_treeDatabases->SetBitmaps(&loader->GetBitmaps());

    int img_folder = loader->GetMimeImageId(FileUtil::TypeFolder);
    int img_table = loader->GetMimeImageId(gsgs::ImageManager::kTable);
    int img_find = loader->GetMimeImageId(gsgs::ImageManager::kFind);
    int img_database = loader->GetMimeImageId(gsgs::ImageManager::kDatabase);
    int img_column = loader->GetMimeImageId(gsgs::ImageManager::kColumn);
    ...
    try {
        if(evt.GetId() == XRCID("IDR_DBVIEWER_COPY_COLUMN")) {
            ...
            if(data) {
                ...
                if(pTab) {
#ifdef __WXMSW__
                    gsgs::WindowUpdateLocker locker(m_mgr->GetEditorPaneNotebook());
#endif
...
{
    LexerStyle::Ptr_t lexer = gsgs_EditorConfig().GetLexer("SQL");
...
void DbViewerPanel::AddEditorPage(wxWindow* page, const wxString& name)
{
    ...
    DbExplorerFrame* frame = new DbExplorerFrame(gsgs_Event().TopFrame(), page, name, this);
    frame->Show();
...
void DbViewerPanel::OpenSQLiteFile(const wxFileName& fileName, bool openDefaultSQLPanel)
{
    try {
        ...
        wxString errorMessage = wxString::Format(_("Error (%d): %s"), e.GetErrorCode(), e.GetErrorMessage().c_str());
        ::wxMessageBox(errorMessage, "gsgs", wxICON_ERROR | wxCENTER | wxOK, gsgs_Event().TopFrame());
    } catch(...) {
        ::wxMessageBox(_("Unknown error."), "gsgs", wxICON_ERROR | wxCENTER | wxOK,
                       gsgs_Event().TopFrame());
    }
}
