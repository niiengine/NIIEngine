...
CL_PLUGIN_API PluginInfo* GetPluginInfo()
{
    ...
    info.SetDescription(_("A plugin that allows user to launch external tools from within gsgs"));
    ...
    , m_parentMenu(NULL)
{
    ...
    m_longName = _("A plugin that allows user to launch external tools from within gsgs");
    ...
    gsgs_Event().Bind(wxEVT_FILE_SAVED, &ExternalToolsPlugin::OnFileSave, this);
...
void ExternalToolsPlugin::CreateToolBar(gsgs::ToolBar* toolbar)
{
    // support both toolbars icon size
    int size = m_mgr->GetToolbarIconSize();
    toolbar->AddSpacer();
    toolbar->AddButton(XRCID("external_tools_settings"), m_mgr->GetStdIcons()->GetMimeImageId("tools"),
                       _("Configure external tools..."));
    toolbar->AddButton(XRCID("external_tools_monitor"), m_mgr->GetStdIcons()->GetMimeImageId("monitor"),
...
{
    gsgs_Event().Unbind(wxEVT_FILE_SAVED, &ExternalToolsPlugin::OnFileSave, this);
...
void ExternalToolsPlugin::DoRecreateToolbar()
{
    ...
    gsgs::ToolBar* toolbar = GetManager()->GetToolBar();
    ...
    for(size_t i = 0; i < tools.size(); i++) {
        ...
        gsgs::DefaultToolBarButton* button = new gsgs::DefaultToolBarButton(toolbar, wxXmlResource::GetXRCID(ti.GetId()), gsgs_Image().GetMimeImageId("cog"), ti.GetName());
...
{
    wxUnusedVar(e);
    ExternalToolsManager dlg(gsgs_Event().TopFrame());
    dlg.ShowModal();
}

void ExternalToolsPlugin::OnFileSave(gsgs::Event& event)
{
