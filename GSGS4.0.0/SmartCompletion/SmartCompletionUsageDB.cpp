...
void SmartCompletionUsageDB::Open()
{
    try {
        ...
        wxFileName fn(gsgs_Path().GetUserDataDir(), "SmartCompletions.db");
        fn.AppendDir("config");
        m_db.Open(fn.GetFullPath());
        CreateScheme();
    } catch(wxSQLite3Exception& e) {
        gsgsWARNING() << "Failed to open SmartCompletions DB:" << e.GetMessage() << gsgs::clEndl;
    }
}
...
void SmartCompletionUsageDB::CreateScheme()
{
    try {
        ...
        gsgsWARNING() << "SmartCompletionUsageDB::CreateScheme():" << e.GetMessage() << gsgs::clEndl;
    }
}
...
void SmartCompletionUsageDB::LoadCCUsageTable(std::unordered_map<wxString, int>& weightTable)
{
    try {
        ...
        gsgsWARNING() << "SQLite 3 error:" << e.GetMessage() << gsgs::clEndl;
    }
}
