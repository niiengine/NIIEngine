...
void wxSplitterWindow::OnMouseEvent(wxMouseEvent& event)
{
    ...
    else if (event.Dragging() && (m_dragMode == wxSPLIT_DRAG_DRAGGING))
    {
        ...
        if ( !isLive )
        {
            ...
            // Draw new one
            //DrawSashTracker(m_oldX, m_oldY);
            DrawSashTracker(tempx, tempy, m_oldX, m_oldY);
        }
        else
        {
...
void wxSplitterWindow::DrawSashTracker(int x, int y, int newx, int newy)
{
    int w, h;
    GetClientSize(&w, &h);
    ...
    if (newx || newy)
    {
        if (m_splitMode == wxSPLIT_VERTICAL)
        {
            x1 = x2 = wxClip(newx, 0, w) + m_sashTrackerPen->GetWidth() / 2;
            y1 = 2;
            y2 = h - 2;
        }
        else
        {
            y1 = y2 = wxClip(newy, 0, h) + m_sashTrackerPen->GetWidth() / 2;
            x1 = 2;
            x2 = w - 2;
        }
        ClientToScreen(&x1, &y1);
...
void wxSplitterWindow::SizeWindows()
{
    ...
    {
        wxWindowUpdateLocker lockwin1(GetWindow1());
    ...
    else if ( GetWindow1() && GetWindow2() )
    {
        ...
        wxWindowUpdateLocker lockwin2(GetWindow2());
        wxWindowUpdateLocker lockwin1(GetWindow1());
                               ...
                               int sashPosition)
{
    ...
    if (!window1->IsShown())
    {
        wxWindowUpdateLocker lockwin1(window1);
        window1->Show();
    }
    if (!window2->IsShown())
    {
        wxWindowUpdateLocker lockwin2(window2);
        window2->Show();
    }
