...
#include "OgreShadowGenCameraPSSM.h"
...
namespace Ogre
{
	...
	TerrainMaterialGeneratorA::TerrainMaterialGeneratorA()
	{
		...
		mProfiles.push_back(N_new SM2Profile(this, "SM2", "Profile for rendering on Shader Model 2 capable cards"));
	...
	MaterialPtr TerrainMaterialGeneratorA::SM2Profile::generate(const Terrain * terrain)
	{
		...
		if(mat.isNull())
		{
			...
			if(mat.isNull())
			{
				mat = matMgr.create(matName, GroupDefault);
			}
		}
		// clear everything
		...
		if(mCompositeMapEnabled)
		{
			...
			ShaderFusion* lowLodTechnique = mat->get(1);
	...
	{
		ShaderFusion * tech = mat->createTechnique();

		// Only supporting one pass
		ShaderCh * pass = tech->create();
		GpuProgramManager& gmgr = GpuProgramManager::getSingleton();
		...
		if (!mShaderGen)
		{
			...
			if (hmgr.isShaderSupport(SL_CG))
				mShaderGen = N_new ShaderHelperCg();
			else if (hmgr.isShaderSupport(SL_HLSL) &&
				((check2x && gmgr.isSyntaxSupported(SL_PS_4_0)) ||
				(check2x && gmgr.isSyntaxSupported(SL_PS_2_X)) ||
				(!check2x && gmgr.isSyntaxSupported(SL_PS_2_0))))
				mShaderGen = N_new ShaderHelperHLSL();
			else if (hmgr.isShaderSupport(SL_GLSL))
				mShaderGen = N_new ShaderHelperGLSL();
			else if (hmgr.isShaderSupport(SL_GLSLES))
				mShaderGen = N_new ShaderHelperGLSLES();
			else
			{
				// todo
			}
        ...
        pass->getProgram().set(ST_VS, 0, vprog->getOriginID());
        pass->getProgram().set(ST_FS, 0, fprog->getOriginID());

		if(tt == HIGH_LOD || tt == RENDER_COMPOSITE_MAP)
		{
			// global normal map
			ShaderChTextureUnit * tu = pass->getTexture().create();
			tu->setSrc(terrain->getTerrainNormalMap()->getName(), Texture::T_2D);
            tu->setUAddressing(TAM_CLAMP);
            tu->setVAddressing(TAM_CLAMP);
            tu->setWAddressing(TAM_CLAMP);

			// global colour map
			if (terrain->getGlobalColourMapEnabled() && isGlobalColourMapEnabled())
			{
				tu = pass->getTexture().create(terrain->getGlobalColourMap()->getName());
                tu->setUAddressing(TAM_CLAMP);
                tu->setVAddressing(TAM_CLAMP);
                tu->setWAddressing(TAM_CLAMP);
			}
			...
			{
				tu = pass->getTexture().create(terrain->getLightmap()->getName());
                tu->setUAddressing(TAM_CLAMP);
                tu->setVAddressing(TAM_CLAMP);
                tu->setWAddressing(TAM_CLAMP);
			}
			...
			{
				tu = pass->getTexture().create(terrain->getBlendTextureName(i));
                tu->setUAddressing(TAM_CLAMP);
                tu->setVAddressing(TAM_CLAMP);
                tu->setWAddressing(TAM_CLAMP);
			}
			...
			{
				// diffuse / specular
				pass->getTexture().create(terrain->getLayerTextureName(i, 0));
				// normal / height
				pass->getTexture().create(terrain->getLayerTextureName(i, 1));
			}
		}
		else
		{
			...
			// composite map
			ShaderChTextureUnit* tu = pass->getTexture().create();
			tu->setSrc(terrain->getCompositeMap()->getName(), Texture::T_2D);
            tu->setUAddressing(TAM_CLAMP);
            tu->setVAddressing(TAM_CLAMP);
            tu->setWAddressing(TAM_CLAMP);
		...
		if (isShadowingEnabled(tt, terrain))
		{
			...
			{
				ShaderChTextureUnit * tu = pass->getTexture().create();
				tu->setContentType(ShaderChTexture::T_SHADOW);
                tu->setUAddressing(TAM_BORDER);
                tu->setVAddressing(TAM_BORDER);
                tu->setWAddressing(TAM_BORDER);
				tu->setBorderColour(Colour::White);
			}
		}
	}
	//---------------------------------------------------------------------
			...
			const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		ret->setProgramCode(sourceStr.str());
		ret->load();
	...
	{
		GpuProgramParam * params = prog->createDefaultParam();

		params->setAllowMissing(true);
		params->set("worldMatrix", GpuProgramParam::GSP_Matrix_Space);
		params->set("viewProjMatrix", GpuProgramParam::GSP_Matrix_Proj_X_View);
		params->set("lodMorph", GpuProgramParam::GSP_Ch_Custom,
			Terrain::LOD_MORPH_CUSTOM_PARAM);
		params->set("fogParams", GpuProgramParam::GSP_Fog_Params);
		...
		if (prof->isShadowingEnabled(tt, terrain))
		{
			...
			for (uint i = 0; i < numTextures; ++i)
			{
				params->set("texViewProjMatrix" + StrConv::conv(i),
					GpuProgramParam::GSP_Texture_Matrix_Proj_X_View_ParamIndex, i);
				if (prof->getReceiveDynamicShadowsDepth())
				{
					params->set("depthRange" + StrConv::conv(i),
						GpuProgramParam::GSP_Shadow_ChDepthRange_ParamIndex, i);
				}
			}
		}
		...
		if (terrain->_getUseVertexCompression() && tt != RENDER_COMPOSITE_MAP)
		{
			...
			params->set("posIndexToObjectSpace", posIndexToObjectSpace);
		}
	}
	//---------------------------------------------------------------------
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, const HighLevelGpuProgramPtr& prog)
	{
		...
		params->setAllowMissing(true);
		params->set("ambient", GpuProgramParam::GSP_Ambient);
		params->set("lightPosObjSpace", GpuProgramParam::GSP_Light_Pos_ChSpace_ParamIndex, 0);
		params->set("lightDiffuseColour", GpuProgramParam::GSP_Light_Diffuse_ParamIndex, 0);
		params->set("lightSpecularColour", GpuProgramParam::ACT_LIGHT_SPECULAR_COLOUR, 0);
		params->set("eyePosObjSpace", GpuProgramParam::GSP_Camera_Pos_ChSpace);
		params->set("fogColour", GpuProgramParam::GSP_Fog_Params); /// 修改
		...
		if(prof->isShadowingEnabled(tt, terrain))
		{
			...
			if (prof->getReceiveDynamicShadowsPSSM())
			{
				PSSMShadowSetup * pssm = prof->getReceiveDynamicShadowsPSSM();
				numTextures = pssm->getSplitCount();
				Vector4 splitPoints;
				const PSSMShadowSetup::SplitPointList& splitPointList = pssm->getSplitPoints();
				// Populate from split point 1, not 0, since split 0 isn't useful (usually 0)
				for (uint i = 1; i < numTextures; ++i)
				{
					splitPoints[i-1] = splitPointList[i];
				}
				params->set("pssmSplitPoints", splitPoints);
			}
			...
			if(prof->getReceiveDynamicShadowsDepth())
			{
				...
				for(uint i = 0; i < numTextures; ++i)
				{
					params->set("inverseShadowmapSize" + StrConv::conv(i),
						GpuProgramParam::GSP_Texture_InverseDim_ParamIndex, i + samplerOffset);
				}
			}
		}
	}
            ...
            bool compositeMap)
	{
		...
		if(compositeMap)
		{
			...
			updateVpParams(prof, terrain, RENDER_COMPOSITE_MAP, param);
            p->getProgram().get(0, ST_FS, NULL, param);
			updateFpParams(prof, terrain, RENDER_COMPOSITE_MAP, param);
		}
		else
		{
			// high lod
            p->getProgram().get(0, ST_VS, NULL, param);
			updateVpParams(prof, terrain, HIGH_LOD, param);
            p->getProgram().get(0, ST_FS, NULL, param);
			updateFpParams(prof, terrain, HIGH_LOD, param);
			...
			if(prof->isCompositeMapEnabled())
			{
				...
				p = mat->get(1)->get(0);
                p->getProgram().get(0, ST_VS, NULL, param);
				updateVpParams(prof, terrain, LOW_LOD, param);
                p->getProgram().get(0, ST_FS, NULL, param);
				updateFpParams(prof, terrain, LOW_LOD, param);
			}
		}
	}
	//---------------------------------------------------------------------
	void TerrainMaterialGeneratorA::SM2Profile::ShaderHelper::updateVpParams(
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, const GpuProgramParam *& params)
	{
		params->setAllowMissing(true);
		uint maxLayers = prof->getMaxLayers(terrain);
		uint numLayers = std::min(maxLayers, static_cast<uint>(terrain->getLayerCount()));
		...
		for (uint i = 0; i < numUVMul; ++i)
		{
			...
			params->set("uvMul_" + StrConv::conv(i), uvMul);
		}

		if (terrain->_getUseVertexCompression() && tt != RENDER_COMPOSITE_MAP)
		{
			NIIf baseUVScale = 1.0f / (terrain->getSize() - 1);
			params->set("baseUVScale", baseUVScale);
		}
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, const GpuProgramParam *& params)
	{
		...
		params->set("scaleBiasSpecular", scaleBiasSpecular);
		...
		const SM2Profile * prof, const Terrain * terrain, TechniqueType tt)
	{
		...
		if(ret.isNull())
		{
			ret = mgr.createProgram(progName, GroupDefault, "cg", GPT_VERTEX_PROGRAM);
		}
		else
		{
			ret->unload();
		...
		ret->set("profiles", "vs_4_0 vs_3_0 vs_2_0 arbvp1");
		ret->set("entry_point", "main_vp");
			...
			const SM2Profile * prof, const Terrain * terrain, TechniqueType tt)
	{
		...
		if (ret.isNull())
		{
			ret = mgr.createProgram(progName, GroupDefault, "cg", GPT_FRAGMENT_PROGRAM);
		}
		else
		{
			ret->unload();
		...
		if(prof->isLayerNormalMappingEnabled() || prof->isLayerParallaxMappingEnabled())
			ret->set("profiles", "ps_4_0 ps_3_0 ps_2_x fp40 arbfp1");
		else
			ret->set("profiles", "ps_4_0 ps_3_0 ps_2_0 fp30 arbfp1");
		ret->set("entry_point", "main_fp");
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, StringUtil::StrStreamType& outStream)
	{
		...
		bool fog = terrain->getSceneManager()->getFog().mMode != FM_NONE && tt != RENDER_COMPOSITE_MAP;
		if(fog)
		{
			outStream <<
	...
	void TerrainMaterialGeneratorA::SM2Profile::ShaderHelperCg::generateFpLayer(
		const SM2Profile * prof, const Terrain * terrain, TechniqueType tt, uint layer,
            StringUtil::StrStreamType & outStream)
	{
		...
		String blendWeightStr = String("blendTexVal") + StrConv::conv(blendIdx) +
			"." + blendChannel;
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, StringUtil::StrStreamType& outStream)
	{
		...
		if(fog)
		{
			if(terrain->getSceneManager()->getFog().mMode == FM_LINEAR)
			{
				outStream <<
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		if (ret.isNull())
		{
			ret = mgr.createProgram(progName, GroupDefault,
		...
		if (prof->_isSM4Available())
			ret->set("target", "vs_4_0");
		else if (prof->_isSM3Available())
			ret->set("target", "vs_3_0");
		else
			ret->set("target", "vs_2_0");
		ret->set("entry_point", "main_vp");
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		if (prof->_isSM4Available())
			ret->set("target", "ps_4_0");
		else if (prof->_isSM3Available())
			ret->set("target", "ps_3_0");
		else
			ret->set("target", "ps_2_x");
		ret->set("entry_point", "main_fp");
			...
			const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		if (ret.isNull())
		{
			ret = mgr.createProgram(progName, GroupDefault, "glsles", GPT_FRAGMENT_PROGRAM);
		}
		else
		{
			ret->unload();
