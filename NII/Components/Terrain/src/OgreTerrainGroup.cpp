...
namespace Ogre
{
	...
	const uint32 TerrainGroup::CHUNK_ID = N_MAGIC4('T','E','R','G');
	const uint16 TerrainGroup::CHUNK_VERSION = 1;
	//---------------------------------------------------------------------
	TerrainGroup::TerrainGroup(SpaceManager * sm, Terrain::Alignment align,
		uint16 terrainSize, NIIf terrainWorldSize) : 
        mSceneManager(sm), 
        mAlignment(align), 
        ...
        mResourceGroup(ResourceSchemeManager::GroupDefault)
	{
	...
	//---------------------------------------------------------------------
	TerrainGroup::TerrainGroup(SpaceManager * sm) : 
        mSceneManager(sm), 
	...
	void TerrainGroup::setOrigin(const Vector3 & pos)
	{
		if(pos != mOrigin)
		{
			...
			for(TerrainSlotMap::iterator i = mTerrainSlots.begin(); i != mTerrainSlots.end(); ++i)
			{
				...
				if(slot->instance)
				{
					slot->instance->setPos(getTerrainSlotPosition(slot->x, slot->y));
				}
			}
		}
	}
	...
	void TerrainGroup::defineTerrain(long x, long y, float constantHeight)
	{
		...
		// Copy all settings, but make sure our primary settings are  
		...
		const Terrain::LayerInstanceList* layers /*= 0*/)
	{
		...
		*slot->def.importData = mDefaultImportData;
		...
		slot->def.importData->inputImage = N_new Image(*img);
		if (layers)
		{
			// copy (held by value)
	...
	void TerrainGroup::saveAllTerrains(bool onlyIfModified, bool replaceFilenames)
	{
		for(TerrainSlotMap::iterator i = mTerrainSlots.begin(); i != mTerrainSlots.end(); ++i)
		{
			...
			if(t)
			{
				if(t->isLoad() && (!onlyIfModified || t->isModified()))
				{
	...
	void TerrainGroup::loadTerrainImpl(TerrainSlot* slot, bool synchronous)
	{
			...
			(!slot->def.filename.empty() || slot->def.importData))
		{
			...
			slot->instance = N_new Terrain(mSceneManager);
	...
	//---------------------------------------------------------------------
	float TerrainGroup::getHeightAtWorldPosition(NIIf x, NIIf y, NIIf z,
        Terrain** ppTerrain /* = 0*/)
	{
	...
	//---------------------------------------------------------------------
	float TerrainGroup::getHeightAtWorldPosition(const Vector3 & pos,
        Terrain** ppTerrain /*= 0*/)
	{
		long x, y;
		convertWorldPositionToTerrainSlot(pos, &x, &y);
		TerrainSlot* slot = getTerrainSlot(x, y);
		if (slot && slot->instance && slot->instance->isLoad())
		{
			if (ppTerrain)
	...
	//---------------------------------------------------------------------
	TerrainGroup::RayResult TerrainGroup::rayIntersects(const Ray& ray, NIIf distanceLimit /* = 0*/) const
	{
		long curr_x, curr_z;
		...
		while(keepSearching)
		{
			if(Math::isEqual(inc.x, 0.0f) && Math::isEqual(inc.z, 0.0f))
				keepSearching = false;
            ...
            while((!slot || !slot->instance) && keepSearching)
			{
				...
				if (offset.x >= 1.0f && offset.z >= 1.0f)
				{
					...
					NIIf diffz = 1.0f - oldoffset.z;
					NIIf diffx = 1.0f - oldoffset.x;
					NIIf distz = diffz / inc.z;
					NIIf distx = diffx / inc.x;
					if (distx < distz)
					{
						curr_x += xdir;
						offset.x -= 1.0f;
	...
	//---------------------------------------------------------------------
	void TerrainGroup::boxIntersects(const AABox & box, TerrainList * resultList) const
	{
		resultList->clear();
		// Much simpler test
        TerrainSlotMap::const_iterator i, iend = mTerrainSlots.end();
		for(i = mTerrainSlots.begin(); i != iend; ++i)
		{
			if(i->second->instance && box.intersects(i->second->instance->getSpaceAABB()))
	...
	void TerrainGroup::sphereIntersects(const Sphere & sphere, TerrainList * resultList) const
	{
		...
		for(i = mTerrainSlots.begin(); i != iend; ++i)
		{
			if(i->second->instance && sphere.intersects(i->second->instance->getSpaceAABB()))
				resultList->push_back(i->second->instance);
		}
	...
	void TerrainGroup::convertWorldPositionToTerrainSlot(const Vector3& pos, long *x, long *y) const
	{
		...
		NIIf offset = mTerrainWorldSize * 0.5f;
		terrainPos.x += offset;
		terrainPos.y += offset;
	...
	WorkQueue::Response* TerrainGroup::handleRequest(const WorkQueue::Request* req, const WorkQueue* srcQ)
	{
		...
		try
		{
			...
			response = N_new WorkQueue::Response(req, true, Any());
		}
		catch (Exception& e)
		{
			// oops
			response = N_new WorkQueue::Response(req, false, Any(),
	...
	{
		LoadRequest lreq = any_cast<LoadRequest>(res->getJob()->getData());
		// only deal with own requests
		if (lreq.origin != this)
			return false;
		else
	...
	void TerrainGroup::handleResponse(const WorkQueue::Response * res, const WorkQueue * srcQ)
	{
		...
		if(res->isComplete())
		{
			...
			if (terrain)
			{
				...
				terrain->setPos(getTerrainSlotPosition(slot->x, slot->y));
	...
	void TerrainGroup::connectNeighbour(TerrainSlot* slot, long offsetx, long offsety)
	{
		...
		if(neighbourSlot && neighbourSlot->instance && neighbourSlot->instance->isLoad())
		{
	...
	TerrainGroup::TerrainSlot* TerrainGroup::getTerrainSlot(long x, long y, bool createIfMissing)
	{
		...
		{
			TerrainSlot* slot = N_new TerrainSlot(x, y);
	...
	{
        TerrainSlotMap::iterator i, iend = mTerrainSlots.end();
		for(i = mTerrainSlots.begin(); i != iend; ++i)
		{
	...
	{
		DataStream * stream = N_Engine().createFile(filename, getResourceGroup(), true);
		FileStreamSerializer ser(stream);
		saveGroupDefinition(ser);
        N_delete stream;
	}
	//---------------------------------------------------------------------
	void TerrainGroup::saveGroupDefinition(FileStreamSerializer& ser)
	{
	...
	{
		DataStream * stream = N_Engine().openFile(filename, getResourceGroup());
		FileStreamSerializer ser(stream);
		loadGroupDefinition(ser);
        N_delete stream;
	}
	//---------------------------------------------------------------------
	void TerrainGroup::loadGroupDefinition(FileStreamSerializer& ser)
	{
		if(!ser.readChunkBegin(CHUNK_ID, CHUNK_VERSION))
			N_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, "Stream does not contain TerrainGroup data");
	...
	//---------------------------------------------------------------------
    void TerrainGroup::setTerrainWorldSize(NIIf newWorldSize)
    {
		if (newWorldSize != mTerrainWorldSize)
		{
			...
			for (TerrainSlotMap::iterator i = mTerrainSlots.begin(); i != mTerrainSlots.end(); ++i)
			{
				if (i->second->instance)
				{
					...
					i->second->instance->setPos(getTerrainSlotPosition(i->second->x, i->second->y));
				}
			}
		}
    }
	...
	{
		N_delete_t(importData, ImportData, MEMCATEGORY_GEOMETRY);
		importData = 0;
	}
	//---------------------------------------------------------------------
