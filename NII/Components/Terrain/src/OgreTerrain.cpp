...
{
	//---------------------------------------------------------------------
	const uint32 Terrain::TERRAIN_CHUNK_ID = N_MAGIC4('T','E','R','R');
	const uint16 Terrain::TERRAIN_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERDECLARATION_CHUNK_ID = N_MAGIC4('T','D','C','L');
	const uint16 Terrain::TERRAINLAYERDECLARATION_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERSAMPLER_CHUNK_ID = N_MAGIC4('T','S','A','M');
	const uint16 Terrain::TERRAINLAYERSAMPLER_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERSAMPLERELEMENT_CHUNK_ID = N_MAGIC4('T','S','E','L');
	const uint16 Terrain::TERRAINLAYERSAMPLERELEMENT_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERINSTANCE_CHUNK_ID = N_MAGIC4('T','L','I','N');
	const uint16 Terrain::TERRAINLAYERINSTANCE_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINDERIVEDDATA_CHUNK_ID = N_MAGIC4('T','D','D','A');
	...
	TerrainGlobalOptions::TerrainGlobalOptions()
		...
		, mCompositeMapAmbient(Colour::White)
		, mCompositeMapDiffuse(Colour::White)
		, mCompositeMapDistance(4000)
		, mResourceGroup(GroupDefault)
		, mUseVertexCompressionWhenAvailable(true)
	{
        Vector3(1, -1, 0).normalise(mLightMapDir);
	}
	//---------------------------------------------------------------------
	...
	TerrainMaterialGeneratorPtr TerrainGlobalOptions::getDefaultMaterialGenerator()
	{
		...
		{
			// default
			mDefaultMaterialGenerator.bind(N_new TerrainMaterialGeneratorA());
		}
	...
	//---------------------------------------------------------------------
	Terrain::Terrain(SpaceManager * sm) : 
        mSceneMgr(sm), 
	...
	{
		mRootNode = sm->getOriginNode()->createChildSpaceNode();
		sm->add(this);
		...
		mMaterialName = "OgreTerrain/" + StrConv::conv(FastHash((const char*)&pTerrain, sizeof(Terrain*)));
	...
	Terrain::~Terrain()
	{
		...
		if(mSceneMgr)
		{
			mSceneMgr->destroySpaceNode(mRootNode->getID());
			mSceneMgr->remove(this);
		}
	}
	//---------------------------------------------------------------------
	const AABox & Terrain::getAABB() const
	{
		if(!mQuadTree)
			return AABox::Null;
		else
	...
	//---------------------------------------------------------------------
	AABox Terrain::getSpaceAABB() const
	{
	...
	//---------------------------------------------------------------------
	NIIf Terrain::getMinHeight() const
	{
		if (!mQuadTree)
			return 0;
		else
	...
	//---------------------------------------------------------------------
	NIIf Terrain::getMaxHeight() const
	{
		if (!mQuadTree)
			return 0;
		else
	...
	//---------------------------------------------------------------------
	NIIf Terrain::getCenterRange() const
	{
		if (!mQuadTree)
			return 0;
		else
			return mQuadTree->getCenterRange();
	}
	//---------------------------------------------------------------------
	void Terrain::save(const String & filename)
	{
		DataStream * stream = N_Engine().createFile(filename, _getDerivedResourceGroup(), true);
		// Compress
		DataStream * compressStream = N_new DeflateStream(filename, stream);
		FileStreamSerializer ser(compressStream);
		save(ser);
        N_delete compressStream;
        N_delete stream;
	}
	//---------------------------------------------------------------------
	void Terrain::save(FileStreamSerializer& stream)
	{
		...
		else
		{
			...
			for (TexturePtrList::iterator i = mBlendTextureList.begin(); i != mBlendTextureList.end(); ++i, ++texIndex)
			{
				...
				PixelBlock dst(mLayerBlendMapSizeActual, mLayerBlendMapSizeActual, 1, cpuFormat, tmpData);
				(*i)->getBuffer()->read(Box(0,0,0,mWidth,mHeight,mDepth), dst);
		...
		else
		{
			...
			PixelBlock dst(mSize, mSize, 1, PF_BYTE_RGB, tmpData);
			mTerrainNormalMap->getBuffer()->read(Box(0,0,0,mWidth,mHeight,mDepth), dst);
		...
		if (mGlobalColourMapEnabled)
		{
			...
			else
			{
				...
				PixelBlock dst(mGlobalColourMapSize, mGlobalColourMapSize, 1, PF_BYTE_RGB, tmpData);
				mColourMap->getBuffer()->read(Box(0,0,0,mWidth,mHeight,mDepth), dst);
		...
		if (mLightMapRequired)
		{
			...
			else
			{
				...
				PixelBlock dst(mLightmapSize, mLightmapSize, 1, PF_L8ui, tmpData);
				mLightmap->getBuffer()->read(Box(0,0,0,mWidth,mHeight,mDepth), dst);
		...
		if (mCompositeMapRequired)
		{
			...
			else
			{
				...
				PixelBlock dst(mCompositeMapSize, mCompositeMapSize, 1, PF_BYTE_RGBA, tmpData);
				mCompositeMap->getBuffer()->read(Box(0,0,0,mWidth,mHeight,mDepth), dst);
	...
	//---------------------------------------------------------------------
	void Terrain::writeLayerDeclaration(const TerrainLayerDeclaration& decl, FileStreamSerializer& stream)
	{
		// Layer declaration
	...
	//---------------------------------------------------------------------
	bool Terrain::readLayerDeclaration(FileStreamSerializer& stream, TerrainLayerDeclaration& targetdecl)
	{
	...
	//---------------------------------------------------------------------
	void Terrain::writeLayerInstanceList(const Terrain::LayerInstanceList& layers, FileStreamSerializer& stream)
	{
		...
		for (LayerInstanceList::const_iterator i = layers.begin(); i != layers.end(); ++i)
		{
			...
			for (StringList::const_iterator t = inst.textureNames.begin();
	...
	//---------------------------------------------------------------------
	bool Terrain::readLayerInstanceList(FileStreamSerializer& stream, size_t numSamplers, Terrain::LayerInstanceList& targetlayers)
	{
		uint8 numLayers;
	...
	{
        bool temp;
		DataStream * stream = N_Engine().openFile(filename, _getDerivedResourceGroup());
		...
		DataStream * uncompressStream = N_new DeflateStream(filename, stream);
		FileStreamSerializer ser(uncompressStream);

		temp = prepare(ser);
        N_delete uncompressStream;
        N_delete stream;
        return temp;
	}
	//---------------------------------------------------------------------
	bool Terrain::prepare(FileStreamSerializer& stream)
	{
		freeTemporaryResources();
		freeCPUResources();
			...
			stream.peekNextChunkID() == TERRAINDERIVEDDATA_CHUNK_ID)
		{
			...
			if (name == "normalmap")
			{
				...
				mCpuTerrainNormalMap = N_new PixelBlock(sz, sz, 1, PF_BYTE_RGB, pData);
		...
		mQuadTree = N_new TerrainQuadTreeNode(this, 0, 0, 0, mSize, mNumLodLevels - 1, 0, 0);
	...
	bool Terrain::prepare(const ImportData & importData)
	{
		...
		if (importData.maxBatchSize > TERRAIN_MAX_BATCH_SIZE)
		{
			N_EXCEPT(Exception::ERR_INVALIDPARAMS,
				"maxBatchSize must be no larger than " +
					StrConv::conv(TERRAIN_MAX_BATCH_SIZE));
		}
		...
		{
			if (Math::isEqual(importData.inputBias, 0.0) && Math::isEqual(importData.inputScale, 1.0))
			{
				// straight copy
		...
		else if (importData.inputImage)
		{
			...
			for(size_t i = 0; i < mSize; ++i)
			{
				...
				unsigned char * pSrc = pSrcBase + srcy * img->getWidth() * img->getUnitSize();
	...
	void Terrain::copyGlobalOptions()
	{
		...
		mRenderQueueGroup = opts.getRenderGroup();
	...
	//---------------------------------------------------------------------
	void Terrain::load(FileStreamSerializer& stream)
	{
		if (prepare(stream))
			load();
		else
	...
	//---------------------------------------------------------------------
	float Terrain::getHeightAtTerrainPosition(NIIf x, NIIf y)
	{
		// get left / bottom points (rounded down)
		NIIf factor = (NIIf)mSize - 1.0f;
		NIIf invFactor = 1.0f / factor;
		...
		NIIf startXTS = startX * invFactor;
		NIIf startYTS = startY * invFactor;
		NIIf endXTS = endX * invFactor;
		NIIf endYTS = endY * invFactor;
		...
		NIIf xParam = (x - startXTS) / invFactor;
		NIIf yParam = (y - startYTS) / invFactor;
	...
	//---------------------------------------------------------------------
	float Terrain::getHeightAtWorldPosition(NIIf x, NIIf y, NIIf z)
	{
		Vector3 terrPos;
			...
			case TERRAIN_SPACE:
				...
				case POINT_SPACE:
					if (translation)
					{
						...
						outVec.x = static_cast<NIIf>(static_cast<int>(outVec.x + 0.5));
						outVec.y = static_cast<NIIf>(static_cast<int>(outVec.y + 0.5));
					}
	...
	//---------------------------------------------------------------------
	void Terrain::getVector(NIIf x, NIIf y, NIIf z, Vector3 * outVec)
	{
	...
	//---------------------------------------------------------------------
	void Terrain::getVectorAlign(NIIf x, NIIf y, NIIf z, Alignment align, Vector3* outVec)
	{
	...
	//---------------------------------------------------------------------
	void Terrain::getPos(const Vector3 & TSpos, Vector3 * outWSpos)
	{
		getPositionAlign(TSpos, mAlign, outWSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPos(NIIf x, NIIf y, NIIf z, Vector3 * outWSpos)
	{
	...
	//---------------------------------------------------------------------
	void Terrain::getTerrainPosition(NIIf x, NIIf y, NIIf z, Vector3 * outTSpos)
	{
		getTerrainPositionAlign(x, y, z, mAlign, outTSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPositionAlign(const Vector3& TSpos, Alignment align,
        Vector3 * outWSpos)
	{
		getPositionAlign(TSpos.x, TSpos.y, TSpos.z, align, outWSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPositionAlign(NIIf x, NIIf y, NIIf z, Alignment align,
        Vector3 * outWSpos)
	{
		switch(align)
		{
	...
	//---------------------------------------------------------------------
	void Terrain::getTerrainPositionAlign(const Vector3& WSpos, Alignment align,
        Vector3 * outTSpos)
	{
		getTerrainPositionAlign(WSpos.x, WSpos.y, WSpos.z, align, outTSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainPositionAlign(NIIf x, NIIf y, NIIf z, Alignment align,
        Vector3 * outTSpos)
	{
		switch(align)
		{
	...
	//---------------------------------------------------------------------
	void Terrain::getTerrainVector(NIIf x, NIIf y, NIIf z, Vector3 * outVec)
	{
		getTerrainVectorAlign(x, y, z, mAlign, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainVectorAlign(const Vector3 & inVec, Alignment align,
        Vector3 * outVec)
	{
		getTerrainVectorAlign(inVec.x, inVec.y, inVec.z, align, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainVectorAlign(NIIf x, NIIf y, NIIf z, Alignment align,
        Vector3 * outVec)
	{
	...
	//---------------------------------------------------------------------
	NIIf Terrain::getWorldSize() const
	{
		return mWorldSize;
	}
	//---------------------------------------------------------------------
	NIIf Terrain::getLayerWorldSize(uint8 index) const
	{
	...
	//---------------------------------------------------------------------
	void Terrain::setLayerWorldSize(uint8 index, NIIf size)
	{
		if (index < mLayers.size())
		{
	...
	//---------------------------------------------------------------------
	NIIf Terrain::getLayerUVMultiplier(uint8 index) const
	{
		if (index < mLayerUVMultiplier.size())
		{
	...
	//---------------------------------------------------------------------
	void Terrain::setPos(const Vector3 & pos)
	{
		if(pos != mPos)
		{
			mPos = pos;
			mRootNode->setPos(pos);
			updateBaseScale();
			mModified = true;
		}
	}
	//---------------------------------------------------------------------
	SpaceNode * Terrain::_getRootSpaceNode() const
	{
		return mRootNode;
	}
	//---------------------------------------------------------------------
	void Terrain::updateBaseScale()
	{
		...
		mScale =  mWorldSize / (NIIf)(mSize-1);
	}
	//---------------------------------------------------------------------
	void Terrain::dirty()
	{
	...
	void Terrain::freeGPUResources()
	{
		...
		if(tmgr)
		{
            TexturePtrList::iterator i, iend = mBlendTextureList.end();
			for(i = mBlendTextureList.begin(); i != iend; ++i)
			{
				tmgr->remove((*i)->getPrc());
			}
		...
		{
			tmgr->remove(mTerrainNormalMap->getPrc());
			mTerrainNormalMap.setNull();
		}

		if(!mColourMap.isNull())
		{
			tmgr->remove(mColourMap->getPrc());
			mColourMap.setNull();
		}

		if(!mLightmap.isNull())
		{
			tmgr->remove(mLightmap->getPrc());
			mLightmap.setNull();
		}

		if(!mCompositeMap.isNull())
		{
			tmgr->remove(mCompositeMap->getPrc());
			mCompositeMap.setNull();
		}

		if(!mMaterial.isNull())
		{
			MaterialManager::getSingleton().remove(mMaterial->getPrc());
			mMaterial.setNull();
		}
		if(!mCompositeMapMaterial.isNull())
		{
			MaterialManager::getSingleton().remove(mCompositeMapMaterial->getPrc());
...
//			int higherstep = step >> 1;
				...
				for (int i = lodRect.left; i < lodRect.right - step; i += step )
				{
					...
					for ( int y = 0; y <= yubound; y++ )
					{
						...
						for ( int x = 0; x <= xubound; x++ )
						{
							...
							NIIf ypct = (NIIf)y / (NIIf)step;
							NIIf xpct = (NIIf)x / (NIIf)step;
							...
							NIIf actual_h = actualPos.z;
							NIIf delta = interp_h - actual_h;
	...
	//---------------------------------------------------------------------
	void Terrain::onFindGeometry(SpaceManager* source, ShadowType irs, Viewport* v)
	{
		// Early-out
		if (!mIsLoaded)
			return;
		...
		unsigned long currMillis = Root::getSingleton().getTimer()->getMS();
		...
		const Camera* lodCamera = v->getCamera()->getExtLod();
		unsigned long frameNum = Root::getSingleton().getCurrentFrame();
		int vpHeight = v->getPixelHeight();
	...
	//---------------------------------------------------------------------
	void Terrain::onDestroy(SpaceManager * source)
	{
		unload();
		unprepare();
		if(source == mSceneMgr)
	...
	void Terrain::calculateCurrentLod(Viewport* vp)
 	{
		if (mQuadTree)
		{
			...
			const Camera* cam = vp->getCamera()->getExtLod();

			// W. de Boer 2000 calculation
			// A = vp_near / abs(vp_top)
			// A = 1 / tan(fovy*0.5)    (== 1 for fovy=45*2)
			NIIf A = 1.0f / Math::Tan(cam->getFOV() * 0.5f);
			// T = 2 * maxPixelError / vertRes
			NIIf maxPixelError = TerrainGlobalOptions::getSingleton().getMaxPixelError() * (1.0f / cam->getLodBias());
			NIIf T = 2.0f * maxPixelError / (NIIf)vp->getPixelHeight();
	...
	std::pair<bool, Vector3> Terrain::rayIntersects(const Ray& ray,
		bool cascadeToNeighbours /* = false */, NIIf distanceLimit /* = 0 */)
	{
		...
		Vector3 rayOrigin = ray.getOrigin() - getPos();
		...
		NIIf maxHeight = getMaxHeight();
		NIIf minHeight = getMinHeight();

		AABox aabb(Vector3(0, minHeight, 0), Vector3(mSize, maxHeight, mSize));
		std::pair<bool, NIIf> aabbTest = localRay.intersects(aabb);
		if (!aabbTest.first)
		{
		...
		NIIf dummyHighValue = (NIIf)mSize * 10000.0f;
		...
		while (cur.y >= (minHeight - 1e-3) && cur.y <= (maxHeight + 1e-3))
		{
			...
			NIIf xDist = Math::isEqual(rayDirection.x, 0.0) ? dummyHighValue :
				(quadX - cur.x + flipX) / rayDirection.x;
			NIIf zDist = Math::isEqual(rayDirection.z, 0.0) ? dummyHighValue :
	...
	std::pair<bool, Vector3> Terrain::checkQuadIntersection(int x, int z, const Ray& ray)
	{
		...
		Vector3 v1 ((NIIf)x, *getHeightData(x,z), (NIIf)z);
		Vector3 v2 ((NIIf)x+1, *getHeightData(x+1,z), (NIIf)z);
		Vector3 v3 ((NIIf)x, *getHeightData(x,z+1), (NIIf)z+1);
		Vector3 v4 ((NIIf)x+1, *getHeightData(x+1,z+1), (NIIf)z+1);
		...
		std::pair<bool, NIIf> planeInt = ray.intersects(p1);
		if(planeInt.first)
		{
	...
	void Terrain::checkLayers(bool includeGPUResources)
	{
		for (LayerInstanceList::iterator it = mLayers.begin(); it != mLayers.end(); ++it)
		{
			...
			// If we're missing sampler entries compared to the declaration, init them
	...
	//---------------------------------------------------------------------
    void Terrain::replaceLayer(uint8 index, bool keepBlends, NIIf worldSize, const StringList* textureNames)
    {
        if (getLayerCount() > 0)
        {
	...
	//---------------------------------------------------------------------
	void Terrain::addLayer(NIIf worldSize, const StringList* textureNames)
	{
		addLayer(getLayerCount(), worldSize, textureNames);
	}
	//---------------------------------------------------------------------
	void Terrain::addLayer(uint8 index, NIIf worldSize, const StringList* textureNames)
	{
		if (!worldSize)
	...
	TerrainLayerBlendMap* Terrain::getLayerBlendMap(uint8 layerIndex)
	{
		...
		if (!mLayerBlendMapList[idx])
		{
			...
			mLayerBlendMapList[idx] = N_new TerrainLayerBlendMap(this, layerIndex, tex->getBuffer().getPointer());
		}
    ...
    {
        FrameBuffer * srcBuffer = getLayerBlendTexture(srcIndex)->getBuffer();
        FrameBuffer * destBuffer = getLayerBlendTexture(destIndex)->getBuffer();

        unsigned char rgbaShift[4];
		Box box(0, 0, destBuffer->getWidth(), destBuffer->getHeight());

        uint8* pDestBase = static_cast<uint8*>(destBuffer->lock(box, Buffer::MM_READ | Buffer::MM_WRITE).data);
        ...
        {
            pSrc = static_cast<uint8*>(srcBuffer->lock(box, Buffer::MM_READ).data);
    ...
    {
        FrameBuffer * buffer = getLayerBlendTexture(index)->getBuffer();

        unsigned char rgbaShift[4];
		Box box(0, 0, buffer->getWidth(), buffer->getHeight());

        uint8* pData = static_cast<uint8*>(buffer->lock(box, Buffer::MM_READ | Buffer::MM_WRITE).data);
	...
	void Terrain::createGPUBlendTextures()
	{
		...
		{
			tmgr->remove(mBlendTextureList.back()->getPrc());
		...
		for(uint8 i = currentTex; i < numTex; ++i)
		{
			...
			// Use Buffer::Buffer::M_DEV because although we will update this, we won't do it every frame
			// in normal circumstances, so we don't want Buffer::M_READ | Buffer::M_WRITE. Also we will
			// read it (if we've cleared local temp areas) so no WRITE_ONLY
			mBlendTextureList[i] = N_Only(Texture).createTexture(
				msBlendTextureGenerator.generate(), _getDerivedResourceGroup(),
                    TEX_TYPE_2D, fmt, mLayerBlendMapSize, mLayerBlendMapSize,
                        1, 0,  Buffer::Buffer::M_DEV);
			...
			{
				// Load blend data
				PixelBlock src(mLayerBlendMapSize, mLayerBlendMapSize, 1, fmt, mCpuBlendMapStorage[i]);
				mBlendTextureList[i]->getBuffer()->write(src, Box(0, 0, 0, mWidth, mHeight, mDepth));
			...
			else
			{
				...
				FrameBuffer * buf = mBlendTextureList[i]->getBuffer();
				uint8* pInit = static_cast<uint8*>(buf->lock(box, Buffer::MM_WHOLE).data);
	...
	void Terrain::createLayerBlendMaps()
	{
		...
		{
			N_delete mLayerBlendMapList.back();
	...
	void Terrain::createOrDestroyGPUNormalMap()
	{
		...
		{
			// create
			mTerrainNormalMap = N_Only(Texture).createTexture(
				mMaterialName + "/nm", _getDerivedResourceGroup(), TEX_TYPE_2D,
                    PF_BYTE_RGB, mSize, mSize, 1, 0, Buffer::Buffer::M_DEV);

			// Upload loaded normal data if present
			if (mCpuTerrainNormalMap)
			{
				mTerrainNormalMap->getBuffer()->write(*mCpuTerrainNormalMap, Box(0, 0, 0, mWidth, mHeight, mDepth));
				OGRE_FREE(mCpuTerrainNormalMap->data, MEMCATEGORY_GENERAL);
				N_delete mCpuTerrainNormalMap;
				mCpuTerrainNormalMap = 0;
			}
		...
		{
			// destroy
			TextureManager::getSingleton().remove(mTerrainNormalMap->getPrc());
			mTerrainNormalMap.setNull();
		}
	...
	void Terrain::_setNormalMapRequired(bool normalMap)
	{
		if (normalMap != mNormalMapRequired)
		{
			...
			if(!mNormalMapRequired && Root::getSingleton().getRender()
				->getFeature()->hasCapability(GF_Tex_NonPow2))
			{
				mNormalMapRequired = false;
	...
	{
		DerivedDataRequest ddreq = any_cast<DerivedDataRequest>(res->getJob()->getData());
		// only deal with own requests
		// we do this because if we delete a terrain we want any pending tasks to be discarded
	...
	WorkQueue::Response* Terrain::handleRequest(const WorkQueue::Request* req, const WorkQueue* srcQ)
	{
		...
		WorkQueue::Response* response = N_new WorkQueue::Response(req, true, Any(ddres));
		return response;
	}
	//---------------------------------------------------------------------
	...
	//---------------------------------------------------------------------
	PixelBlock* Terrain::calculateNormals(const Rect &rect, Rect& finalRect)
	{
		...
		PixelBlock* pixbox = N_new PixelBlock(widenedRect.width(), widenedRect.height(), 1, PF_BYTE_RGB, pData);
		...
		for (long y = widenedRect.top; y < widenedRect.bottom; ++y)
		{
			for (long x = widenedRect.left; x < widenedRect.right; ++x)
			{
				...
				for (int i = 0; i < 8; ++i)
				{
					plane.set(centrePoint, adjacentPoints[i], adjacentPoints[(i+1)%8]);
	...
	//---------------------------------------------------------------------
	void Terrain::finaliseNormals(const Ogre::Rect &rect, Ogre::PixelBlock *normalsBox)
	{
		createOrDestroyGPUNormalMap();
		...
		if (!mTerrainNormalMap.isNull())
		{
			...
			{
				mTerrainNormalMap->getBuffer()->write(*normalsBox, Box(0, 0, 0, mWidth, mHeight, mDepth));
			}
			else
			{
				...
				mTerrainNormalMap->getBuffer()->write(*normalsBox, dstBox);
			}
		}
	...
	void Terrain::widenRectByVector(const Vector3& vec, const Rect& inRect,
		NIIf minHeight, NIIf maxHeight, Rect& outRect)
	{
		...
		{
		case ALIGN_X_Y:
			p.set(Vector3::Z, Vector3(0, 0, vec.z < 0.0 ? minHeight : maxHeight));
			break;
		case ALIGN_X_Z:
			p.set(Vector3::Y, Vector3(0, vec.y < 0.0 ? minHeight : maxHeight, 0));
			break;
		case ALIGN_Y_Z:
			p.set(Vector3::X, Vector3(vec.x < 0.0 ? minHeight : maxHeight, 0, 0));
			break;
		}
		float verticalVal = vec.dotProduct(p.normal);

		if (Math::isEqual(verticalVal, 0.0))
			return;

		Vector3 corners[4];
		NIIf startHeight = verticalVal < 0.0 ? maxHeight : minHeight;
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			std::pair<bool, NIIf> rayHit = ray.intersects(p);
			if(rayHit.first)
			{
	...
	//---------------------------------------------------------------------
	PixelBlock* Terrain::calculateLightmap(const Rect& rect, const Rect& extraTargetRect, Rect& outFinalRect)
	{
		...
		PixelBlock* pixbox = N_new PixelBlock(widenedRect.width(), widenedRect.height(), 1, PF_L8ui, pData);

		NIIf heightPad = (getMaxHeight() - getMinHeight()) * 1.0e-3f;
		...
		for (long y = widenedRect.top; y < widenedRect.bottom; ++y)
		{
			for (long x = widenedRect.left; x < widenedRect.right; ++x)
			{
				...
				getPos(Tx, Ty, getHeightAtTerrainPosition(Tx, Ty) + heightPad, &wpos);
				wpos += getPos();
	...
	//---------------------------------------------------------------------
	void Terrain::finaliseLightmap(const Rect& rect, PixelBlock* lightmapBox)
	{
		createOrDestroyGPULightmap();
		...
		if (!mLightmap.isNull())
		{
			...
			{
				mLightmap->getBuffer()->write(*lightmapBox, Box(0, 0, 0, mWidth, mHeight, mDepth));
			}
			else
			{
				// content of PixelBlock is already inverted in Y, but rect is still
				// in terrain space for dealing with sub-rect, so invert
				Box dstBox;
				dstBox.left = rect.left;
				dstBox.right = rect.right;
				...
				mLightmap->getBuffer()->write(*lightmapBox, dstBox);
			}
		}

		// delete memory
		OGRE_FREE(lightmapBox->data, MEMCATEGORY_GENERAL);
		N_delete(lightmapBox);
	...
	//---------------------------------------------------------------------
	void Terrain::updateCompositeMapWithDelay(NIIf delay)
	{
	...
	{
		if(layerIndex == 0 || layerIndex-1 >= (uint8)mLayerBlendMapList.size())
			N_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid layer index");
	...
	const String & Terrain::getBlendTextureName(uint8 textureIndex) const
	{
		if(textureIndex >= (uint8)mBlendTextureList.size())
			N_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid texture index");
	...
	void Terrain::createOrDestroyGPUColourMap()
	{
		...
		{
			// create
			mColourMap = N_Only(Texture).createTexture(mMaterialName + "/cm", 
                _getDerivedResourceGroup(), TEX_TYPE_2D, PF_BYTE_RGB, 
                    mGlobalColourMapSize, mGlobalColourMapSize, -1,  Buffer::Buffer::M_DEV);

			if(mCpuColourMapStorage)
			{
				// Load cached data
				PixelBlock src(mGlobalColourMapSize, mGlobalColourMapSize, 1, PF_BYTE_RGB, mCpuColourMapStorage);
				mColourMap->getBuffer()->write(src, Box(0, 0, 0, mWidth, mHeight, mDepth));
				// release CPU copy, don't need it anymore
		...
		{
			// destroy
			TextureManager::getSingleton().remove(mColourMap->getPrc());
			mColourMap.setNull();
		}
	}
	//---------------------------------------------------------------------
	void Terrain::createOrDestroyGPULightmap()
	{
		...
		{
			// create
			mLightmap = N_Only(Texture).createTexture(mMaterialName + "/lm", 
                _getDerivedResourceGroup(), TEX_TYPE_2D, PF_L8ui, mLightmapSize, 
                    mLightmapSize, 0, Buffer::Buffer::M_DEV);
			...
			{
				// Load cached data
				PixelBlock src(mLightmapSize, mLightmapSize, 1, PF_L8ui, mCpuLightmapStorage);
				mLightmap->getBuffer()->write(src, Box(0, 0, 0, mWidth, mHeight, mDepth));
				// release CPU copy, don't need it anymore
			...
			else
			{
				...
				FrameBuffer * buf = mLightmap->getBuffer();
				uint8* pInit = static_cast<uint8*>(buf->lock(box, Buffer::MM_WHOLE).data);
		...
		{
			// destroy
			TextureManager::getSingleton().remove(mLightmap->getPrc());
			mLightmap.setNull();
		}
	}
	//---------------------------------------------------------------------
	void Terrain::createOrDestroyGPUCompositeMap()
	{
		...
		{
			// create
			mCompositeMap = N_Only(Texture).createTexture(mMaterialName + "/comp", 
                _getDerivedResourceGroup(), TEX_TYPE_2D, PF_BYTE_RGBA,
                    mCompositeMapSize, mCompositeMapSize, 0,  Buffer::Buffer::M_DEV);
			...
			{
				// Load cached data
				PixelBlock src(mCompositeMapSize, mCompositeMapSize, 1, PF_BYTE_RGBA, mCpuCompositeMapStorage);
				mCompositeMap->getBuffer()->write(src, Box(0, 0, 0, mWidth, mHeight, mDepth));
				// release CPU copy, don't need it anymore
			...
			else
			{
				...
				FrameBuffer * buf = mCompositeMap->getBuffer();
				uint8* pInit = static_cast<uint8*>(buf->lock(box, Buffer::MM_WHOLE).data);
		...
		{
			// destroy
			TextureManager::getSingleton().remove(mCompositeMap->getPrc());
			mCompositeMap.setNull();
		}
	...
	void Terrain::notifyNeighbours()
	{
		...
		if (!mDirtyGeometryRectForNeighbours.isNull())
		{
			...
			for (int i = 0; i < (int)NEIGHBOUR_COUNT; ++i)
			{
				...
				Rect heightEdgeRect = edgeRect.intersects(dirtyRectForNeighbours);
				Rect lightmapEdgeRect = edgeRect.intersects(lightmapRect);
	...
	void Terrain::neighbourModified(NeighbourIndex index, const Rect& edgerect, const Rect& shadowrect)
	{
		...
		if (!edgerect.isNull())
		{
			...
			heightMatchRect = heightMatchRect.intersects(edgerect);
			...
			for (long y = heightMatchRect.top; y < heightMatchRect.bottom; ++y)
			{
				for (long x = heightMatchRect.left; x < heightMatchRect.right; ++x)
				{
					...
					if (!Math::isEqual(neighbourHeight, getHeightAtPoint(x, y), 1e-3f))
					{
	...
	void Terrain::getPointFromSelfOrNeighbour(long x, long y, Vector3 * outpos)
	{
		...
		else
		{
			...
			if(neighbour)
			{
				...
				*outpos = neighbourPos + neighbour->getPos() - getPos();
			}
			else
			{
	...
	//---------------------------------------------------------------------
	Terrain* Terrain::raySelectNeighbour(const Ray & ray, NIIf distanceLimit /* = 0 */)
	{
		...
		if(Math::isEqual(tDir.x, 0.0f, 1e-4) && Math::isEqual(tDir.y, 0.0f, 1e-4))
			return 0;
		...
		NIIf dist = std::numeric_limits<NIIf>::max();
		std::pair<bool, NIIf> intersectResult;
		if(tDir.x < 0.0f)
		{
			intersectResult = terrainRay.intersects(Plane(Vector3::X, Vector3::ZERO));
			if(intersectResult.first && intersectResult.second < dist)
				dist = intersectResult.second;
		}
		else if(tDir.x > 0.0f)
		{
			intersectResult = terrainRay.intersects(Plane(Vector3::INV_X, Vector3(1, 0, 0)));
			if(intersectResult.first && intersectResult.second < dist)
				dist = intersectResult.second;
		}
		if(tDir.y < 0.0f)
		{
			intersectResult = terrainRay.intersects(Plane(Vector3::Y, Vector3::ZERO));
			if (intersectResult.first && intersectResult.second < dist)
				dist = intersectResult.second;
		}
		else if(tDir.y > 0.0f)
		{
			intersectResult = terrainRay.intersects(Plane(Vector3::INV_Y, Vector3(0,1,0)));
			if (intersectResult.first && intersectResult.second < dist)
				dist = intersectResult.second;
		}
		...
		NIIf x = terrainIntersectPos.x;
		NIIf y = terrainIntersectPos.y;
		NIIf dx = tDir.x;
		NIIf dy = tDir.y;

		// Never return diagonal directions, we will navigate those recursively anyway
		if(Math::isEqual(x, 1.0f, 1e-4f) && dx > 0)
			return getNeighbour(NEIGHBOUR_EAST);
		else if(Math::isEqual(x, 0.0f, 1e-4f) && dx < 0)
			return getNeighbour(NEIGHBOUR_WEST);
		else if(Math::isEqual(y, 1.0f, 1e-4f) && dy > 0)
			return getNeighbour(NEIGHBOUR_NORTH);
		else if(Math::isEqual(y, 0.0f, 1e-4f) && dy < 0)
	...
	{
		if(mTerrainNormalMap != 0)
		{
			Image img;
			Texture::convert(img, mTerrainNormalMap);
		...
		{
			Image img;
			Texture::convert(img, mColourMap);
		...
		{
			Image img;
			Texture::convert(img, mCompositeMap);
		...
		for(TexturePtrList::iterator i = mBlendTextureList.begin(); i != mBlendTextureList.end(); ++i, ++blendTexture)
		{
			...
			{
				Image img;
				Texture::convert(img, (*i));
				img.save(prefix + "_blendtexture" + StrConv::conv(blendTexture) + suffix);
			}
		}
	}
	//---------------------------------------------------------------------
	...
	//---------------------------------------------------------------------
	void Terrain::setWorldSize(NIIf newWorldSize)
	{
	...
	void Terrain::setSize(uint16 newSize)
	{
		if(mSize != newSize)
		{
			...
			PixelBlock src(mSize, mSize, 1, Ogre::PF_R32, (void*)getHeightData());

			float* tmpData = OGRE_ALLOC_T(float, numVertices, MEMCATEGORY_GENERAL);

			PixelBlock dst(newSize, newSize, 1, Ogre::PF_R32, tmpData);

			PixelBlock::scale(src, dst, PixelBlock::::F_Bilinear);

            if (!mTerrainNormalMap.isNull())
            {
                TextureManager::getSingletonPtr()->remove(mTerrainNormalMap->getPrc());
                mTerrainNormalMap.setNull();
            }
	...
	void Terrain::DefaultGpuBufferAllocator::allocateVertexBuffers(Terrain* forTerrain,
		size_t numVertices, VertexBufferSPtr& destPos, VertexBufferSPtr& destDelta)
	{
	...
	//---------------------------------------------------------------------
	VertexBufferSPtr Terrain::DefaultGpuBufferAllocator::getVertexBuffer(
		VBufList& list, size_t vertexSize, size_t numVertices)
	{
        ...
        VertexBuffer * re;
		// Didn't find one?
		GBufferManager::getSingleton().create(re, vertexSize,
            numVertices, Buffer::Buffer::M_DEV | Buffer::M_WRITE);
        return VertexBufferSPtr(re);
	}
	//---------------------------------------------------------------------
	void Terrain::DefaultGpuBufferAllocator::freeVertexBuffers(
		const VertexBufferSPtr & posbuf, const VertexBufferSPtr & deltabuf)
	{
		mFreePosBufList.push_back(posbuf);
		mFreeDeltaBufList.push_back(deltabuf);
	}
	//---------------------------------------------------------------------
	IndexBufferSPtr Terrain::DefaultGpuBufferAllocator::getSharedIndexBuffer(uint16 batchSize,
		uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
            uint16 skirtRowColSkip)
	{
		...
		if(i == mSharedIBufMap.end())
		{
			...
			IndexBuffer * ret;
            N_Only(GBuffer).create(ret, 16, indexCount, Buffer::Buffer::M_DEV | Buffer::M_WRITE);
			uint16* pI = static_cast<uint16 *>(ret->lock(Buffer::MM_WHOLE));
    ...
    /// 结合boost::hash_combine相同哈希风格
    template <typename T> Nui32 HashCombine(Nui32 ext, const T & data)
    {
        return FastHash((const NIIb *)&data, sizeof(T), ext);
    }
	//---------------------------------------------------------------------
