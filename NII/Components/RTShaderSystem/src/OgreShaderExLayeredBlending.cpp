		...
		for(size_t i = mTextureBlends.size() - 1; i != (size_t)-1 ; --i)
		{
				...
				(texBlend.sourceModifier != SM_None))
			{
				...
				texBlend.modControlParam = psProgram->resolveAutoParameterInt(
					GpuProgramParam::GSP_Ch_Custom, texBlend.customNum);
...
//-----------------------------------------------------------------------
void LayeredBlending::write(const SubRenderState& rhs)
{
	FFPTexturing::write(rhs);
										 ...
										 int targetChannels)
{
	...
	else 
	{
		...
		{
			FunctionInvocation* curFuncInvocation = N_new FunctionInvocation(funcName, groupOrder, internalCounter++);
    ...
    PropertyScriptNode * prop, ShaderChTextureUnit* texState, SGScriptAnalyze * translator)
{
	...
	{
		String blendType;
		if(false == SGScriptAnalyze::get(prop->values.front(), &blendType))
		{
		...
		unsigned short texIndex = texState->getLocalIndex();
	...
	if (prop->name == "source_modifier")
	{
		...
		{
			compiler->error(prop, ScriptCompiler::CE_INVALIDPARAMETERS, "Expected three or more parameters.");
			return NULL;
		}
		...
		ScriptNodeList::const_iterator itValue = prop->values.begin();
		isParseSuccess = SGScriptAnalyze::get(*itValue, &modifierString); 
		...
		++itValue;
		isParseSuccess = SGScriptAnalyze::get(*itValue, &paramType); 
		...
		++itValue;
		isParseSuccess = SGScriptAnalyze::get(*itValue, &customNum); 
		if(isParseSuccess == false)
		{
			compiler->error(prop, ScriptCompiler::CE_INVALIDPARAMETERS, 
...
//-----------------------------------------------------------------------
void LayeredBlendingFactory::writeInstance(MaterialSerializer * ser, 
    SubRenderState * subRenderState, const ShaderChTextureUnit * srcTextureState,
        const ShaderChTextureUnit * dstTextureState)
{
	unsigned short texIndex = srcTextureState->getLocalIndex();
	...
	if (layeredBlendingSubRenderState->getSourceModifier(texIndex, modType, customNum) == true)
	{
		...
		ser->writeValue(StrConv::conv(customNum));
	}
...
//-----------------------------------------------------------------------
LayeredBlending* LayeredBlendingFactory::createOrRetrieveSubRenderState(SGScriptAnalyze* translator)
{
