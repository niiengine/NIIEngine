...
void GLSLProgramWriter::initializeStringMaps()
{
	...
	mGpuConstTypeMap[GDT_SamplerArray2D] = "sampler2DArray";
	mGpuConstTypeMap[GDT_Sampler3D] = "sampler3D";
	mGpuConstTypeMap[GDT_SamplerCube] = "samplerCube";
	mGpuConstTypeMap[GDT_Matrix2X2] = "mat2";
	mGpuConstTypeMap[GDT_Matrix2X3] = "mat2x3";
	mGpuConstTypeMap[GDT_Matrix2X4] = "mat2x4";
	mGpuConstTypeMap[GDT_Matrix3X2] = "mat3x2";
	mGpuConstTypeMap[GDT_Matrix3X3] = "mat3";
	mGpuConstTypeMap[GDT_Matrix3X4] = "mat3x4";
	mGpuConstTypeMap[GDT_Matrix4X2] = "mat4x2";
	mGpuConstTypeMap[GDT_Matrix4X3] = "mat4x3";
...
void GLSLProgramWriter::writeForwardDeclarations(std::ostream& os, Program* program)
{
	...
	StringList forwardDecl; // holds all generated function declarations 
	const ShaderFunctionList& functionList = program->getFunctions();
	ShaderFunctionConstIterator itFunction;
	...
	StringList::iterator endIt = std::unique(forwardDecl.begin(), forwardDecl.end()); 

	// Finally write all function declarations to the shader file
	for (StringList::iterator it = forwardDecl.begin(); it != endIt; it++)
	{
		os << *it;
	}
}
