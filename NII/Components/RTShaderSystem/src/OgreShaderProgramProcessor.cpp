...
//-----------------------------------------------------------------------------
void ProgramProcessor::bindAutoParameters(Program * pCpuProgram, GpuProgram * pGpuProgram)
{
	...
	for (itParams=progParams.begin(); itParams != progParams.end(); ++itParams)
	{
        ...
        if(pGpuParams->getDefine() != 0)
        {
            const GpuParamUnit * gpuConstDef = pGpuParams->getDefine()->get(pCurParam->getName());
            ...
            if(gpuConstDef != NULL)
            {
                ...
                if(pCurParam->isAutoConstantParameter())
                {
                    ...
                    {
                        pGpuParams->set(pCurParam->getName(),
                ...
                // Case this is not auto constant - we have to update its typemark ourself.
                else
                {
                    gpuConstDef->mGBTMark |= pCurParam->getVariability();

                    // Update typemark in the float map.
                    if((gpuConstDef->isSampler()) == false)
                    {
                        GpuParamMap * floatLogical = pGpuParams->getValue()->mFloatList;
                        if(floatLogical.get())
                        {
                            for(GpuParamBlockMap::const_iterator i = floatLogical->map.begin(); i != floatLogical->map.end(); ++i)
                            {
                                if(i->second.mMemIndex == gpuConstDef->mMemIndex)
                                {
                                    i->second.mGBTMark |= gpuConstDef->mGBTMark;
                                    break;
                                }
                            }
                        }
...
bool ProgramProcessor::compactVsOutputs(Function* vsMain, Function* fsMain)
{
	...
	// Build output parameter tables - each row represents different parameter type (GDT_Float-4).
...
void ProgramProcessor::generateLocalSplitParameters(Function* func, GpuProgramType progType,
    MergeParameterList& mergedParams, ShaderParameterList& splitParams, LocalParameterMap& localParamsMap)
{
	...
	for (unsigned int i=0; i < mergedParams.size(); ++i)
	{
		...
		for (unsigned int p=0; p < curMergeParameter.getSourceParameterCount(); ++p)
		{
			...
			if (itFind != localParamsMap.end())
			{
				...
				{
					FunctionInvocation* curFuncInvocation = N_new FunctionInvocation(FFP_FUNC_ASSIGN, FFP_VS_POST_PROCESS, invocationCounter++);
				...
				{
					FunctionInvocation* curFuncInvocation = N_new FunctionInvocation(FFP_FUNC_ASSIGN, FFP_PS_PRE_PROCESS, invocationCounter++);
...
//-----------------------------------------------------------------------------
int	ProgramProcessor::getParameterFloatCount(GpuDataType type)
{
	int floatCount = 0;
...
//-----------------------------------------------------------------------------
int ProgramProcessor::getParameterMaskByType(GpuDataType type)
{
	int paramMask = 0;
...
{
	GpuDataType dstParamType = GDT_Unknow;

	switch (getUsedFloatCount())
	{
	case 1:
		dstParamType = GDT_Float;
		break;

	case 2:
		dstParamType = GDT_Float2X;
		break;

	case 3:
		dstParamType = GDT_Float3X;
		break;

	case 4:
		dstParamType = GDT_Float4X;
		break;
