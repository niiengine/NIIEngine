...
namespace Ogre {
    namespace RTShader {
        ...
        FunctionInvocation * GLSLESProgramWriter::createInvocationFromString(const String & input)
        {
            ...
            StringList leftTokens = StringUtil::split(input, "(");
            StringList leftTokens2 = StringUtil::split(leftTokens[0], " ");
            StringUtil::trim(leftTokens2[0]);
            StringUtil::trim(leftTokens2[1]);
            returnType      = leftTokens2[0];
            functionName    = leftTokens2[1];

            invoc = N_new FunctionInvocation(functionName, 0, 0, returnType);
            ...
            {
                StringList tokens = StringUtil::split(input, "(");
            ...
            for(itParam = parameters.begin(); itParam != parameters.end(); ++itParam, i++)
            {
                ...
                if(paramTokens.size() == 3)
                {
                    ...
                    GpuDataType gpuType = GDT_Unknow;
                    ...
                    if (gpuType == GDT_Unknow)
                    {
                        N_EXCEPT( Exception::ERR_INTERNAL_ERROR, 
                            "Can not convert Operand::OpMask to GpuDataType", 
                            "GLSLESProgramWriter::createInvocationFromString" );	
                    }

                    if(gpuType == GDT_Sampler1D)
                        gpuType = GDT_Sampler2D;

                    ParameterPtr p = ParameterPtr(N_new Parameter(gpuType, paramTokens[2],
        ...
        void GLSLESProgramWriter::discoverFunctionDependencies(const FunctionInvocation &invoc, FunctionVector &depVector)
        {
            ...
            if(body != StringUtil::BLANK)
            {
                ...
                StringList tokens = StringUtil::split(body, "(");

                for (StringList::const_iterator it = tokens.begin(); it != tokens.end(); it++)
                {
                    StringList moreTokens = StringUtil::split(*it, " ");
        ...
        void GLSLESProgramWriter::initializeStringMaps()
        {
            ...
            mGpuConstTypeMap[GDT_Float2X] = "vec2";
            mGpuConstTypeMap[GDT_Float3X] = "vec3";
            mGpuConstTypeMap[GDT_Float4X] = "vec4";
            mGpuConstTypeMap[GDT_Sampler1D] = "sampler2D";
            mGpuConstTypeMap[GDT_Sampler2D] = "sampler2D";
            mGpuConstTypeMap[GDT_SamplerCube] = "samplerCube";
            mGpuConstTypeMap[GDT_Matrix2X2] = "mat2";
            mGpuConstTypeMap[GDT_Matrix3X3] = "mat3";
            mGpuConstTypeMap[GDT_Matrix4X4] = "mat4";
                ...
                {		
                    ...
                    for (; itOperand != itOperandEnd; )
                    {
                        ...
                        {	
                            ...
                            if (gpuType == GPT_FRAGMENT_PROGRAM)
                            {	
                                StringList::iterator itFound = std::find(mFragInputParams.begin(), mFragInputParams.end(), paramName);	
        ...
        void GLSLESProgramWriter::cacheDependencyFunctions(const String & libName)
        {
            ...
            DataStream * stream = N_Only(ResourceScheme).openResource(libFileName);
            ...
            while(!stream->end())
            {
                ...
                if(line.length() > 0)
                {
                    ...
                    {
                        // Break up the line.
                        StringList tokens = StringUtil::tokenise(line, " (\n\r");
                        ...
                        if(isBasicType(tokens[0]) && ((tokens.size() < 3) || (tokens[2] != "=")) )
                        {
                            ...
                            while(!foundEndOfSignature)
                            {
                                ...
                                StringList paramTokens;
                                String::size_type lparen_pos = line.find('(', 0);
                                if(lparen_pos != String::npos)
                                {
                                    StringList lineTokens = StringUtil::split(line, "(");
