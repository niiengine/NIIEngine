...
namespace Ogre {
    ...
    void AndroidEGLWindow::initNativeCreatedWindow(const PropertyData *miscParams)
    {
    }
    ...
    void AndroidEGLWindow::pos( int left, int top )
    {
    }
    ...
    void AndroidEGLWindow::_notifySurfaceCreated(void* window, void* config)
    {
        ...
        else
        {
            ...
            if (config)
            {
                ...
                Root::getSingletonPtr()->getRenderSystem()->setConfig("Orientation", isLandscape ? "Landscape" : "Portrait");
            }
        }
