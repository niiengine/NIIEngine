...
namespace Ogre {
    ...
    void EmscriptenEGLWindow::initNativeCreatedWindow(const PropertyData *miscParams)
    {
    }
    ...
    void EmscriptenEGLWindow::pos( int left, int top )
    {
    }
    ...
    void EmscriptenEGLWindow::resize(uint width, uint height)
    {
        ...
        LogManager::getSingleton().logMessage("EmscriptenEGLWindow::resize "+mCanvasSelector+" w:" + Ogre::StrConv::conv(mWidth) + " h:" + Ogre::StrConv::conv(mHeight));
    ...
    void EmscriptenEGLWindow::notify()
    {
        if(!mActive) return;
