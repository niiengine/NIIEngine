...
namespace Ogre {
    ...
    void X11EGLWindow::getPlatformData( const String& name, void* pData )
    {
        EGLWindow::getPlatformData(name, pData);
        if (name == "ATOM")
        {
    ...
    void X11EGLWindow::initNativeCreatedWindow(const PropertyData *miscParams)
    {
        mExternalWindow = 0;
        mNativeDisplay = mGLSupport->getNativeDisplay();
        ...
        if (miscParams)
        {
            ...
            {
                StringList tokens = StringUtil::split(opt->second, " :");
    ...
    void X11EGLWindow::pos( int left, int top )
    {
