...
namespace Ogre
{
                           ...
                           bool fullScreen, const PropertyData *miscParams)
    {
        ...
        if(miscParams)
        {
                ...
                StringConverter::parseBool(opt->second))
            {
                ...
                if (!glxContext)
                {
                    N_EXCEPT(RenderingAPI, "currentGLContext was specified with no current GL context", "GLXWindow::create");
                }
        ...
        if (parentWindow != DefaultRootWindow(xDisplay))
        {
            ...
            {
                N_EXCEPT(RenderingAPI, "Invalid parentWindowHandle (wrong server or screen)", "GLXWindow::create");
            }
        }
        ...
        if (externalWindow != 0)
        {
            ...
            if (! XGetWindowAttributes(xDisplay, externalWindow, &windowAttrib) ||
                windowAttrib.root != DefaultRootWindow(xDisplay))
            {
                N_EXCEPT(RenderingAPI, "Invalid externalWindowHandle (wrong server or screen)", "GLXWindow::create");
            }
        ...
        else
        {
            ...
            N_EXCEPT(RenderingAPI, "Unexpected failure to determine a GLXFBConfig","GLXWindow::create");
        }
        ...
        if (! mIsExternal)
        {
            ...
            if(!mWindow)
            {
                N_EXCEPT(RenderingAPI, "Unable to create an X Window", "GLXWindow::create");
            }
        ...
        LogManager::getSingleton().logMessage("GLXWindow::create used FBConfigID = " + StrConv::conv(fbConfigID));
    ...
    //-------------------------------------------------------------------------------------------------//
    unsigned int GLXWindow::getSwapInterval() const
    {
        return mVSyncInterval;
    }

    //-------------------------------------------------------------------------------------------------//
    void GLXWindow::pos(int left, int top)
    {
        if (mClosed || ! mIsTopLevel)
            return;
    ...
    //-------------------------------------------------------------------------------------------------//
    void GLXWindow::getPlatformData( const String& name, void* pData )
    {
    ...
    void GLXWindow::copyContentsToMemory(const Box& src, const PixelBox &dst, FrameBuffer buffer)
    {
        ...
        {
            N_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid box.", "GLXWindow::copyContentsToMemory");
        }
