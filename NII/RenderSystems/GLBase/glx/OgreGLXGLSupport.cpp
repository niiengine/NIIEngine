...
namespace Ogre
{
    ...
    //-------------------------------------------------------------------------------------------------//
    RenderWindow* GLXGLSupport::newWindow(const String &name, unsigned int width, unsigned int height, bool fullScreen, const PropertyData *miscParams)
    {
    ...
    Display* GLXGLSupport::getGLDisplay(void)
    {
        if (! mGLDisplay)
        {
            ...
            if(! mGLDisplay)
            {
                N_EXCEPT(RenderingAPI, "Couldn`t open X display " + String((const char*)XDisplayName (0)), "GLXGLSupport::getGLDisplay");
            }
        }
    ...
    Display* GLXGLSupport::getXDisplay(void)
    {
        if (! mXDisplay)
        {
            ...
            if (! mXDisplay)
            {
                N_EXCEPT(RenderingAPI, "Couldn`t open X display " + String((const char*)displayString), "GLXGLSupport::getXDisplay");
            }
	...
	OgreAssert(_glXCreateContextAttribsARB, "glXCreateContextAttribsARB() function not found");
        ...
        while(!glxContext && (context_attribs[1] >= minVersion))
        {
            ...
            {
                LogManager::getSingleton().logMessage("Created GL " + StrConv::conv(context_attribs[1]) + "." + StrConv::conv(context_attribs[3]) + " context" );
            }
            else
            {
    ...
    void GLXGLSupport::switchMode(uint& width, uint& height, short& frequency)
    {
        ...
        if (newMode && *newMode != mCurrentMode)
        {
            ...
            if (screenConfig)
            {
                ...
                LogManager::getSingleton().logMessage("Entered video mode " + mCurrentMode.getDescription() + " @ " + StrConv::conv(mCurrentMode.refreshRate) + "Hz");
            }
        }
    }
