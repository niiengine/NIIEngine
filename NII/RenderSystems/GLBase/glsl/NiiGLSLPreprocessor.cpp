...
#define snprintf _snprintf
    ...
    void CPreprocessor::Error(int iLine, const char *iError, const Token *iToken)
    {
        char line [1000];
        if (iToken)
            NsnprintfV(line, sizeof (line), "line %d: %s: `%.*s'\n",
                      iLine, iError, int (iToken->Length), iToken->String);
        else
            NsnprintfV(line, sizeof (line), "line %d: %s\n", iLine, iError);
        String temp;
        StrConv::conv(line, temp);
        N_Only(Log).log (temp, LL_Fatal);
    }
        ...
        Token &oResult, int iLine, int iOpPriority)
    {
        ...
        if (oResult.Type == Token::TK_PUNCTUATION && oResult.Length == 1)
        {
            ...
            else if (oResult.String [0] == '(')
            {
                ...
                N_assert (op.Type == Token::TK_PUNCTUATION &&
                        op.Length == 1 &&
                        op.String [0] == ')');
                op = GetToken (true);
            }
    ...
    bool CPreprocessor::HandleDefine (Token &iBody, int iLine)
    {
        ...
        default:
            ...
            N_assert (t.String + t.Length == cpp.Source);
