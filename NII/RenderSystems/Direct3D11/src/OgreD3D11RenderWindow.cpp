...
#pragma region D3D11RenderWindowBase
        ...
        bool fullScreen, const PropertyData *miscParams)
    {
        ...
        mName = name;
        mDSPool = depthBuffer ? DepthBuffer::GroupInner : DepthBuffer::GroupUnknow;
    ...
    {
        N_assert(mpBackBuffer && !mRenderTargetView && !mDepthStencilView);
        ...
        N_assert(BBDesc.SampleDesc.Count == mFSAAType.Count && BBDesc.SampleDesc.Quality == mFSAAType.Quality);
        ...
        if( mDSPool != DepthBuffer::GroupUnknow )
        {
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowBase::getPlatformData( const String& name, void* pData )
    {
        ...
        {
            RenderWindow::getPlatformData(name, pData);
        }
    }
    //---------------------------------------------------------------------
    void D3D11RenderWindowBase::copyContentsToMemory(const Box& src, const PixelBox &dst, FrameBuffer buffer)
    {
        ...
        {
            N_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid box.");
        }
        ...
        else if(mpBackBufferNoMSAA)
        {
            ...
            mDevice.throwIfFailed("Error resolving MSAA subresource");
        }
        else
        {
            ...
            mDevice.throwIfFailed(hr, "Error creating texture without MSAA");
        ...
        mDevice.throwIfFailed(hr, "Error creating staging texture");
        ...
        mDevice.throwIfFailed("Error while copying to staging texture");
        ...
        mDevice.throwIfFailed(hr, "Error while mapping staging texture");
...
#pragma region D3D11RenderWindowSwapChainBased
    ...
    void D3D11RenderWindowSwapChainBased::_resizeSwapChainBuffers(unsigned width, unsigned height)
    {
        ...
        if(hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
        {
            ...
            N_EXCEPT_EX(RenderingAPI, hr, "Unable to resize swap chain", "D3D11RenderWindowSwapChainBased::_resizeSwapChainBuffers");
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowSwapChainBased::swap( )
    {
        D3D11RenderSystem* rsys = static_cast<D3D11RenderSystem*>(Root::getSingleton().getRenderSystem());
        ...
        if( !mDevice.isNull() )
        {
            ...
            if(mUseFlipMode && mFSAAType.Count > 1)
            {
                ...
                if(FAILED(hr))
                    N_EXCEPT_EX(RenderingAPI, hr, "Error obtaining backbuffer", "D3D11RenderWindowSwapChainBased::swap");
                if(!isHardwareGammaEnabled())
                {
                    N_assert(_getRenderFormat() == _getSwapChainFormat());
                    mDevice.GetImmediateContext()->ResolveSubresource(swapChainBackBuffer.Get(), 0, mpBackBuffer.Get(), 0, _getRenderFormat());
				}
				else
                {
                    N_assert(mpBackBufferNoMSAA);
            ...
            if( FAILED(hr) )
                N_EXCEPT_EX(RenderingAPI, hr, "Error Presenting surfaces", "D3D11RenderWindowSwapChainBased::swap");
        }
    }
    //---------------------------------------------------------------------
...
#pragma region D3D11RenderWindowHwnd
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowHwnd::pos(int top, int left)
    {
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowHwnd::notify()
    {
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowHwnd::getPlatformData( const String& name, void* pData )
    {
        // Valid attributes and their equvalent native functions:
        // D3DDEVICE            : getD3DDevice
        // WINDOW               : getWindowHandle
        ...
        D3D11RenderWindowSwapChainBased::getPlatformData(name, pData);
    }
    //---------------------------------------------------------------------
...
#pragma region D3D11RenderWindowCoreWindow
    ...
    float D3D11RenderWindowCoreWindow::getViewRatio()
    {
        ...
        bool fullScreen, const PropertyData *miscParams)
    {
        ...
        if (!externalHandle)
        {
            N_EXCEPT( Exception::ERR_INVALIDPARAMS, "External window handle is not specified.", "D3D11RenderWindow::create" );
        }
        else
        {
    ...
    void D3D11RenderWindowCoreWindow::destroy()
    {
        ...
        {
            N_EXCEPT( Exception::ERR_INVALIDPARAMS, "Only external window handles are supported."
    ...
    HRESULT D3D11RenderWindowCoreWindow::_createSwapChainImpl(IDXGIDeviceN* pDXGIDevice)
    {
        ...
        N_assert(mUseFlipMode);                                                       // i.e. no FSAA for swapchain, but can be enabled in separate backbuffer
        mSwapChainDesc.SampleDesc.Count     = 1;
        mSwapChainDesc.SampleDesc.Quality   = 0;
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowCoreWindow::notify()
    {
        float scale = getViewRatio();
        Windows::Foundation::Rect rc = mCoreWindow->Bounds;
        mLeft = (int)(rc.X * scale + 0.5f);
...
#pragma region D3D11RenderWindowSwapChainPanel
    ...
    float D3D11RenderWindowSwapChainPanel::getViewRatio()
    {
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowSwapChainPanel::notify()
    {
        Windows::Foundation::Size sz = Windows::Foundation::Size(static_cast<float>(mSwapChainPanel->ActualWidth), static_cast<float>(mSwapChainPanel->ActualHeight));
        mCompositionScale = Windows::Foundation::Size(mSwapChainPanel->CompositionScaleX, mSwapChainPanel->CompositionScaleY);
        mLeft = 0;
...
#pragma region D3D11RenderWindowImageSource
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowImageSource::create(const String& name, unsigned width, unsigned height, bool fullScreen, const PropertyData *miscParams)
    {
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowImageSource::update(bool _swapBuffers)
    {
        if(!mImageSourceNative)
            return;

        D3D11RenderWindowBase::update(_swapBuffers);
    }
    //---------------------------------------------------------------------
    void D3D11RenderWindowImageSource::swap()
    {
        if(!mImageSourceNative)
            return;
        ...
        if(FAILED(hr))
        {
			N_EXCEPT_EX(RenderingAPI, hr,
                ...
                "D3D11RenderWindowImageSource::swap");
        }
    ...
    //---------------------------------------------------------------------
    void D3D11RenderWindowImageSource::getPlatformData( const String& name, void* pData )
    {
        if( name == "ImageBrush" )
        {
        ...
        D3D11RenderWindowBase::getPlatformData(name, pData);
    }
