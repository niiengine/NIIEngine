...
#include "OgreD3D11HardwareStructBuffer.h"
...
namespace Ogre {
    ...
    void D3D11HLSLProgram::getDefines(String& stringBuffer, std::vector<D3D_SHADER_MACRO>& defines, const String& definesString)
    {
		...
		switch (this->mType)
		{
			...
			break;
			default:
				N_EXCEPT(Exception::ERR_INVALID_STATE,"Could not find a compatible HLSL program type",
...
//      READ_ENUM(MinPrecision, D3D_MIN_PRECISION) // not needed and doesn't exist in June 2010 SDK
    ...
    void D3D11HLSLProgram::compileMicrocode(void)
    {
        ...
        N_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, message,
...
#pragma comment(lib, "d3dcompiler.lib")
        ...
        if (FAILED(hr))
        {
			...
			N_EXCEPT_EX(RenderingAPI, hr, message,
                "D3D11HLSLProgram::compileMicrocode");
        }
        else
        {
            ...
            if(warnings && LogManager::getSingletonPtr())
            {
                ...
                LogManager::getSingleton().logMessage(message, LL_Info);
            }
#endif
...
//              WRITE_ENUM(MinPrecision, D3D_MIN_PRECISION)  // not needed and doesn't exist in June 2010 SDK
            ...
            case D3D_CT_TBUFFER:
                {
                    ...
                    if (!it->mStructBuffer)
                    {
                        HardwareStructBufferSharedPtr uBuffer = GBufferManager::getSingleton().createStructBuffer(mD3d11ShaderBufferDescs[b].Size, Buffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE, false);
                        it->mStructBuffer = HardwareStructBufferSharedPtr(uBuffer);
                    }
                    else
                    {
                        N_assert (it->mStructBuffer->getSizeInBytes() == mD3d11ShaderBufferDescs[b].Size);
                    }
    ...
    void D3D11HLSLProgram::createLowLevelImpl(void)
    {
        ...
        mAssemblerProgram = GpuProgram(dynamic_cast<GpuProgram*>(this), SPFM_NONE);
    }
    //-----------------------------------------------------------------------
    ...
    void D3D11HLSLProgram::populateParameterNames(GpuProgramParam * params)
    {
        ...
        const GpuProgramParameters::GpuSharedParamUsageList& sharedParams = params->getValueList();
        ...
        for (it = sharedParams.begin(); it != end; ++it)
        {
            ...
            HardwareStructBufferSharedPtr uniformBuffer = 
                        GBufferManager::getSingleton()
                                              .createStructBuffer(bufferDesc.Size, 
                                                                   Buffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE, 
                                                                   false, it->getName());
    ...
    //-----------------------------------------------------------------------
    void D3D11HLSLProgram::populateDef(D3D11_SHADER_TYPE_DESC& d3dDesc, GpuParamUnit& def) const
    {
        ...
        case D3D10_SVT_INT:
            ...
            {
            case 1:
                def.constType = GDT_Int;
                break;
            case 2:
                def.constType = GDT_Int2X;
                break;
            case 3:
                def.constType = GDT_Int3X;
                break;
            case 4:
                def.constType = GDT_Int4X;
                break;
            } // columns
            break;
        case D3D10_SVT_UINT:
            ...
            {
            case 1:
                def.constType = GDT_UInt;
                break;
            case 2:
                def.constType = GDT_UInt2X;
                break;
            case 3:
                def.constType = GDT_UInt3X;
                break;
            case 4:
                def.constType = GDT_UInt4X;
                break;
            } // columns
            break;
        case D3D10_SVT_FLOAT:
            ...
            case 1:
                ...
                {
                case 1:
                    def.constType = GDT_Float;
                    break;
                case 2:
                    def.constType = GDT_Float2X;
                    break;
                case 3:
                    def.constType = GDT_Float3X;
                    break;
                case 4:
                    def.constType = GDT_Float4X;
                    break;
                } // columns
                break;
            case 2:
                ...
                {
                case 2:
                    def.constType = GDT_Matrix2X2;
                    break;
                case 3:
                    def.constType = GDT_Matrix2X3;
                    break;
                case 4:
                    def.constType = GDT_Matrix2X4;
                    break;
                } // columns
                break;
            case 3:
                ...
                {
                case 2:
                    def.constType = GDT_Matrix3X4;
                    break;
                case 3:
                    def.constType = GDT_Matrix3X3;
                    break;
                case 4:
                    def.constType = GDT_Matrix3X4;
                    break;
            ...
            case 4:
                ...
                {
                case 2:
                    def.constType = GDT_Matrix4X2;
                    break;
                case 3:
                    def.constType = GDT_Matrix4X3;
                    break;
                case 4:
                    def.constType = GDT_Matrix4X4;
                    break;
                } // columns
                break;
        ...
        def.elementSize = GpuParamUnit::getElementSize(def.constType, true);
    ...
    bool D3D11HLSLProgram::isSupported(void) const
    {
        ...
        // Get the supported syntaxed from RenderFeature 
        return rs->getCapabilities()->isShaderSupport(getCompatibleTarget()) && GpuProgram::isSupported();
    }
    //-----------------------------------------------------------------------
    GpuProgramParam * D3D11HLSLProgram::createParameters(void)
    {
        // Call superclass
    ...
    {
        return StrConv::conv(static_cast<const D3D11HLSLProgram*>(target)->getColumnMajorMatrices());
    }
    ...
    {
        return StrConv::conv(static_cast<const D3D11HLSLProgram*>(target)->getEnableBackwardsCompatibility());
    }
    ...
    {
        N_assert(index<mD3d11ShaderInputParameters.size());
    ...
    {
        N_assert(index<mD3d11ShaderOutputParameters.size());
    ...
    //-----------------------------------------------------------------------------
    ID3D11Buffer* D3D11HLSLProgram::getConstantBuffer(GpuProgramParam * params, uint16 variabilityMask)
    {
		...
		if(!mBufferInfoMap.empty())
        {
            ...
            if (it->mStructBuffer)
            {
                HardwareBufferLockGuard uniformLock(it->mStructBuffer, Buffer::HBL_DISCARD);
                ...
                {
                    const GpuParamUnit& def = params->getConstantDefinition(iter->name);
                ...
                return static_cast<D3D11HardwareStructBuffer*>(it->mStructBuffer.get())->getD3DConstantBuffer();
            }
        }
    ...
    void D3D11HLSLProgram::getConstantBuffers(ID3D11Buffer** buffers, unsigned int& numBuffers,
                                              ...
                                              GpuProgramParam * params, uint16 variabilityMask)
    {
        ...
        while (it != end)
        {
            if (it->mStructBuffer)
            {
                ...
                // Add buffer to list
                buffers[numBuffers] = static_cast<D3D11HardwareStructBuffer*>(it->mStructBuffer.get())->getD3DConstantBuffer();
        ...
        {
            // Get constant name
            const GpuParamUnit& def = params->getConstantDefinition(sit->first);
    ...
    { 
        N_assert(mType == GPT_VERTEX_PROGRAM);
        N_assert(mVertexShader);
    ...
    { 
        N_assert(mType == GPT_FRAGMENT_PROGRAM);
        N_assert(mPixelShader);
        return mPixelShader.Get(); 
    }
    //-----------------------------------------------------------------------------
    ID3D11GeometryShader* D3D11HLSLProgram::getGeometryShader(void) const 
    { 
        N_assert(mType == GPT_GEOMETRY_PROGRAM);
        N_assert(mGeometryShader);
