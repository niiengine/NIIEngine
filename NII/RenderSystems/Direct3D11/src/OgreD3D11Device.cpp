...
namespace Ogre
{
    ...
    {
        N_assert(mD3D11Device.Get() != d3d11device);
        ReleaseAll();
        ...
        if (d3d11device)
        {
            ...
            && SUCCEEDED(pDXGIDevice->GetParent(__uuidof(IDXGIAdapterN), (void **)pDXGIAdapter.GetAddressOf())))
            {
                ...
                if(FAILED(pDXGIAdapter->CheckInterfaceSupport(IID_ID3D10Device /* intentionally D3D10, not D3D11 */, &mVersion)))
                    mVersion.QuadPart = 0;
            }
    ...
    void D3D11Device::throwIfFailed(HRESULT hr, const char* desc, const char* src)
    {
        ...
        {
            String description = std::string(desc).append("\nError Description:").append(getErrorString(hr));
            N_EXCEPT_EX(RenderingAPI, hr, description, src);
        }
    }
    //---------------------------------------------------------------------
    String D3D11Device::getErrorString(const HRESULT lastResult /* = NO_ERROR */) const
    {
        if (!mD3D11Device)
        {
            return "NULL device";
