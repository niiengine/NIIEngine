...
namespace Ogre
{
	...
	bool D3D11StereoDriverNVIDIA::addRenderWindow(D3D11RenderWindowBase* renderWindow)
	{
		...
		void* d3dDevice = NULL;
		renderWindow->getPlatformData("D3DDEVICE", &d3dDevice);
	...
	bool D3D11StereoDriverNVIDIA::isStereoEnabled(const String& renderWindowName)
	{
		...
		if (NVAPI_OK != nvStatus || !isStereoOn)
		{
			...
			void* d3dDevice = NULL;
			stereoHandle.renderWindow->getPlatformData("D3DDEVICE", &d3dDevice);
	...
	//---------------------------------------------------------------------
	bool D3D11StereoDriverNVIDIA::getBufferType(ViewportType colourBuffer)
	{
