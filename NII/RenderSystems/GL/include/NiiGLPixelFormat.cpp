...
namespace NII
{
    ...
    static GLPixelFormatDescription _pixelFormats[int(PF_COUNT)] = {
            ...
            {GL_RED, GL_UNSIGNED_SHORT, GL_R16},                 // PF_L16ui
            ...
            {GL_RGB, GL_HALF_FLOAT, GL_RGB16F},                  // PF_R16G16B16
            ...
            {GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, GL_RGBA8},        // PF_R8G8B8A8ui
            {GL_DEPTH_COMPONENT, GL_UNSIGNED_SHORT, GL_DEPTH_COMPONENT16}, // PF_DEPTH16
            {GL_RGBA, GL_UNSIGNED_SHORT, GL_RGBA16},             // PF_R16G16B16A16ui
            {GL_RGB, GL_UNSIGNED_BYTE_3_3_2, GL_R3_G3_B2},       // PF_R3G3B2
            {GL_RED, GL_HALF_FLOAT, GL_R16F},                    // PF_R16
            {GL_RED, GL_FLOAT, GL_R32F},                         // PF_R32
            {GL_RG, GL_UNSIGNED_SHORT, GL_RG16},                 // PF_G16R16ui
            {GL_RG, GL_HALF_FLOAT, GL_RG16F},                    // PF_G16R16
            {GL_RG, GL_FLOAT, GL_RG32F},                         // PF_G32R32
            {GL_RGB, GL_UNSIGNED_SHORT, GL_RGB16},               // PF_R16G16B16ui
            {GL_NONE},                                           // PF_PVRTC_RGB_2BPP
            {GL_NONE},                                           // PF_PVRTC_RGBA_2BPP
            {GL_NONE},                                           // PF_PVRTC_RGB_4BPP
            {GL_NONE},                                           // PF_PVRTC_RGBA_4BPP
            ...
            {GL_RED, GL_UNSIGNED_BYTE, GL_R8},                   // PF_R8ui
            {GL_RG, GL_UNSIGNED_BYTE, GL_RG8},                   // PF_R8G8ui
	...
	//-----------------------------------------------------------------------------
	PixelFormat GLPixelUtil::getClosestNIIFormat(GLenum fmt)
	{
        switch(format)
        {
    ...
    //-----------------------------------------------------------------------------
    GLenum GLPixelUtil::getGLImageInternalFormat(PixelFormat format)
    {
        switch(format)
        {
        case PF_L8ui:
    ...
    //-----------------------------------------------------------------------------
    GLenum GLPixelUtil::getClosestGLImageInternalFormat(PixelFormat format)
    {
        GLenum GLformat = getGLImageInternalFormat(format);
        return (format == GL_NONE ? GL_RGBA8 : GLformat);
    }
    //-----------------------------------------------------------------------------
    NCount GLPixelUtil::optionalPO2(Nui32 value)
    {
        const RenderFeature * caps = N_Engine().getRender()->getFeature();
        if(caps->isSupport(GF_Tex_NonPow2))
            return value;
        else
