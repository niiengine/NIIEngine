...
//-----------------------------------------------------------------------------    
    GLES2FBORenderTexture::GLES2FBORenderTexture(GLES2FBOManager *manager, const String &name,
        ...
        GLTextureFrame(name, target, writeGamma, std::min(manager->getMaxFSAASamples(), (int)fsaa)),
        mFB(manager, mFSAA)
    {
    ...
    void GLES2FBORenderTexture::getPlatformData(const String& name, void* pData)
    {
    ...
    void GLES2FBORenderTexture::swap()
    {
        mFB.swap();
    }
    ...
    {
        bool result;
        if( (result = GLTextureFrame::attachDepthBuffer( depthBuffer )) )
    ...
    void GLES2FBORenderTexture::detachDepthBuffer()
    {
        ...
        GLTextureFrame::detachDepthBuffer();
    }
    //-----------------------------------------------------------------------------
    void GLES2FBORenderTexture::_detachDepthBuffer()
    {
        mFB.detachDepthBuffer();
        GLTextureFrame::_detachDepthBuffer();
    }
    ...
    GLES2FBOManager::GLES2FBOManager() : mMaxFSAASamples(0)
    {
        ...
        GLCheck(glGenFramebuffers(1, &mTempFBO));

        // Check multisampling if supported
        if(getGLES2RenderSystem()->hasMinGLVersion(3, 0))
        {
            // Check samples supported
            GLCheck(glGetIntegerv(GL_MAX_SAMPLES_APPLE, &mMaxFSAASamples));
        }
    }
    ...
    GLES2FBOManager::~GLES2FBOManager()
    {
        ...
        GLCheck(glDeleteFramebuffers(1, &mTempFBO));
    }
...
//                            LogManager::getSingleton().logMessage(l.str());
                    ...
                    else if(hasGLES3 || rs->checkGpuApi("GL_OES_packed_depth_stencil") )
                    {
    ...
    void GLES2FBOManager::bind(RenderTarget *target)
    {
        ...
        else
        {
            ...
            target->getPlatformData("GLFBO", &mainfbo);
#endif
            GLCheck(glBindFramebuffer(GL_FRAMEBUFFER, mainfbo));
        }
    }
    ...
    GLSurfaceDesc GLES2FBOManager::requestRenderBuffer(GLenum format, uint32 width, uint32 height, uint fsaa)
    {
        ...
        if(format != GL_NONE)
        {
            ...
            {
                // New one
                GLES2RenderBuffer *rb = N_new GLES2RenderBuffer(format, width, height, fsaa);
