...
namespace Ogre {
    ...
    {
        N_assert( msSingleton );  return ( *msSingleton );
    }
    ...
    //---------------------------------------------------------------------
    void GLSLESProgramManager::convertGLUniformtoOgreType(GLenum gltype, GpuParamUnit& defToUpdate)
    {
        ...
        {
        case GL_FLOAT:
            defToUpdate.constType = GDT_Float;
            break;
        case GL_FLOAT_VEC2:
            defToUpdate.constType = GDT_Float2X;
            break;
        case GL_FLOAT_VEC3:
            defToUpdate.constType = GDT_Float3X;
            break;
        case GL_FLOAT_VEC4:
            defToUpdate.constType = GDT_Float4X;
            break;
        case GL_SAMPLER_3D:
        case GL_INT_SAMPLER_3D:
        case GL_UNSIGNED_INT_SAMPLER_3D:
            defToUpdate.constType = GDT_Sampler3D;
            break;
        ...
        case GL_SAMPLER_2D:
            defToUpdate.constType = GDT_Sampler2D;
            break;
        case GL_SAMPLER_CUBE_SHADOW:
        case GL_INT_SAMPLER_CUBE:
        case GL_UNSIGNED_INT_SAMPLER_CUBE:
        case GL_SAMPLER_CUBE:
            defToUpdate.constType = GDT_SamplerCube;
            break;
        case GL_SAMPLER_2D_SHADOW_EXT:
            defToUpdate.constType = GDT_SamplerShadow2D;
            break;
        case GL_SAMPLER_EXTERNAL_OES:
            defToUpdate.constType = GCT_SAMPLER_EXTERNAL_OES;
            break;
        case GL_INT:
            defToUpdate.constType = GDT_Int;
            break;
        case GL_INT_VEC2:
            defToUpdate.constType = GDT_Int2X;
            break;
        case GL_INT_VEC3:
            defToUpdate.constType = GDT_Int3X;
            break;
        case GL_INT_VEC4:
            defToUpdate.constType = GDT_Int4X;
            break;
        case GL_FLOAT_MAT2:
            defToUpdate.constType = GDT_Matrix2X2;
            break;
        case GL_FLOAT_MAT3:
            defToUpdate.constType = GDT_Matrix3X3;
            break;
        case GL_FLOAT_MAT4:
            defToUpdate.constType = GDT_Matrix4X4;
            break;
        case GL_FLOAT_MAT2x3:
            defToUpdate.constType = GDT_Matrix2X3;
            break;
        case GL_FLOAT_MAT3x2:
            defToUpdate.constType = GDT_Matrix3X4;
            break;
        case GL_FLOAT_MAT2x4:
            defToUpdate.constType = GDT_Matrix2X4;
            break;
        case GL_FLOAT_MAT4x2:
            defToUpdate.constType = GDT_Matrix4X2;
            break;
        case GL_FLOAT_MAT3x4:
            defToUpdate.constType = GDT_Matrix3X4;
            break;
        case GL_FLOAT_MAT4x3:
            defToUpdate.constType = GDT_Matrix4X3;
            break;
        default:
        ...
        // GL doesn't pad
        defToUpdate.elementSize = GpuParamUnit::getElementSize(defToUpdate.constType, false);
    }

    //---------------------------------------------------------------------
    bool GLSLESProgramManager::completeParamSource(const String& paramName,
        const GpuParamUnitList* vertexConstantDefs, const GpuParamUnitList* fragmentConstantDefs, GLUniformReference& refToUpdate)
    {
        if (vertexConstantDefs)
        {
            GpuParamUnitList::const_iterator parami = 
    ...
    void GLSLESProgramManager::extractUniforms(GLuint programObject,
        const GpuParamUnitList* vertexConstantDefs, 
        const GpuParamUnitList* fragmentConstantDefs,
        GLUniformReferenceList& list, GLUniformBufferList& sharedList)
    {
        ...
        GLCheck(glGetProgramiv(programObject, GL_ACTIVE_UNIFORM_MAX_LENGTH, &maxLength));
        ...
        for (GLuint index = 0; index < (GLuint)uniformCount; index++)
        {
            ...
            GLCheck(glGetActiveUniform(programObject, index, maxLength, NULL,
            ...
            if (newGLUniformReference.mLocation >= 0)
            {
                ...
                if (foundSource)
                {
                    N_assert(size_t (arraySize) == newGLUniformReference.mConstantDef->arraySize
        ...
        for (int index = 0; index < blockCount; index++)
        {
            ...
            GpuSharedParametersPtr blockSharedParams = N_Only(GpuProgram).getShare(uniformName);

            GLint blockSize, blockBinding;
            GLCheck(glGetActiveUniformBlockiv(programObject, index, GL_UNIFORM_BLOCK_DATA_SIZE, &blockSize));
            GLCheck(glGetActiveUniformBlockiv(programObject, index, GL_UNIFORM_BLOCK_BINDING, &blockBinding));
            StructBuffer * newUniformBuffer = GBufferManager::getSingleton().createUniformBuffer(blockSize, Buffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE, false, uniformName);
