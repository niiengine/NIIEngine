...
namespace Ogre 
{
    ...
    {   
        ...
        while (it != mMapDeviceToTextureResources.end())
        {
            ...
            N_delete_t(textureResource, TextureResources, MEMCATEGORY_RENDERSYS);
            ++it;
        }       
        ...
        while (it != mMapDeviceToTextureResources.end())
        {
            ...
            {           
                ...
                if( FAILED( hr = srcTextureResource->pNormTex->GetSurfaceLevel(0, &pSrcSurface) ) )
                {
                    ...
                    N_EXCEPT(RenderingAPI, "Couldn't blit : " + msg, "D3D9Texture::copyToTexture" );
                }
    ...
    /****************************************************************************************/
    void D3D9Texture::freeInternalImpl()
    {
        D3D9_DEVICE_ACCESS_CRITICAL_SECTION
    ...
    {
        N_assert(mMapDeviceToTextureResources.find(d3d9Device) == mMapDeviceToTextureResources.end());
    ...
    {
        size_t instanceSize = getNumFaces() * PixelUtil::getSize(mWidth, mHeight, mDepth, mFormat);
    ...
    /****************************************************************************************/
    void D3D9Texture::createInternalImpl(void)
    {
        D3D9_DEVICE_ACCESS_CRITICAL_SECTION
    ...
    /****************************************************************************************/
    void D3D9Texture::createInternalImpl(IDirect3DDevice9* d3d9Device)
    {       
        ...
        default:
            freeInternalResources();
            N_EXCEPT( Exception::ERR_INTERNAL_ERROR, "Unknown texture type", "D3D9Texture::createInternalResources" );
        }
    }
    ...
    void D3D9Texture::_createNormTex(IDirect3DDevice9* d3d9Device)
    {
        ...
        {
            freeInternalResources();
            N_EXCEPT(RenderingAPI, "Error creating texture: " + String(DXGetErrorDescription(hr)), 
        ...
        {
            freeInternalResources();
            N_EXCEPT(RenderingAPI, "Can't get base texture: " + String(DXGetErrorDescription(hr)), 
                "D3D9Texture::_createNormTex" );
        }
        ...
        {
            freeInternalResources();
            N_EXCEPT(RenderingAPI, "Can't get texture description: " + String(DXGetErrorDescription(hr)), 
                "D3D9Texture::_createNormTex" );
        }
        ...
        if(mMipmapsHardwareGenerated)
        {
            ...
            if(FAILED(hr))
            {
                N_EXCEPT(RenderingAPI, "Could not set best autogen filter type: "  + String(DXGetErrorDescription(hr)), 
                    "D3D9Texture::_createNormTex" );
            }
        }
    ...
    void D3D9Texture::_createVolumeTex(IDirect3DDevice9* d3d9Device)
    {
        ...
        if (mUsage & TU_RENDERTARGET)
        {
            N_EXCEPT(RenderingAPI, "D3D9 Volume texture can not be created as render target !!", 
    ...
    { 
            ...
            mHeight != mSrcHeight)
        {
            ...
            LogManager::getSingleton().logMessage("D3D9 : ***** Source image dimensions : " + StrConv::conv(mSrcWidth) + "x" + StrConv::conv(mSrcHeight));
            LogManager::getSingleton().logMessage("D3D9 : ***** Texture dimensions : " + StrConv::conv(mWidth) + "x" + StrConv::conv(mHeight));
        }
    ...
    { 
        ...
        case TEX_TYPE_1D:
            if (mUsage & TU_RENDERTARGET)
                LogManager::getSingleton().logMessage("D3D9 : Creating 1D RenderTarget, name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            else
                LogManager::getSingleton().logMessage("D3D9 : Loading 1D Texture, image name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            break;
        case TEX_TYPE_2D:
            if (mUsage & TU_RENDERTARGET)
                LogManager::getSingleton().logMessage("D3D9 : Creating 2D RenderTarget, name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            else
                LogManager::getSingleton().logMessage("D3D9 : Loading 2D Texture, image name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            break;
        case TEX_TYPE_3D:
            if (mUsage & TU_RENDERTARGET)
                LogManager::getSingleton().logMessage("D3D9 : Creating 3D RenderTarget, name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            else
                LogManager::getSingleton().logMessage("D3D9 : Loading 3D Texture, image name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            break;
        case TEX_TYPE_CUBE_MAP:
            if (mUsage & TU_RENDERTARGET)
                LogManager::getSingleton().logMessage("D3D9 : Creating Cube map RenderTarget, name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            else
                LogManager::getSingleton().logMessage("D3D9 : Loading Cube Texture, base image name : '" + getName() + "' with " + StrConv::conv(mNumMipmaps) + " mip map levels");
            break;
        default:
            freeInternalResources();
            N_EXCEPT( Exception::ERR_INTERNAL_ERROR, "Unknown texture type", "D3D9Texture::_setSrcAttributes" );
        }
    }
    /****************************************************************************************/
    ...
    void D3D9Texture::_createSurfaceList(IDirect3DDevice9* d3d9Device, TextureResources* textureResources)
    {
        ...
        N_assert(textureResources != NULL);
        N_assert(textureResources->pBaseTex);
        ...
        {
            bufusage = Buffer::HBU_DYNAMIC;
        }
        else
        {
            bufusage = Buffer::HBU_STATIC;
        }
        if (mUsage & TU_RENDERTARGET)
        {
        ...
        {           
            ...
            for(size_t face=0; face<getNumFaces(); ++face)
            {
                ...
                {
                    buffer = N_new D3D9HardwarePixelBuffer((Buffer::Usage)bufusage, this);
        ...
        case TEX_TYPE_2D:
        case TEX_TYPE_1D:
            N_assert(textureResources->pNormTex);
            ...
            for(mip=0; mip<=mNumMipmaps; ++mip)
            {
                if(textureResources->pNormTex->GetSurfaceLevel(static_cast<UINT>(mip), &surface) != D3D_OK)
                    N_EXCEPT(RenderingAPI, "Get surface level failed",
            ...
            break;
        case TEX_TYPE_CUBE_MAP:
            N_assert(textureResources->pCubeTex);
            ...
            for(face=0; face<6; ++face)
            {
                for(mip=0; mip<=mNumMipmaps; ++mip)
                {
                    if(textureResources->pCubeTex->GetCubeMapSurface((D3DCUBEMAP_FACES)face, static_cast<UINT>(mip), &surface) != D3D_OK)
                        N_EXCEPT(RenderingAPI, "Get cubemap surface failed",
            ...
            break;
        case TEX_TYPE_3D:
            N_assert(textureResources->pVolumeTex);
            // For all mipmaps, store surfaces as HardwarePixelBufferSharedPtr
            for(mip=0; mip<=mNumMipmaps; ++mip)
            {
                if(textureResources->pVolumeTex->GetVolumeLevel(static_cast<UINT>(mip), &volume) != D3D_OK)
                    N_EXCEPT(RenderingAPI, "Get volume level failed",
                        "D3D9Texture::getBuffer");  
    ...
    //---------------------------------------------------------------------
    void D3D9RenderTexture::getPlatformData( const String& name, void *pData )
    {
        ...
        {
            *static_cast<FrameBuffer**>(pData) = mBuffer;
        }
    }    
    //---------------------------------------------------------------------
    void D3D9RenderTexture::swap()
    {
