...
namespace Ogre
{
	...
	//-----------------------------------------------------------------------------
	bool D3D9StereoDriverAMD::getBufferType(ViewportType colourBuffer)
	{
		// Note, getBufferType is normally called at the beginning of rendering to the viewport
	...
	//-----------------------------------------------------------------------------
	void D3D9StereoDriverAMD::eventOccurred(const String& eventName, const PropertyData* parameters)
	{
	...
	//-----------------------------------------------------------------------------
	void D3D9StereoDriverAMD::deviceCreatedEvent(const PropertyData* parameters)
	{
		PropertyData::const_iterator iter = parameters->find("D3DDEVICE");
	...
	//-----------------------------------------------------------------------------
	void D3D9StereoDriverAMD::deviceLostEvent(const PropertyData* parameters)
	{
		releaseResources();
	}
	//-----------------------------------------------------------------------------
	void D3D9StereoDriverAMD::afterDeviceResetEvent(const PropertyData* parameters)
	{
		PropertyData::const_iterator iter = parameters->find("D3DDEVICE");
		mDevice = reinterpret_cast<IDirect3DDevice9*>(StringConverter::parseSizeT(iter->second));
	...
	//-----------------------------------------------------------------------------
	void D3D9StereoDriverAMD::beforeDevicePresentEvent(const PropertyData* parameters)
	{
		if (!mStereoEnabled)
			return;
	...
	void D3D9StereoDriverAMD::postViewportUpdate(const RenderTargetViewportEvent& evt)
	{
		...
		switch (evt.source->getBufferType())
		{
		case VT_Back:
		case VT_BackLeft:
		...
		RECT sourceRect;
		sourceRect.left = evt.source->getPixelLeft();
		sourceRect.top = evt.source->getPixelTop();
		sourceRect.right = evt.source->getPixelWidth();
		sourceRect.bottom = evt.source->getPixelHeight();
