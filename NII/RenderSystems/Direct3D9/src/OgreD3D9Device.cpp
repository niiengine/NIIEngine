...
namespace Ogre
{
    ...
    void D3D9Device::detachRenderWindow(D3D9RenderWindow* renderWindow)
    {
        ...
        {       
            ...
            N_delete_t(renderWindowResources, RenderWindowResources, MEMCATEGORY_RENDERSYS);
    ...
    {   
        ...
        {   
            ...
            N_delete_t(it->second, RenderWindowResources, MEMCATEGORY_RENDERSYS);
            ++it;
        }
    ...
    void D3D9Device::clearDeviceStreams()
    {
        ...
        for (DWORD stage = 0; stage < mD3D9DeviceCaps.MaxSimultaneousTextures; ++stage)
        {
            ...
            if( hr != S_OK )
            {
                String str = "Unable to disable texture '" + StrConv::conv((unsigned int)stage) + "' in D3D9";
                N_EXCEPT(RenderingAPI, str, "D3D9Device::clearDeviceStreams" );
            }
            ...
            if (dwCurValue != D3DTOP_DISABLE)
            {
                ...
                if( hr != S_OK )
                {
                    String str = "Unable to disable texture '" + StrConv::conv((unsigned)stage) + "' in D3D9";
                    N_EXCEPT(RenderingAPI, str, "D3D9Device::clearDeviceStreams" );
                }
            }           
            ...
            renderSystem->mTexStageDesc[stage].autoTexCoordType = TCP_NONE;
    ...
    {       
        ...
        N_assert(it != mMapRenderWindowToResources.end());
        ...
        const Box& src, const PixelBox &dst, RenderTarget::FrameBuffer buffer)
    {
        ...
        if (buffer == RenderTarget::FB_FRONT)
        {
            ...
            {
                N_EXCEPT(RenderingAPI, "Can't get display mode: " + DXGetErrorDescription(hr));
            }
            ...
            if (FAILED(hr = mDevice->CreateOffscreenPlainSurface(desc.Width, desc.Height,
                ...
                0)))
            {
                N_EXCEPT(RenderingAPI, "Can't create offscreen buffer: " + DXGetErrorDescription(hr));
            }
                ...
                mDevice->GetFrontBufferData(0, pTempSurf)))
            {
                ...
                N_EXCEPT(RenderingAPI, "Can't get front buffer: " + DXGetErrorDescription(hr));
            }
            ...
            if(renderWindow->isFullScreen())
            {
                ...
                {
                    SAFE_RELEASE(pTempSurf);
                    N_EXCEPT(RenderingAPI, "Can't lock rect: " + DXGetErrorDescription(hr));
                } 
            }
            else
            {
        ...
        else
        {
            ...
            {
                N_EXCEPT(RenderingAPI, "Can't get back buffer: " + DXGetErrorDescription(hr));
            }

            if(FAILED(hr = pSurf->GetDesc(&desc)))
            {
                N_EXCEPT(RenderingAPI, "Can't get description: " + DXGetErrorDescription(hr));
            }
            ...
            if (FAILED(hr = mDevice->CreateOffscreenPlainSurface(desc.Width, desc.Height,
                ...
                0)))
            {
                N_EXCEPT(RenderingAPI, "Can't create offscreen surface: " + DXGetErrorDescription(hr));
            }
            ...
            if (desc.MultiSampleType == D3DMULTISAMPLE_NONE)
            {
                ...
                {
                    SAFE_RELEASE(pTempSurf);
                    N_EXCEPT(RenderingAPI, "Can't get render target data: " + DXGetErrorDescription(hr));
                }
            }
            else
            {
                ...
                {
                    SAFE_RELEASE(pTempSurf);
                    N_EXCEPT(RenderingAPI, "Can't create render target: " + DXGetErrorDescription(hr));
                }
        ...
        {
            SAFE_RELEASE(pTempSurf);
            N_EXCEPT(RenderingAPI, "Unsupported format");
        }
