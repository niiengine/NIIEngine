...
namespace Ogre
{
    ...
    void D3D9RenderWindow::setFullscreen(bool fullScreen, unsigned int width, unsigned int height)
    {
        if (fullScreen != mIsFullScreen || width != mWidth || height != mHeight)
        {
            ...
            // NB don't use notify since Win32 doesn't know
    ...
    {       
        ...
        presentParams->EnableAutoDepthStencil   = (mDSPool != DepthBuffer::GroupUnknow);
    ...
    unsigned int D3D9RenderWindow::getSwapInterval() const
    {
        return mVSyncInterval;
    }

    void D3D9RenderWindow::pos(int top, int left)
    {
    ...
    void D3D9RenderWindow::swap( )
    {
        if (mDeviceValid)
            mDevice->present(this);     
    }

    void D3D9RenderWindow::getPlatformData( const String& name, void* pData )
    {
    ...
    {
        return (mDSPool != DepthBuffer::GroupUnknow);
    }
