...
namespace Ogre {
    ...
    //---------------------------------------------------------------------
    D3D9HardwareIndexBuffer::D3D9HardwareIndexBuffer(GBufferManager* mgr, IndexBuffer::IndexType idxType, 
        size_t numIndexes, Buffer::Usage usage,
            ...
            D3D9RenderSystem::getResourceManager()->getAutoHardwareBufferManagement()))
   {
        ...
        eResourcePool = useSystemMemory? D3DPOOL_SYSTEMMEM : 
            ...
            ((usage & Buffer::HBU_DISCARDABLE) || (D3D9RenderSystem::isDirectX9Ex())) ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED;
#else
        ...
        void* pDest)
    {
       // There is no functional interface in D3D, just do via manual 
        // lock, copy & unlock
        void* pSrc = this->lock(offset, length, Buffer::HBL_READ_ONLY);
    ...
    void D3D9HardwareIndexBuffer::updateBufferContent(BufferResources* bufferResources)
    {
        if (bufferResources->mOutOfDate)
        {
            ...
            else if (mSourceBuffer != bufferResources && (mUsage & Buffer::HBU_WRITE_ONLY) == 0)
            {               
    ...
    bool D3D9HardwareIndexBuffer::updateBufferResources(const char* systemMemoryBuffer,
        BufferResources* bufferResources)
    {
        N_assert(bufferResources != NULL);
        N_assert(bufferResources->mBuffer != NULL);
        N_assert(bufferResources->mOutOfDate);
