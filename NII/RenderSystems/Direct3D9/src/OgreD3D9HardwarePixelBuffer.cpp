...
D3D9HardwarePixelBuffer::D3D9HardwarePixelBuffer(Buffer::Usage usage, 
                                                 D3D9Texture* ownerTexture):
    FrameBuffer(0, 0, 0, PF_UNKNOWN, usage, false, false),
                                   ...
                                   IDirect3DBaseTexture9 *mipTex)
{
    ...
    if(surface->GetDesc(&desc) != D3D_OK)
        N_EXCEPT(RenderingAPI, "Could not get surface information",
         "D3D9HardwarePixelBuffer::D3D9HardwarePixelBuffer");
...
void D3D9HardwarePixelBuffer::bind(IDirect3DDevice9 *dev, IDirect3DVolume9 *volume, IDirect3DBaseTexture9 *mipTex)
{
    ...
    if(volume->GetDesc(&desc) != D3D_OK)
        N_EXCEPT(RenderingAPI, "Could not get volume information",
         "D3D9HardwarePixelBuffer::D3D9HardwarePixelBuffer");
    mWidth = desc.Width;
    mHeight = desc.Height;
    mDepth = desc.Depth;
...
void fromD3DLock(PixelBox &rval, const D3DLOCKED_RECT &lrect)
{
    ...
    if (bpp != 0)
    {
        ...
        N_assert((lrect.Pitch % bpp)==0);
    }
    else if (PixelUtil::isCompress(rval.format))
    {
...
void fromD3DLock(PixelBox &rval, const D3DLOCKED_BOX &lbox)
{
    ...
    if (bpp != 0)
    {
        ...
        N_assert((lbox.RowPitch % bpp)==0);
        N_assert((lbox.SlicePitch % bpp)==0);
    }
    else if (PixelUtil::isCompress(rval.format))
    {
        rval.rowPitch = rval.getWidth();
...
{   
    ...
    {
        N_EXCEPT(RenderingAPI, "There are no resources attached to this pixel buffer !!",
            "D3D9HardwarePixelBuffer::lockImpl");   
    }
                                                   ...
                                                   DWORD flags)
{
    ...
    if (bufferResources->surface != NULL) 
    {
        ...
        if (FAILED(hr))     
            N_EXCEPT(RenderingAPI, "Surface locking failed",
            "D3D9HardwarePixelBuffer::lockImpl");
    ...
    else if(bufferResources->volume) 
    {
        ...
        if (FAILED(hr))     
        {
            N_EXCEPT(RenderingAPI, "Volume locking failed",
                "D3D9HardwarePixelBuffer::lockImpl");
        }
                                   ...
                                   const Box &dstBox)
{
    ...
    while (it != mMapDeviceToBufferResources.end())
    {
        ...
        {
            N_EXCEPT(RenderingAPI, "There are no matching resources attached to the source pixel buffer !!",
                                   ...
                                   BufferResources* dstBufferResources)
{
    if(dstBufferResources->surface && srcBufferResources->surface)
    {
            ...
            && srcDesc.MultiSampleType == D3DMULTISAMPLE_NONE)
        {
            ...
            {
                N_EXCEPT(RenderingAPI, "Create temporary texture failed",
                    "D3D9HardwarePixelBuffer::blit");
            }
            ...
            if(d3d9Device->GetRenderTargetData(srcBufferResources->surface, tmpsurface) == D3D_OK)
            {
                    ...
                    D3DX_DEFAULT, 0) != D3D_OK)
                {
                    tmpsurface->Release();
                    tmptex->Release();
                    N_EXCEPT(RenderingAPI, "D3DXLoadSurfaceFromSurface failed",
                        "D3D9HardwarePixelBuffer::blit");
                }
                tmpsurface->Release();
                tmptex->Release();
    ...
    else if(dstBufferResources->volume && srcBufferResources->volume)
    {
        ...
        if(D3DXLoadVolumeFromVolume(
            ...
            D3DX_DEFAULT, 0) != D3D_OK)
        {
            N_EXCEPT(RenderingAPI, "D3DXLoadVolumeFromVolume failed",
                "D3D9HardwarePixelBuffer::blit");
        }
    }
    else
    {
        // Software fallback   
        FrameBuffer::blit(rsrc, srcBox, dstBox);
    }
}
...
void D3D9HardwarePixelBuffer::blitFromMemory(const PixelBox &src, const Box &dstBox, BufferResources* dstBufferResources)
{
    ...
    {
        buf.reset(N_new MemoryDataStream(
            PixelUtil::getSize(src.getWidth(), src.getHeight(), src.getDepth(),
            mFormat)));
    ...
    size_t rowWidth;
    if (PixelUtil::isCompress(converted.format))
    {
    ...
    if (dstBufferResources->surface)
    {
        ...
        {
            N_EXCEPT(RenderingAPI, "D3DXLoadSurfaceFromMemory failed",
                                           ...
                                           IDirect3DDevice9* d3d9Device)
{
    ...
    if (srcBufferResources->surface)
    {
        ...
        if(surface->LockRect(&lrect, NULL,  D3DLOCK_READONLY) != D3D_OK)
        {
            surface->Release();
            tmp->Release();
            N_EXCEPT(RenderingAPI, "surface->LockRect",
                "D3D9HardwarePixelBuffer::blitToMemory");
        }
        // Copy it
    ...
    else if (srcBufferResources->volume)
    {
        ...
        if(surface->LockBox(&lbox, NULL,  D3DLOCK_READONLY) != D3D_OK)
        {
            surface->Release();
            tmp->Release();
            N_EXCEPT(RenderingAPI, "surface->LockBox",
                "D3D9HardwarePixelBuffer::blitToMemory");
        }
        // Copy it
        PixelBox locked(dst.getWidth(), dst.getHeight(), dst.getDepth(), tmpFormat);
...
//-----------------------------------------------------------------------------   
void D3D9HardwarePixelBuffer::destroyImpl(size_t zoffset)
{
    mRenderTexture = NULL;
}
...
void D3D9HardwarePixelBuffer::updateRenderTexture(bool writeGamma, uint fsaa, const String& srcName)
{
    ...
    {
        String name;
        name = "rtt/" +Ogre::StrConv::conv((size_t)this) + "/" + srcName;

        mRenderTexture = N_new D3D9RenderTexture(name, this, writeGamma, fsaa);      
