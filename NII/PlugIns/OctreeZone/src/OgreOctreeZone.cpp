...
namespace Ogre
{
	...
	*/
	...
	{
		OctreeZoneData * ozd = N_new OctreeZoneData(node, this);
		if (ozd)
		{
                                               ...
                                               Portal * ignorePortal)
    {
		for ( PortalList::iterator it = mPortals.begin(); it != mPortals.end(); ++it )
		{
			...
			if (p != ignorePortal)
			{
				...
				Vector3 lightToPortal = p->getDerivedCP() - light->getSpacePos();
				if (portalFrustum->isIntersect(p))
				{
					...
					case LT_POINT:
						...
						if (lightToPortal.length() <= light->getRange())
						{
 							// if portal is quad portal it must be pointing towards the light
							if ((p->getType() == Portal::PORTAL_TYPE_QUAD && lightToPortal.dotProduct(p->getSpaceDirection()) < 0.0) ||
                    ...
                    case LT_DIRECTIONAL:
                        ...
                        if (lightToPortal.dotProduct(light->getSpaceDirection()) >= 0.0)
                        {
 							// if portal is quad portal it must be pointing towards the light
							if ((p->getType() == Portal::PORTAL_TYPE_QUAD && lightToPortal.dotProduct(p->getSpaceDirection()) < 0.0) ||
								(p->getType() != Portal::PORTAL_TYPE_QUAD))
							{
								if (!light->affectsZone(targetZone))
								{
									...
									// recurse into the target zone of the portal
									p->getTargetZone()->_checkLightAgainstPortals(light,
                                        frameCount, portalFrustum, p->getTargetPortal());
									// remove the planes added by this portal
									portalFrustum->removePortalCullingPlanes(p);
								}
							}
    ...
    */
								  ...
								  bool aabbview)
    {
		...
		const Vector3& cameraOrigin(camera->getSpacePos());
		...
		antiPortalFrustum.setProjType(camera->getProjType());
		...
		for (size_t i = 0; i < sortedPortalListCount; ++i)
		{
			...
			if (portalBase->getTypeMark() == PortalFactory::FACTORY_TYPE_FLAG)
			{
								...
								bool aabbview)
	{
		...
		if ( v != PCZCamera::NONE )
		{
			...
			if ( (child = octant -> mChildren[ 0 ][ 0 ][ 0 ]) != 0 )
				walkOctree( camera, visibleNodeList, queue, child, visibleBounds, childfoundvisible, onlyShadowCasters, displayNodes, aabbview );
	...
	void OctreeZone::_findNodes(const AABox &t,
								...
								PCZSceneNode *exclude )
    {
		...
		if (mEnclosureNode)
		{
			if (!mEnclosureNode->getSpaceAABB().intersects(t))
			{
    ...
    void OctreeZone::_findNodes(const PlaneSpace &t,
							    PCZSceneNodeList &list,
                                PortalList &visitedPortals,
						 	    bool includeVisitors,
							    bool recurseThruPortals,
							    PCZSceneNode *exclude)
    {
		...
		if (mEnclosureNode)
		{
			if (!t.intersects(mEnclosureNode->getSpaceAABB()))
			{
				// AABB of zone does not intersect t, just return.
				return;
			}
							    ...
							    PCZSceneNode *exclude )
    {
		...
		if (mEnclosureNode)
		{
			std::pair<bool, NIIf> nsect = t.intersects(mEnclosureNode->getSpaceAABB());
			if (!nsect.first)
			{
				// AABB of zone does not intersect t, just return.
				return;
			...
			while ( pit != mPortals.end() )
			{
				...
				if (portal->intersects(t))
				{
					...
					if (pit2 == visitedPortals.end())
					{
						...
						visitedPortals.push_front(portal);
						// recurse into the connected zone
						portal->getTargetZone()->_findNodes(t, list, visitedPortals,
                            includeVisitors, recurseThruPortals, exclude);
					}
				}
				pit++;
			}
	...
	*/
	...
	void OctreeZone::setZoneGeometry(const String &filename, PCZSceneNode * parentNode)
	{
		...
		GeometrySpace *ent = mPCZSM->createGeo(entityName , filename);
		// create a node for the entity
		PCZSceneNode * node;
		node = (PCZSceneNode*)(parentNode->create(nodeName));
	...
	//-------------------------------------------------------------------------
	void OctreeZone::getAABB(AABox & aabb)
	{
		// get the Octree bounding box
		aabb = mOctree->mBox;
	}
	//-------------------------------------------------------------------------
	void OctreeZone::init(AABox &box, int depth)
	{
		if ( mOctree != 0 )
			N_delete mOctree;

		mOctree = N_new Octree( this, 0 );
	...
	void OctreeZone::resize( const AABox &box )
	{
		// delete the octree
		N_delete mOctree;
		// create a new octree
	...
	bool OctreeZone::setProperty(PropertyID key, const void * val )
	{
		if ( key == "Size" )
		{
...
/*		else if ( key == "ShowOctree" )
	...
	{
		const AABox& box = zoneData -> mOctreeWorldAABB;
	...
	*/
	...
	void OctreeZone::addNodeToOctree( PCZSceneNode * n, Octree *octant, int depth )
	{
		...
		if ( ( depth < mMaxDepth ) && octant -> _isTwiceSize( bx ) )
		{
			...
			{
				octant -> mChildren[ x ][ y ][ z ] = N_new Octree( this, octant );
	...
	************************************************************************/
	...
	void OctreeZoneData::update(void)
	{
        ...
        SpaceNode::AttachList::iterator oit, oitend = mAssociatedNode->getAttachList().end();
        for(oit = mAssociatedNode->getAttachList().begin(); oit != oitend; ++oit)
		{
			SpaceObj * m = oit->second;
			// merge world bounds of object
			mOctreeWorldAABB.merge(m->getSpaceAABB(true));
		}
	...
	*/
	bool OctreeZoneData::_isIn(AABox & box)
	{
		// Always fail if not in the scene graph or box is null
		if(!mAssociatedNode->isEnable() || box.isNull())
            return false;

		Vector3 center = mAssociatedNode->getSpaceAABB().getMax().middle( mAssociatedNode->getSpaceAABB().getMin() );
		...
		Vector3 nodeSize = mAssociatedNode->getSpaceAABB().getMax() - mAssociatedNode->getSpaceAABB().getMin();
	...
	{
		return N_new OctreeZone(pczsm, zoneName);
	}
