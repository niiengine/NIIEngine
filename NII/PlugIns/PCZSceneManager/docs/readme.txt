...
a node require a node which has the correct size AAB (node->getSpaceAABB()).
...
Vector3::UNIT_NEGATIVE_Z or Vector3::Z.  The first corresponds to a portal
...
The user should use SpaceManager::createNodeImpl() to create all scene nodes.
...
SpaceNode.  Use the functions "PCZSceneNode::setHomeZone(PCZone * zone)" followed
...
using XXXSpaceQuery::setStartZone(zone) where "XXX" is Ray, Sphere, AABox, etc.
