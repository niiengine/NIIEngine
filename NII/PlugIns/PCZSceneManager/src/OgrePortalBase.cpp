...
PortalBase::PortalBase(SpaceID sid, const PORTAL_TYPE type) :
    SpaceObj(sid),
	mType(type),
	mCurrentHomeZone(0),
	mNewHomeZone(0),
...
const AABox & PortalBase::getSpaceAABB(bool derive) const
{
//	if (derive)
//	{
//		updateDerivedValues();
...
const Sphere& PortalBase::getSpaceBS(bool derive) const
{
//	if (derive)
//	{
//		updateDerivedValues();
...
void PortalBase::setNode(SpaceNode* sn)
{
	if(mSpaceNode) 
        ((SpaceNode*)mSpaceNode)->detach(this);
	if(sn) 
...
void PortalBase::calcDirectionAndRadius(void) const
{
	...
	case PORTAL_TYPE_QUAD:
		...
		for (int i=0;i<4;i++)
		{
			...
			min = min.floor(mCorners[i]);
			max = max.ceil(mCorners[i]);
		}
		mLocalCP *= 0.25f;
...
void PortalBase::updateDerivedValues(void) const
{
	...
	if (mSpaceNode)
	{
		...
		Matrix4 transform = mSpaceNode->getSpaceMatrix();
		Matrix3 rotation;
		...
		case PORTAL_TYPE_AABB:
			{
				AABox aabb;
                aabb.setMin(mCorners[0]);
                aabb.setMax(mCorners[1]);

				aabb = ((SpaceNode*)mSpaceNode)->getSpaceAABB();
				//aabb.transform(mSpaceNode->getSpaceMatrix());
				mDerivedCorners[0] = aabb.getMin();
				mDerivedCorners[1] = aabb.getMax();
	...
	// rebuild AAB.
	mPortalAAB = mSpaceAABB;
	mPortalAAB.merge(mPrevPortalAAB);
	mPrevPortalAAB = mSpaceAABB;
...
// NOTE: The node will become the portal's "associated" node (mSpaceNode).
void PortalBase::adjustNodeToMatch(SpaceNode *node)
{
	int i;
	...
	if(mType != PORTAL_TYPE_AABB && mType != PORTAL_TYPE_SPHERE)
	{
		// NOTE: Z is the basis for our local direction
		// orient the node to match the direction
		Quaternion q;
		q = Vector3::Z.getRotation(mDirection);
		node->setOri(q);
	}
...
bool PortalBase::intersects(const AABox & aab)
{
		...
		case PORTAL_TYPE_AABB:
			{
				// aab to aab check
				AABox aabb;
                aabb.setMin(mDerivedCorners[0]);
                aabb.setMax(mDerivedCorners[1]);
...
bool PortalBase::intersects(const PlaneSpace & pbv)
{
	// Only check if portal is enabled
	if(mEnabled)
	{
		...
		case PORTAL_TYPE_QUAD:
			{
				...
				PlaneList::const_iterator it = pbv.mPlanes.begin();
				while(it != pbv.mPlanes.end())
				{
					...
					for(int i = 0; i < 4; ++i)
					{
						if(plane.getSide(mDerivedCorners[i]) != pbv.mOuterType)
						{
							allOutside = false;
						}
					}
...
bool PortalBase::intersects(const Ray & ray)
{
	...
	if(mEnabled)
	{
		if(mType == PORTAL_TYPE_QUAD)
		{
			...
			std::pair<bool, NIIf> result = ray.intersects(mDerivedPlane);
		...
		else if (mType == PORTAL_TYPE_AABB)
		{
            ...
            aabb.setMin(mDerivedCorners[0])
            aabb.setMax(mDerivedCorners[1]);
			std::pair<bool, NIIf> result = ray.intersects(aabb);
			return result.first;
		}
		else // sphere
		{
			std::pair<bool, NIIf> result = ray.intersects(mDerivedSphere);
			return result.first;
		}
	}
	return false;
...
PortalBase::PortalIntersectResult PortalBase::intersects(PCZSceneNode * pczsn)
{
	...
	if (mEnabled)
	{
		...
		if (mType == PORTAL_TYPE_QUAD)
		{
			...
			Segment nodeSegment;
			nodeSegment.set(pczsn->getPrevPosition(), pczsn->getSpacePos());
			...
			if (getCapsule().intersects(nodeSegment))
			{
				...
				if (mDerivedPlane.getSide(pczsn->getSpacePos()) == Plane::PT_Negative &&
					mPrevDerivedPlane.getSide(pczsn->getPrevPosition()) != Plane::PT_Negative)
				{
					...
					Vector3 nodeHalfVector = pczsn->getSpaceAABB().getSize() * 0.5;
					Vector3 portalBox = Vector3(mRadius, mRadius, mRadius);
					portalBox = portalBox.floor(nodeHalfVector);
			...
			if(mDerivedSphere.intersects(pczsn->getSpaceAABB()) &&
				mDerivedPlane.getSide(pczsn->getSpaceAABB()) == Plane::PT_Merge)
			{
		...
		else if(mType == PORTAL_TYPE_AABB)
		{
            ...
            aabb.setMin(mDerivedCorners[0]);
            aabb.setMax(mDerivedCorners[1]);
			bool currentInside = aabb.intersects(pczsn->getSpacePos());
			if(mDirection == Vector3::Z)
			{
			...
			if (aabb.intersects(pczsn->getSpaceAABB()))
			{
				...
				AABox overlap = aabb.intersection(pczsn->getSpaceAABB());
				if(overlap != pczsn->getSpaceAABB())
				{
					return PortalBase::INTERSECT_NO_CROSS;
				}
			}
		...
		else
		{
			...
			//NIIf previousDistance2 = mPrevDerivedCP.distanceSquared(pczsn->getPrevPosition());
			NIIf currentDistance2 = mDerivedCP.distanceSquared(pczsn->getSpacePos());
			NIIf mRadius2 = mRadius * mRadius;
			if(mDirection == Vector3::Z)
			{
				// portal norm is "outward" pointing, look for going from outside to inside
	...
	if (otherPortal->getEnabled())
	{
		...
		if(getCapsule().intersects(otherPortalCapsule))
		{
			...
			case PORTAL_TYPE_QUAD:
				...
				if(otherPortal->getDerivedPlane().getSide(mDerivedCP) == Plane::PT_Negative &&
					otherPortal->getPrevDerivedPlane().getSide(mPrevDerivedCP) != Plane::PT_Negative)
				{
					// crossing occurred!
					return true;
				}
			...
			case PORTAL_TYPE_AABB:
				{
					...
					// the aabb (or vice versa) for crossing.
					AABox aabb;
					aabb.setMin(otherPortal->getDerivedCorner(0));
                    aabb.setMax(otherPortal->getDerivedCorner(1));
					bool currentInside = aabb.intersects(mDerivedCP);
					if (otherPortal->getSpaceDirection() == Vector3::Z)
					{
						// portal norm is "outward" pointing, look for going from outside to inside
						//if (previousInside == false &&
						if (currentInside == true)
			...
			case PORTAL_TYPE_SPHERE:
				{
					// for spheres we check if the center point went from being inside to being outside
					// the sphere surface (or vice versa) for crossing.
					//NIIf previousDistance2 = mPrevDerivedCP.distanceSquared(otherPortal->getPrevDerivedCP());
					NIIf currentDistance2 = mDerivedCP.distanceSquared(otherPortal->getDerivedCP());
					NIIf mRadius2 = Math::Sqr(otherPortal->getRadius());
					if (otherPortal->getSpaceDirection() == Vector3::Z)
					{
						// portal norm is "outward" pointing, look for going from outside to inside
						//if (previousDistance2 >= mRadius2 &&
...
const AABox& PortalBase::getAABB() const
{
	if (!mLocalsUpToDate)
	{
		calcDirectionAndRadius();
...
{
	PCZSceneNode* pczNode = (PCZSceneNode*)mSpaceNode;
...
PortalBase::PORTAL_TYPE PortalBaseFactory::getPortalType(const PropertyData * params)
{
	if(params)
	{
		PropertyData::const_iterator ni = params->find("type");
		if(ni != params->end())
		{
			if(ni->second == "Quad") 
                return PortalBase::PORTAL_TYPE_QUAD;
			else if(ni->second == "AABB") 
                return PortalBase::PORTAL_TYPE_AABB;
			else if(ni->second == "Sphere") 
                return PortalBase::PORTAL_TYPE_SPHERE;
		}
	}
