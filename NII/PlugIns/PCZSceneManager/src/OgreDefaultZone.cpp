...
namespace Ogre
{
	...
	*/
	...
	{
		if(pczsn == mEnclosureNode || pczsn->allowedToVisit() == false)
		{
    ...
    */
								  ...
								  bool aabbview)
    {
        ...
        while ( it != mHomeNodeList.end() )
        {
				...
				pczsn->getLastVisibleFromCamera() != camera)
            {
				...
				vis = camera ->isIntersect( pczsn -> getSpaceAABB() );
				if ( vis )
				{
	...
	void DefaultZone::_findNodes(const AABox & t, 
								  ...
								  PCZSceneNode * exclude)
	{
	    ...
	    while ( it != mHomeNodeList.end() )
	    {
			...
			if ( pczsn != exclude )
			{
				...
				if (it2 == list.end())
				{
					bool nsect = t.intersects(pczsn -> getSpaceAABB());
					if ( nsect )
					{
						list.insert( pczsn );
					}
    ...
    void DefaultZone::_findNodes(const PlaneSpace &t, 
							      PCZSceneNodeList &list, 
                                  PortalList &visitedPortals,
						 	      bool includeVisitors,
							      bool recurseThruPortals,
							      ...
							      PCZSceneNode *exclude )
    {
	    ...
	    while ( it != mHomeNodeList.end() )
	    {
			...
			if ( pczsn != exclude )
			{
				...
				if (it2 == list.end())
				{
					std::pair<bool, NIIf> nsect = t.intersects( pczsn -> getSpaceAABB() );
					if ( nsect.first )
					{
						list.insert( pczsn );
					}
	...
	bool DefaultZone::setProperty(PropertyID pid, const void * value)
	{
		return false;
	}
