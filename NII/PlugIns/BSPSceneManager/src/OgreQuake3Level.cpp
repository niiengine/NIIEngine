...
namespace Ogre 
{
    ...
    //-----------------------------------------------------------------------
    void Quake3Level::loadHeaderFromStream(DataStream * inStream)
    {
        ...
        mChunk = N_new MemDataStream((Nui8 *)pHeader, sizeof(bsp_header_t), false);
        // Grab all the counts, header only
        init(true);
		// Delete manually since delete and delete[] (as used by MemDataStream)
		// are not compatible
		OGRE_FREE(pHeader, MEMCATEGORY_RESOURCE);
    }
    //-----------------------------------------------------------------------
    void Quake3Level::loadFromStream(DataStream * stream)
    {
        mChunk = N_new MemDataStream(stream);
        init();
   ...
   //-----------------------------------------------------------------------
    void Quake3Level::init(bool headerOnly)
    {
        mHeader = (bsp_header_t*)mChunk->getData();
    ...
    //-----------------------------------------------------------------------
    void Quake3Level::extractLightmaps() const
    {
        // Lightmaps are always 128x128x24 (RGB)
        Nui8 * pLightmap = mLightmaps;
        for(int i = 0; i < mNumLightmaps; ++i)
        {
			...
			DataStream * stream = N_new MemDataStream(pLightmap, 128 * 128 * 3, false);
            Image img; 
			img.loadRawData(stream, 128, 128, PF_BYTE_RGB);
            N_Only(Texture).load(name.str(), N_Only(ResourceScheme).getWorldResourceGroupName(),
                img, TEX_TYPE_2D, PF_UNKNOWN, 0, 4.0f);
