...
namespace NII_MEDIA
{
    ...
    bool FLACCodec::Init(const CStdString &strFile, unsigned int filecache)
    {
        ...
        if (m_dll.FLAC__stream_decoder_init_stream(
            m_pFlacDecoder,
            DecoderReadCallback,
            DecoderSeekCallback,
            DecoderTellCallback,
        ...
        if (m_SampleRate==0 || m_Channels==0 || m_BitsPerSample==0 || m_TotalTime==0 || m_MaxFrameSize==0 || m_DataFormat == SF_Unknow)
        {
    ...
    //------------------------------------------------------------------------
    FLAC__StreamDecoderReadStatus FLACCodec::DecoderReadCallback(
        const FLAC__StreamDecoder * decoder, FLAC__byte buffer[], size_t * bytes,
            void * data)
    {
        FLACCodec* pThis = (FLACCodec *)data;
        if (!pThis)
    ...
    //------------------------------------------------------------------------
    FLAC__StreamDecoderSeekStatus FLACCodec::DecoderSeekCallback(
    ...
    //------------------------------------------------------------------------
    FLAC__StreamDecoderTellStatus FLACCodec::DecoderTellCallback(
        const FLAC__StreamDecoder * decoder, FLAC__uint64 * absolute_byte_offset,
            void * data)
    {
        FLACCodec * pThis=(FLACCodec*)data;
    ...
    //------------------------------------------------------------------------
    FLAC__StreamDecoderLengthStatus FLACCodec::DecoderLengthCallback(
    ...
    //------------------------------------------------------------------------
    FLAC__bool FLACCodec::DecoderEofCallback(const FLAC__StreamDecoder * decoder, void * data)
    {
        FLACCodec * pThis = (FLACCodec *)data;
        if (!pThis)
            return true;
    ...
    //------------------------------------------------------------------------
    FLAC__StreamDecoderWriteStatus FLACCodec::DecoderWriteCallback(
            ...
            const FLAC__int32 * const buffer[], void * data)
    {
        ...
        {
            for(NCount channel = 0; channel < frame->header.channels; ++channel)
            {
    ...
    //------------------------------------------------------------------------
    void FLACCodec::DecoderMetadataCallback(const FLAC__StreamDecoder * decoder,
        const FLAC__StreamMetadata * metadata, void * data)
    {
        ...
        {
            static AudioChannelType map[6][7] =
            {
                {ACT_FC, ACT_Unknow},
                {ACT_FL, ACT_FR, ACT_Unknow},
                {ACT_FL, ACT_FR, ACT_FC, ACT_Unknow},
                {ACT_FL, ACT_FR, ACT_BL, ACT_BR, ACT_Unknow},
                {ACT_FL, ACT_FR, ACT_FC, ACT_BL, ACT_BR, ACT_Unknow},
                {ACT_FL, ACT_FR, ACT_FC, ACT_LEF, ACT_BL, ACT_BR, ACT_Unknow}
            };
            ...
            if (metadata->data.stream_info.channels > 6)
                ...
                pThis->m_ChannelInfo = AudioChannel(map[metadata->data.stream_info.channels - 1]);
            ...
            {
            case  8:
                pThis->m_DataFormat = SF_U8;
                break;
            case 16:
                pThis->m_DataFormat = SF_S16;
                break;
            case 24:
                pThis->m_DataFormat = SF_S24W3;
                break;
            case 32:
                pThis->m_DataFormat = SF_F;
                break;
            }
    ...
    //------------------------------------------------------------------------
    void FLACCodec::DecoderErrorCallback(const FLAC__StreamDecoder * decoder,
