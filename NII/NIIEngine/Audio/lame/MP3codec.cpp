...
namespace NII_MEDIA
{
    ...
    #define ID3V2HEADERSIZE 10
        ...
        for (int i = 0; i < iScanSize; i++)
        {
            unsigned long mpegheader =
                (unsigned long)(((buffer[i] & 255) << 24) |
                    ((buffer[i + 1] & 255) << 16) | ((buffer[i + 2] & 255) << 8) |
                        ((buffer[i + 3] & 255)));

            // Do we have a Xing header before the first mpeg frame?
            if (buffer[i ] == 'X' && buffer[i + 1] == 'i' &&
            ...
            if ((i == firstValidFrameLocation) ||(
             (firstValidFrameLocation == -1) && (IsMp3FrameHeader(mpegheader))))
            {
                // skip mpeg header
                i += 4;
                int version = 0;
                ...
                */
                ...
                if (vbri[0] == 'V' && vbri[1] == 'B' && vbri[2] == 'R' && vbri[3] == 'I')
                {
    ...
    {
        if (b[0x9c] != 'L' || b[0x9d] != 'A' || b[0x9e] != 'M' || b[0x9f] != 'E')
            return false;
    ...
    unsigned int MP3Codec::IsID3v2Header(Nui8* pBuf, size_t bufLen)
    {
    ...
    //------------------------------------------------------------------------
    AudioChannel MP3Codec::GetChannelInfo()
    {
