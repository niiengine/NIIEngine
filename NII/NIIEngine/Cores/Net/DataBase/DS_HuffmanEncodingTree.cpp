...
namespace NII_NET
{
    ...
    void HuffmanEncodingTree::FreeMemory( void )
    {
        ...
        for (int i = 0; i < 256; i++)
            N_free(encodingTable[i].encoding);

        root = 0;
    }
    //------------------------------------------------------------------------
    // Given a frequency table of 256 elements, all with a frequency of 1 or more, generate the tree
    void HuffmanEncodingTree::GenerateFromFrequencyTable(Nui32 frequencyTable[256])
    {
        NCount counter;
        HuffmanEncodingTreeNode * node;
        ...
        {
            node = N_new HuffmanEncodingTreeNode();
            node->left = 0;
            node->right = 0;
            node->value = (Nui8) counter;
    ...
    #pragma warning(disable : 4127) // warning C4127: conditional expression is constant
        ...
        for(counter = 0; counter < 256; ++counter)
        {
            ...
            {
                if(tempPath[tempPathLength])   // write 1's and 0's because writing a bool will write the NetSerializer TYPE_CHECKING validation bits if that is defined along with the actual data bit, which is not what we want
                    ser.write1();
                else
                    ser.write0();
            }

            encodingTable[counter].bitLength = ser.getWriteOffset();
            encodingTable[counter].encoding = (Nui8 *)N_alloc(N_B_TO_b(ser.getWriteOffset()));
            memcpy(encodingTable[counter].encoding, ser.getData(), N_B_TO_b(ser.getWriteOffset()));
    ...
    //------------------------------------------------------------------------
    // Pass an array of bytes to array and a preallocated NetSerializer to receive the output
    void HuffmanEncodingTree::EncodeArray(Nui8 * input, size_t sizeInBytes,
        NetSerializer * output)
    {
        ...
        {
            output->writeBit(encodingTable[input[counter]].encoding, encodingTable[input[counter]].bitLength, false); // Data is left aligned
        }

        // Byte align the output so the unassigned remaining bits don't equate to some actual value
        if (output->getWriteOffset() % 8 != 0)
        {
            // Find an input that is longer than the remaining bits.  write out part of it to pad the output to be byte aligned.
            Nui8 remainingBits = (Nui8)(8 - (output->getWriteOffset() % 8));

            for (counter = 0; counter < 256; ++counter)
            {
                if (encodingTable[counter].bitLength > remainingBits)
                {
                    output->writeBit(encodingTable[counter].encoding, remainingBits, false); // Data is left aligned
                    break;
                }
            }
            N_assert(counter != 256);  // Given 256 elements, we should always be able to find an input that would be >= 7 bits
        }
    }
    //------------------------------------------------------------------------
    Nui32 HuffmanEncodingTree::DecodeArray(NetSerializer * input,
        NBitCount sizeInBits, size_t maxCharsToWrite, Nui8 * output)
    {
        HuffmanEncodingTreeNode * currentNode;

        Nui32 outputWriteIndex;
        outputWriteIndex = 0;
        currentNode = root;
        ...
        for(Nui32 counter = 0; counter < sizeInBits; ++counter)
        {
            bool tb;
            input->read(tb);
            if(tb == false)   // left!
    ...
    //------------------------------------------------------------------------
    // Pass an array of encoded bytes to array and a preallocated NetSerializer to receive the output
    void HuffmanEncodingTree::DecodeArray(Nui8 * input, NBitCount sizeInBits,
        NetSerializer * output)
    {
        HuffmanEncodingTreeNode * currentNode;
        ...
        NetSerializer ser(input, N_B_TO_b(sizeInBits), false);

        currentNode = root;

        // For each bit, go left if it is a 0 and right if it is a 1.  When we reach a leaf, that gives us the desired value and we restart from the root
        for(NCount counter = 0; counter < sizeInBits; ++counter)
        {
            bool tb;
            ser.read(tb);
            if(tb == false)   // left!
            ...
            if(currentNode->left == 0 && currentNode->right == 0)   // Leaf
            {
                output->writeBit(&(currentNode->value), sizeof(char) * 8, true); // Use writeBit instead of write(char) because we want to avoid TYPE_CHECKING
                currentNode = root;
            }
        }
    }
