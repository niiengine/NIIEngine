...
#ifndef __MULTILIST_H_
#define __MULTILIST_H_
...
namespace DataStructures
{
    ...
    void DeletePtr_RakNet(templateType &ptr) {N_delete ptr;}
    ...
    typedef Nui32 DefaultIndexType;
    ...
    public:
        ...
        void Push(const _DataType &d);
        void Push(const _DataType &d, const _KeyType &key);
        ...
        void PushOpposite(const _DataType &d);
        void PushOpposite(const _DataType &d, const _KeyType &key);
        ...
        void InsertAtIndex(const _DataType &d, _IndexType index);    
        ...
        void RemoveAtIndex(_IndexType position);
        ...
        void Clear(bool deallocateSmallBlocks = true);

        /// \brief Empties the list, first calling OP_Delete on all items.
        /// \details The list is not deallocated if it is small, unless \a deallocateSmallBlocks is true
        void ClearPointers(bool deallocateSmallBlocks = true);

        /// \brief Empty one item from the list, first calling OP_Delete on that item.
        void ClearPointer(_KeyType key);
        ...
        void Reallocate(_IndexType size);
    ...
    void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::CopySource( const Multilist& source )
    {
        ...
        else
        {
            ...
            mData = N_alloc_t(_DataType, dataSize);
            _IndexType i;
    ...
    {
        N_assert(position<GetSize());

        if (GetMultilistType()==ML_QUEUE)
        {
            if ( queueHead + position >= allocationSize )
                return mData[ queueHead + position - allocationSize ];
            else
                return mData[ queueHead + position ];
        }
    ...
    void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Push(const _DataType &d, const _KeyType &key)
    {
        ...
        {
            N_assert(GetMultilistType()==ML_ORDERED_LIST);
    ...
    _DataType &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Pop()
    {
        ...
        {
            N_assert(GetMultilistType()==ML_QUEUE);

            if ( ++queueHead == allocationSize )
                queueHead = 0;

            if ( queueHead == 0 )
                return mData[ allocationSize -1 ];

            dataSize--;
            return mData[ queueHead -1 ];
        }
    }
    ...
    _DataType &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Peek(void) const
    {
        ...
        if (GetMultilistType()==ML_UNORDERED_LIST || GetMultilistType()==ML_STACK || GetMultilistType()==ML_ORDERED_LIST)
        {
            return mData[dataSize-1];
        }
        else
        {
            N_assert(GetMultilistType()==ML_QUEUE);
            return mData[ queueHead ];
        }
    }
    ...
    void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::InsertAtIndex(const _DataType &d, _IndexType index)
    {
        ...
        else
        {
            ...
            while (readIndex >= index)
            {
                ...
                mData[trueWriteIndex]=mData[trueReadIndex];
    ...
    {
        N_assert(position < dataSize);
        N_assert(IsEmpty()==false);

        if (GetMultilistType()==ML_UNORDERED_LIST)
        {
            // Copy tail to current
            mData[position]=mData[dataSize-1];
        }
    ...
    bool Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::RemoveAtKey(_KeyType key, bool assertIfDoesNotExist)
    {
        ...
        {
            N_assert(assertIfDoesNotExist==false && "RemoveAtKey element not found");
            return false;
        }
    ...
    _IndexType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetIndexOf(_KeyType key) const
    {
        ...
        else if (GetMultilistType()==ML_UNORDERED_LIST || GetMultilistType()==ML_STACK)
        {
            for (i=0; i < dataSize; i++)
            {
                if (MLKeyRef<_KeyType>(key)==mData[i])
                    return i;
            }
            return (_IndexType)-1;
        }
    ...
    void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Clear( bool deallocateSmallBlocks)
    {
        ...
        if (deallocateSmallBlocks && allocationSize < 128 && mData)
        {
            N_free(mData);
            mData=0;
            allocationSize=0;
    ...
    void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::QSortAscending(_IndexType leftEdge, _IndexType rightEdge)
    {
        ...
        while (left<right)
        {
            if (mData[left] <= mData[pivotIndex])
            {
                ++left;
            }
            else
    ...
    {
        N_assert(_MultilistType==ML_VARIABLE_DURING_RUNTIME);
    ...
    {
        _DataType * newData = N_alloc_t(_DataType, newAllocationSize);
        _IndexType i;
        for (i=0; i < dataSize; i++)
    ...
    //        N_assert("Duplicate insertion into ordered list" && false);
    //        return;
    //    }
    ...
    {
        N_assert(_MultilistType!=ML_QUEUE);
...
void MultilistUnitTest(void)
{
    ...
    ml1.Reallocate(64);
    N_assert(ml1.IsEmpty());
    ml1.Push(53);
    N_assert(ml1.Peek()==53);
    N_assert(ml1.IsEmpty()==false);
    N_assert(ml1.Pop()==53);
    N_assert(ml1.IsEmpty()==true);
    for (int i=0; i < 512; i++)
    ml1.Push(i);
    N_assert(ml1.GetIndexOf(200)==200);
    N_assert(ml1.PeekOpposite()==0);
    N_assert(ml1.PopOpposite()==0);
    N_assert(ml1.PeekOpposite()==1);
    N_assert(ml1.Peek()==511);
    ml1.ReverseList();
    for (int i=0; i < 511; i++)
    N_assert(ml1[i]==511-i);
    N_assert(ml1.PeekOpposite()==511);
    N_assert(ml1.Peek()==1);
    oldSize = ml1.GetSize();
    ml1.RemoveAtIndex(0);
    N_assert(ml1.GetSize()==oldSize-1);
    N_assert(ml1.PeekOpposite()==1);
    ml1.Clear();
    N_assert(ml1.IsEmpty()==true);
    ...
    N_assert(ml1[3]==150);
    N_assert(ml1[4]==175);
    N_assert(ml1.GetIndexOf(25)==0);
    N_assert(ml1.GetIndexOf(50)==1);
    N_assert(ml1.GetIndexOf(100)==2);
    N_assert(ml1.GetIndexOf(150)==3);
    N_assert(ml1.GetIndexOf(175)==4);
    ml1.Clear();
    ...
    N_assert(ml1[3]==4);
    N_assert(ml1[4]==5);
    N_assert(ml1.GetIndexOf(1)==0);
    N_assert(ml1.GetIndexOf(2)==1);
    N_assert(ml1.GetIndexOf(3)==2);
    N_assert(ml1.GetIndexOf(4)==3);
    N_assert(ml1.GetIndexOf(5)==4);
    ml1.Clear();
    ...
    ml2.Reallocate(64);
    N_assert(ml2.IsEmpty());
    ml2.Push(53);
    N_assert(ml2.Peek()==53);
    N_assert(ml2.IsEmpty()==false);
    N_assert(ml2.Pop()==53);
    N_assert(ml2.IsEmpty()==true);
    for (int i=0; i < 512; i++)
    ml2.Push(i);
    N_assert(ml2.GetIndexOf(200)==200);
    N_assert(ml2.PeekOpposite()==0);
    N_assert(ml2.PopOpposite()==0);
    N_assert(ml2.PeekOpposite()==1);
    N_assert(ml2.Peek()==511);
    ml2.ReverseList();
    for (int i=0; i < 511; i++)
    N_assert(ml2[i]==511-i);
    N_assert(ml2.PeekOpposite()==511);
    N_assert(ml2.Peek()==1);
    oldSize = ml2.GetSize();
    ml2.RemoveAtIndex(0);
    N_assert(ml2.GetSize()==oldSize-1);
    N_assert(ml2.Peek()==1);
    N_assert(ml2.PeekOpposite()==510);
    ml2.Clear();
    N_assert(ml2.IsEmpty()==true);

    DataStructures::Multilist<ML_QUEUE, int> ml3;
    N_assert(ml3.IsEmpty());
    ml3.Push(53);
    N_assert(ml3.Peek()==53);
    N_assert(ml3.IsEmpty()==false);
    N_assert(ml3.Pop()==53);
    N_assert(ml3.IsEmpty()==true);
    for (int i=0; i < 512; i++)
    ml3.Push(i);
    N_assert(ml3.GetIndexOf(200)==200);
    N_assert(ml3.PeekOpposite()==511);
    N_assert(ml3.PopOpposite()==511);
    N_assert(ml3.PeekOpposite()==510);
    N_assert(ml3.Peek()==0);
    ml3.ReverseList();
    for (int i=0; i < 511; i++)
    N_assert(ml3[i]==511-1-i);
    N_assert(ml3.PeekOpposite()==0);
    N_assert(ml3.Peek()==510);
    oldSize = ml3.GetSize();
    ml3.RemoveAtIndex(0);
    N_assert(ml3.GetSize()==oldSize-1);
    N_assert(ml3.Peek()==509);
    N_assert(ml3.PeekOpposite()==0);
    ml3.Clear();
    N_assert(ml3.IsEmpty()==true);
    ...
    N_assert(ml3[3]==150);
    N_assert(ml3[4]==175);
    N_assert(ml3.GetIndexOf(25)==0);
    N_assert(ml3.GetIndexOf(50)==1);
    N_assert(ml3.GetIndexOf(100)==2);
    N_assert(ml3.GetIndexOf(150)==3);
    N_assert(ml3.GetIndexOf(175)==4);
    ml3.Clear();
    ...
    N_assert(ml3[3]==4);
    N_assert(ml3[4]==5);
    N_assert(ml3.GetIndexOf(1)==0);
    N_assert(ml3.GetIndexOf(2)==1);
    N_assert(ml3.GetIndexOf(3)==2);
    N_assert(ml3.GetIndexOf(4)==3);
    N_assert(ml3.GetIndexOf(5)==4);
    ml3.Clear();
    ...
    N_assert(ml3[3]==2);
    N_assert(ml3[4]==1);
    N_assert(ml3.GetIndexOf(1)==4);
    N_assert(ml3.GetIndexOf(2)==3);
    N_assert(ml3.GetIndexOf(3)==2);
    N_assert(ml3.GetIndexOf(4)==1);
    N_assert(ml3.GetIndexOf(5)==0);

    ml3.Clear();

    DataStructures::Multilist<ML_ORDERED_LIST, int> ml4;
    ml4.Reallocate(64);
    N_assert(ml4.IsEmpty());
    ml4.Push(53);
    N_assert(ml4.Peek()==53);
    N_assert(ml4.IsEmpty()==false);
    N_assert(ml4.Pop()==53);
    N_assert(ml4.IsEmpty()==true);
    for (int i=0; i < 512; i++)
    ml4.Push(i);
    N_assert(ml4.GetIndexOf(200)==200);
    N_assert(ml4.PeekOpposite()==0);
    N_assert(ml4.PopOpposite()==0);
    N_assert(ml4.PeekOpposite()==1);
    N_assert(ml4.Peek()==511);
    ml4.ReverseList();
    for (int i=0; i < 511; i++)
    N_assert(ml4[i]==511-i);
    N_assert(ml4.PeekOpposite()==511);
    N_assert(ml4.Peek()==1);
    oldSize = ml4.GetSize();
    ml4.RemoveAtIndex(0);
    N_assert(ml4.GetSize()==oldSize-1);
    N_assert(ml4.Peek()==1);
    N_assert(ml4.PeekOpposite()==510);
    ml4.Clear();
    N_assert(ml4.IsEmpty()==true);
    ...
    N_assert(ml5.GetIndexOf(0)==0);
    N_assert(ml5.GetIndexOf(5)==5);
    N_assert(ml5.GetIndexOf(15)==15);
    N_assert(ml5.GetIndexOf(16)==-1);
    ml5.RemoveAtKey(0,true);
    N_assert(ml5.GetIndexOf(1)==0);
    KeyAndValue *iPtr = ml5.GetPtr(5);
    N_assert(iPtr);
    N_assert(iPtr->value=105);
