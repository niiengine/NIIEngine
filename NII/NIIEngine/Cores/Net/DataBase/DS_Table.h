...
namespace DataStructures
{
    ...
    public:
        ...
        struct _EngineAPI Cell
        {
            Cell();
            ~Cell();
            Cell(NIId numericValue, char *charValue, void *ptr, ColumnType type);
            void SetByType(NIId numericValue, char *charValue, void *ptr, ColumnType type);
            void Clear(void);
            
            /// Numeric
            void Set(int input);
            void Set(Nui32 input);
            void Set(NIId input);
            ...
            RakString ToString(ColumnType columnType);
        ...
        /// \internal
        // Note: If this structure is changed the struct in the swig files need to be changed as well
        struct _EngineAPI ColumnDescriptor
        {
            ColumnDescriptor();
            ~ColumnDescriptor();
        ...
        // Note: If this structure is changed the struct in the swig files need to be changed as well
        struct _EngineAPI Row
        {
            // list of cells
            DataStructures::List<Cell*> cells;

            /// Numeric
            void UpdateCell(Nui32 columnIndex, NIId value);

            /// String
            void UpdateCell(Nui32 columnIndex, const char *str);

            /// Binary
            void UpdateCell(Nui32 columnIndex, int byteLength, const char *data);
        };
        ...
        // Note: If this structure is changed the struct in the swig files need to be changed as well
        struct _EngineAPI FilterQuery
        {
            FilterQuery();
            ~FilterQuery();
            FilterQuery(Nui32 column, Cell *cell, FilterQueryType op);
        ...
        Nui32 AddColumn(const char columnName[_TABLE_MAX_COLUMN_NAME_LENGTH], ColumnType columnType);

        /// \brief Removes a column by index
        /// \param[in] columnIndex The index of the column to remove
        void RemoveColumn(Nui32 columnIndex);

        /// \brief Gets the index of a column by name
        /// \details Column indices are stored in the order they are added.
        /// \param[in] columnName The name of the column
        /// \return The index of the column, or (Nui32)-1 if no such column
        Nui32 ColumnIndex(char columnName[_TABLE_MAX_COLUMN_NAME_LENGTH]) const;
        Nui32 ColumnIndex(const char *columnName) const;
        ...
        char* ColumnName(Nui32 index) const;

        /// \brief Returns the type of a column, referenced by index
        /// \param[in] index The index of the column
        /// \return The type of the column
        ColumnType GetColumnType(Nui32 index) const;

        /// Returns the number of columns
        /// \return The number of columns in the table
        Nui32 GetColumnCount(void) const;

        /// Returns the number of rows
        /// \return The number of rows in the table
        Nui32 GetRowCount(void) const;
        ...
        Table::Row* AddRow(Nui32 rowId);
        Table::Row* AddRow(Nui32 rowId, DataStructures::List<Cell> &initialCellValues);
        Table::Row* AddRow(Nui32 rowId, DataStructures::List<Cell*> &initialCellValues, bool copyCells=false);

        /// \brief Removes a row specified by rowId.
        /// \param[in] rowId The ID of the row
        /// \return true if the row was deleted. False if not.
        bool RemoveRow(Nui32 rowId);
        ...
        bool UpdateCell(Nui32 rowId, Nui32 columnIndex, int value);
        bool UpdateCell(Nui32 rowId, Nui32 columnIndex, char *str);
        bool UpdateCell(Nui32 rowId, Nui32 columnIndex, int byteLength, char *data);
        bool UpdateCellByIndex(Nui32 rowIndex, Nui32 columnIndex, int value);
        bool UpdateCellByIndex(Nui32 rowIndex, Nui32 columnIndex, char *str);
        bool UpdateCellByIndex(Nui32 rowIndex, Nui32 columnIndex, int byteLength, char *data);

        /// \brief Note this is much less efficient to call than GetRow, then working with the cells directly.
        /// Numeric, string, binary
        void GetCellValueByIndex(Nui32 rowIndex, Nui32 columnIndex, int *output);
        void GetCellValueByIndex(Nui32 rowIndex, Nui32 columnIndex, char *output);
        void GetCellValueByIndex(Nui32 rowIndex, Nui32 columnIndex, char *output, int *outputLength);
        ...
        Row* GetRowByID(Nui32 rowId) const;
        ...
        /// \return The desired row, or 0 if no such row.
        Row* GetRowByIndex(Nui32 rowIndex, Nui32 *key) const;
        ...
        void QueryTable(Nui32 *columnIndicesSubset, Nui32 numColumnSubset, FilterQuery *inclusionFilters, Nui32 numInclusionFilters, Nui32 *rowIds, Nui32 numRowIDs, Table *result);
        ...
        void SortTable(Table::SortQuery *sortQueries, Nui32 numSortQueries, Table::Row** out);
        ...
        /// \brief Direct access to make things easier.
        const DataStructures::BPlusTree<Nui32, Row*, _TABLE_BPLUS_TREE_ORDER>& GetRows(void) const;

        /// \brief Get the head of a linked list containing all the row data.
        DataStructures::Page<Nui32, Row*, _TABLE_BPLUS_TREE_ORDER> * GetListHead(void);

        /// \brief Get the first free row id.
        /// This could be made more efficient.
        Nui32 GetAvailableRowId(void) const;

        Table& operator = ( const Table& input );

    protected:
        Table::Row* AddRowColumns(Nui32 rowId, Row *row, DataStructures::List<Nui32> columnIndices);

        void DeleteRow(Row *row);

        void QueryRow(DataStructures::List<Nui32> &inclusionFilterColumnIndices, DataStructures::List<Nui32> &columnIndicesToReturn, Nui32 key, Table::Row* row, FilterQuery *inclusionFilters, Table *result);
        ...
        DataStructures::BPlusTree<Nui32, Row*, _TABLE_BPLUS_TREE_ORDER> rows;
