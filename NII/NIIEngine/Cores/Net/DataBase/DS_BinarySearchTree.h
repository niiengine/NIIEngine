...
namespace DataStructures
{
     ...
     * Nui32 Height(<index>) - Returns the height of the tree at the optional specified starting index.  Default is the root
     ...
     * Nui32 Height() - Returns the height of the tree
     * Nui32 size() - returns the size of the BinarySearchTree
     ...
     * A.Add(15);
     * A.Add(5);
     * int * array = N_alloc(A.Size() * sizeof(int));
    ...
    class _EngineAPI BinarySearchTree
    {
    public:
        struct node
        {
        ...
        Nui32 Size(void);
        void Clear();
        Nui32 Height(node * starting_node = 0);
    ...
    protected:
        ...
        Nui32 HeightRecursive( node* current );
        Nui32 BinarySearchTree_size;
        node*& Find( const BinarySearchTreeType& element, node** parent );
        node*& FindParent( const BinarySearchTreeType& element );
        void DisplayPostorderRecursive( node* current, BinarySearchTreeType* return_array, Nui32& index );
    ...
    template <class BinarySearchTreeType>
    class _EngineAPI AVLBalancedBinarySearchTree : public BinarySearchTree<BinarySearchTreeType>
    {
    ...
    template <class BinarySearchTreeType>
    Nui32 BinarySearchTree<BinarySearchTreeType>::Size( void )
    {
        return BinarySearchTree_size;
    }

    template <class BinarySearchTreeType>
    Nui32 BinarySearchTree<BinarySearchTreeType>::Height( typename BinarySearchTree::node* starting_node )
    {
    ...
    template <class BinarySearchTreeType>
    Nui32 BinarySearchTree<BinarySearchTreeType>::HeightRecursive( typename BinarySearchTree::node* current )
    {
        Nui32 left_height = 0, right_height = 0;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
    ...
    typename BinarySearchTree<BinarySearchTreeType>::node * BinarySearchTree<BinarySearchTreeType>::Del( const BinarySearchTreeType& input)
    {
        ...
        if ( ( current->right ) == 0 && ( current->left ) == 0 )    // Leaf node, just remove it
        {
            ...
            N_delete node_to_delete->item;
            N_delete node_to_delete;
            ...
            if ( ( current->right ) != 0 && ( current->left ) == 0 )   // Node has only one child, delete it and cause the parent to point to that child
            {
                ...
                else // Go right, then as left as far as you can
                {
                    ...
                    // Replace the value held by the node to N_delete with the value pointed to by current;
    ...
    typename BinarySearchTree<BinarySearchTreeType>::node * BinarySearchTree<BinarySearchTreeType>::Add ( const BinarySearchTreeType& input)
    {
        ...
        if (BinarySearchTree_size == 0)
        {
            ...
            root = N_new typename BinarySearchTree::node();
            root->item = N_new BinarySearchTreeType();
            *( root->item ) = input;
            root->left = 0;
            root->right = 0;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
            ...
            while ( true )    // This loop traverses the tree to find a spot for insertion
            {
                ...
                if ( input < *( current->item ) )
                {
                    if ( current->left == 0 )
                    {
                        current->left = N_new typename BinarySearchTree::node();
                        current->left->item = N_new BinarySearchTreeType();
                        current = current->left;
                        current->left = 0;
                        current->right = 0;
                        *( current->item ) = input;
                    ...
                    if(input > *(current->item))
                    {
                        if (current->right == 0)
                        {
                            current->right = N_new typename BinarySearchTree::node();
                            current->right->item = N_new BinarySearchTreeType();
                            current = current->right;
                            current->left = 0;
                            current->right = 0;
                            *(current->item) = input;
    ...
    template <class BinarySearchTreeType>
    void BinarySearchTree<BinarySearchTreeType>::DisplayPostorderRecursive( typename BinarySearchTree::node* current, BinarySearchTreeType* return_array, Nui32& index )
    {
        if ( current->left != 0 )
