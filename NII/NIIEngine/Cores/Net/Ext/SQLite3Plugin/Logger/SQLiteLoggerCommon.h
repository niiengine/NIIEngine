...
namespace NII_NET
{
    ...
    struct LogParameter
    {
        ...
        template <> LogParameter(const RGBImageBlob *t) : type(SQLLPDT_IMAGE), size(t->size), data(t->data), imageDescriptorFormat(t->imageDescriptorFormat), mPixelSize(t->mPixelSize), imageWidth(t->imageWidth)  {}
        template <> LogParameter(const BlobDescriptor t) : type(SQLLPDT_BLOB), size(t.size), data(t.data) {}
        template <> LogParameter(const RGBImageBlob t) : type(SQLLPDT_IMAGE), size(t.size), data(t.data), imageDescriptorFormat(t.imageDescriptorFormat), mPixelSize(t.mPixelSize), imageWidth(t.imageWidth) {}
        */
