...
{
    //------------------------------------------------------------------------
    int VoiceChannelComp( const UniqueID &key, VoiceChannel * const &data )
    {
        if (key < data->guid)
            return -1;
        if (key == data->guid)
    ...
    void RakVoice::Init(unsigned short sampleRate, unsigned bufferSizeBytes)
    {
        ...
        N_assert(sampleRate==8000 || sampleRate==16000 || sampleRate==32000);
        this->sampleRate=sampleRate;
        this->bufferSizeBytes=bufferSizeBytes;
        bufferedOutputCount=bufferSizeBytes/SAMPLESIZE;
        bufferedOutput = (NIIf*) rakMalloc_Ex(sizeof(NIIf)*bufferedOutputCount);
        unsigned i;
    ...
    void RakVoice::SetLoopbackMode(bool enabled)
    {
        if (enabled)
        {
            ...
            p.data = out.getData();
            p.mAddress = 0;
            p.guid = UniqueID::INVALID;
            p.length = out.getSize();
            OpenChannel(&p);
        }
        else
        {
            FreeChannelMemory(UniqueID::INVALID);
        }
    ...
    //------------------------------------------------------------------------
    void RakVoice::RequestVoiceChannel(UniqueID recipient)
    {
        // send a reliable ordered message to the other system to open a voice channel
        NetSerializer out;
        out.write((unsigned char)ID_RAKVOICE_OPEN_CHANNEL_REQUEST);
        out.write((int32_t)sampleRate);
        send(&out, HIGH_PRIORITY, RELIABLE_ORDERED,0,recipient,false);
    }
    //------------------------------------------------------------------------
    void RakVoice::CloseVoiceChannel(UniqueID recipient)
    {
    ...
    void RakVoice::CloseAllChannels(void)
    {
        ...
        {
            send(&out, HIGH_PRIORITY, RELIABLE_ORDERED,0,voiceChannels[index]->guid,false);
    ...
    //------------------------------------------------------------------------
    bool RakVoice::SendFrame(UniqueID recipient, void *inputBuffer)
    {
        bool objectExists;
        unsigned index;
    ...
    #ifdef _DEBUG
            N_assert(remainingBufferSize>0 && remainingBufferSize <= totalBufferSize);
        //	printf("SendFrame: buff=%i writeIndex=%i readIndex=%i\n",remainingBufferSize, channel->outgoingWriteIndex, channel->outgoingReadIndex);
            ...
            // Copy encoded sound to the outgoing buffer for that channel.  This has to be fast, since this function is likely to be called  a locked buffer
    ...
    #ifdef _DEBUG
            N_assert(channel->outgoingWriteIndex+bufferSizeBytes <= totalBufferSize);
    #endif
            ...
            N_assert(channel->outgoingWriteIndex <= totalBufferSize);
    #endif
    ...
    //------------------------------------------------------------------------
    bool RakVoice::IsSendingVoiceDataTo(UniqueID recipient)
    {
        bool objectExists;
        unsigned index;
        index = voiceChannels.GetIndexFromKey(recipient, &objectExists);
    ...
    //------------------------------------------------------------------------
    unsigned RakVoice::GetBufferedBytesToSend(UniqueID guid) const
    {
        bool objectExists;
        VoiceChannel *channel;
        unsigned totalBufferSize=bufferSizeBytes * FRAME_OUTGOING_BUFFER_COUNT;
        if (guid!=UniqueID::INVALID)
        {
    ...
    //------------------------------------------------------------------------
    unsigned RakVoice::GetBufferedBytesToReturn(UniqueID guid) const
    {
        bool objectExists;
        VoiceChannel *channel;
        unsigned totalBufferSize=bufferSizeBytes * FRAME_OUTGOING_BUFFER_COUNT;
    ...
    void RakVoice::update(void)
    {
        ...
        // 1 byte for ID, and 2 bytes(short) for SocketMessage number
        ...
        TimeDurMS currentTime = N_Engine().getTimer().getMS();
            ...
            if (currentTime - channel->lastSend > 50) // Throttle to 20 sends a second
            {
                ...
                if (speexFramesAvailable > 0)
                {
                    ...
                    while (speexFramesAvailable-- > 0)
                    {
                        ...
                        {
    #ifdef _DEBUG
                            N_assert(speexBlockSize < 2048-1);
    #endif
                            unsigned t;
    ...
    */
                        ...
                        N_assert(bytesWritten!=2048-headerSize);
    #endif
    ...
    printf("%i ", voicePacketsSent++);
                        ...
                        NetSerializer tempOutputBs((unsigned char*) tempOutput,bytesWritten+headerSize,false);
                        send(&tempOutputBs, HIGH_PRIORITY, UNRELIABLE,0,channel->guid,false);
                        ...
                        if (loopbackMode)
                        {
                            ...
                            p.guid=channel->guid;
                            p.mAddress = mPrc2->getAddress(p.guid);
                            OnVoiceData(&p);
                        }
                    }
            ...
            if (channel->copiedOutgoingBufferToBufferedOutput==false)
            {
                ...
                {
                    // Block running this again until the user calls ReceiveFrame since every call to ReceiveFrame only gets zero or one output blocks 
                    // each channel
                    ...
                    {
                        // write short to NIIf so if the range goes over the range of a NIIf we can still add and subtract the correct final value.
    ...
    //------------------------------------------------------------------------
    ConnectReceiveType RakVoice::onMessage(SocketMessage * msg)
    {
        N_assert(msg);
        ...
        case ID_RAKVOICE_OPEN_CHANNEL_REQUEST:
            OnOpenChannelRequest(msg);
        break;
        case ID_RAKVOICE_OPEN_CHANNEL_REPLY:
            OnOpenChannelReply(msg);
            break;
        case ID_RAKVOICE_CLOSE_CHANNEL:
            FreeChannelMemory(msg->guid);
        break;
    ...
    //------------------------------------------------------------------------
    void RakVoice::onConnectClose(const Address & address, UniqueID id, ConnectCloseType lostConnectionReason)
    {
        (void)address;

        if (lostConnectionReason == CCT_Close)
            CloseVoiceChannel(id);
        else
            FreeChannelMemory(id);
    }
    //------------------------------------------------------------------------
    void RakVoice::OnOpenChannelRequest(SocketMessage * msg)
    {
        if (voiceChannels.HasData(msg->guid))
            return;
        ...
        out.write((int32_t)sampleRate);
        send(&out, HIGH_PRIORITY, RELIABLE_ORDERED,0,msg->mAddress,false);
    }
    //------------------------------------------------------------------------
    void RakVoice::OnOpenChannelReply(SocketMessage * msg)
    {
        if (voiceChannels.HasData(msg->guid))
            return;
        OpenChannel(msg);
    ...
    //------------------------------------------------------------------------
    void RakVoice::OpenChannel(SocketMessage * msg)
    {
        NetSerializer in(msg->data, msg->length, false);
        in.skipRead(1, 0);
        FreeChannelMemory(msg->guid);

        VoiceChannel *channel=OP_NEW<VoiceChannel>();
        channel->guid=msg->guid;
        channel->isSendingVoiceData=false;
        int sampleRate;
        in.read(sampleRate);
        ...
        N_assert((channel->enc_state)&&(channel->dec_state));
        ...
        voiceChannels.Insert(msg->guid, channel, true);
    }
    //------------------------------------------------------------------------
    ...
    {
        N_assert((complexity>=0)&&(complexity<=10));
    ...
    //------------------------------------------------------------------------
    void RakVoice::FreeChannelMemory(UniqueID recipient)
    {
        bool objectExists;
        unsigned index;
        index = voiceChannels.GetIndexFromKey(recipient, &objectExists);
    ...
    //------------------------------------------------------------------------
    void RakVoice::OnVoiceData(SocketMessage *msg)
    {
        bool objectExists;
        unsigned index;
        ...
        index = voiceChannels.GetIndexFromKey(msg->guid, &objectExists);
        if (objectExists)
        {
            SpeexBits speexBits;
            speex_bits_init(&speexBits);
            channel=voiceChannels[index];
            memcpy(&packetMessageNumber, msg->data+1, sizeof(unsigned short));
            ...
            int maxSkip = (int)(100.0f / (NIIf) sampleRate);
            ...
            speex_bits_read_from(&speexBits, (char*)(msg->data+headerSize), msg->length-headerSize);
