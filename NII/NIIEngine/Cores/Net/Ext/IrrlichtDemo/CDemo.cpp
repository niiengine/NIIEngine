...
void CDemo::run()
{
	...
	// set ambient light
	smgr->setAmbient ( video::SColorf ( 0x00c0c0c0 ) );
...
//	wchar_t tmp[255];
	...
	playerReplica->playerName = RakString(dest);
...
*/
			RakString curMsg = GetCurrentMessage();
...
void CDemo::loadSceneData()
{
	...
	*/
	...
	campFire = sm->addParticleSpaceSceneNode(false);
...
TimeDurMS CDemo::shootFromOrigin(core::vector3df camPosition, core::vector3df camAt)
{
	scene::ISceneManager* sm = device->getSceneManager();
...
void CDemo::shoot()
{
	...
	br->shotLifetime = N_Engine().getTimer().getMS() + shootFromOrigin(camPosition, camAt);
...
void CDemo::createParticleImpacts()
{
	...
	for (s32 i=0; i<(s32)Impacts.size(); ++i)
		if (now > Impacts[i].when)
		{
			...
			scene::IParticleSpaceSceneNode* pas = 0;

			pas = sm->addParticleSpaceSceneNode(false, 0, -1, Impacts[i].pos);
...
{
	Address * facilitatorSystemAddress(DEFAULT_NAT_PUNCHTHROUGH_FACILITATOR_IP, DEFAULT_NAT_PUNCHTHROUGH_FACILITATOR_PORT);
	SocketMessage * msg;
	TimeDurMS curTime = N_Engine().getTimer().getMS();
	RakString targetName;
	for(msg = mLink->recevie(); msg; N_delete msg, msg=mLink->recevie())
	{
        String temp;
        msg->mAddress.read(temp, false)
		if(strcmp(temp, DEFAULT_NAT_PUNCHTHROUGH_FACILITATOR_IP)==0)
		{
			targetName="NATPunchthroughServer";
		}
		else
		{
			msg->mAddress.read(targetName, true);
		}
		...
		case ID_IP_RECENTLY_CONNECTED:
			{
				PushMessage(RakString("This IP address recently connected from ") + targetName + RakString("."));
				if (msg->mAddress==facilitatorSystemAddress)
					PushMessage("Multiplayer will not work without the NAT punchthrough server!");
			}
			break;
		case ID_INCOMPATIBLE_PROTOCOL_VERSION:
			{
				PushMessage(RakString("Incompatible protocol version from ") + targetName + RakString("."));
				if (msg->mAddress==facilitatorSystemAddress)
					PushMessage("Multiplayer will not work without the NAT punchthrough server!");
			}
			break;
		case ID_DISCONNECTION_NOTIFICATION:
			{
				PushMessage(RakString("Disconnected from ") + targetName + RakString("."));
				if (msg->mAddress==facilitatorSystemAddress)
		...
		case ID_CONNECTION_LOST:
			{
				PushMessage(RakString("Connection to ") + targetName + RakString(" lost."));
				if (msg->mAddress==facilitatorSystemAddress)
					isConnectedToNATPunchthroughServer=false;
			}
			break;
		case ID_NO_FREE_INCOMING_CONNECTIONS:
			{
				PushMessage(RakString("No free incoming connections to ") + targetName + RakString("."));
				if (msg->mAddress==facilitatorSystemAddress)
					PushMessage("Multiplayer will not work without the NAT punchthrough server!");
			}
			break;
		case ID_NEW_INCOMING_CONNECTION:
			{
				...
				{
					PushMessage(RakString("Sending player list to new connection"));
					fullyConnectedMesh2->StartVerifiedJoin(msg->guid);
				}
			}
			break;
		case ID_FCM2_VERIFIED_JOIN_START:
			{
				...
				fullyConnectedMesh2->GetVerifiedJoinRequiredProcessingList(msg->guid, addresses, guids);
		...
		case ID_FCM2_VERIFIED_JOIN_FAILED:
			{
				PushMessage(RakString("Failed to join game session"));
			}
			break;
		case ID_FCM2_VERIFIED_JOIN_CAPABLE:
			{
				fullyConnectedMesh2->RespondOnVerifiedJoinCapable(msg, true, 0);
			}
			break;
		case ID_FCM2_VERIFIED_JOIN_ACCEPTED:
			{
				...
				fullyConnectedMesh2->GetVerifiedJoinAcceptedAdditionalData(msg, &thisSystemAccepted, systemsAccepted, 0);
                ...
                {
                    String temp4;
                    systemsAccepted[0].read(temp4);
					PushMessage(RakString("System %s joined the mesh\n", temp4));
                }
				// DataStructures::List<UniqueID> participantList;
				// fullyConnectedMesh2->GetParticipantList(participantList);

				for (unsigned int i=0; i < systemsAccepted.Size(); i++)
					replicaManager3->PushConnection(replicaManager3->AllocConnection(mLink->GetSystemAddressFromGuid(systemsAccepted[i]), systemsAccepted[i]));
			}
			break;
		case ID_CONNECTION_REQUEST_ACCEPTED:
			{
				PushMessage(RakString("Connection request to ") + targetName + RakString(" accepted."));
				if (msg->mAddress==facilitatorSystemAddress)
				{
					...
					cloudQuery.keys.Push(CloudKey("IrrlichtDemo",0));
					cloudClient->Get(&cloudQuery, msg->guid);
				}
			}
			break;
		case ID_FCM2_NEW_HOST:
			{
				if (msg->guid == mLink->getGuid())
				{
					// Original host dropped. I am the new session host. Upload to the cloud so new players join this system.
					CloudKey cloudKey("IrrlichtDemo",0);
					cloudClient->Post(&cloudKey, 0, 0, mLink->getGUID(facilitatorSystemAddress));
				}
			}
			break;
		...
		case ID_CONNECTION_ATTEMPT_FAILED:
			{
				PushMessage(RakString("Connection attempt to ") + targetName + RakString(" failed."));
				if (msg->mAddress==facilitatorSystemAddress)
					PushMessage("Multiplayer will not work without the NAT punchthrough server!");
			}
			break;
		case ID_NAT_TARGET_NOT_CONNECTED:
			{
				UniqueID recipientGuid;
				NetSerializer bs(msg->data,msg->length,false);
				bs.skipRead(1, 0);
				bs.read(recipientGuid);
				recipientGuid.read(targetName);
				PushMessage(RakString("NAT target ") + targetName + RakString(" not connected."));
			}
			break;
		case ID_NAT_TARGET_UNRESPONSIVE:
			{
				...
				bs.read(recipientGuid);
				recipientGuid.read(targetName);
				PushMessage(RakString("NAT target ") + targetName + RakString(" unresponsive."));
			}
			break;
		case ID_NAT_CONNECTION_TO_TARGET_LOST:
			{
				...
				bs.read(recipientGuid);
				recipientGuid.read(targetName);
				PushMessage(RakString("NAT target connection to ") + targetName + RakString(" lost."));
			}
			break;
		case ID_NAT_ALREADY_IN_PROGRESS:
			{
				...
				bs.read(recipientGuid);
				recipientGuid.read(targetName);
				PushMessage(RakString("NAT punchthrough to ") + targetName + RakString(" in progress (skipping)."));
			}
			break;
		...
		case ID_NAT_PUNCHTHROUGH_SUCCEEDED:
			{
				...
				if (msg->data[1]==1)
				{
					PushMessage(RakString("Connecting to existing game instance"));
                    msg->mAddress.read(temp3, false)
					ConnectResultType car = mLink->Connect(temp3.c_str(), msg->mAddress.getHSBPort(), 0, 0);
		...
		case ID_ADVERTISE_SYSTEM:
			if (msg->guid != mLink->getGUID(0))
			{
				String hostIP;
				msg->mAddress.read(hostIP, false);
				ConnectResultType car = mLink->Connect(hostIP.c_str(), 
                    msg->mAddress.getHSBPort(),0,0);
				N_assert(car == CONNECTION_ATTEMPT_STARTED);
			}
			break;
		}
...
void CDemo::PushMessage(RakString rs)
{
	outputMessages.Push(rs);
	if (whenOutputMessageStarted == 0)
	{
		whenOutputMessageStarted = N_Engine().getTimer().getMS();
	}
}
