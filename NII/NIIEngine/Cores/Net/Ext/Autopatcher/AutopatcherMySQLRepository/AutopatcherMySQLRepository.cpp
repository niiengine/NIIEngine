...
namespace NII_NET
{
    ...
    bool AutopatcherMySQLRepository::GetChangelistSinceDate(const char *applicationName, FileList *addedOrModifiedFilesWithHashData, FileList *deletedFiles, NIId sinceDate)
    {
        char query[512];
    ...
    bool AutopatcherMySQLRepository::GetMostRecentChangelistWithPatches(RakString &applicationName, FileList *patchedFiles, FileList *addedFiles, FileList *addedOrModifiedFileHashes, FileList *deletedFiles, NIId *priorRowPatchTime, NIId *mostRecentRowPatchTime)
    {
        // Not yet implemented
        return false;
    }
    ...
    bool AutopatcherMySQLRepository::UpdateApplicationFiles(const char *applicationName, const char *applicationDirectory, const char *userName, FileListProgress *cb)
    {
        ...
        while ((row = mysql_fetch_row (result)) != 0)
        {
            ...
            if (fi.createFile)
            {
                N_assert(mysql_fetch_lengths (result) [1] == HASH_LENGTH);  // check the data is sensible
    ...
    unsigned int AutopatcherMySQLRepository::GetFilePart(const char * filename, 
