...
namespace NII_NET
{
    ...
    bool AutopatcherPostgreRepository::GetChangelistSinceDate(const char * applicationName,
        FileList * addedOrModifiedFilesWithHashData, FileList * deletedFiles, NIId sinceDate)
    {
        PGresult * result;
        char query[512];
        if (strlen(applicationName)>100)
            return false;
        RakString escapedApplicationName = GetEscapedString(applicationName);
        ...
        int rowIndex;
        int fileLength;
        N_assert(PQfsize(result, fileLengthColumnIndex)==sizeof(fileLength));
    ...
    //				unsigned int hash2 = SuperFastHashFile("C:/temp/AutopatcherClient/RakNet_Icon_Final.psd");
                ...
                //			formats[0]=PQEXECPARAM_FORMAT_BINARY;
                            ...
                            int fileIndex=0;
                            char *file = OP_NEW char[fileLengthInt];
    ...
    */
                            ...
                            unsigned int hash1 = SuperFastHashFile("C:/temp/RakNet_Icon_Final.psd");
                            unsigned int hash2 = FastHash(content, contentLength);
                            */
    ...
    //						OP_DELETE_ARRAY file;
                        ...
                        else
                        {
                            ...
                            if (useReference==false)
                                temp = OP_NEW_ARRAY<char>(patchLength + HASH_LENGTH);
                            else
                                temp = 0;
    ...
    bool AutopatcherPostgreRepository::GetMostRecentChangelistWithPatches(RakString &applicationName,
        FileList *patchedFiles, FileList *addedFiles, FileList *addedOrModifiedFileHashes,
            FileList *deletedFiles, NIId *priorRowPatchTime, NIId *mostRecentRowPatchTime)
    {
        PGresult *result;
        char query[1024];
        ...
        RakString escapedApplicationName2 = GetEscapedString(applicationName);
        PGresult *result2;
        char *ts;
        int numRows2;
        ...
        {
            sprintf(lastError,"ERROR: Query is bad in function GetMostRecentChangelistWithPatches\n");
            PQclear(result);
            PQclear(result2);
            return false;
        }
        ...
        *mostRecentRowPatchTime=atof(ts);
        ...
        if (ExecuteBlockingCommand(query, &result2, false) == false)
        {
            sprintf(lastError,"ERROR: Query is bad function GetMostRecentChangelistWithPatches\n");
            PQclear(result);
            PQclear(result2);
            return false;
        }
    ...
    bool AutopatcherPostgreRepository2::GetMostRecentChangelistWithPatches(RakString &applicationName,
        FileList *patchedFiles, FileList *addedFiles, FileList *addedOrModifiedFileHashes,
            FileList *deletedFiles, NIId *priorRowPatchTime, NIId *mostRecentRowPatchTime)
    {
        PGresult *result;
        ...
        for (rowIndex=0; rowIndex < numRows; rowIndex++)
        {
            ...
            if (createFileResult[0]==1)
            {
                ...
                if (patchLength==0)
                {
                    ...
                    if (fp == 0)
                    {
                        sprintf(lastError,"ERROR: Cannot open file %s in function GetMostRecentChangelistWithPatches\n",pathToContent);
                        PQclear(result);
                        return false;
                    }
    ...
    bool AutopatcherPostgreRepository::UpdateApplicationFiles(const char *applicationName, const char *applicationDirectory, const char *userName, FileListProgress *cb)
    {
        ...
        int rowIndex;
        RakString hardDriveFilename;
        char *hardDriveHash;
        RakString queryFilename;
        char *createFileResult;
        char *hash;
        bool addFile;
        FileList newFiles;
    ...
    //		hardDriveData=newFiles.fileList[fileListIndex].data+HASH_LENGTH;
            ...
            formats[0]=PQEXECPARAM_FORMAT_TEXT;
            N_assert(formats[1]==PQEXECPARAM_FORMAT_BINARY);
            N_assert(formats[2]==PQEXECPARAM_FORMAT_BINARY);
    ...
    bool AutopatcherPostgreRepository2::MakePatch(const char * oldFile, const char * newFile,
        char ** patch, unsigned int * patchLength, int * patchAlgorithm)
    {
        *patchAlgorithm=0;
    ...
    bool AutopatcherPostgreRepository2::MakePatchBSDiff(FILE * fpOld, int contentLengthOld,
        FILE * fpNew, int contentLengthNew, char ** patch, unsigned int * patchLength)
    {
    ...
    unsigned int AutopatcherPostgreRepository::GetFilePart(const char * filename,
    ...
    unsigned int AutopatcherPostgreRepository2::GetFilePart(const char * filename,
        unsigned int startReadBytes, unsigned int numBytesToRead,
            void * preallocatedDestination, FileListNodeContext context)
    {
        if(context.op == PC_HASH_1_WITH_PATCH)
        ...
        else
        {
            ...
            N_assert(context.op == PC_WRITE_FILE)
