...
namespace NII_NET
{
    ...
    void Rackspace2::update()
    {
        ...
        // This is kind of crappy, but for TCP plugins, always do HasCompletedConnectionAttempt, then recevie(), then onConnectFail(),onConnectClose()
        //Address * sa = tcp->onClientConnect(sa);
        for (msg = tcp->recevie(); msg; N_delete msg, msg = tcp->recevie())
            ;
        //sa = tcp->onConnectFail(sa);
        //sa = tcp->onConnectClose(sa);

        Address hostReceived;
        int contentOffset;
    ...
    //------------------------------------------------------------------------
    void Rackspace2::AuthenticateInt(const char * authenticationURL, 
        const char * rackspaceCloudUsername, const char * apiAccessKey)
    {
    ...
    //------------------------------------------------------------------------
    void Rackspace2::Authenticate(const char * authenticationURL, 
        const char * rackspaceCloudUsername, const char * apiAccessKey)
    {
    ...
    //------------------------------------------------------------------------
    void Rackspace2::AddOperation(RakString URL, OpType opType, json_t * data,
        bool setAuthToken)
    {
        if (tcp == 0)
        {
            tcp = OP_NEW<NetworkObj>();
