...
namespace NII_NET
{
    ...
    //------------------------------------------------------------------------
    int Lobby2Client_Steam_Impl::SystemAddressAndRoomMemberComp(const Address & key,
        const Lobby2Client_Steam_Impl::RoomMember & data)
    {
        if (key<data.mAddress)
            return -1;
        if (key==data.mAddress)
            return 0;
        return 1;
    ...
    //------------------------------------------------------------------------
    int Lobby2Client_Steam_Impl::SteamIDAndRoomMemberComp(const uint64_t & key,
        const Lobby2Client_Steam_Impl::RoomMember & data)
    {
    ...
    {
        // Must recompile RakNet with N_Net_MaxMtuSize set to 1200 in the preprocessor settings
        N_assert(N_Net_MaxMtuSize<=1200);
        roomId=0;
    ...
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::update()
    {
    ...
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::CallCBWithResultCode(Lobby2Message * msg,
        Lobby2ResultCode rc)
    {
        if (msg)
        {
        ...
        for (i=0; i < deferredCallbacks.GetSize(); i++)
        {
            ...
            if (deferredCallbacks[i]->GetID()==L2MID_Console_SearchRooms)
            {
                ...
                for(uint32 iLobby = 0; iLobby < pCallback->m_nLobbiesMatching; iLobby++ )
                {
                    ...
                    RakString s = SteamMatchmaking()->GetLobbyData( steamId, "name" );
    ...
    void Lobby2Client_Steam_Impl::OnLobbyCreated( LobbyCreated_t *pCallback, bool bIOFailure )
    {
        ...
        for (i=0; i < deferredCallbacks.GetSize(); i++)
        {
            if (deferredCallbacks[i]->GetID()==L2MID_Console_CreateRoom)
            {
                ...
                roomMember.mAddress.mSrc.sin_addr.s_addr=nextFreeSystemAddress++;
                roomMember.mAddress.setHSBPort(STEAM_UNUSED_PORT);
                roomMembersByAddr.Insert(roomMember.mAddress,roomMember,true);
    ...
    void Lobby2Client_Steam_Impl::CallRoomCallbacks()
    {
        ...
        NCount currentMemberIndex=0, oldMemberIndex=0;
        while (currentMemberIndex < currentMembers.Size() && oldMemberIndex < roomMembersById.Size())
        {
            if (currentMembers[currentMemberIndex]<roomMembersById[oldMemberIndex].steamIDRemote)
            {
                ...
                // new member
                NotifyNewMember(currentMembers[currentMemberIndex], roomMember.mAddress);
                currentMemberIndex++;
            }
            else if (currentMembers[currentMemberIndex]>roomMembersById[oldMemberIndex].steamIDRemote)
            {
                ...
                // dropped member
                NotifyDroppedMember(roomMembersById[oldMemberIndex].steamIDRemote, roomMembersById[oldMemberIndex].mAddress);
                oldMemberIndex++;
            }
            else
            {
        ...
        if (anyChanges)
        {
            ...
            {
                roomMembersByAddr.Insert(roomMembersById[currentMemberIndex].mAddress, roomMembersById[currentMemberIndex], true);
            }
        }
    }
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::NotifyNewMember(uint64_t memberId, Address * remote)
    {
        ...
        notification.mRemote=remote;


        CallCBWithResultCode(&notification, L2RC_SUCCESS);
    }
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::NotifyDroppedMember(uint64_t memberId, Address * remote)
    {
        /// const char *pchName = SteamFriends()->GetFriendPersonaName( memberId );
        ...
        if (objectExists)
        {
            mPrc2->close(roomMembersById[i].mAddress, false);
            // Is this necessary?
    ...
    void Lobby2Client_Steam_Impl::ClearRoom(void)
    {
        ...
        if (SteamNetworking())
        {
            for (NCount i=0; i < roomMembersById.Size(); i++)
            {
    ...
    void Lobby2Client_Steam_Impl::OnP2PSessionConnectFail( P2PSessionConnectFail_t *pCallback )
    {
        ...
        // we've sent a msg to the user, but it never got through
    ...
    //------------------------------------------------------------------------
    int Lobby2Client_Steam_Impl::RakNetSendTo(const char * data, int length,
        const Address & address)
    {
        bool objectExists;
        NCount i = roomMembersByAddr.GetIndexFromKey(address, &objectExists);
        if(objectExists)
        {
        ...
        else if (address.getHSBPort() != STEAM_UNUSED_PORT)
        {
            return -1;
        }
        return 0;
    ...
    //------------------------------------------------------------------------
    int Lobby2Client_Steam_Impl::RakNetRecvFrom(char dataOut[N_Net_MaxMtuSize],
        Address *& senderOut, bool calledFromMainThread)
    {
        ...
        if (SteamNetworking() && SteamNetworking()->IsP2PPacketAvailable(&pcubMsgSize))
        {
            ...
            if(SteamNetworking()->ReadP2PPacket(dataOut, N_Net_MaxMtuSize,
                &pcubMsgSize, &psteamIDRemote))
            {
                ...
                if (objectExists)
                {
                    senderOut = roomMembersById[i].mAddress.clone();
                }
                return pcubMsgSize;
            }
        }
    ...
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::shutdown()
    {
        ClearRoom();
    }
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::onConnectClose(const Address & address, UniqueID id,
    ...
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::onConnectFail(SocketMessage * msg, ConnectFailType failedConnectionAttemptReason)
    {
        (void) failedConnectionAttemptReason;

        bool objectExists;
        NCount i = roomMembersByAddr.GetIndexFromKey(msg->mAddress, &objectExists);
        if (objectExists)
        {
            uint64_t steamIDRemote = roomMembersByAddr[i].steamIDRemote;
    ...
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::onAttach()
    {
        nextFreeSystemAddress = (uint32_t) mPrc2->getGuid().g;

        // If this asserts, call NetworkSys::Startup() before attaching Lobby2Client_Steam
        DataStructures::List<SocketObj *> sockets;
        mPrc2->getSockets(sockets);
        ((RNS2_Windows*)sockets[0])->SetSocketLayerOverride(this);
    }
    //------------------------------------------------------------------------
    void Lobby2Client_Steam_Impl::onDetach()
    {
        DataStructures::List<SocketObj *> sockets;
        mPrc2->getSockets(sockets);
        ((RNS2_Windows*)sockets[0])->SetSocketLayerOverride(this);
