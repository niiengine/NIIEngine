...
namespace NII_NET
{
    ...
    {
        Nui32 idx = eventCallbacks.GetIndexOf(callback);
        if (idx == (Nui32)-1)
    ...
    //------------------------------------------------------------------------
    Address * Rackspace::Authenticate(NetworkObj *_tcpInterface, const char *_authenticationURL, const char *_rackspaceCloudUsername, const char *_apiAccessKey)
    {
        Nui32 index = GetOperationOfTypeIndex(RO_CONNECT_AND_AUTHENTICATE);
        if (index!=(Nui32)-1)
        {
            // In progress
        ...
        N_assert(mPrc->isRun());
        ro.connectionAddress = mPrc->Connect(_authenticationURL, 443, true);
        if (ro.connectionAddress == 0 || ro.connectionAddress->isInvalid())
        {
        ...
        mPrc->setSSL(ro.connectionAddress);
    #endif
        ...
        mPrc->send(command.C_String(), (Nui32) command.GetLength(), ro.connectionAddress, false);
    ...
    //------------------------------------------------------------------------
    void Rackspace::onMessage(SocketMessage * msg)
    {
        NCount i;
        for (i = 0; i < operations.Size(); ++i)
        {
            if (operations[i].isPendingAuthentication == false &&
                operations[i].connectionAddress == msg->mAddress)
            {
                operations[i].incomingStream += msg->mData;
            }
        }
    }
    //------------------------------------------------------------------------
    ...
    void Rackspace::ReadLine(const char * data, const char * stringStart, RakString & output)
    {
        ...
        output.Truncate((Nui32) (resultEnd-result));
    }
    //------------------------------------------------------------------------
    bool Rackspace::ConnectToServerManagementDomain(RackspaceOperation &ro)
    {
        ro.connectionAddress = mPrc->Connect(serverManagementDomain.C_String(),443,true);
        if (ro.connectionAddress == Address::invalid)
        {
            for (Nui32 i = 0; i < eventCallbacks.Size(); ++i)
    ...
    {
        for (Nui32 i=0; i < operations.Size(); i++)
        {
    ...
    //------------------------------------------------------------------------
    Nui32 Rackspace::GetOperationOfTypeIndex(RackspaceOperationType t)
    {
        for (Nui32 i=0; i < operations.Size(); i++)
        {
            if (operations[i].type==t)
