...
static Nui32 _state[ N + 1 ];     // state vector + 1 extra to not violate ANSI C
static Nui32 *_next;        // next random value is computed from here
...
namespace NII_NET
{
    void seedMT(Nui32 seed, Nui32 * state, Nui32 *& next, int & left);
    Nui32 reloadMT(Nui32 * state, Nui32 *&next, int & left);
    Nui32 randomMT(Nui32 * state, Nui32 *&next, int & left);
    void fillBufferMT(void * buffer, Nui32 bytes, Nui32 * state, Nui32 *& next, int & left);
    NIIf frandomMT(Nui32 * state, Nui32 *& next, int & left);
    ...
    //------------------------------------------------------------------------
    void seedMT( Nui32 seed, Nui32 *state, Nui32 *&next, int &left )   // Defined in cokus_c.c
    {
        (void) next;
        ...
        register Nui32 x = ( seed | 1U ) & 0xFFFFFFFFU, *s = state;
        register int j;
    ...
    Nui32 reloadMT( Nui32 *state, Nui32 *&next, int &left )
    {
        register Nui32 * p0 = state, *p2 = state + 2, *pM = state + M, s0, s1;
        register int j;
    ...
    NIIf frandomMT( Nui32 *state, Nui32 *&next, int &left )
    {
        return ( NIIf ) ( ( NIId ) randomMT(state, next, left) / 4294967296.0 );
    }
    //------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------
    void RakNetRandom::SeedMT( Nui32 seed )
    {
        printf("%i\n",seed);
        seedMT(seed, state, next, left);
    }
    //------------------------------------------------------------------------
    Nui32 RakNetRandom::ReloadMT( void )
    {
        return reloadMT(state, next, left);
    }
    //------------------------------------------------------------------------
    Nui32 RakNetRandom::RandomMT( void )
    {
        return randomMT(state, next, left);
    }
    //------------------------------------------------------------------------
    NIIf RakNetRandom::FrandomMT( void )
    {
        return frandomMT(state, next, left);
    }
    //------------------------------------------------------------------------
    void RakNetRandom::FillBufferMT( void *buffer, Nui32 bytes )
    {
    ...
    //------------------------------------------------------------------------
    void RakNetRandomSync::SeedMT(Nui32 _seed)
    {
        seed = _seed;
        rnr.SeedMT(seed);
        callCount = 0;
    ...
    //------------------------------------------------------------------------
    void RakNetRandomSync::SeedMT(Nui32 _seed, Nui32 skipValues)
    {
        SeedMT(_seed);
        Skip(skipValues);
    }
    //------------------------------------------------------------------------
    NIIf RakNetRandomSync::FrandomMT(void)
    {
        return ( NIIf ) ( ( NIId ) RandomMT() / (NIId) UINT_MAX );
    }
    //------------------------------------------------------------------------
    Nui32 RakNetRandomSync::RandomMT(void)
    {
    ...
    //------------------------------------------------------------------------
    Nui32 RakNetRandomSync::GetSeed(void) const
    {
        return seed;
    }
    //------------------------------------------------------------------------
    Nui32 RakNetRandomSync::GetCallCount(void) const
    {
        return callCount;
    }
    //------------------------------------------------------------------------
    void RakNetRandomSync::SetCallCount(Nui32 i)
    {
        callCount = i;
    }
    //------------------------------------------------------------------------
    void RakNetRandomSync::SerializeConstruction(NetSerializer *constructionBitstream)
    {
        constructionBitstream->write(seed);
        constructionBitstream->write(callCount);
    }
    //------------------------------------------------------------------------
    bool RakNetRandomSync::DeserializeConstruction(NetSerializer *constructionBitstream)
    {
        Nui32 _seed;
        Nui32 _skipValues;
    ...
    //------------------------------------------------------------------------
    void RakNetRandomSync::read(NetSerializer * out)
    {
        out->write(callCount);
    }
    //------------------------------------------------------------------------
    void RakNetRandomSync::import(NetSerializer * in)
    {
        Nui32 _callCount;
        in->read(_callCount);
        ...
        else if(_callCount > callCount)
        {
            ...
            Nui32 diff = _callCount - callCount;
    ...
    //------------------------------------------------------------------------
    void RakNetRandomSync::Skip(Nui32 count)
    {
...
// you can seed with any Nui32, but the best are odds in 0..(2^32 - 1)
...
for(j=0; j<2002; j++)
N_printf(" %10lu%s", (Nui32) randomMT(), (j%7)==6 ? "\n" : "");
