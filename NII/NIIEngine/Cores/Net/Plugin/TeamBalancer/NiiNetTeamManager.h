...
namespace NII_NET
{
    ...
    /// \details When used with ReplicaManager3 and FullyConnected, provides a complete solution to managing a distributed list of teams and team member objects with support for host migration.
    ...
    /// \ingroup TEAM_MANAGER_GROUP
    class _EngineAPI TM_TeamMember
    {
    public:
        TM_TeamMember();
        ...
        /// \param[out] constructionBitstream This object serialized to a NetSerializer
        void SerializeConstruction(NetSerializer *constructionBitstream);
        ...
        /// \param[in] constructionBitstream This object serialized to a NetSerializer
        bool DeserializeConstruction(TeamManager *teamManager, NetSerializer *constructionBitstream);
        ...
        Nui32 GetWorldIndex(void) const;

        /// \internal
        static Nul ToUint32(const NetworkID & g);

        /// \internal
        struct RequestedTeam
        {
            TimeDurMS whenRequested;
            Nui32 requestIndex;
            TM_Team * requested;
            bool isTeamSwitch;
            TM_Team * teamToLeave;
    ...
    protected:
        ...
        TimeDurMS whenJoinAnyRequested;
        Nui32 joinAnyRequestIndex;
        void * owner;
    ...
    public:
        ...
        Nui32 GetTeamMembersCount() const;
        ...
        TM_TeamMember * GetTeamMemberByIndex(Nui32 index) const;
    ...
    protected:
        ...
        Nui32 GetMemberWithRequestedSingleTeamSwitch(TM_Team * team);
    ...
    public:
        ...
        /// \param[in] id UniqueID of the system you are removing. See SocketMessage::mID or LinkBase::GetGUIDFromSystemAddress()
        void RemoveParticipant(UniqueID id);
        ...
        TM_Team * GetTeamByIndex(Nui32 index) const;
        ...
        Nui32 GetTeamIndex(const TM_Team *team) const;
        ...
        Nui32 GetTeamMemberCount(void) const;
        ...
        NetworkID GetTeamMemberIDByIndex(Nui32 index) const;
        ...
        Nui32 GetTeamMemberIndex(const TM_TeamMember *teamMember) const;
        ...
        /// For a client/server game, call SetHost() with the server's UniqueID value on all systems (including the server itself). If you call TeamManager::SetTopology(TM_CLIENT_SERVER), the server will also relay messages between participants.
        ...
        /// \note If using FullyConnected, SetHost() is called automatically when ID_FCM2_NEW_HOST is returned.
        ...
        void SetHost(UniqueID _hostGuid);

        /// \return Returns the current host, or UniqueID::INVALID if unknown
        UniqueID GetHost(void) const;
        ...
        {
            TimeDurMS whenRequestMade;
            Nui32 teamMemberIndex;
            Nui32 indexIntoTeamsRequested;
            Nui32 requestIndex;
        };
        /// \internal
    ...
    protected:
        ...
        Nui32 GetAvailableTeamIndexWithFewestMembers(TeamMemberLimit secondaryLimit, JoinPermissions joinPermissions);
        ...
        void BroadcastToParticipants(NetSerializer *bsOut, UniqueID exclusionGuid);
        void BroadcastToParticipants(Nui8 *data, const int length, UniqueID exclusionGuid);
        ...
        DataStructures::List<UniqueID> participants;
        DataStructures::List<TM_Team *> teams;
        DataStructures::List<TM_TeamMember *> teamMembers;
    ...
    /// 9. As the peer to peer session host changes, call SetHost() (Not necessary if using FullyConnected). If using client/server, you must set the host<BR>
    /// \note This replaces TeamBalancer. You cannot use TeamBalancer and TeamManager at the same time.
    /// \ingroup TEAM_MANAGER_GROUP
    class _EngineAPI TeamManager : public Plugin
    {
    public:
        ...
        TM_World * GetWorldAtIndex(Nui32 index) const;
        ...
        /// \brief When you get ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, pass the msg to this function to read out parameters
        /// \param[in] A msg where msg->mData[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_FULL
        /// \return true on success, false on read error
        void DecomposeTeamFull(SocketMessage * msg,
        ...
        /// \brief When you get ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED, pass the msg to this function to read out parameters
        /// \param[in] A msg where msg->mData[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED
        /// \return true on success, false on read error
        void DecomposeTeamLocked(SocketMessage * msg,
            TM_World ** world, TM_TeamMember ** teamMember, TM_Team ** team,
            Nui16 & currentMembers, Nui16 & memberLimitIncludingBalancing,
                bool & balancingIsActive, JoinPermissions & joinPermissions);
        ...
        /// \param[in] A msg where msg->mData[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
        ...
        void DecodeTeamAssigned(SocketMessage * msg, TM_World **world,
        ...
        /// \param[in] A msg where msg->mData[0]==ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED
        /// \param[out] world Set to the world this \a teamMember is on. 0 on bad lookup.
        /// \param[out] teamMember Set to the teamMember affected. 0 on bad lookup.
        /// \param[out] teamCancelled Set to the team that was cancelled. 0 for all teams.
        void DecodeTeamCancelled(SocketMessage * msg, TM_World **world,
    ...
    protected:
        ...
        virtual void onConnectClose(const Address & address, UniqueID id, ConnectCloseType lostConnectionReason);
        virtual void onConnect(const Address & address, UniqueID id);
        void send(const NetSerializer * bitStream, const UniqueAddress usys, bool broadcast);

        void EncodeTeamFullOrLocked(NetSerializer * bitStream, TM_TeamMember * teamMember, TM_Team *team);
        void DecomposeTeamFullOrLocked(NetSerializer * bsIn, TM_World ** world,
            TM_TeamMember **teamMember, TM_Team ** team, Nui16 & currentMembers,
            Nui16 &memberLimitIncludingBalancing, bool & balancingIsActive,
                JoinPermissions & joinPermissions);
        void ProcessTeamAssigned(NetSerializer * bsIn);

        void EncodeTeamAssigned(NetSerializer * bitStream, TM_TeamMember * teamMember);
        ...
        void PushBitStream(NetSerializer * bitStream);
        void OnUpdateListsToNoTeam(SocketMessage * msg, TM_World * world);
        void OnUpdateTeamsRequestedToAny(SocketMessage * msg, TM_World * world);
        void OnJoinAnyTeam(SocketMessage * msg, TM_World * world);
        void OnJoinRequestedTeam(SocketMessage * msg, TM_World * world);
        void OnUpdateTeamsRequestedToNoneAndAddTeam(SocketMessage * msg, TM_World * world);
        void OnRemoveFromTeamsRequestedAndAddTeam(SocketMessage * msg, TM_World * world);
        void OnAddToRequestedTeams(SocketMessage * msg, TM_World * world);
        bool OnRemoveFromRequestedTeams(SocketMessage * msg, TM_World * world);
        void OnLeaveTeam(SocketMessage * msg, TM_World * world);
        void OnSetMemberLimit(SocketMessage * msg, TM_World * world);
        void OnSetJoinPermissions(SocketMessage * msg, TM_World * world);
        void OnSetBalanceTeams(SocketMessage * msg, TM_World * world);
        void OnSetBalanceTeamsInitial(SocketMessage * msg, TM_World * world);

        void EncodeTeamFull(NetSerializer * bitStream, TM_TeamMember * teamMember,
            TM_Team * team);
        void EncodeTeamLocked(NetSerializer * bitStream, TM_TeamMember * teamMember,
            TM_Team * team);

        /// \brief When you get ID_TEAM_BALANCER_TEAM_ASSIGNED, pass the msg to this function to read out parameters
        /// \param[in] A msg where msg->mData[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
        /// \return true on success, false on read error
        void DecodeTeamAssigned(NetSerializer * bsIn, TM_World ** world,
            TM_TeamMember ** teamMember, NoTeamId & noTeamSubcategory,
