...
namespace NII_NET
{
    ...
    /// \deprecated Use TeamManager intead
    /// \ingroup TEAM_BALANCER_GROUP
    class _EngineAPI TeamBalancer : public Plugin
    {
    public:
        TeamBalancer();
        ...
        void SetTeamSizeLimit(TeamId team, Nui16 limit);
        ...
        /// Defaults to false
        /// \note SetLockTeams(true) takes plevel over SetForceEvenTeams(), so if teams are currently locked, this function will have no effect until teams become unlocked.
    ...
    protected:
        ...
        void OnStatusUpdateToNewHost(Message * msg);
        void OnCancelTeamRequest(Message * msg);
        void OnRequestAnyTeam(Message * msg);
        void OnRequestSpecificTeam(Message * msg);
        ...
        DataStructures::List<Nui16> teamLimits;
        DataStructures::List<Nui16> teamMemberCounts;
        DataStructures::List<TeamMember> teamMembers;
        Nui32 GetMemberIndex(NetworkID memberId, UniqueID id) const;
        Nui32 AddTeamMember(const TeamMember & tm); // Returns index of new member
        void RemoveTeamMember(Nui32 index);
        void EvenTeams(void);
        Nui32 GetMemberIndexToSwitchTeams(const DataStructures::List<TeamId> &sourceTeamNumbers, TeamId targetTeamNumber);
        void GetOverpopulatedTeams(DataStructures::List<TeamId> &overpopulatedTeams, int maxTeamSize);
        void SwitchMemberTeam(Nui32 teamMemberIndex, TeamId destinationTeam);
        void NotifyTeamAssigment(Nui32 teamMemberIndex);
        bool WeAreHost(void) const;
        ConnectReceiveType OnTeamAssigned(Message *msg);
        ConnectReceiveType OnRequestedTeamChangePending(Message *msg);
        ConnectReceiveType OnTeamsLocked(Message *msg);
        ...
        bool TeamWouldBeOverpopulatedOnAddition(TeamId teamId, Nui32 teamMemberSize); // Accounting for team balancing and team limits, would this team be overpopulated if a member was added to it?
        bool TeamWouldBeUnderpopulatedOnLeave(TeamId teamId, Nui32 teamMemberSize);
        ...
        void NotifyTeamsLocked(UniqueID target, TeamId requestedTeam);
        void NotifyTeamSwitchPending(UniqueID target, TeamId requestedTeam, NetworkID memberId);
        void NotifyNoTeam(NetworkID memberId, UniqueID target);
        void SwapTeamMembersByRequest(Nui32 memberIndex1, Nui32 memberIndex2);
        void RemoveByGuid(UniqueID id);
