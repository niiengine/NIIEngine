...
namespace NII_NET
{
    ...
    typedef Nui8 WorldId;

    /// \internal
    /// \ingroup REPLICA_MANAGER_GROUP3
    struct PRO
    {
        /// Passed to LinkBase::send(). Defaults to ReplicaManager3::SetDefaultPacketPriority().
        PacketLevel mPacketLevel;

        /// Passed to LinkBase::send(). Defaults to ReplicaManager3::SetDefaultPacketReliability().
        PacketType mPacketType;

        /// Passed to LinkBase::send(). Defaults to ReplicaManager3::SetDefaultOrderingChannel().
        char mPacketCh;

        /// Passed to LinkBase::send(). Defaults to 0.
        Nui32 sendReceipt;
    ...
    /// \ingroup REPLICA_MANAGER_GROUP3
    class _EngineAPI ReplicaManager3 : public Plugin
    {
    public:
        ReplicaManager3();
        ...
        /// \param[in] address Address of the system you are adding
        /// \param[in] id UniqueID of the system you are adding. See SocketMessage::mID or LinkBase::GetGUIDFromSystemAddress()
        /// \return The new connection instance.
        virtual Connection_RM3 * AllocConnection(const Address & address, UniqueID id) const=0;
        ...
        /// \details ReplicaManager3 can automatically create and/or destroy Connection_RM3 as systems connect or disconnect from LinkBase.<BR>
        ...
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        ...
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        bool PushConnection(Connection_RM3 * newConnection, WorldId gid = 0);
        ...
        /// \param[in] id of the connection to get. Passed to ReplicaManager3::AllocConnection() originally.
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        Connection_RM3 * PopConnection(UniqueID id, WorldId gid = 0);
        ...
        /// Remote systems already connected will potentially download this object the next time ReplicaManager3::update() is called, which happens every time you call LinkBase::receive().<BR>
        ...
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        void Reference(Replica3 * replica3, WorldId gid = 0);
        ...
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        void Dereference(Replica3 * replica3, WorldId gid = 0);
        ...
        /// \param[in] id UniqueID of the system we are referring to. Originally passed as the id parameter to ReplicaManager3::AllocConnection()
        /// \param[out] List of Replica3 instances to be returned
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        void GetReplicasCreatedByGuid(UniqueID id, DataStructures::List<Replica3 *> & replicaListOut,
            WorldId gid = 0);

        /// \brief Returns all objects originally created by your system
        /// \details Calls GetReplicasCreatedByGuid() for your own system id.
        /// \param[out] List of Replica3 instances to be returned
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        ...
        Nui32 GetReplicaCount(WorldId gid = 0) const;
        ...
        Replica3 *GetReplicaAtIndex(Nui32 index, WorldId gid = 0);
        ...
        Nui32 GetConnectionCount(WorldId gid = 0) const;
        ...
        Connection_RM3 * GetConnectionAtIndex(Nui32 index, WorldId gid = 0) const;

        /// \brief Returns a connection pointer previously added with PushConnection()
        /// \param[in] sa The system address of the connection to return
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        /// \return A Connection_RM3 pointer, or 0 if not found
        Connection_RM3 * GetConnectionBySystemAddress(const Address & sa, WorldId gid = 0) const;

        /// \brief Returns a connection pointer previously added with PushConnection.()
        /// \param[in] id The guid of the connection to return
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        /// \return A Connection_RM3 pointer, or 0 if not found
        Connection_RM3* GetConnectionByGUID(UniqueID id, WorldId gid = 0) const;
        ...
        /// \param[in] Default packet plevel to use for object creation, destruction, and serializations
        void SetDefaultPacketPriority(PacketLevel plevel);

        /// \param[in] Default packet ptype to use for object creation, destruction, and serializations
        void SetDefaultPacketReliability(PacketType ptype);

        /// \details Every \a intervalMS milliseconds, Connection_RM3::OnAutoserializeInterval() will be called.<BR>
        /// Defaults to 30.<BR>
        /// Pass with <0 to disable. Pass 0 to export() every time NetworkSys::Recieve() is called<BR>
        /// If you want to control the update interval with more granularity, use the return values from Replica3::export().<BR>
        ...
        void SetAutoSerializeInterval(TimeDurMS intervalMS);
        ...
        bool GetAllConnectionDownloadsCompleted(WorldId gid = 0) const;
        ...
        /// \param[in] gid A unique identifier for this world. User-defined
        void AddWorld(WorldId gid);
        ...
        /// \param[in] gid A \a gid value previously added with AddWorld()
        void RemoveWorld(WorldId gid);
        ...
        WorldId GetWorldIdAtIndex(Nui32 index);
        ...
        Nui32 GetWorldCount(void) const;

        /// \details Sets the networkIDManager instance that this plugin relys upon.<BR>
        /// Uses whatever instance is attached to LinkBase if unset.<BR>
        ...
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        void SetNetworkIDManager(NetworkIDManager *_networkIDManager, WorldId gid = 0);

        /// Returns what was passed to SetNetworkIDManager(), or the instance on LinkBase if unset.
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        NetworkIDManager * GetNetworkIDManager(WorldId gid = 0) const;
        ...
        /// \param[in] exclusionAddress Which system to not send to. Address::invalid to send to all.
        /// \param[in] gid Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
        void BroadcastDestructionList(DataStructures::List<Replica3 *> &replicaListSource,
            const Address & exclusionAddress, WorldId gid = 0);
        ...
        void BroadcastDestruction(Replica3 * replica, const Address & exclusionAddress);
    ...
    protected:
        ...
        ConnectReceiveType OnConstruction(SocketMessage * msg, Nui8 * data,
            int count, UniqueID senderGuid, Nui8 offset, WorldId gid);
        ConnectReceiveType OnSerialize(SocketMessage * msg, Nui8 * data,
            int count, UniqueID senderGuid, TimeDurMS timestamp, Nui8 offset, WorldId gid);
        ConnectReceiveType OnDownloadStarted(SocketMessage * msg, Nui8 * data,
            int count, UniqueID senderGuid, Nui8 offset, WorldId gid);
        ConnectReceiveType OnDownloadComplete(SocketMessage * msg, Nui8 * data,
            int count, UniqueID senderGuid, Nui8 offset, WorldId gid);

        void DeallocReplicaNoBroadcastDestruction(Connection_RM3 * connection,
            Replica3 * replica3);
        Connection_RM3 * PopConnection(Nui32 index, WorldId gid);
        Replica3 * GetReplicaByNetworkID(NetworkID networkId, WorldId gid);
        Nui32 ReferenceInternal(Replica3 *replica3, WorldId gid);

        PRO defaultSendParameters;
        TimeDurMS autoSerializeInterval;
        TimeDurMS lastAutoSerializeOccurance;
    ...
    {
        NetSerializer bitStream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
    ...
    struct LastSerializationResult
    {
        ...
        Replica3 * replica;
        TimeDurMS whenLastSerialized;

        void AllocBS(void);
        LastSerializationResultBS* lastSerializationResultBS;
    };

    /// Parameters passed to Replica3::export()
    /// \ingroup REPLICA_MANAGER_GROUP3
    struct SerializeParameters
    {
        ...
        NetSerializer outputBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];

        /// Last bitstream we sent for this replica to this system.
        /// read, but DO NOT MODIFY
        NetSerializer* lastSentBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];

        /// Set to non-zero to transmit a timestamp with this message.
        /// Defaults to 0
        /// Use N_Engine().getTimer().getMS() for this
        TimeDurMS messageTimestamp;
    ...
    {
        NetSerializer serializationBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
    ...
    /// \ingroup REPLICA_MANAGER_GROUP3
    class _EngineAPI Connection_RM3
    {
    public:

        Connection_RM3(const Address &_systemAddress, UniqueID _guid);
        ...
        /// {RakString typeName; allocationIdBitstream->read(typeName); if (typeName=="Soldier") return new Soldier; return 0;}<BR>
        ...
        virtual Replica3 * AllocReplica(NetSerializer * allocationIdBitstream, ReplicaManager3 * replicaManager3) = 0;
        ...
        bool HasReplicaConstructed(Replica3 * replica);
        ...
        virtual void SerializeOnDownloadStarted(NetSerializer * bitStream)
        {
            (void) bitStream;
        }
        ...
        virtual void DeserializeOnDownloadStarted(NetSerializer * bitStream)
        {
            (void) bitStream;
        }
        ...
        virtual void SerializeOnDownloadComplete(NetSerializer * bitStream)
        {
            (void) bitStream;
        }
        ...
        virtual void DeserializeOnDownloadComplete(NetSerializer * bitStream)
        {
            (void) bitStream;
        }

        /// \return The system address passed to the constructor of this object
        Address * GetSystemAddress(void) const {return mAddress;}

        /// \return Returns the UniqueID passed to the constructor of this object
        UniqueID GetRakNetGUID(void) const {return mID;}
        ...
        /// \brief Return whether or not downloads to our system should all be processed the same tick (call to NetworkSys::receive() )
        ...
        /// When QuerySerializationList(), sort objects by plevel, and write the list to \a replicasToSerialize, optionally skipping objects with a lower serialization frequency
        ...
        /// \param[in] link Instance of LinkBase to send on
        /// \param[in] gid Which world, see ReplicaManager3::AddWorld()
        /// \param[in] curTime The current time
        virtual SendSerializeIfChangedResult SendSerialize(Replica3 * replica,
            bool indicesToSend[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS],
        ...
        /// \internal
        /// \details Calls Connection_RM3::SendSerialize() if Replica3::export() returns a different result than what is contained in \a lastSerializationResult.<BR>
        ...
        /// \param[in] gid Which world, see ReplicaManager3::AddWorld()
        /// \param[in] curTime The current time
        virtual SendSerializeIfChangedResult SendSerializeIfChanged(LastSerializationResult * lsr,
            SerializeParameters * sp, LinkBase * link, Nui8 gid, ReplicaManager3 * replicaManager, TimeDurMS curTime);
        ...
        /// \internal
        void SendValidation(LinkBase * link, WorldId gid);

        /// \internal
        void AutoConstructByQuery(ReplicaManager3 * replicaManager3, WorldId gid);
        ...
        // Internal
        void ClearDownloadGroup(LinkBase * linkbase);
    protected:
        Address * mAddress;
        UniqueID mID;
        ...
        */
        ...
        void OnNeverConstruct(Nui32 queryToConstructIdx, ReplicaManager3 * replicaManager);
        void OnConstructToThisConnection(Nui32 queryToConstructIdx, ReplicaManager3 * replicaManager);
        ...
        void OnReplicaAlreadyExists(Nui32 queryToConstructIdx, ReplicaManager3 * replicaManager);
        void OnDownloadExisting(Replica3 * replica3, ReplicaManager3 * replicaManager);
        void OnSendDestructionFromQuery(Nui32 queryToDestructIdx, ReplicaManager3 * replicaManager);
        void OnDoNotQueryDestruction(Nui32 queryToDestructIdx, ReplicaManager3 * replicaManager);
        void ValidateLists(ReplicaManager3 * replicaManager) const;
        void SendSerializeHeader(Replica3 * replica, TimeDurMS timestamp, NetSerializer * bs, WorldId gid);
        ...
        DataStructures::Queue<SocketMessage *> downloadGroup;
    ...
    enum RM3ConstructionState
    {
        ...
        /// Will call export() and SerializeConstructionExisting() to the object on the remote system
    ...
    {
        /// Call export() to see if this object should be serializable for this connection
        RM3QSR_CALL_SERIALIZE,
        /// Do not call export() this tick to see if this object should be serializable for this connection
        RM3QSR_DO_NOT_CALL_SERIALIZE,
        /// Never call export() for this object and connection. This system will not serialize this object for this topology
    ...
    /// \ingroup REPLICA_MANAGER_GROUP3
    class _EngineAPI Replica3 : public NetworkIDObject
    {
    public:
        Replica3();
        ...
        /// Sample implementation:<BR>
        /// {allocationIdBitstream->write(RakString("Soldier");}<BR>
        ...
        virtual void WriteAllocationID(Connection_RM3 * destinationConnection,
            NetSerializer * allocationIdBitstream) const=0;
        ...
        virtual RM3ConstructionState QueryConstruction(Connection_RM3 * destinationConnection,
        ...
        virtual RM3DestructionState QueryDestruction(Connection_RM3 * destinationConnection,
        ...
        /// After SerializeConstruction() is called, export() will be called immediately thereafter. However, they are sent in different messages, so export() may arrive a later frame than SerializeConstruction()
        ...
        virtual void SerializeConstruction(NetSerializer * constructionBitstream, Connection_RM3 * destinationConnection) = 0;
        ...
        virtual bool DeserializeConstruction(NetSerializer * constructionBitstream, Connection_RM3 * sourceConnection) = 0;
        ...
        virtual void SerializeConstructionExisting(NetSerializer * constructionBitstream, Connection_RM3 * destinationConnection)
        {
        ...
        /// Used if you return RM3CS_ALREADY_EXISTS_REMOTELY from QueryConstruction
        virtual void DeserializeConstructionExisting(NetSerializer * constructionBitstream, Connection_RM3 * sourceConnection)
        {
            (void) constructionBitstream;
            (void) sourceConnection;
        };
        ...
        /// \param[in] destinationConnection System that will receive this network message.
        virtual void SerializeDestruction(NetSerializer * destructionBitstream, Connection_RM3 * destinationConnection) = 0;
        ...
        virtual bool DeserializeDestruction(NetSerializer * destructionBitstream, Connection_RM3 * sourceConnection) = 0;
        ...
        virtual void DeallocReplica(Connection_RM3 * sourceConnection) = 0;
        ...
        /// If more than one system can serialize the same object then you will need to override to return true, and control the serialization result from Replica3::export(). Be careful not to send back the same data to the system that just sent to you!
        /// \return True to allow calling Replica3::export() for this connection, false to not call.
        virtual RM3QuerySerializationResult QuerySerialization(Connection_RM3 * destinationConnection) = 0;

        /// \brief Called for each replica owned by the user, once per Serialization tick, before export() is called.
        ...
        /// \brief export our class to a bitstream
        ...
        /// Transmitted over the network if it changed from the last time we called export().<BR>
        ...
        virtual RM3SerializationResult export(SerializeParameters * serializeParameters)=0;

        /// \brief Called when the class is actually transmitted via export()
        /// \details Use to track how much bandwidth this class it taking
        virtual void OnSerializeTransmission(NetSerializer * bitStream, Connection_RM3 * destinationConnection,
            NBitCount bitsPerChannel[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], TimeDurMS curTime)
        {
            (void) bitStream;
            (void) destinationConnection;
            (void) bitsPerChannel;
        ...
        /// \brief read what was written in export()
        /// \details Reads the contents of the class from SerializationParamters::serializationBitstream.<BR>
        /// Called whenever export() is called with different data from the last send.
        /// \param[in] serializationBitstream Bitstream passed to export()
        ...
        virtual void import(DeserializeParameters * deserializeParameters)=0;
        ...
        virtual void PostSerializeConstruction(NetSerializer * constructionBitstream, Connection_RM3 * destinationConnection)
        {
            (void) constructionBitstream;
            (void) destinationConnection;
        }
        ...
        /// \param[in] constructionBitstream NetSerializer written in PostSerializeConstruction()
        /// \param[in] sourceConnection System that sent us this network message.
        virtual void PostDeserializeConstruction(NetSerializer * constructionBitstream, Connection_RM3 * sourceConnection)
        {
            (void) constructionBitstream;
            (void) sourceConnection;
        }
        ...
        virtual void PostSerializeConstructionExisting(NetSerializer * constructionBitstream, Connection_RM3 *destinationConnection)
        {
            (void) constructionBitstream;
            (void) destinationConnection;
        }
        ...
        virtual void PostDeserializeConstructionExisting(NetSerializer * constructionBitstream, Connection_RM3 * sourceConnection)
        {
            (void) constructionBitstream;
            (void) sourceConnection;
        }
        ...
        virtual RM3ConstructionState QueryConstruction_ClientConstruction(Connection_RM3 * destinationConnection, bool isThisTheServer);
        ...
        /// \param[in] isThisTheServer True if this system is the server, false if not.
        virtual bool QueryRemoteConstruction_ClientConstruction(Connection_RM3 * sourceConnection, bool isThisTheServer);

        /// \brief Default call for QueryConstruction().
        /// \details Only the server is allowed to create this object. The network topology is client/server
        /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
        /// \param[in] isThisTheServer True if this system is the server, false if not.
        virtual RM3ConstructionState QueryConstruction_ServerConstruction(Connection_RM3 * destinationConnection, bool isThisTheServer);
        ...
        virtual bool QueryRemoteConstruction_ServerConstruction(
            Connection_RM3 * sourceConnection, bool isThisTheServer);
        ...
        /// \details All clients are allowed to create all objects. The object is not relayed when remotely created
        /// \param[in] sourceConnection destinationConnection parameter passed to QueryConstruction()
        virtual bool QueryRemoteConstruction_PeerToPeer(Connection_RM3 * sourceConnection);
        ...
        /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
        /// \param[in] isThisTheServer True if this system is the server, false if not.
        virtual RM3QuerySerializationResult QuerySerialization_ClientSerializable(
            Connection_RM3 * destinationConnection, bool isThisTheServer);
        /// \brief Default call for QuerySerialization().
        ...
        /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
        /// \param[in] isThisTheServer True if this system is the server, false if not.
        virtual RM3QuerySerializationResult QuerySerialization_ServerSerializable(
            Connection_RM3 * destinationConnection, bool isThisTheServer);
        /// \brief Default call for QuerySerialization().
        ...
        /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
        /// \param[in] p2pMode If controlled only by this system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
        virtual RM3QuerySerializationResult QuerySerialization_PeerToPeer(
            Connection_RM3 * destinationConnection,
                Replica3P2PMode p2pMode = R3P2PM_SINGLE_OWNER);
        ...
        virtual RM3ActionOnPopConnection QueryActionOnPopConnection_Client(
            Connection_RM3 * droppedConnection) const;
        ...
        UniqueID GetCreatingSystemGUID(void) const;

        /// \return If ReplicaManager3::Reference() was called on this object.
        bool WasReferenced(void) const {return replicaManager!=0;}

        /// UniqueID of the system that first called Reference() on this object.
        /// Transmitted automatically when the object is constructed
        UniqueID creatingSystemGUID;
        /// UniqueID of the system that caused the item to send a deletion command over the network
        UniqueID deletingSystemGUID;
    ...
    class _EngineAPI Replica3Composite : public Replica3
    {
    protected:
    ...
    public:
        ...
        virtual void SerializeConstructionExisting(NetSerializer * constructionBitstream,
            Connection_RM3 * destinationConnection)
        {
            r3CompositeOwner->SerializeConstructionExisting(constructionBitstream,
                destinationConnection);
        }
        virtual void DeserializeConstructionExisting(NetSerializer * constructionBitstream,
            Connection_RM3 * sourceConnection)
        {
            r3CompositeOwner->DeserializeConstructionExisting(constructionBitstream,
                sourceConnection);
        }
        virtual void SerializeDestruction(NetSerializer * destructionBitstream,
            Connection_RM3 * destinationConnection)
        {
            r3CompositeOwner->SerializeDestruction(destructionBitstream,
                destinationConnection);
        }
        virtual bool DeserializeDestruction(NetSerializer * destructionBitstream,
            Connection_RM3 * sourceConnection)
        {
        ...
        virtual RM3SerializationResult export(SerializeParameters * serializeParameters)
        {
            return r3CompositeOwner->export(serializeParameters);
        }
        virtual void OnSerializeTransmission(NetSerializer * bitStream,
            Connection_RM3 * destinationConnection,
                NBitCount bitsPerChannel[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS],
                    TimeDurMS curTime)
        {
            r3CompositeOwner->OnSerializeTransmission(bitStream,
                destinationConnection, bitsPerChannel, curTime);
        }
        virtual void import(DeserializeParameters * deserializeParameters)
        {
            r3CompositeOwner->import(deserializeParameters);
        }
        virtual void PostSerializeConstruction(NetSerializer * constructionBitstream,
            Connection_RM3 *destinationConnection)
        {
            r3CompositeOwner->PostSerializeConstruction(constructionBitstream, destinationConnection);}
        virtual void PostDeserializeConstruction(NetSerializer * constructionBitstream,
            Connection_RM3 *sourceConnection)
        {
            r3CompositeOwner->PostDeserializeConstruction(constructionBitstream,
                sourceConnection);
        }
        virtual void PostSerializeConstructionExisting(NetSerializer * constructionBitstream,
            Connection_RM3 *destinationConnection)
        {
            r3CompositeOwner->PostSerializeConstructionExisting(constructionBitstream,
                destinationConnection);
        }
        virtual void PostDeserializeConstructionExisting(NetSerializer * constructionBitstream,
            Connection_RM3 *sourceConnection)
        {
