...
namespace NII_NET
{
    ...
    /// \sa FullyConnected
    /// \ingroup READY_EVENT_GROUP
    class ReadyEvent : public Plugin
    {
    public:
        // Constructor
        ReadyEvent();
        ...
        Nui32 GetEventListSize(void) const;
        ...
        int GetEventAtIndex(Nui32 index) const;
        ...
        /// \param[in] id An address to wait for event replies from.  Pass Address::invalid for all currently connected systems. Until all systems in this list have called SetEvent with this ID and true, and have this system in the list, we won't get ID_READY_EVENT_COMPLETE
        /// \return True on success, false on unknown eventId (this should be considered an error)
        bool AddToWaitList(int eventId, UniqueID id);
        ...
        /// \param[in] id The system to remove from the wait list. Pass UniqueID::INVALID for all currently connected systems.
        /// \return True on success, false on unknown eventId (this should be considered an error)
        bool RemoveFromWaitList(int eventId, UniqueID id);

        /// Returns if a particular system is waiting on a particular event.
        /// \param[in] eventId A user-defined identifier
        /// \param[in] id The system we are checking up on
        /// \return True if this system is waiting on this event, false otherwise.
        bool IsInWaitList(int eventId, UniqueID id);
        ...
        Nui32 GetRemoteWaitListSize(int eventId) const;
        ...
        UniqueID GetFromWaitListAtIndex(int eventId, Nui32 index) const;

        /// For a remote system, find out what their ready status is (waiting, signaled, complete).
        /// \param[in] eventId A user-defined identifier
        /// \param[in] id Which system we are checking up on
        /// \return The status of this system, for this particular event. \sa ReadyEventSystemStatus
        ReadyEventSystemStatus GetReadyStatus(int eventId, UniqueID id);

        /// This channel will be used for all NetworkSys::send calls
        /// \param[in] newChannel The channel to use for internal NetworkSys::send calls from this system.  Defaults to 0.
        void SetSendChannel(Nui8 newChannel);

        /// \internal
        /// Status of a remote system
        struct RemoteSystem
        {
            Token lastSentStatus, lastReceivedStatus;
            UniqueID mID;
        };
        static int RemoteSystemCompByGuid(const UniqueID & key, const RemoteSystem & data);
        /// \internal
        ...
        struct ReadyEventNode
        {
            ...
            Token eventStatus;
            DataStructures::OrderedList<UniqueID,RemoteSystem,ReadyEvent::RemoteSystemCompByGuid> systemList;
        };
    ...
    protected:
        ...
        void Clear(void);
        /*
        bool AnyWaitersCompleted(Nui32 eventIndex) const;
        bool AllWaitersCompleted(Nui32 eventIndex) const;
        bool AllWaitersReady(Nui32 eventIndex) const;
        void SendAllReady(Nui32 eventId, UniqueID id);
        void BroadcastAllReady(Nui32 eventIndex);
        void SendReadyStateQuery(Nui32 eventId, UniqueID id);
        void BroadcastReadyUpdate(Nui32 eventIndex);
        bool AddToWaitListInternal(Nui32 eventIndex, UniqueID id);
        bool IsLocked(Nui32 eventIndex) const;
        bool IsAllReadyByIndex(Nui32 eventIndex) const;
        */

        void SendReadyStateQuery(Nui32 eventId, UniqueID id);
        void SendReadyUpdate(Nui32 eventIndex, Nui32 systemIndex, bool forceIfNotDefault);
        void BroadcastReadyUpdate(Nui32 eventIndex, bool forceIfNotDefault);
        void RemoveFromAllLists(UniqueID id);
        void OnReadyEventQuery(SocketMessage * msg);
        void PushCompletionPacket(Nui32 eventId);
        bool AddToWaitListInternal(Nui32 eventIndex, UniqueID id);
        void OnReadyEventForceAllSet(SocketMessage * msg);
        void OnReadyEventPacketUpdate(SocketMessage * msg);
        void UpdateReadyStatus(Nui32 eventIndex);
        bool IsEventCompletedByIndex(Nui32 eventIndex) const;
        Nui32 CreateNewEvent(int eventId, bool isReady);
