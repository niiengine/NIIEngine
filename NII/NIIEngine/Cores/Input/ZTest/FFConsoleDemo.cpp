...
class VariableEffect
{
  ...
  public:
    ...
    string from() const
    {
	  ...
	  for (iterVars = _mapVariables.begin(); iterVars != _mapVariables.end(); iterVars++)
		str += iterVars->first + ":" + iterVars->second->from() + " ";
	  return str;
	}
};
...
class EffectManager
{
  ...
  public:
    ...
    {
	  cout << "* #" << nEffInd << " : " << _vecEffects[nEffInd]->getVerbose() << endl;
	}
    ...
    string from() const
    {
	  ...
	  if (_nCurrEffectInd >= 0)
		oss << _vecPlayableEffectInd[_nCurrEffectInd] 
			<< " " << _vecEffects[_vecPlayableEffectInd[_nCurrEffectInd]]->from();
	  else
		oss << "--";
	  return oss.str();
	}
...
class Application
{
  ...
  public:
    ...
    int initialize()
    {
	  ...
	  ParamList pl;
	  pl.insert(Npair(string("WINDOW"), wnd.str()));
    ...
    int run()
    {
	  ...
	  try
	  {
		...
		while(!_bMustStop)
		{
		  ...
		  // Update state line.
		  cout << "\r" << _pJoystickMgr->from() << " " << _pEffectMgr->from()
			   << "                           ";
