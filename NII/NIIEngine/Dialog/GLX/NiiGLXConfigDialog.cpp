...
print "NIIb %s[%i]={%s};" % (sys.argv[1],len(pngstring), ",".join([str(ord(x)) for x in pngstring]))
...
class GLXConfigurator {
	/* GUI constants */
	static const NIIi wWidth = 400;		// Width of window
	static const NIIi wHeight = 320;		// Height of window
	static const NIIi col1x = 20;		// Starting x of column 1 (labels)
	static const NIIi col2x = 180;		// Starting x of column 2 (options)
	static const NIIi col1w = 150;		// Width of column 1 (labels)
	static const NIIi col2w = 200;		// Width of column 2 (options)
	static const NIIi ystart = 105;		// Starting y of option table rows
	static const NIIi rowh = 20;		// Height of one row in the option table
...
protected:
	 ...
	 */
	virtual Pixmap CreateBackdrop(Window rootWindow, NIIi depth);
	/**
...
public:
	/* Local */
	bool accept;
	/* Class that binds a callback to a RenderSys */
	class RendererCallbackData {
	public:
		RendererCallbackData(GLXConfigurator *parent, RenderSys *renderer, Widget optionmenu):
			parent(parent),
			renderer(renderer),
	...
	std::list<Widget> mRenderOptionWidgets; // List of RenderSys specific
	...
	void SetRenderSystem(RenderSys *sys) {
		mRenderer = sys;
	}
private:
...
{
	const NIIb *bla[] = {"Rendering Settings", "-bg", "honeydew3", "-fg", "black","-bd","darkseagreen4"};
	NIIi argc = sizeof(bla)/sizeof(*bla);

	toplevel = XtVaOpenApplication(&appContext, "NII", NULL, 0, &argc, const_cast<NIIb**>(bla), NULL,sessionShellWidgetClass,
		XtNwidth, mWidth,
		XtNheight, mHeight,
		XtNminWidth, mWidth,
		XtNmaxWidth, mWidth,
	...
	NIIi screen = DefaultScreen(mDisplay);
	Window rootWindow = RootWindow(mDisplay,screen);

	/* Move to center of display */
	NIIi w = DisplayWidth(mDisplay, screen);
	NIIi h = DisplayHeight(mDisplay, screen);
	...
	const NIIb *curRenderName = " Select One "; // Name of current renderer, or hint to select one
	if(mRenderer)
	...
	for(RenderSysList::const_iterator pRend = renderers.begin(); pRend != renderers.end(); pRend++)
    {
...
Pixmap GLXConfigurator::CreateBackdrop(Window rootWindow, NIIi depth) {
	NIIi bpl;
	/* Find out number of bytes per pixel */
	switch(depth)
    {
	default:
		N_Only(Log).log("GLX backdrop: Unsupported bit depth");
	...
	Nui8 *data = 0; // Must be allocated with malloc
	...
	try
    {
        ...
        // Load backdrop image using NII
        imgStream = N_new MemStream(GLX_backdrop_data, sizeof(GLX_backdrop_data), false);
		img.load(imgStream, imgType);

        PixelBlock src = img.getPixel(0, 0);

		// Convert and copy image
		data = (Nui8*)malloc(mWidth * mHeight * bpl); // Must be allocated with malloc

        PixelBlock dst(src, bpl == 2 ? PF_B5G6R5 : PF_A8R8G8B8, data );
    ...
    catch(Exception & e)
    {
		...
		N_Only(Log).log("WARNING: Can not load backdrop for config dialog. " + e.getVerbose(), LML_TRIVIAL);
		return 0;
	}
...
void GLXConfigurator::SetRenderer(RenderSys *r)
{
	mRenderer = r;
	...
	// Create option GUI
	NIIi cury = ystart + 1*rowh + 10;

	SysOptionList options = mRenderer->getConfig();
	// Process each option and create an optionmenu widget for it
	for(SysOptionList::iterator it = options.begin(); it != options.end(); it++) 
    {
		...
		Widget mb1 = XtVaCreateManagedWidget("Menu", menuButtonWidgetClass, box, XtNlabel, it->second.mValue.c_str(),
			XtNresize, false,
			XtNresizable, false,
			XtNwidth, col2w, 	// Fixed width
			XtNheight, 18,
		...
		for (opt_it = it->second.mReserve.begin();
        opt_it != it->second.mReserve.end(); opt_it++) 
        {
			// Create callback data
...
void GLXConfigurator::SetConfigOption(const std::string &optionName, const std::string &valueName) 
{
	if(!mRenderer)
		...
		return;
	mRenderer->setConfig(optionName, valueName);
	SetRenderer(mRenderer);
}
//------------------------------------------------------------------------------------//
...
bool ConfigDialog::display()
{
    ...
    N_Engine().get(rsys);
	if(rsys)
		test.SetRenderSystem(rsys);
	/* Attempt to create the window */
	if(!test.CreateWindow())
		N_EXCEPT(InternalError, "Could not create configuration dialog", "GLXConfig::display");
