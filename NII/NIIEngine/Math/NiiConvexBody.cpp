...
namespace NII
{
    ...
    */
    void getSingleEdges(const ConvexBody::PolygonList & in, Polygon::EdgeMap & edgeMap)
    {
        // put all edges of all polygons into a list every edge has to be
        // walked in each direction once
        for (NCount i = 0; i < in.size(); ++i)
        {
            const Polygon & p = *in[i];

            for (NCount j = 0; j < p.getVertexCount(); ++j)
            {
                const Vector3f & a = p.getVertex(j);
                const Vector3f & b = p.getVertex((j + 1) % p.getVertexCount());

                edgeMap.push_back(Polygon::Edge(a, b));
            }
        }
        ...
        Polygon::EdgeMap::const_iterator itEnd = edgeMap.end();
        while (!edgeMap.empty())
        {
            ...
            for (; it != itEnd; ++it)
            {
                if (itStart->first.equal(it->second) &&
                    itStart->second.equal(it->first))
                {
                    edgeMap.erase(it);
    ...
    //-----------------------------------------------------------------------
    bool findAndEraseEdgePair(const Vector3f & vec, Polygon::EdgeMap & intersectionEdges,
        Vector3f & vNext)
    {
        Polygon::EdgeMap::iterator it, itend = intersectionEdges.end();
        for (it = intersectionEdges.begin(); it != itend; ++it)
        {
            if (it->first.equal(vec))
            {
                vNext = it->second;
    ...
    // Statics
    //-----------------------------------------------------------------------
    ConvexBody::PolygonList ConvexBody::mFreeList;
#if NII_THREAD_SUPPORT
    N_mutex(ConvexBody::msFreePolygonsMutex)
#endif
    //-----------------------------------------------------------------------
    void ConvexBody::setupPool()
    {
        N_lock_mutex(msFreePolygonsMutex)

        if(mFreeList.empty())
        {
            const NCount initialSize = 30;

            // Initialise polygon pool with 30 polys
            mFreeList.resize(initialSize);
            for(NCount i = 0; i < initialSize; ++i)
            {
                mFreeList[i] = N_new_t(Polygon)();
            }
        }
    }
    //-----------------------------------------------------------------------
    void ConvexBody::shutdownPool()
    {
        N_lock_mutex(msFreePolygonsMutex)

        PolygonList::iterator i, iend = mFreeList.end();
        for(i = mFreeList.begin(); i != iend; ++i)
        {
            N_delete_t(*i, Polygon);
        }
        mFreeList.clear();
    }
    //-----------------------------------------------------------------------
    Polygon* ConvexBody::allocatePolygon()
    {
        ...
        if (mFreeList.empty())
        {
            ...
            return N_new_t(Polygon)();
        }
        else
        {
            Polygon * re = mFreeList.back();
            re->cleanAll();
    ...
    {
        for(NCount i = 0; i < cpy.getPolygonCount(); ++i)
        {
    ...
    void ConvexBody::define(const Frustum & frustum)
    {
        ...
        const Vector3f * pts = frustum.getClipPlaneIntersect();
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::define(const AABox& aab)
    {
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::clip(const AABox & aab)
    {
        // ordering of the AAB points:
        //      1-----2
        //     /|    /|
        ...
        // front
        p.set(Vector3f::Z, max);
        clip(p);

        // back
        p.set(Vector3f::INV_Z, min);
        clip(p);

        // left
        p.set(Vector3f::INV_X, min);
        clip(p);

        // right
        p.set(Vector3f::X, max);
        clip(p);

        // bottom
        p.set(Vector3f::INV_Y, min);
        clip(p);

        // top
        p.set(Vector3f::Y, max);
        clip(p);
    ...
    void ConvexBody::clip(const Frustum & fr)
    {
        ...
        for(NCount i = 0; i < 6; ++i)
        {
            ...
            clip(fr.getClipPlane((Frustum::PlaneType)i), false);
        }
    }
    //-----------------------------------------------------------------------
    void ConvexBody::clip(const ConvexBody & body)
    {
        ...
        for(NCount iPoly = 0; iPoly < body.getPolygonCount(); ++iPoly)
        {
            const Polygon & p = body.getPolygon( iPoly );

            N_assert(p.getVertexCount() >= 3, "A valid polygon must contain at least three vertices." );

            // set up plane with first three vertices of the polygon (a polygon is always planar)
            pl.set(p.getVertex(0), p.getVertex(1), p.getVertex(2));

            clip(pl);
        }
    }
    //-----------------------------------------------------------------------
    void ConvexBody::extend(const Vector3f & pt)
    {
        ...
        for(NCount i = 0; i < getPolygonCount(); ++i)
        {
            const Vector3f & normal = getNormal(i);
            Vector3f ptDir  = pt - getVertex(i, 0);
            ptDir.normalise();

            // remove polygon if dot product is greater or equals null.
            if(normal.dotProduct(ptDir) >= 0)
            {
                mPolygons[i]->getEdge(&edgeMap);
                deletePolygon(i);
        ...
        Polygon::EdgeMap::iterator itStart, itEnd = edgeMap.end();
        for(itStart = edgeMap.begin(); itStart != itEnd;)
        {
            ...
            for(; it != itEnd; ++it)
            {
                if (itStart->first.equal(it->second) && itStart->second.equal(it->first))
                {
                    edgeMap.erase(it);
    ...
    //-----------------------------------------------------------------------
    NCount ConvexBody::getPolygonCount() const
    {
        return mPolygons.size();
    }
    //-----------------------------------------------------------------------
    NCount ConvexBody::getVertexCount( NCount poly ) const
    {
        N_assert(poly < getPolygonCount(), "Search position out of range" );
    ...
    bool ConvexBody::hasClosedHull() const
    {
        ...
        Polygon::EdgeMap edgeMap;
        getSingleEdges(mPolygons, edgeMap);
    ...
    void ConvexBody::mergePolygons()
    {
        ...
        for(NCount iPolyA = 0; iPolyA < getPolygonCount(); ++iPolyA)
        {
            for(NCount iPolyB = iPolyA + 1; iPolyB < getPolygonCount(); ++iPolyB)
            {
                const Vector3f & n1 = getNormal(iPolyA);
                const Vector3f & n2 = getNormal(iPolyB);

                NIIf dot = n1.dotProduct(n2);
                Radian angle = Math::ACos(dot);

                // if the normals point into the same direction
                if(Math::Abs(angle.getValue()) <= Radian(Angle(0.00001f)).getValue())
                {
                    // indicates if a neighbor has been found and joined
                    bool bFound = false;

                    // search the two fitting vertices (if there are any) for the common edge
                    const NCount numVerticesA = getVertexCount( iPolyA );
                    for ( NCount iVertexA = 0; iVertexA < numVerticesA; ++iVertexA )
                    {
                        const NCount numVerticesB = getVertexCount( iPolyB );
                        for ( NCount iVertexB = 0; iVertexB < numVerticesB; ++iVertexB )
                        {
                            const Vector3f& aCurrent    = getVertex( iPolyA, iVertexA );
                            const Vector3f& aNext        = getVertex( iPolyA, (iVertexA + 1) % getVertexCount( iPolyA ) );
                            const Vector3f& bCurrent    = getVertex( iPolyB, iVertexB );
                            const Vector3f& bNext        = getVertex( iPolyB, (iVertexB + 1) % getVertexCount( iPolyB ) );

                            // if the edge is the same the current vertex of A has to be equal to the next of B and the other
                            // way round
                            if ( aCurrent.equal(bNext) &&
                                 bCurrent.equal(aNext))
                            {
                                ...
                                for ( NCount i = 0; i <= iVertexB; ++i )
                                {
                                    pNew->insertVertex( getVertex( iPolyB, i%numVerticesB ) );
                                }

                                // insert all vertices of A _after_ the join to the end
                                for ( NCount i = iVertexA + 2; i < numVerticesA; ++i )
                                {
                                    pNew->insertVertex( getVertex( iPolyA, i ) );
                                }

                                // in case there are NIId vertices (in special cases), remove them
                                for ( NCount i = 0; i < pNew->getVertexCount(); ++i )
                                {
                                    const Vector3f& a = pNew->getVertex( i );
                                    const Vector3f& b = pNew->getVertex( (i + 1) % pNew->getVertexCount() );
                                ...
                                N_assert( iPolyA != iPolyB, "PolyA and polyB are the same!" );
    ...
    //-----------------------------------------------------------------------
    const Vector3f& ConvexBody::getNormal( NCount poly )
    {
        N_assert( poly < getPolygonCount(), "Search position out of range" );
    ...
    //-----------------------------------------------------------------------
    AABox ConvexBody::getAABB() const
    {
        AABox aab;

        for(NCount i = 0; i < getPolygonCount(); ++i)
        {
            for(NCount j = 0; j < getVertexCount(i); ++j)
            {
    ...
    bool ConvexBody::operator == (const ConvexBody & rhs) const
    {
        ...
        bool *bChecked = N_alloc_t(bool, getPolygonCount());
        for ( NCount i=0; i<getPolygonCount(); ++i )
        {
            bChecked[ i ] = false;
        }
        ...
        for ( NCount i=0; i<getPolygonCount(); ++i )
        {
            ...
            for ( NCount j=0; j<getPolygonCount(); ++j )
            {
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::insertPolygon(Polygon* pdata, NCount poly )
    {
        N_assert(poly <= getPolygonCount(), "Insert position out of range" );
        N_assert( pdata != NULL, "Polygon is NULL" );
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::insertVertex(NCount poly, NCount veridx, const Vector3f & vertex)
    {
        N_assert(poly < getPolygonCount(), "Search position (polygon) out of range" );

        mPolygons[poly]->insertVertex(vertex, veridx);
    }
    //-----------------------------------------------------------------------
    void ConvexBody::insertVertex(NCount poly, const Vector3f & vdata)
    {
        N_assert(poly < getPolygonCount(), "Search position (polygon) out of range" );
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::deletePolygon(NCount poly)
    {
        N_assert(poly < getPolygonCount(), "Search position out of range" );
    ...
    //-----------------------------------------------------------------------
    Polygon* ConvexBody::popPolygon(NCount poly)
    {
        N_assert( poly < getPolygonCount(), "Search position out of range" );
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::swap(ConvexBody & body)
    {
        body.mPolygons.swap(mPolygons);
    }
    //-----------------------------------------------------------------------
    void ConvexBody::deleteVertex(NCount poly, NCount vertex)
    {
        N_assert(poly < getPolygonCount(), "Search position out of range" );
    ...
    //-----------------------------------------------------------------------
    const Polygon & ConvexBody::getPolygon(NCount poly) const
    {
        N_assert(poly < getPolygonCount(), "Search position out of range");
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::setPolygon(NCount poly, Polygon * pdata)
    {
        N_assert(poly < getPolygonCount(), "Search position out of range" );
        N_assert(pdata != NULL, "Polygon is NULL" );
    ...
    //-----------------------------------------------------------------------
    const Vector3f & ConvexBody::getVertex(NCount poly, NCount vertex) const
    {
        N_assert( poly < getPolygonCount(), "Search position out of range" );
    ...
    //-----------------------------------------------------------------------
    void ConvexBody::setVertex(NCount poly, NCount vertex, const Vector3f & vdata)
    {
        N_assert(poly < getPolygonCount(), "Search position out of range");
    ...
    void ConvexBody::clip(const Plane & pl, bool keepNegative)
    {
        ...
        N_assert(this->getPolygonCount() == 0, "Body not empty!");
        N_assert(current.getPolygonCount() != 0, "Body empty!");
        ...
        for ( NCount iPoly = 0; iPoly < current.getPolygonCount(); ++iPoly )
        {
            ...
            Plane::PositionType clipSide = keepNegative ? Plane::PT_Positive : Plane::PT_Negative;
            // - side is clipSide: vertex will be clipped
            // - side is !clipSide: vertex will be untouched
            // - side is NOSIDE:   vertex will be untouched
            Plane::PositionType * side = N_alloc_t(Plane::PositionType, vertexCount);
            for(NCount iVertex = 0; iVertex < vertexCount; ++iVertex)
            {
            ...
            for ( NCount iVertex = 0; iVertex < vertexCount; ++iVertex )
            {
                ...
                NCount iNextVertex = ( iVertex + 1 ) % vertexCount;

                const Vector3f& vCurrent = p.getVertex( iVertex );
                const Vector3f& vNext    = p.getVertex( iNextVertex );
                          ...
                          side[ iNextVertex ]    == clipSide )
                {
                    ...
                    Vector3f vDirection = vCurrent - vNext;
                    vDirection.normalise();
                    Ray ray(vNext, vDirection);
                    std::pair<bool, NIIf> intersect;
                    intersect.first = ray.intersects(pl, intersect.second);

                    // store intersection
                    if(intersect.first)
                    {
                        // convert distance to vector
                        Vector3f vIntersect = ray.getPoint(intersect.second);
                ...
                else if(side[iVertex] == clipSide && side[iNextVertex] != clipSide)
                {
                    ...
                    // intersect from the outside vertex towards the inside one
                    Vector3f vDirection = vNext - vCurrent;
                    vDirection.normalise();
            ...
            {
                intersectionEdges.push_back( Polygon::Edge( pIntersect->getVertex( 0 ),
        ...
        if ( intersectionEdges.size() >= 3 )
        {
            ...
            Vector3f vFirst  = it->first;
            Vector3f vSecond = it->second;
            ...
            if(findAndEraseEdgePair(vSecond, intersectionEdges, vNext))
            {
                ...
                Vector3f vCross = (vFirst - vSecond).crossProduct(vNext - vSecond);

                NIIf dot = (pl.mNormal).dotProduct(vCross);
                Radian angle = Math::ACos(dot);

                bool frontside = Math::Abs(angle.getValue()) <= Angle(1).getRadian();
                ...
                // continue this operation as NIIl as edges are present
    ...
    //-----------------------------------------------------------------------
    Nostream & operator<< (Nostream & out, const ConvexBody & obj)
    {
        out << "ConvexBody (" << obj.getPolygonCount() << ")" << std::endl;

        for (NCount i = 0; i < obj.getPolygonCount(); ++i)
        {
            out << "POLYGON (" << i << "), ";
            out << obj.getPolygon(i);
        }
