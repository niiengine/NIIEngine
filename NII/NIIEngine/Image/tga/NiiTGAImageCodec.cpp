...
namespace NII
{
    ...
    TGAImageCodec::TGAImageCodec() :
        ImageCodec(N_IMAGE_TGA, "tga")
    {
    }
    //-------------------------------------------------------------------------
    CodecResult TGAImageCodec::decode(DataStream * in, Image * out) const
    {
        N_Only(Log).log(_T("TGAImageCodec::load()"));
        ImageTGA * img = loadTGA(in.getData(), in.size());
        if(img != 0)
        {
            PixelFormat fmt = (img->channels == 3) ? PF_B8G8R8UN : PF_A8B8G8R8;
            out->load(img->data, img->sizeX, img->sizeY, fmt, true);
            N_delete img;
    ...
    void TGAImageCodec::flipVertImageTGA(ImageTGA * img)
    {
        ...
        for(int line = 0; line < img->sizeY / 2; ++line)
        {
            ...
            {
                Nui8 tmp = img->data[dstOffest + colBit];
    ...
    void TGAImageCodec::flipHorzImageTGA(ImageTGA * img)
    {
        ...
        for(int line = 0; line < img->sizeY; ++line)
        {
            ...
            for(int pix = 0; pix < img->sizeX / 2; ++pix)
            {
                ...
                {
                    const Nui8 tmp = img->data[dstOffest + c];
    ...
    /////
    ///////////////////////////////// LOAD TGA \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
     TGAImageCodec::ImageTGA * TGAImageCodec::loadTGA(const unsigned char * buffer,
        NCount /*buffer_size*/)
    {
        typedef unsigned char byte;

        ImageTGA * pImageData = 0;		// This stores our important image data
        short width = 0, height = 0;	// The size of the image
        ...
        if(imageType != TGA_RLE)
        {
            ...
            if(bits == 24 || bits == 32)
            {
                ...
                pImageData->data = N_alloc_t(Nui8, stride * height);
