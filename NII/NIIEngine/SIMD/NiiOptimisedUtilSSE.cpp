...
#include "NiiSIMDHelper.h"

// I'd like to merge this file with NiiOptimisedUtil.cpp, but it's
...
#define NII_SSE_SKINNING_UNROLL_VERTICES  16

namespace NII 
{
    class _EngineInner OptimisedUtilSSE : public SIMDUtil
    {
    protected:
    ...
    public:
        ...
        /// @copydoc SIMDUtil::softwareVertexSkinning
        virtual void NII_SIMD_ALIGN_ATTRIBUTE softwareVertexSkinning(
        ...
        /// @copydoc SIMDUtil::softwareVertexMorph
        virtual void NII_SIMD_ALIGN_ATTRIBUTE softwareVertexMorph(
            float t,
        ...
        /// @copydoc SIMDUtil::affine
        virtual void NII_SIMD_ALIGN_ATTRIBUTE affine(
            Matrix4f* dstMatrices,
            const Matrix4f & baseMatrix,
            const Matrix4f* srcMatrices,
            size_t numMatrices);

        /// @copydoc SIMDUtil::calculateFaceNormals
        virtual void NII_SIMD_ALIGN_ATTRIBUTE calculateFaceNormals(
            const float *positions,
            const GeoEdget::Triangle *triangles,
            Vector4f *faceNormals,
            size_t numTriangles);

        /// @copydoc SIMDUtil::calculateLightFacing
        virtual void NII_SIMD_ALIGN_ATTRIBUTE calculateLightFacing(
            const Vector4f& lightPos,
            const Vector4f* faceNormals,
            char* lightFacings,
            size_t numFaces);

        /// @copydoc SIMDUtil::extrude
        virtual void NII_SIMD_ALIGN_ATTRIBUTE extrude(
            const Vector4f& lightPos,
            float extrudeDist,
...
#if defined(NII_SIMD_ALIGN_STACK)
    /** Stack-align implementation of SIMDUtil.
    @remarks
    ...
    @note
        Don't use this class directly, use SIMDUtil instead.
    */
    class _EngineInner OptimisedUtilWithStackAlign : public SIMDUtil
    {
    protected:
    ...
    public:
        /// Constructor
        OptimisedUtilWithStackAlign(SIMDUtil* impl)
            : mImpl(impl)
        {
        }
...
#endif  // !defined(NII_SIMD_ALIGN_STACK)
...
//
// :) Thank row-major matrix used in NII, it make we accessing affine matrix easy.
//---------------------------------------------------------------------
...
#define _collapseOneMatrix(                                                     \
    ...
    {                                                                           \
        ...
        const Matrix4f* pMatrix0, *pMatrix1, *pMatrix2, *pMatrix3;               \
...
#define _collapseFourMatrices(                                                  \
    ...
    static N_FORCEINLINE void softwareVertexSkinning_SSE_PosNorm_Shared_Packed(
        const float* pSrcPos, float* pDestPos,
        const float* pBlendWeight, const unsigned char* pBlendIndex,
        const Matrix4f* const* blendMatrices,
        size_t blendWeightStride, size_t blendIndexStride,
    ...
    static N_FORCEINLINE void softwareVertexSkinning_SSE_PosNorm_Separated_Packed(
        const float* pSrcPos, float* pDestPos,
        const float* pSrcNorm, float* pDestNorm,
        const float* pBlendWeight, const unsigned char* pBlendIndex,
        const Matrix4f* const* blendMatrices,
    ...
    static N_FORCEINLINE void softwareVertexSkinning_SSE_PosOnly_Packed(
        const float* pSrcPos, float* pDestPos,
        const float* pBlendWeight, const unsigned char* pBlendIndex,
        const Matrix4f* const* blendMatrices,
        size_t blendWeightStride, size_t blendIndexStride,
        ...
        : mPreferGeneralVersionForSharedBuffers(false)
    {
        ...
        //
#if N_HAVE_NEON == 0
        if (PlatInfo::getIdentifier().find(_T("AuthenticAMD")) != String::npos)
        {
            ...
            //
            if (!(PlatInfo::isSupport(SimdInfo::CS_SSE2) || PlatInfo::isSupport(SimdInfo::CS_SSE3)))
            {
        ...
        size_t numVertices)
    {
        ...
        NII_CHECK_STACK_ALIGNED_FOR_SSE();
        ...
        if (numVertices > NII_SSE_SKINNING_UNROLL_VERTICES)
        {
            if (pSrcNorm)
            {
    ...
    //---------------------------------------------------------------------
    void OptimisedUtilSSE::affine(
        Matrix4f* pDstMat,
        const Matrix4f& baseMatrix,
        const Matrix4f* pSrcMat,
        size_t numMatrices)
    {
        NII_CHECK_STACK_ALIGNED_FOR_SSE();
...
#define __LOAD_VECTOR3(p)   _mm_loadh_pi(_mm_load_ss(p), (const __m64*)((p)+1))
        ...
        N_SIMD_ALIGNED_DECL(static const Nui32, msSignMask[4]) =
        {
...
#define __LOAD_FOUR_VECTOR3(x, y, z, p0, p1, p2, p3)                    \
            ...
            __LOAD_FOUR_VECTOR3(x0, y0, z0,
                positions + triangles[0].mVertIndex[0] * 3,
                positions + triangles[1].mVertIndex[0] * 3,
                positions + triangles[2].mVertIndex[0] * 3,
                positions + triangles[3].mVertIndex[0] * 3);

            // Load vertex 1 of four triangles, packed as component-major format: xxxx yyyy zzzz
            __LOAD_FOUR_VECTOR3(x1, y1, z1,
                positions + triangles[0].mVertIndex[1] * 3,
                positions + triangles[1].mVertIndex[1] * 3,
                positions + triangles[2].mVertIndex[1] * 3,
                positions + triangles[3].mVertIndex[1] * 3);

            // Load vertex 2 of four triangles, packed as component-major format: xxxx yyyy zzzz
            __LOAD_FOUR_VECTOR3(x2, y2, z2,
                positions + triangles[0].mVertIndex[2] * 3,
                positions + triangles[1].mVertIndex[2] * 3,
                positions + triangles[2].mVertIndex[2] * 3,
                positions + triangles[3].mVertIndex[2] * 3);
...
#undef __LOAD_FOUR_VECTOR3
        ...
        for (size_t j = 0; j < numTriangles; ++j)
        {
            ...
            __m128 v0 = __LOAD_VECTOR3(positions + triangles->mVertIndex[0] * 3);
            __m128 v1 = __LOAD_VECTOR3(positions + triangles->mVertIndex[1] * 3);
            __m128 v2 = __LOAD_VECTOR3(positions + triangles->mVertIndex[2] * 3);
            ++triangles;
...
#undef __LOAD_VECTOR3
        ...
        size_t numFaces)
    {
        ...
        for (size_t i = 0; i < numIterations; ++i)
        {
            ...
            *reinterpret_cast<uint32*>(lightFacings) =
                ...
                */
            memcpy(lightFacings, msMaskMapping[bitmask], sizeof(Nui32));
    ...
    //---------------------------------------------------------------------
    //---------------------------------------------------------------------
    //extern SIMDUtil* _getOptimisedUtilSSE();
    extern SIMDUtil* _getOptimisedUtilSSE()
    {
