...
inline Colour col(const vrmllib::col3 &c) { return Colour(c.r, c.g, c.b); }
inline void copyVec(NIIf *d, const Vector3 &s) { d[0] = s.x; d[1] = s.y; d[2] = s.z; }
inline void copyVec(NIIf *d, const vec2 &s) { d[0] = s.x; d[1] = s.y; }
...
try
{
	...
	Math math;
    ResourceSchemeManager resGrpMgr;
...
void parseNode(Mesh * mesh, const vrmllib::node * n, Matrix4 m)
{
	if(const Transform * tr = dynamic_cast<const Transform *>(n))
    {
		...
		TranslationMatrix4 trans(vec(tr->translation));
...
try
{
	...
	if (!material->isNull() && app) {
		...
		sub->setMaterial(material->getName());
	} else {
		String matName;
		const String *mn;
		if (mn = findName(app))
	...
	const Shape *sh, const Matrix4 & mat)
{
	...
	geom.pVertices = new NIIf[nvertices*3];
	if (tcs)
		geom.pTexCoords[0] = new NIIf[nvertices*2];
	if (norm)
		geom.pNormals = new NIIf[nvertices*3];
	if (color)
	...
	for (int i=0; i!=nvertices; ++i) {
		...
		NIIf *pos = geom.pVertices + i*3;
		NIIf *tc = geom.pTexCoords[0] + i*2;
		NIIf *n = geom.pNormals + i*3;
...
Ogre::MaterialPtr parseMaterial(const Appearance *app, const String &name)
{
	...
	Ogre::MaterialPtr m = MaterialManager::getSingleton().create(name, GroupDefault);

	Colour diffuse = texture ? Colour::White : col(vm->diffuseColor);
    ...
    m->setColour(ShaderChColour(a, diffuse, col(vm->specularColor), col(vm->emissiveColor),
        vm->shininess, CM_M_AMBIENT | CM_M_DIFFUSE | CM_M_SPECULAR | CM_M_EMISSIVE));
	...
	if(texture && !texture->url.empty()) 
    {
		...
		Ogre::ShaderChTextureUnit * l = m->addTextureLayer(texName);
        TextureAddressingMode uwv = texture->repeatS ? TAM_REPEAT : TAM_CLAMP;
        l->setUAddressing(uwv);
        l->setVAddressing(uwv);
        l->setWAddressing(uwv);
	}
...
{
	if(inverse)
		return TranslationMatrix4(-v.x, -v.y, -v.z);
	else
		return TranslationMatrix4(v.x, v.y, v.z);
}
...
{
	if(inverse)
		return ScaleMatrix4(1/v.x, 1/v.y, 1/v.z);
	else
		return ScaleMatrix4(v.x, v.y, v.z);
}
