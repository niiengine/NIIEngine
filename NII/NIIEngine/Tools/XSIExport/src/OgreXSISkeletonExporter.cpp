...
namespace Ogre
{
	...
	//-----------------------------------------------------------------------------
	const AABox& XsiSkeletonExporter::exportSkeleton(const String& skeletonFileName,
	   ...
	   */
		...
		for(DeformerMap::iterator i = deformers.begin(); i != deformers.end(); ++i)
		{
			...
			if(!deformer->parentName.empty())
			{
				...
				parent->pBone->add(deformer->pBone);
		...
		AnimationList& animList)
	{
	   ...
	   */
		...
		{
			String name = XSItoOgre(deformer->obj.GetName());
			deformer->pBone = pSkeleton->create(name, deformer->boneID);
			...
			deformer->pBone->setPos(XSItoOgre(trans.GetTranslation()));
			deformer->pBone->setOri(XSItoOgre(trans.GetRotation().GetQuaternion()));
			...
			if (deformer->parentIsChainEndEffector)
			{
				...
				deformer->pBone->setPos(deformer->pBone->getPos() + Vector3::X * boneLen);
			}
	...
	void XsiSkeletonExporter::createAnimations(Skeleton* pSkel,
		DeformerMap& deformers, float fps, AnimationList& animList, AABox& AABBPadding)
	{
		for (AnimationList::iterator ai = animList.begin(); ai != animList.end(); ++ai)
		{
			...
			TimeDurMS animLength = (animEntry.endFrame - animEntry.startFrame) / fps;
            ...
            AABox & AABBPadding)
	{
		...
		std::vector<NodeKeyFrameTrack *> deformerTracks;
		deformerTracks.resize(deformers.size());
		for(DeformerMap::iterator di = deformers.begin(); di != deformers.end(); ++di)
		{
			DeformerEntry * deformer = di->second;

            NodeKeyFrameTrack * track = N_new NodeKeyFrameTrack(pAnim, deformer->boneID);
            track->setTarget(deformer->pBone);
		...
		NIIf time, float fps, AABox & AABBPadding)
	{
		...
		{
			DeformerEntry * deformer = di->second;
			NodeKeyFrameTrack * track = deformerTracks[deformer->boneID];
			...
			{
				// Based on global
				transformation = deformer->obj.GetKinematics().GetGlobal().GetTransform();
			}
			else
			{
				// Based on local
				transformation = deformer->obj.GetKinematics().GetLocal().GetTransform();
			}
