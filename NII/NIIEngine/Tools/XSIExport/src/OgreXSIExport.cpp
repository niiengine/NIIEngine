...
XSI::CStatus OgreMeshExportCommand_Execute( XSI::CRef& in_context )
{
	...
	try
	{
		...
		{
			N_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, 
		...
		NII::VertexType tangentSemantic = (tangentSem == true)?
		...
		// create singletons
		Ogre::ResourceSchemeManager rgm;
		Ogre::MeshManager meshMgr;
		Ogre::SkeletonManager skelMgr;
		Ogre::MaterialManager matMgr;
		Ogre::MemGBufferManager hardwareBufMgr;
	...
	catch (Ogre::Exception& e)
	{
		...
		// Tell XSI
		app.LogMessage(OgretoXSI(e.getVerbose()), XSI::siFatalMsg);
...
{	
	...
	try
	{
		...
		if (ret == CStatus::OK)
		{
			...
			Ogre::VertexType tangentSemantic = (tangentSemStr == L"t")?
				NII::VT_Tangent : NII::VT_Tex_Coord;
...
CStatus OgreMeshExportOptions_PPGEvent( const CRef& io_Ctx )
{
    ...
    if ( eventID == PPGEventContext::siOnInit )
	{
		...
		// init it with all detected animations if it's empty
