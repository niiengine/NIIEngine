...
namespace Ogre {
	...
	DeformerMap& XsiMeshExporter::buildMeshForExport(
		...
		bool edgeLists, bool tangents, VertexType tangentSemantic,
		bool tangentsSplitMirrored, bool tangentsSplitRotated, bool tangentsUseParity,
		bool vertexAnimation, AnimationList& animList, NIIf fps, const String& materialPrefix,
		LodData* lod, const String& skeletonName)
    {
        ...
        // Construct mesh
        mMesh = N_Only(Mesh).createManual("XSIExport", GroupDefault);
		...
		{
			mMesh->setSkeletonID(skeletonName);
		}
        ...
        if(tangents)
        {
            ...
            if(!mMesh->checkTangentBuffer(coordtype))
            {
                mMesh->buildTangentBuffer(tangentSemantic, tangentsUseParity,
	...
	//-----------------------------------------------------------------------
	void XsiMeshExporter::exportMesh(const String& fileName, const AABox& skelAABB)
	{

		// Pad bounds
		AABox currBounds = mMesh->getAABB();
		currBounds.merge(skelAABB);
		mMesh->setAABB(currBounds);
		...
		bool lookForBoneAssignments, unsigned short progressUpdates)
	{
		...
		for (long t = 0; t < xsiMesh->geometry.GetTriangleCount(); ++t)
		{
            ...
            for (long p = 0; p < 3; ++p)
            {
				...
				if (first)
				{
					squaredRadius = vertex.position.lengthSquared();
					min = max = vertex.position;
					first = false;
				}
				else
				{
					squaredRadius = std::max(squaredRadius, vertex.position.lengthSquared());
					min = min.floor(vertex.position);
					max = max.ceil(vertex.position);
				}
			}
								...
								if (poli != ps->polygonMeshOffsetMap.end())
								{
									...
									if(remi != ps->posIndexRemap.end())
									{
										...
										while(moreVerts)
										{
											...
											SkeletonVertex vba;
											vba.mBoneIndex = deformerEntry->boneID;
											vba.mVertexIndex = vertIndex;
											vba.mWeight = weight;
											ps->boneAssignments.insert(Mesh::SkeletonVertexList::value_type(vertIndex, vba));
				...
				mi != mMaterialProtoSubmeshMap.end(); ++mi)
			{
					...
					psi != mi->second->end(); ++psi)
				{
					...
					if (poli != ps->polygonMeshOffsetMap.end())
					{
						...
						VertexOffset pose(0, XSItoOgre(shapeKey.GetName()));
		...
		NIIf fps)
	{
		...
		for (AnimationList::iterator i = animList.begin(); i != animList.end(); ++i)
		{
			...
			for (ushort targetIndex = 1; targetIndex <= pMesh->getSubCount(); ++targetIndex)
			{
				...
				if(!submeshClipList.empty())
				{
					...
					if(!anim)
					{
						NIIf len = (float)(animEntry.endFrame - animEntry.startFrame + 1) / fps;
						anim = pMesh->createAnimation(animEntry.animationName, len);
					}

					// Create track
                    VertexOffsetKeyFrameTrack * track = N_new VertexOffsetKeyFrameTrack(anim, targetIndex);
						...
						k != keyframeList.end(); ++k)
					{
						// Create a key
						long frameNum = *k;
						NIIf keyTime = (float)(frameNum - animEntry.startFrame) / fps;
						VertexOffsetKeyFrame * keyFrame =
							static_cast<VertexOffsetKeyFrame *>(track->create(keyTime)));
								...
								frameNum <= sce.endFrame)
							{
								...
								for (int p = 0; p < params.GetCount(); ++p)
								{
									...
									if (param.GetSource().IsA(siFCurveID))
									{
										...
										if ((float)val != 0.0f)
										{
											keyFrame->add(sce.keytoPose->poseIndex, val);
										}
								...
								if (!fcurveFound)
								{
									...
									keyFrame->add(sce.keytoPose->poseIndex, 1.0f);
	...
	void XsiMeshExporter::exportProtoSubMesh(Mesh * pMesh, ProtoSubMesh * proto)
	{
        ...
        else
        {
            // named submesh
            sm = pMesh->createSub();
            sm->setName(proto->name);
        }

        // Set material
        sm->setMaterial(proto->materialName);
        ...
        sm->getIndexData(0)->mCount = proto->indices.size();
		smvd->mCount = proto->uniqueVertices.size();
        ...
        N_Only(GBuffer).create(sm->getIndexData(0)->mBuffer, use32BitIndexes ? 32: 16, sm->getIndexData(0)->mCount, Buffer::M_NORMAL );
        if(use32BitIndexes)
        {
        ...
        {
			DataType uvType = DataTypeUtil::getMultiType(
                DT_Float1x, proto->textureCoordDimensions[uvi]);
            smvd->add(uvType, VT_Tex_Coord, buf, offset, uvi);
            offset += VertexUnit::getTypeSize(uvType);
        }

        // create & fill buffer(s)
        for(unsigned short b = 0; b < smvd->mBufferCount; ++b)
        {
            createVertexBuffer(smvd, b, proto->uniqueVertices);
        }
		...
		if(!proto->boneAssignments.empty())
		{
			...
			SkeletonManager::opt(proto->boneAssignments, smvd->mCount);
            Mesh::SkeletonVertexList::iterator bi, biend = proto->boneAssignments.end();
			for(bi = proto->boneAssignments.begin(); bi != biend; ++bi)
			{
				sm->addSkeleton(bi->second);
			}
		}

		// poses
		// derive target index (current submesh index + 1 since 0 is shared geom)
		ushort targetIndex = pMesh->getSubCount();
		ushort sk = 0;
		for(std::list<VertexOffset>::iterator pi = proto->poseList.begin();
			pi != proto->poseList.end(); ++pi, ++sk)
		{
			VertexOffset * pose = pMesh->createOffset(pi->getName(), targetIndex);
			VertexOffset::OffsetMap i, iend = pi->getVertexOffsets().end();
			while (i.isNextValid())
			{
				pose->add(i.getCurrentKey(), i.getCurrentValue());
				i.getAndNext();
			}
			...
			se.poseIndex = pMesh->getOffsetCount() - 1;
    ...
    {
        IndexList::const_iterator i, iend = indexes.end();
    ...
    //-----------------------------------------------------------------------
    void XsiMeshExporter::createVertexBuffer(VertexData * vd, unsigned short bufIdx, UniqueVertexList & uniqueVertexList)
    {
        VertexBuffer * vbuf;
        N_Only(GBuffer).create(vbuf, vd->getUnitSize(bufIdx), vd->mCount, Buffer::Buffer::M_NORMAL );
        ...
        for(size_t v = 0; v < vd->mCount; ++v, ++srci)
        {
            for(ei = elems.begin(); ei != eiend; ++ei)
            {
                ...
                case VT_Tex_Coord:
                    elem.getData(pBase, pFloat);
					for (int t = 0; t < VertexUnit::getTypeCount(elem.mType); ++t)
					{
						NIIf val = srci->uv[elem.mIndex][t];
						*pFloat++ = val;
					}
                    break;
                }
