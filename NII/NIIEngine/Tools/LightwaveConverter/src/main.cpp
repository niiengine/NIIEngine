...
LogManager* logMgr;
ResourceSchemeManager* resourceGroupMgr;
Math* mth;
...
int main( int argc, char *argv[] )
{
	...
	if (!flags[InfoOnly])
	{
		...
		resourceGroupMgr = new ResourceSchemeManager();
		mth = new Math();
		...
		bufferMgr = new MemGBufferManager(); // needed because we don't have a rendersystem
	}
