...
void WorkspacePanel::appendNewMenu(wxMenu* menu)
{
	...
	newMenu->Append(ID_MENU_NEW_TECHNIQUE, wxT("ShaderFusion"));
	newMenu->Append(ID_MENU_NEW_PASS, wxT("ShaderCh"));
...
TechniqueController* WorkspacePanel::getShaderFusion(wxTreeItemId id)
{
...
{
	return getShaderFusion(id) != NULL;
}
...
void WorkspacePanel::OnSelectionChanged(wxTreeEvent & event)
{
	...
	if(isProject(id))
        ...
        list.push_back(getMaterial(id));
	else if(isTechnique(id))
        list.push_back(getShaderFusion(id));
	else if(isPass(id))
...
void WorkspacePanel::OnNewTechnique(wxCommandEvent& event)
{
	...
	wizard->Create(this, wxID_ANY, wxT("New ShaderFusion"), wxNullBitmap, wxDefaultPosition, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
...
void WorkspacePanel::OnNewPass(wxCommandEvent& event)
{
	...
	else if(isTechnique(selId))
	{
		...
		technique = getShaderFusion(selId);
	}

	PassWizard* wizard = new PassWizard();
	wizard->Create(this, wxID_ANY, wxT("New ShaderCh"), wxNullBitmap, wxDefaultPosition, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
...
void WorkspacePanel::materialTechniqueAdded(EventArgs & args)
{
	...
	wxTreeItemId materialId = mMaterialIdMap[mc];
	wxTreeItemId id = mTreeCtrl->AppendItem(materialId, tc->getShaderFusion()->getName().c_str(), TECHNIQUE_IMAGE);
	mTreeCtrl->SelectItem(id, true);
...
void WorkspacePanel::techniqueNameChanged(EventArgs & args)
{
	...
	mTreeCtrl->SetItemText(techniqueId, tc->getShaderFusion()->getName().c_str());
}
