...
void PassPage::createPage()
{
	...
	// ShaderFusion Label
	mTechniqueLabel = new wxStaticText(this, wxID_ANY, wxT("ShaderFusion:"), wxDefaultPosition, wxDefaultSize, 0);
	mSizer->Add(mTechniqueLabel, 0, wxALL, 0);

	// ShaderFusion Combo Box
...
TechniqueController* PassPage::getShaderFusion() const
{
...
{
	mTechniqueComboBox->SetValue(tc != NULL ? tc->getShaderFusion()->getName().c_str() : wxEmptyString);
}
...
void PassPage::populateTechniques(const TechniqueControllerList* techniques)
{
	...
	TechniqueControllerList::const_iterator it, itend = techniques->end();
	for(it = techniques->begin(); it != itend; ++it)
	{
		techniqueNames.Add((*it)->getShaderFusion()->getName().c_str());
	}
