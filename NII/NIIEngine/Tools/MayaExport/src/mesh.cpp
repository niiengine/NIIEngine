	...
	MStatus Mesh::getVertexBoneWeights(const MDagPath& meshDag, OgreMayaExporter::ParamList &params)
	{
		...
		for (int i=0; !iterGeom.isDone(); iterGeom.next(), i++)
		{
			...
			if(m_pSkeleton)
			{
				...
				newjointIds[i].setDur(newweights[i].length());
...
/*********************************** Export mesh data **************************************/
	...
	MStatus Mesh::writeOgreBinary(ParamList &params)
	{
		...
		// Construct mesh
		Ogre::MeshPtr obj = NII::N_Only(Mesh).createManual(m_name.asChar(), NII::GroupDefault);

        // Write shared geometry data
		if (params.useSharedGeom)
		{
			createOgreSharedGeometry(obj,params);
		}
		// Write submeshes data
		for (int i=0; i<m_submeshes.size(); i++)
		{
			m_submeshes[i]->createOgreSubmesh(obj,params);
		}
		...
		if (m_pSkeleton && params.exportSkeleton)
		{
			...
			obj->setSkeletonID(filename.asChar());
		}
		// Write poses
		if (params.exportBlendShapes)
		{
			createOgrePoses(obj,params);
		}
		// Write vertex animations
		if (params.exportVertAnims)
		{
			createOgreVertexAnimations(obj,params);
		}
		// Write pose animations
		if (params.exportBSAnims)
		{
			createOgrePoseAnimations(obj,params);
		}
		// Create a bounding box for the mesh
		Ogre::AABox bbox = obj->getAABB();
		for(int i=0; i<m_submeshes.size(); i++)
		{
		...
		{
			NII::VertexType targetSemantic = params.tangentSemantic == TS_TANGENT ?
				NII::VT_Tangent : NII::VT_Tex_Coord;
			bool canBuild = true;
			unsigned short srcTex, destTex;
			try
            {
                bool coordtype = (targetSemantic == VT_Tex_Coord) ? true : false;
				canBuild = !obj->checkTangentBuffer(coordtype);
			} catch(NII::Exception e)
            {
				canBuild = false;
			}
			if(canBuild)
				obj->buildTangentBuffer(targetSemantic, params.tangentsUseParity,
					params.tangentsSplitMirrored, params.tangentsSplitRotated);
		}
		// Export the binary mesh
		Ogre::MeshSerializer serializer;
		serializer.exportMesh(obj.getPointer(),params.meshFilename.asChar());
		obj.setNull();
		return MS::kSuccess;
	}

	// Create shared geometry data for an Ogre mesh
	MStatus Mesh::createOgreSharedGeometry(NII::MeshPtr obj, ParamList & params)
	{
		MStatus stat;
		obj->setVertexData(N_new NII::VertexData());
		obj->getVertexData()->mCount = m_sharedGeom.vertices.size();

		// Create the vertex buffer using the newly created vertex declaration
		stat = createOgreVertexBuffer(obj, m_sharedGeom.vertices,
            params.exportVertNorm, params.exportVertCol);
		// Write vertex bone assignements list
		if(params.exportVBA)
		{
			// Create a new vertex bone assignements list
			NII::Mesh::SkeletonVertexList vbas;
	...
	MStatus Mesh::createOgreVertexBuffer(NII::MeshPtr obj,
        const std::vector<vertex> & vertices, bool nrls, bool cool)
	{
		// Define a new vertex declaration
		NII::VertexData * vd = obj->getVertexData();
		size_t offset = 0;
		// Add vertex position
		vd->add(NII::DT_Float3x, NII::VT_Pos, 0, offset);
		offset += NII::VertexUnit::getTypeSize(NII::DT_Float3x);
		// Add vertex normal
		if(nrls)
		{
			vd->add(NII::DT_Float3x, NII::VT_Normals, 0, offset);
			offset += NII::VertexUnit::getTypeSize(NII::DT_Float3x);
		}
		// Add vertex colour
		if(cool)
	...
	MStatus Mesh::createOgrePoses(Ogre::MeshPtr obj,ParamList & params)
	{
		int poseCounter = 0;
		if(params.useSharedGeom)
		{
			...
			for (int i=0; i<m_submeshes.size(); i++)
			{
				...
				if (pBS)
				{
					...
					for (int j=0; j<pg.poses.size(); j++)
					{
						...
						// Create a new pose for the ogre mesh
						NII::VertexOffset* pPose = obj->createOffset(p->name.asChar(), p->index);
						// Set the pose attributes
						for (int k=0; k<p->offsets.size(); k++)
						{
							Ogre::Vector3 offset(p->offsets[k].x,p->offsets[k].y,p->offsets[k].z);
	...
	MStatus Mesh::createOgreVertexAnimations(Ogre::MeshPtr obj,ParamList& params)
	{
		// Read the list of vertex animation clips
		for (int i=0; i<m_vertexClips.size(); i++)
		{
			// Create a new animation
			Ogre::Animation* pAnimation = obj->createAnimation(m_vertexClips[i].m_name.asChar(),m_vertexClips[i].m_length);
			// Create all tracks for current animation
			for(int j = 0; j < m_vertexClips[i].m_tracks.size(); ++j)
			{
				Track * t = &(m_vertexClips[i].m_tracks[j]);
				// Create a new track
				Ogre::VertexKeyFrameTrack * pTrack;
				if(t->m_target == T_MESH)
                {
                    pTrack = N_new VertexUnitKeyFrameTrack(pAnimation, 0);
                    pTrack->setTarget(obj->getVertexData());
				}
                else
				{
                    pTrack = N_new VertexUnitKeyFrameTrack(pAnimation, t->m_index + 1);
                    pTrack->setTarget(obj->getSub(t->m_index)->getVertexData());
				}
				// Create keyframes for current track
				for (int k = 0; k<t->m_vertexKeyframes.size(); k++)
				{
					// Create a new keyframe
					Ogre::VertexUnitKeyFrame * pKeyframe = static_cast<NII::VertexUnitKeyFrame *>(pTrack->create(t->m_vertexKeyframes[k].time)));
					// Create vertex buffer for current keyframe
					NII::VertexBuffer * pBuffer;
                    NII::N_Only(GBuffer).create(pBuffer,
						NII::VertexUnit::getTypeSize(NII::DT_Float3x),
                            t->m_vertexKeyframes[k].positions.size(),
                                NII::Buffer::M_NORMAL);
					float* pFloat = static_cast<float*>(pBuffer->lock(Ogre::Buffer::MM_WHOLE));
	...
	MStatus Mesh::createOgrePoseAnimations(Ogre::MeshPtr obj,ParamList& params)
	{
		// Get all loaded blend shape clips
		for (int i=0; i<m_BSClips.size(); i++)
		{
			// Create a new animation for each clip
			Ogre::Animation* pAnimation = obj->createAnimation(m_BSClips[i].m_name.asChar(),m_BSClips[i].m_length);
			...
			for (int j=0; j<m_BSClips[i].m_tracks.size(); j++)
			{
				...
				if (t->m_target == T_MESH)
                {
                    VertexOffsetKeyFrameTrack * pTrack = N_new VertexOffsetKeyFrameTrack(pAnimation, 0);
                    pTrack->setTarget(obj->getVertexData());
                }
                else
				{
                    VertexOffsetKeyFrameTrack * pTrack = N_new VertexOffsetKeyFrameTrack(pAnimation, t->m_index);
                    pTrack->setTarget(obj->getSub(t->m_index - 1)->getVertexData());
				}
				// Create keyframes for current track
				for (int k=0; k<t->m_vertexKeyframes.size(); ++k)
				{
					NII::VertexOffsetKeyFrame * pKeyframe = static_cast<NII::VertexOffsetKeyFrame *>(pTrack->create(t->m_vertexKeyframes[k].time));
					for (int pri=0; pri<t->m_vertexKeyframes[k].poserefs.size(); pri++)
					{
						...
						pKeyframe->add(poseIndex, pr->poseWeight);
					}
				}
			}
		}
