...
namespace OgreMayaExporter
{
	...
	MStatus Skeleton::writeOgreBinary(ParamList & params)
	{
		...
		Ogre::SkeletonPtr pSkeleton = N_Only(Skeleton).create(name.asChar(),
			NII::GroupDefault);
	...
	MStatus Skeleton::createOgreBones(Ogre::SkeletonPtr pSkeleton,ParamList& params)
	{
		...
		for(int i = 0; i < m_joints.size(); i++)
		{
			...
			int jointId = j->id;
			Ogre::Bone * pBone = pSkeleton->create(jointName, jointId);
			// Set bone position (relative to it's parent)
			pBone->setPos(Vector3f(j->posx, j->posy, j->posz));
			// Set bone orientation (relative to it's parent)
			Ogre::Quaternion orient;
			orient.from(NII::Radian(j->angle), NII::Vector3(j->axisx,j->axisy,j->axisz));
			pBone->setOri(orient);
			// Set bone scale (relative to it's parent)
			pBone->setScale(Vector3f(j->scalex, j->scaley, j->scalez));
		}
		...
		for(int i = 0; i < m_joints.size(); i++)
		{
			...
			if(parentIdx >= 0)
			{
				...
				Ogre::Bone * pParent = pSkeleton->get(parentName);
				// Get current joint from skeleton
				unsigned short jointId = j->id;
				Ogre::String jointName = j->name.asChar();
				Ogre::Bone* pBone = pSkeleton->get(jointName);
	...
	MStatus Skeleton::createOgreSkeletonAnimations(Ogre::SkeletonPtr pSkeleton,ParamList& params)
	{
		...
		for(int i = 0; i < m_animations.size(); ++i)
		{
			...
			Ogre::Animation * pAnimation = 
                pSkeleton->createAnimation(m_animations[i].m_name.asChar(), m_animations[i].m_length);
			// Create tracks for current animation
			for(int j = 0; j < m_animations[i].m_tracks.size(); ++j)
			{
				Track * t = &m_animations[i].m_tracks[j];
				// Create a new track
                NodeKeyFrameTrack * pTrack = N_new NodeKeyFrameTrack(pAnimation, j);
                pTrack->setTarget(pSkeleton->get(t->m_bone.asChar()));
				...
				for(int k = 0; k<t->m_skeletonKeyframes.size(); ++k)
				{
					...
					Ogre::NodeKeyFrame * pKeyframe = static_cast<NodeKeyFrame *>(pTrack->create(keyframe->time));
					// Set translation
					pKeyframe->setTranslate(Ogre::Vector3(keyframe->tx,keyframe->ty,keyframe->tz));
					// Set rotation
					Ogre::Quaternion rot;
					rot.from(NII::Radian(keyframe->angle),
						NII::Vector3(keyframe->axis_x, keyframe->axis_y, keyframe->axis_z));
