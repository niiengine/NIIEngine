...
template<class E> inline static bool waitForWaiters(E& event, int numWaiters, int milliseconds)
{
  ...
  {
    if (event.getWaitCount() == numWaiters)
      return true;
    SleepMillis(1);
  }
  return false;
...
inline static bool waitForThread(std::atomic<long>& mutex, int numWaiters, int milliseconds)
{
  ...
  for( int i = 0; i < milliseconds; i++)
  {
    ...
    {
      ScopeLock tmplock(sec); // kick any memory syncs
    }
    SleepMillis(1);
  }
  return false;
...
public:
  inline explicit thread(ThreadMain& runnable) :
    f(&runnable), cthread(new Thread(f, "DumbThread"))
  {
    cthread->Create();
  }
