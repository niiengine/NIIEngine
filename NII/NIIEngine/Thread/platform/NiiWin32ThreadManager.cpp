...
#include "NiiWin32ThreadMananger.h"
...
{
        /// 线程方法
        struct Thread NII_THREAD_WORKER_INHERIT
        {
            Thread(ThreadManager * q);
            /// boost poco 专用
            void operator()();
            /// tbb专用
            void run();
    ...
    //---------------------------------------------------------------------
    Win32ThreadMananger::Win32ThreadMananger(const String & name) :
        ThreadManager(name){}
    //---------------------------------------------------------------------
    Win32ThreadMananger::~Win32ThreadMananger()
    {
        shutdown();
    }
    //---------------------------------------------------------------------
    void Win32ThreadMananger::startup(bool force)
    {
        if(mRun)
        {
            if(force)
        ...
        mShutDown = false;
        // 创建工作处理函数
        mWorkerFunc = N_new_t(WorkerFunc(this));

        N_Only(Log).stream() <<
            _T("Win32ThreadMananger('") << mName << _T("') initialising on thread ") <<
    ...
    //---------------------------------------------------------------------
    void Win32ThreadMananger::notifyThreadRegistered()
    {
    ...
    //---------------------------------------------------------------------
    void Win32ThreadMananger::shutdown()
    {
        if(!mRun)
            return;

        N_Only(Log).stream() <<
            _T("Win32ThreadMananger('") << mName << _T("') shutting down on thread ") <<
#if NII_THREAD_SUPPORT
            NII_THREAD_CURRENT_ID
#else
            "main"
        ...
        if(mWorkerFunc)
        {   // 删除工作处理函数
            N_delete_t(mWorkerFunc, WorkerFunc);
            mWorkerFunc = 0;
        }
    ...
    //---------------------------------------------------------------------
    void Win32ThreadMananger::notifyFire()
    {
        // 唤起等待工作的线程
        NII_THREAD_NOTIFY_ONE(mRequestCondition)
    }
    //---------------------------------------------------------------------
    void Win32ThreadMananger::waitForNextRequest()
    {
#if NII_THREAD_SUPPORT
    ...
    //---------------------------------------------------------------------
    void Win32ThreadMananger::main()
    {
        // default worker thread
#if NII_THREAD_SUPPORT
        N_Only(Log).stream() << _T("Win32ThreadMananger('") << getName() <<
            _T("')::WorkerFunc - thread ") << NII_THREAD_CURRENT_ID << _T(" starting.");
        ...
        N_Only(Log).stream() << _T("Win32ThreadMananger('") << getName() <<
            _T("')::WorkerFunc - thread ") << NII_THREAD_CURRENT_ID << _T(" stopped.");
#endif
    }
    //---------------------------------------------------------------------
