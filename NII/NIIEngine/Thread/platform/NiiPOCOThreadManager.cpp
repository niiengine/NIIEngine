...
#include "NiiPOCOThreadManager.h"
...
{
    /// 线程方法
    class _EngineInner POCOThread : public Poco::Runnable, public Thread
    {
    public:
        POCOThread(ThreadManager * q):
            mQueue(q) {}
    ...
    //---------------------------------------------------------------------
    POCOThreadMananger::POCOThreadMananger(const String & name) :
        ThreadManager(name){}
    //---------------------------------------------------------------------
    POCOThreadMananger::~POCOThreadMananger()
    {
        shutdown();
    }
    //---------------------------------------------------------------------
    void POCOThreadMananger::startup(bool force)
    {
        if(mRun)
        {
            if(force)
        ...
        mShutDown = false;
        // 创建工作处理函数
        mWorkerFunc = N_new_t(POCOThread(this));

        N_Only(Log).stream() <<
            _T("POCOThreadMananger('") << mName << _T("') initialising on thread ") <<
    ...
    //---------------------------------------------------------------------
    void POCOThreadMananger::notifyThreadRegistered()
    {
    ...
    //---------------------------------------------------------------------
    void POCOThreadMananger::shutdown()
    {
        if(!mRun)
            return;

        N_Only(Log).stream() <<
            _T("POCOThreadMananger('") << mName << _T("') shutting down on thread ") <<
#if NII_THREAD_SUPPORT
            NII_THREAD_CURRENT_ID
#else
            "main"
        ...
        if(mWorkerFunc)
        {   // 删除工作处理函数
            N_delete_t(mWorkerFunc, POCOThread);
            mWorkerFunc = 0;
        }
    ...
    //---------------------------------------------------------------------
    void POCOThreadMananger::notifyFire()
    {
        // 唤起等待工作的线程
        NII_THREAD_NOTIFY_ONE(mRequestCondition)
    }
    //---------------------------------------------------------------------
    void POCOThreadMananger::waitForNextRequest()
    {
#if NII_THREAD_SUPPORT
    ...
    //---------------------------------------------------------------------
    void POCOThreadMananger::main()
    {
        // default worker thread
#if NII_THREAD_SUPPORT
        N_Only(Log).stream() << _T("POCOThreadMananger('") << getName() <<
            _T("')::POCOThread - thread ") << NII_THREAD_CURRENT_ID << _T(" starting.");
        ...
        N_Only(Log).stream() << _T("POCOThreadMananger('") << getName() <<
            _T("')::POCOThread - thread ") << NII_THREAD_CURRENT_ID << _T(" stopped.");
#endif
    }
    //---------------------------------------------------------------------
