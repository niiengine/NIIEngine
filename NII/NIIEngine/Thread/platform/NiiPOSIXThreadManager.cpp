...
#include "NiiPOSIXThreadManager.h"
...
namespace NII
{
    ...
    //---------------------------------------------------------------------
    POSIXThreadMananger::POSIXThreadMananger(const String & name) :
        ThreadManager(name){}
    //---------------------------------------------------------------------
    POSIXThreadMananger::~POSIXThreadMananger()
    {
        shutdown();
    }
    //---------------------------------------------------------------------
    void POSIXThreadMananger::startup(bool force)
    {
        if(mRun)
        {
            if(force)
        ...
        N_Only(Log).stream() <<
            _T("POSIXThreadMananger('") << mName << _T("') initialising on thread ") <<
    ...
    //---------------------------------------------------------------------
    void POSIXThreadMananger::notifyThreadRegistered()
    {
    ...
    //---------------------------------------------------------------------
    void POSIXThreadMananger::shutdown()
    {
        if(!mRun)
            return;

        N_Only(Log).stream() <<
            _T("POSIXThreadMananger('") << mName << _T("') shutting down on thread ") <<
#if NII_THREAD_SUPPORT
            NII_THREAD_CURRENT_ID
#else
            "main"
    ...
    //---------------------------------------------------------------------
    void POSIXThreadMananger::notifyFire()
    {
        // 唤起等待工作的线程
        NII_THREAD_NOTIFY_ONE(mRequestCondition)
    }
    //---------------------------------------------------------------------
    void POSIXThreadMananger::waitForNextRequest()
    {
#if NII_THREAD_SUPPORT
    ...
    //---------------------------------------------------------------------
    void POSIXThreadMananger::main()
    {
        // default worker thread
#if NII_THREAD_SUPPORT
        N_Only(Log).stream() << _T("POSIXThreadMananger('") << getName() <<
