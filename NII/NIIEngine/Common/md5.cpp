...
namespace NII
{
    ...
    static void MD5Transform(Nui32 buf[4], Nui32 const in[16]);
    //------------------------------------------------------------------------
    Md5::Md5(void)
    {
        MD5Init(&m_ctx);
    ...
    Md5::~Md5(void){}
    //------------------------------------------------------------------------
    void Md5::append(const Nui8 * inBuf, size_t inLen)
    {
        MD5Update(&m_ctx, (md5byte *)inBuf, inLen);
    }
    //------------------------------------------------------------------------
    void Md5::append(const String & str)
    {
        append((Nui8 *)str.c_str(), str.length() * sizeof(Ntchar));
    }
    //------------------------------------------------------------------------
    void Md5::getDigest(Nui8 digest[16])
    {
        MD5Final(digest, &m_ctx);
    }
    //------------------------------------------------------------------------
    void Md5::getDigest(String & digest)
    {
        Nui8 szBuf[16] = {'\0'};
        getDigest(szBuf);

        Ntchar buff[256];
        NIIi n = Nsntprintf(buff, sizeof(buff), _T("%02X%02X%02X%02X%02X%02X%02X%02X"\
            ...
            szBuf[15]);
        digest.assign(buff, n);
    }
    //------------------------------------------------------------------------
    String Md5::GetMD5(const String & text)
    {
        if (text.empty())
            return _T("");
        Md5 state;
    ...
    void byteSwap(Nui32 *buf, unsigned words)
    {
        md5byte *p = (md5byte *)buf;

        do
        {
            *buf++ = (Nui32)((unsigned)p[3] << 8 | p[2]) << 16 |
                ((unsigned)p[1] << 8 | p[0]);
            p += 4;
        } while (--words);
    }
