...
{
	AnimationStateSet * set = mEntity->getAnimationCtrl();
	AnimationFusions::iterator i, iend = set->getFusions().end();
    for(i = set->getFusions().begin(); i != iend; ++i)
	{
		AnimationFusion * anim = *i;
		anim->setEnabled(false);
		anim->setWeight(0);
		anim->setTimePosition(0);
	}
	mSource = mEntity->getFusion( animation );
...
void AnimationBlender::blend(const String & animation, BlendingTransition transition, 
    NIIf duration, bool l)
{
	loop = l;
	...
	{ 
		AnimationFusion *newTarget = mEntity->getFusion( animation );
		if( mTimeleft > 0 )
		{
...
void AnimationBlender::addTime(NIIf time)
{
	if( mSource != 0 )
	{
		if( mTimeleft > 0 )
        ...
        if(loop)
            mSource->setPlayMode(RM_Loop);
        else
            mSource->setPlayMode(RM_Once);
	}
}
AnimationBlender::AnimationBlender( GeometrySpace *entity ) : mEntity(entity) {}
