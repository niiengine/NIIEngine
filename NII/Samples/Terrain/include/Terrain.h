...
public:
    ...
    void testCapabilities(const RenderSysFeature* caps)
	{
        ...
        {
			N_EXCEPT(Exception::ERR_NOT_IMPLEMENTED, "Your graphics card does not support vertex or fragment shaders, "
	...
	void doTerrainModify(Terrain* terrain, const Vector3& centrepos, TimeDurMS timeElapsed)
	{
		Vector3 tsPos;
			...
			case MODE_EDIT_HEIGHT:
				{
					...
					NIIf terrainSize = (terrain->getSize() - 1);
					...
					for (long y = starty; y <= endy; ++y)
					{
						...
						{
							NIIf tsXdist = (x / terrainSize) - tsPos.x;
							NIIf tsYdist = (y / terrainSize)  - tsPos.y;

							NIIf weight = std::min((NIIf)1.0,
								Math::sqrt(tsYdist * tsYdist + tsXdist * tsXdist) / NIIf(0.5 * mBrushSizeTerrainSpace));
			...
			case MODE_EDIT_BLEND:
				{
					...
					NIIf imgSize = terrain->getLayerBlendMapSize();
					...
					for (long y = starty; y <= endy; ++y)
					{
						for (long x = startx; x <= endx; ++x)
						{
							NIIf tsXdist = (x / imgSize) - tsPos.x;
							NIIf tsYdist = (y / imgSize)  - tsPos.y;
    ...
    bool onRenderCore(const RenderFrameArgs * evt)
    {
		if(mMode != MODE_NORMAL)
		{
			...
			if(rayResult.hit)
			{
				...
				mEditNode->setPos(rayResult.position);

				// figure out which terrains this affects
				TerrainGroup::TerrainList terrainList;
				NIIf brushSizeWorldSpace = TERRAIN_WORLD_SIZE * mBrushSizeTerrainSpace;
				...
				for(TerrainGroup::TerrainList::iterator ti = terrainList.begin();
					...
					doTerrainModify(*ti, rayResult.position, evt->mCurrent);
			}
			else
			{
		...
		if(!mFly)
		{
			...
			ray.setDirection(Vector3::INV_Y);

			TerrainGroup::RayResult rayResult = mTerrainGroup->rayIntersects(ray);
			NIIf distanceAboveTerrain = 50;
			NIIf fallSpeed = 300;
			NIIf newy = camPos.y;
			if (rayResult.hit)
			{
				...
				{
					mFallVelocity += evt->mCurrent * 20;
					mFallVelocity = std::min(mFallVelocity, fallSpeed);
					newy = camPos.y - mFallVelocity * evt->mCurrent;

				}
				newy = std::max(rayResult.position.y + distanceAboveTerrain, newy);
				mCamera->setPos(Vector3f(camPos.x, newy, camPos.z));
		...
		if (mHeightUpdateCountDown > 0)
		{
			mHeightUpdateCountDown -= evt->mCurrent;
			if (mHeightUpdateCountDown <= 0)
			{
	...
	bool keyPressed (const OIS::KeyEvent &e)
	{
		...
		case OIS::KC_F10:
			// dump
			{
				...
				while(ti.isNextValid())
				{
					Ogre::uint32 tkey = ti.getCurrentKey();
					TerrainGroup::TerrainSlot * ts = ti.getAndNext();
					if(ts->instance && ts->instance->isLoad())
					{
						ts->instance->_dumpTextures("terrain_" + StrConv::conv(tkey), ".png");
					}
				}
			}
			break;
...
protected:
	...
	NIIf mBrushSizeTerrainSpace;
	SpaceNode* mEditNode;
	GeometrySpace* mEditMarker;
	NIIf mHeightUpdateCountDown;
	NIIf mHeightUpdateRate;
	Vector3 mTerrainPos;
	...
	CameraSetup * mPSSMSetup;

	typedef std::list<GeometrySpace*> EntityList;
	...
	void defineTerrain(long x, long y, bool flat = false)
	{
		...
		else
		{
			...
			if (ResourceSchemeManager::getSingleton().isExist(mTerrainGroup->getResourceGroup(), filename))
			{
	...
	void initBlendMaps(Terrain* terrain)
	{
		...
		for (Ogre::uint16 y = 0; y < terrain->getLayerBlendMapSize(); ++y)
		{
			for (Ogre::uint16 x = 0; x < terrain->getLayerBlendMapSize(); ++x)
			{
				...
				NIIf height = terrain->getHeightAtTerrainPosition(tx, ty);
				NIIf val = (height - minHeight0) / fadeDist0;
				val = std::clamp(val, (NIIf)0, (NIIf)1);
				//*pBlend0++ = val;
		...
		blendMap0->dirty();
		blendMap1->dirty();
		//blendMap0->loadImage("blendmap1.png", GroupDefault);
		blendMap0->update();
		blendMap1->update();
		...
		if (!terrain->getGlobalColourMapEnabled())
		{
			...
			Image colourMap;
			colourMap.load("testcolourmap.jpg", GroupDefault);
	...
	void configureTerrainDefaults(Light* l)
	{
		...
		mTerrainGlobals->setLightMapDirection(l->getSpaceDirection());
		mTerrainGlobals->setCompositeMapAmbient(mSceneMgr->getAmbient());
		//mTerrainGlobals->setCompositeMapAmbient(Colour::Red);
		mTerrainGlobals->setCompositeMapDiffuse(l->getDiffuse());
	...
	{
		// Create material
		String matName = "Ogre/DebugTexture" + StrConv::conv(i);
		MaterialPtr debugMat = MaterialManager::getSingleton().getByName(matName);
		if(debugMat.isNull())
		{
			debugMat = MaterialManager::getSingleton().create(matName, GroupDefault);
		}
		ShaderCh * p = debugMat->get(0)->get(0);
		p->getTexture().removeAll();
		p->setLight(false);
		ShaderChTextureUnit * t = p->getTexture().create(texname);
        t->setUAddressing(TAM_CLAMP);
        t->setVAddressing(TAM_CLAMP);
        t->setWAddressing(TAM_CLAMP);

		// create template
		if(!OverlayManager::getSingleton().isGeoExist("Ogre/DebugTexOverlay", true))
		{
			OverlayGeo* e = N_OnlyOverlay).createGeo("Panel", "Ogre/DebugTexOverlay", true);
			e->setSizeMode(SM_Abs);
			e->setWidth(128);
			e->setHeight(128);
		}

		// add widget
		String widgetName = "DebugTex"+ StrConv::conv(i);
		Widget* w = mTrayMgr->getWidget(widgetName);
		if (!w)
		{
			w = mTrayMgr->createDecorWidget(loc, widgetName, "Ogre/DebugTexOverlay");
		}
		w->getOverlayGeo()->setMaterialName(matName);
	...
	void addTextureShadowDebugOverlay(TrayLocation loc, size_t num)
	{
		...
		{
			TexturePtr shadowTex = mSceneMgr->getRenderPattern()->getTexture(i);
	...
	MaterialPtr buildDepthShadowMaterial(const String& textureName)
	{
		...
		if (ret.isNull())
		{
			Material * baseMat = N_Only(Material).getByName("Ogre/shadow/depth/integrated/pssm");
			ret = baseMat->clone(matName);
			Pass* p = ret->get(0)->get(0);
            GpuProgramParam * param;
			p->getTexture().get("diffuse")->setSrc(textureName, Texture::T_2D);

			Vector4 splitPoints;
			const PSSMShadowSetup::SplitPointList& splitPointList =
				static_cast<PSSMShadowSetup*>(mPSSMSetup)->getSplitPoints();
			for(int i = 0; i < 3; ++i)
			{
				splitPoints[i] = splitPointList[i];
			}
			p->getProgram().get(0, ST_FS, NULL, param)
	...
	void configureShadows(bool enabled, bool depthShadows)
	{
		...
		if (enabled)
		{
			...
			{
				// shadow camera setup
				PSSMShadowSetup* pssmSetup = new PSSMShadowSetup();
				pssmSetup->setSplitPadding(mCamera->getNearClipDistance());
				pssmSetup->calculateSplitPoints(3, mCamera->getNearClipDistance(),
                    mSceneMgr->getRenderPattern()->getMaxExtent());
			...
			mSceneMgr->setShadowGenCamera(mPSSMSetup);

			if(depthShadows)
			{
				mSceneMgr->getRenderPattern()->setTextureCount(3);
				mSceneMgr->getRenderPattern()->setTexture(0, 2048, 2048, PF_R32);
				mSceneMgr->getRenderPattern()->setTexture(1, 1024, 1024, PF_R32);
				mSceneMgr->getRenderPattern()->setTexture(2, 1024, 1024, PF_R32);
			...
			{
				mSceneMgr->getRenderPattern()->setTextureCount(3);
				mSceneMgr->getRenderPattern()->setTexture(0, 2048, 2048, PF_X8B8G8R8);
				mSceneMgr->getRenderPattern()->setTexture(1, 1024, 1024, PF_X8B8G8R8);
				mSceneMgr->getRenderPattern()->setTexture(2, 1024, 1024, PF_X8B8G8R8);
			...
			matProfile->setReceiveDynamicShadowsPSSM(static_cast<PSSMShadowSetup*>(mPSSMSetup));
	...
	void setupView()
	{
		...
		mCamera->setPos(mTerrainPos + Vector3(1683, 50, 2116));
		mCamera->lookAt(Vector3f(1963, 50, 1660));
		mCamera->setNearClipDistance(0.1);
		mCamera->setFarClipDistance(50000);

		if (mRoot->getRender()->getFeature()->hasCapability(GF_Infinite_FarPlane))
        {
	...
	void setupContent()
	{
		...
		mTerrainGlobals = N_new TerrainGlobalOptions();

		mEditMarker = mSceneMgr->createGeo("editMarker", "sphere.mesh");
		mEditNode = mSceneMgr->getOriginNode()->create();
		mEditNode->attachObject(mEditMarker);
		mEditNode->setScale(Vector3f(0.05, 0.05, 0.05));
		...
		N_Only(Material).setTextureFilter(TFM_ANISOTROPIC);
		N_Only(Material).setTextureAniso(7);

		mSceneMgr->setFog(ShaderChFog(FM_LINEAR, Colour(0.7, 0.7, 0.8), 0, 10000, 25000));
		...
		l->setSpecular(Colour(0.4, 0.4, 0.4));

		mSceneMgr->setAmbient(Colour(0.2, 0.2, 0.2));


		mTerrainGroup = N_new TerrainGroup(mSceneMgr, Terrain::ALIGN_X_Z, TERRAIN_SIZE, TERRAIN_WORLD_SIZE);
		...
		mTerrainPaging = N_new TerrainPaging(mPageManager);
		...
		if (mTerrainsImported)
		{
			...
			while(ti.isNextValid())
			{
				Terrain* t = ti.getAndNext()->instance;
				initBlendMaps(t);
			}
		}
		...
		GeometrySpace* e = mSceneMgr->createGeo("tudorhouse.mesh");
		...
		rot.from(Angle(Math::random(-180, 180)), Vector3::Y);
		SpaceNode* sn = mSceneMgr->getOriginNode()->create(entPos, rot);
		sn->setScale(Vector3(0.12, 0.12, 0.12));
		sn->attachObject(e);
		mHouseList.push_back(e);

		e = mSceneMgr->createGeo("tudorhouse.mesh");
		entPos = Vector3(mTerrainPos.x + 1850, 0, mTerrainPos.z + 1478);
		entPos.y = mTerrainGroup->getHeightAtWorldPosition(entPos) + 65.5 + mTerrainPos.y;
		rot.from(Angle(Math::random(-180, 180)), Vector3::Y);
		sn = mSceneMgr->getOriginNode()->create(entPos, rot);
		sn->setScale(Vector3(0.12, 0.12, 0.12));
		sn->attachObject(e);
		mHouseList.push_back(e);
	...
	void _shutdown()
	{
		if (mTerrainPaging)
		{
			N_delete mTerrainPaging;
			N_delete mPageManager;
		}
		else
			N_delete mTerrainGroup;

		N_delete mTerrainGlobals;
