 ...
 *  in the materials.
 */
class GBufferSchemeHandler : public NII::MaterialMatchListener
{
public:
	/** @copydoc MaterialMatchListener::miss */
	virtual NII::ShaderFusion * miss(NII::Material * mat, SchemeID schemeName, 
        const NII::GeometryObj * geo, unsigned short lodIndex);
protected:
	...
	struct PassProperties 
	{
		...
		bool isDeferred;
		Ogre::vector<Ogre::ShaderChTextureUnit*>::type regularTextures;
		Ogre::ShaderChTextureUnit* normalMap;
		bool isSkinned;
        bool hasDiffuseColour;
		
		//Example of possible extension : vertex colours
		//Ogre::Nmark vertexColourType;
	};

	//Inspect a technique and return its relevant properties
	PassProperties inspectPass(Ogre::Pass * pass, 
		unsigned short lodIndex, const Ogre::GeometryObj* rend);
	...
	bool checkNormalMap(Ogre::ShaderChTextureUnit* tus, PassProperties& props);
};
