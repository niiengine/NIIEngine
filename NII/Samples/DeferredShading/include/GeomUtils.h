...
public:
	...
	// Fill up a fresh copy of Vertex and Index with a sphere's coords given the number of rings and the number of segments
	static void createSphere(Ogre::VertexData*& vertex, Ogre::IndexData*& index
		, float radius
		, int nRings, int nSegments
		, bool bNormals
		, bool bTexCoords);
	...
	static void createCone(Ogre::VertexData *& vertex, Ogre::IndexData *& index, 
        float radius, float height, int nVerticesInBase);

	// Fill up a fresh copy of VertexData with a normalized quad
	static void createQuad(Ogre::VertexData*& vertex);
};
#endif