...
private:
	...
	std::vector<GeometrySpace*> mMeshes;
...
public:
    ...
    void testCapabilities(const RenderSysFeature* caps)
    {
		...
		if (!(caps->getName() == "OpenGL Rendering Subsystem" || caps->getName() == "Direct3D9 Rendering Subsystem"))
		{
			N_EXCEPT(Exception::ERR_INVALID_STATE, "This demo currently only supports OpenGL and DirectX9. Sorry!",
...
protected:
    ...
    void setupContent()
    {
        mViewport->setBgColour(Colour(0.5, 0.5, 0.5, 1));

        // set our camera to orbit around the origin and show cursor
        mCameraMan->setStyle(CS_FREELOOK);
		mCameraMan->setTopSpeed(20.0);
        mCamera->movePos(Vector3(0, 10, 0));
        mCamera->setFOV(Radian(Angle(45).getRadian())); // i.e. 60deg * 1.3.. maya and ogre use fovX and fovY
        mCamera->setFarClipDistance(400);
        mCamera->setNearClipDistance(0.1);
        mTrayMgr->showCursor();

        // sibenik
        mCamera->setPos(Vector3f(27, 9, -2));
        mCamera->lookAt(Vector3f(-6, 2, 1));;

        // setup all meshes
        for (unsigned int i = 0; i < mMeshNames.size(); i++) {
            GeometrySpace* ent = mSceneMgr->createGeo(mMeshNames[i], mMeshNames[i] + ".mesh");
    ...
    void itemSelected(SelectMenu* menu)
    {
        if (menu->getName() == SSAO_OBJECT_MENU_NAME)
            ...
            changeCompositor(menu->getSelectItem());

        else if (menu->getName() == SSAO_POST_MENU_NAME)
            changePost(menu->getSelectItem());

        else if (menu->getName() == SSAO_CAMERA_MENU_NAME)
        {
            if (menu->getSelectItem() == SSAO_CAMERA_SIBENIK)
            {
                mCamera->setPos(Vector3f(27, 9, -2));
                mCamera->lookAt(Vector3f(-6, 2, 1));
            }
            else if (menu->getSelectItem() == SSAO_CAMERA_CORNELL)
            {
                mCamera->setPos(Vector3f(0, 5, 20));
                mCamera->lookAt(Vector3f(0, 5, 0));
            }
        }
    }
    ...
    void setUniform(Ogre::String compositor, Ogre::String material, Ogre::String uniform, float value, bool setVisible, int position = -1)
    {
        ...
        (static_cast<MaterialPtr>(N_Only(Material).getByName(material)))->get(0)->
            get(0)->getProgram().get(0, ST_FS, NULL, param)
        param->set(uniform, value);
