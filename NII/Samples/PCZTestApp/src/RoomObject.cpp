...
PCZSceneNode * RoomObject::createTestBuilding(Ogre::SpaceManager *scene, const String & name)
{
	count++;
	mPortalCount = 0;
	...
	GeometrySpace *exterior = pczSM->createGeo( name + "_building_exterior", "building_exterior.mesh" );

	// make the enclosure a child node of the root scene node
	PCZSceneNode * exteriorNode, * baseNode;
	baseNode = (PCZSceneNode*)scene->getOriginNode()->create(name +"_base_node");
    exteriorNode = (PCZSceneNode*)baseNode->create(name +"_building_exterior_node",  Vector3( 0.0f, 0.0f, 0.0f ) );
    ...
    GeometrySpace *room = pczSM->createGeo( name +"_room1", "room_nzpz.mesh" );

	// add the room as a child node to the enclosure node
	PCZSceneNode * roomNode;
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room1_node", Vector3( 0.0f, 0.0f, 20.0f ) );
    ...
    room = pczSM->createGeo( name +"_room2", "room_nxpxnypynzpz.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room2_node", Vector3( 0.0f, 0.0f, 0.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room3", "room_nzpz.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room3_node", Vector3( 0.0f, 0.0f, -20.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room4", "room_nxpx.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room4_node", Vector3( -20.0f, 0.0f, 0.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room5", "room_nxpx.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room5_node", Vector3( 20.0f, 0.0f, 0.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room6", "ROOM_NY.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room6_node", Vector3( 0.0f, 10.0f, 0.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room7", "ROOM_PY.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room7_node", Vector3( 0.0f, -50.0f, 0.0f ) );
	roomNode->attachObject(room);
	...
	// create another interior room
    room = pczSM->createGeo( name +"_room8", "room_nypy_4y.mesh" );

	// add the room as a child node to the enclosure node
	roomNode = (PCZSceneNode*)baseNode->create( name +"_room8_node", Vector3( 0.0f, -25.0f, 0.0f ) );
	roomNode->attachObject(room);
...
Ogre::CustomSpaceObj * RoomObject::createRoom(Ogre::SpaceManager *scene,
    const Ogre::String & name, short doorFlags, bool isEnclosure,
        Ogre::Vector3 dimensions, Ogre::Vector3 doorDimensions)
{
	addMaterial(name, Ogre::Colour(1,1,1,.75), Ogre::FBM_INV_ALPHA);

	Ogre::CustomSpaceObj * room = scene->createCustomGeo(name);

	room->begin(Ogre::GeometryRaw::OT_TRIANGLE_LIST, name);
...
//	Ogre::NIIf fade=.5;
	Ogre::NIIf solid=.8;
	Ogre::Colour color = Colour(0, 0, solid, solid);
...
void RoomObject::addMaterial(const Ogre::String & mat, const NII::Colour & clr,
    NII::FrameBlendMode sbt)
{
	...
	NII::Material * matptr = NII::MaterialManager::getSingleton().create(mat, "General");
	matptr->setReceiveShadow(false);
	matptr->get(0)->setLight(true);
    ShaderCh * sh = matptr->get(0)->get(0);

    ShaderChColour * tcc = sh->setColour(ShaderCh::SOT_Set);
    tcc->setAmbient(clr);
	tcc->setDiffuse(clr);
	tcc->setEmissive(clr);
    tcc->setColourTracking(NII::CM_T_DIFFUSE);

	sh->setBlend(ShaderCh::SOT_Set)->setBlend(sbt);
	sh->setLight(false);
}

void RoomObject::createPoints(Ogre::Vector3 dimensions, Ogre::Vector3 doorDimensions)
{
	Ogre::NIIf l = dimensions.x/2;
	Ogre::NIIf h = dimensions.y/2;
	Ogre::NIIf w = dimensions.z/2;
...
//		 1*-------*2
	...
	// doors
	Ogre::NIIf l2 = doorDimensions.x/2;
	Ogre::NIIf h2 = doorDimensions.y/2;
	Ogre::NIIf w2 = doorDimensions.z/2;
...
void RoomObject::createWalls(Ogre::CustomSpaceObj * room, short doorFlags, bool isEnclosure)
{
	if (isEnclosure)
	{
...
void RoomObject::createPortals(Ogre::SpaceManager *scene,
							   Ogre::CustomSpaceObj* room,
							   Ogre::SpaceNode * roomNode,
							   Ogre::PCZone * zone,
							   short doorFlags,
							   bool isEnclosure)
{
