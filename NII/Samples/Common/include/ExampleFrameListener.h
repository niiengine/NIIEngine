...
protected:
	virtual void updateStats()
	{
		...
		try
        {
			...
			guiTris->setCaption(tris + StrConv::conv(mWindow->mTriCount));

			OverlayGeo* guiBatches = OverlayManager::getSingleton().getGeo("Core/NumBatches");
			guiBatches->setCaption(batches + StrConv::conv(mWindow->mBatchCount));
...
public:
            ...
            mJoy(0)
	{
		...
		WindowUtil::addWindowEventListener(mWindow, this);
	}
	...
	virtual ~ExampleFrameListener()
	{
		...
		WindowUtil::removeWindowEventListener(mWindow, this);
	...
	virtual bool processUnbufferedKeyInput(const RenderFrameArgs& evt)
	{
		...
		if(mKeyboard->isKeyDown(OIS::KC_R) && mTimeUntilNextToggle <=0)
		{
			...
			case 0 : 
                ...
                break;
			case 1 : 
                mCamera->setPolygonMode(PM_LINE); 
                break;
			case 2 : 
		...
		if(displayCameraDetails)
			mDebugText = "P: " + StrConv::conv(mCamera->getSpacePos()) +
                " " + "O: " + StrConv::conv(mCamera->getSpaceOri());
