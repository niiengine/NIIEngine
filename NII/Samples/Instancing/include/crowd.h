...
protected:
	...
	vector <AnimationFusion*>::type animations;
...
public:
	...
	//-----------------------------------------------------------------------
	CrowdListener(ViewWindow* win, Camera* cam,CEGUI::Renderer* renderer, CrowdApplication*main);
	//-----------------------------------------------------------------------
	~CrowdListener();
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	void setupInstancedMaterialToEntity(GeometrySpace*);
	//-----------------------------------------------------------------------
...
public:
	...
	ViewWindow * getRenderWindow(void)
	{
		return mWindow;
	}
...
protected:
   ...
   BoxSky * mBoxSky;
    ...
    {
        // Get the SpaceManager, in this case a generic one
        mSceneMgr = mRoot->createSpaceManager("TerrainSceneManager");
    }
	//-----------------------------------------------------------------------
	...
	void createScene(void)
	{
        ...
        mSceneMgr -> setSceneGeo( terrain_cfg );
		// Set ambient light
		...
		dir.normalise();
		l->setDirection(dir);
		l->setDiffuse(1.0, 1.0, 0.8);

		mCamera->setPos(NII::Vector3f(700, 100, 500));
		mCamera->lookAt(NII::Vector3f(600, 80, 600));
	...
	void setupResources(void)
	{
		...
		/*ResourceSchemeManager *rsm = ResourceSchemeManager::getSingletonPtr ();
		GroupIDList groups = rsm->getGroups();
		if (std::find(groups.begin(), groups.end(), String("Crowd")) == groups.end())
		{
			rsm->create(GroupID "Crowd");
			try
			{
				rsm->add("../../../../../ogreaddons/Crowd/Media/materials/programs","FileSystem", "Crowd");
				rsm->add("../../../../../ogreaddons/Crowd/Media/materials/scripts","FileSystem", "Crowd");
				rsm->add("../../../../../ogreaddons/Crowd/Media/models","FileSystem", "Crowd");
			}
			catch (Ogre::Exception& e)
			{
				String error = e.getFullDescription();
				rsm->add("../../../Crowd/Media/materials/programs","FileSystem", "Crowd");
				rsm->add("../../../Crowd/Media/materials/scripts","FileSystem", "Crowd");
				rsm->add("../../../Crowd/Media/models","FileSystem", "Crowd");
			}
		}*/
	}
	void setupGUI(void)
	{
		// Set up GUI system
       mGUIRenderer = new CEGUI::OgreCEGUIRenderer(mWindow, Ogre::RQG_Overlay, true, 3000, mSceneMgr);
	...
	bool handleObjectCountChanged(const CEGUI::EventArgs& e)
	{
		...
		static_cast<CrowdListener*>(mFrameListener)->createCurrentGeomOpt();
		String value=StrConv::conv(objectCount);
	...
	bool handleShadowsChanged(const CEGUI::EventArgs & e)
	{
		if(((CEGUI::Checkbox *)((const CEGUI::WindowEventArgs &)e).window)->isSelected())
		{
			...
			LiSPSMShadowSetup * lispsm = N_new LiSPSMShadowSetup();
			lispsm->setOptimalAdjustFactor(0.3);
			mSceneMgr->setShadowGenCamera(lispsm);
