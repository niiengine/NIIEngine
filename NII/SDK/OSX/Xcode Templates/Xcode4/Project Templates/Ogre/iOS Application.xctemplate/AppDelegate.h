...
- (void)go {
    ...
    if (mDisplayLinkSupported)
    {
        ...
        mDisplayLink = [NSClassFromString(@"CADisplayLink") displayLinkWithTarget:self selector:@selector(renderFrame:)];
...
{
    Ogre::Root::getSingleton().endRender();
...
- (void)renderFrame:(id)sender
{
    if(!OgreFramework::getSingletonPtr()->isOgreToBeShutDown() &&
       Ogre::Root::getSingletonPtr() && Ogre::Root::getSingleton().isInit())
    {
		if(OgreFramework::getSingletonPtr()->m_pRenderWnd->isActive())
		{
			mStartTime = OgreFramework::getSingletonPtr()->m_pTimer->getCostMS();
			...
			mLastFrameTime = OgreFramework::getSingletonPtr()->m_pTimer->getCostMS() - mStartTime;
		}
    }
    else
    {
