...
#endif
    m_pTrayMgr          = 0;
    m_FrameEvent        = Ogre::RenderFrameArgs();
}
...
#endif
{
	...
	m_pSceneMgr->setAmbient(NII::Colour(0.7f, 0.7f, 0.7f));

	m_pCamera = m_pSceneMgr->createCamera("Camera");
	m_pCamera->setPos(Vector3(0, 60, 60));
	m_pCamera->lookAt(Vector3(0, 0, 0));
	m_pCamera->setNearClipDistance(1);

	m_pViewport = m_pRenderWnd->createViewport(m_pCamera);
	m_pViewport->setBgColour(Colour(0.8f, 0.7f, 0.6f, 1.0f));
    ...
    while (seci.isNextValid())
    {
        ...
        for (i = settings->begin(); i != settings->end(); ++i)
        {
            ...
            if (!Ogre::StringUtil::beginsWith(archName, "/", false)) // only adjust relative dirs
                archName = Ogre::String(m_ResourcePath + archName);
#endif
            Ogre::N_Only(ResourceScheme).add(archName, typeName, secName);
        }
    }
...
bool OgreFramework::keyPressed(const OIS::KeyEvent &keyEventRef)
{
	...
	{
		m_pRenderWnd->screenshots("BOF_Screenshot_", ".png");
		return true;
	}
...
{
	m_pCamera->yaw(Angle(evt.state.X.rel * -0.1f));
	m_pCamera->pitch(Angle(evt.state.Y.rel * -0.1f));
...
void OgreFramework::updateOgre(double timeSinceLastFrame)
{
	...
	m_FrameEvent.mCurrent = timeSinceLastFrame;
    m_pTrayMgr->onRenderCore(&m_FrameEvent);
}
