...
#
#  OGRE_FOUND - system has NII
#  OGRE_INCLUDE_DIRS - the NII include directories 
#  OGRE_LIBRARIES - link these to use the NII core
...
#       the NII plugins are located
#  OGRE_PLUGIN_DIR_DBG - The directory where the debug versions of
#       the NII plugins are located
#  OGRE_MEDIA_DIR - The directory where the NII sample media is
...
elseif (UNIX)
  set(OGRE_PREFIX_GUESSES
    ...
    /usr/lib${LIB_SUFFIX}/NII
    /usr/local/lib${LIB_SUFFIX}/ogre
    /usr/local/lib${LIB_SUFFIX}/NII
    $ENV{HOME}/ogre
    $ENV{HOME}/NII
  )
endif ()
...
if(NOT OGRE_BUILD_PLATFORM_APPLE_IOS)
	...
	use_pkgconfig(OGRE_PKGC "NII${OGRE_LIB_SUFFIX}")

	# try to find framework on OSX
	findpkg_framework(NII)
else()
	set(OGRE_LIBRARY_FWK "")
endif()

# locate Ogre include files
find_path(OGRE_CONFIG_INCLUDE_DIR NAMES OgreBuildSettings.h HINTS ${OGRE_INC_SEARCH_PATH} ${OGRE_FRAMEWORK_INCLUDES} ${OGRE_PKGC_INCLUDE_DIRS} PATH_SUFFIXES "NII")
find_path(OGRE_INCLUDE_DIR NAMES OgreRoot.h HINTS ${OGRE_CONFIG_INCLUDE_DIR} ${OGRE_INC_SEARCH_PATH} ${OGRE_FRAMEWORK_INCLUDES} ${OGRE_PKGC_INCLUDE_DIRS} PATH_SUFFIXES "NII")
...
if (OGRE_INCLUDE_DIR)
  ...
  pkg_message(NII "Found Ogre ${OGRE_VERSION_NAME} (${OGRE_VERSION})")
  ...
  if (OGRE_CONFIG_HEADER)
    ...
    elseif (OGRE_CONFIG_STATIC OR OGRE_STATIC)
      pkg_message(NII "Build type (static, dynamic) does not match the requested one.")
      set(OGRE_INCOMPATIBLE TRUE)
    endif ()
  else ()
    pkg_message(NII "Could not determine Ogre build configuration.")
    set(OGRE_INCOMPATIBLE TRUE)
  endif ()
else ()
  set(OGRE_INCOMPATIBLE FALSE)
...
if (OGRE_STATIC)
  ...
  if (NOT OGRE_DEPS_FOUND)
    pkg_message(NII "Could not find all required dependencies for the Ogre package.")
...
macro(ogre_find_component COMPONENT HEADER)
  ...
  find_path(OGRE_${COMPONENT}_INCLUDE_DIR NAMES ${HEADER} HINTS ${OGRE_INCLUDE_DIRS} ${OGRE_PREFIX_SOURCE} PATH_SUFFIXES ${COMPONENT} NII/${COMPONENT} Components/${COMPONENT}/include)
...
macro(ogre_find_plugin PLUGIN HEADER)
  ...
  find_library(OGRE_${PLUGIN}_LIBRARY_REL NAMES ${OGRE_${PLUGIN}_LIBRARY_NAMES}
    HINTS ${OGRE_LIBRARY_DIRS} PATH_SUFFIXES "" NII opt release release/opt relwithdebinfo relwithdebinfo/opt minsizerel minsizerel/opt)
  find_library(OGRE_${PLUGIN}_LIBRARY_DBG NAMES ${OGRE_${PLUGIN}_LIBRARY_NAMES_DBG}
    HINTS ${OGRE_LIBRARY_DIRS} PATH_SUFFIXES "" NII opt debug debug/opt)
